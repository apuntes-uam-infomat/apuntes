...
Sea $B=\{\abs{\mathbb{E}(X|\algb{B})}  \}$ \textcolor{red}{COMPLETAR ESTO}
\end{proof}
...
Y hemos usado que $\mathbb{E}(S_{n}|\algb{M}_n)=S_n$ que es así por definición de esperanza condicionada a una $\salgb$ y porque $S_n$ es $\algb{M_n}$-medible. Lo explico mejor: 

Teníamos que la esperanza condicionada ($\mathbb{E}(X|\algb{B})$) cumple que es $\algb{B}$-medible y que
$\forall B \in \algb{B}$, $\int_{B}\mathbb{E}(X|\algb{B})dP=\int_{B}XdP$, y decíamos que como $\mathbb{E}(X|\algb{B})$ es $\algb{B}$-medible, es que era constante al integrarlo sobre cada elemento $B \in \algb{B}$ y se podía sacar fuera de la integral. Pues entonces, si X fuera también $\algb{B}$-medible, lo podríamos sacar fuera de la integral por el mismo razonamiento y nos quedaría:

\[
\int_{B}\mathbb{E}(X|\algb{B})dP=\int_{B}XdP \Leftrightarrow \mathbb{E}(X|\algb{B}) \int_{B}dP = X \int_{B}dP \Leftrightarrow
\]

\[
\Leftrightarrow \mathbb{E}(X|\algb{B}) P(B) = X P(B) \Leftrightarrow \mathbb{E}(X|\algb{B}) = X
\]

\textcolor{red}{preguntar esto}
