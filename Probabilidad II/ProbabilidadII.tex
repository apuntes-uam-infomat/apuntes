...
$$
$$
\mathbb{E}(X_2|\algb{A}_2)(w)
  \left\lbrace
  \begin{array}{l}
     ???\\
  \end{array}
  \right. \text{   }
\mathbb{E}(X_1|\algb{A}_2)(w)
    \left\lbrace
    \begin{array}{l}
       ???\\
    \end{array}
...
Sea $\{\algb{A}_n \}_{n \geq 0}$ una filtración. $T: \Omega \rightarrow \mathbb{N}\cup \{\infty \}$ es un tiempo de parada si $\forall n \geq 0, \{w \in \Omega \text{ tal que } T(w)=n\} \in \algb{A}_n$. \textcolor{red}{¿Que significa esta definciion?}
\end{defn}
...
\textcolor{red}{Y $X_n$ no converge a $X_{\infty}$ en $L^1$?}
...
Sea $X=\{\{X_n\}_{n=1}^{\infty}, \{\algb{A}_n\}_{n=0}^{\infty}\}$ una martingala en $L^p$ con p>1. Entonces $X_{\infty}=\lim_{n \rightarrow \infty} X_n$ existe casi seguro. Y además $X_n \stackrel{L^p}{\rightarrow} X_{\infty}$, y es fácil ver que si $Y \in L^p$ con $1 \leq p \leq \infty$, entonces $X_n= \mathbb{E}(Y|\algb{A}_n)$ define una martingala.
\end{theorem}
