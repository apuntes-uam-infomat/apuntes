...
\textcolor{red}{Se puede extender a $L^n$?}
...
\]
\end{proof}
\item Si X es $\algb{B}$-medible, entonces $\mathbb{E}(XY|\algb{B}) = X\mathbb{E}(Y|\algb{B})$, suponiendo que $XY,Y \in L^1$. \textcolor{red}{¿Por qué la restricción de $L^1$}
\begin{proof}
...
Del mismo modo $\mathbb{E}(X_{n+1}|\algb{A}_n)=X_n$
...
\begin{proof}
Sea X la martingala que hemos visto anteriormente, con $X_n=  2^n \ind_{(0, \frac{1}{2^n})}$, hemos visto que X pertenece a $L^1$. Y tenemos que $\lim_{n \rightarrow \infty} X_n = X_{\infty} = 0$ \textcolor{red}{No termino de ver muy bien por qué}.
...
\[
\int_{\Omega} \abs{X_n - X_{\infty}} = \int_{\Omega} \abs{X_n - 0} = \mathbb{E}(\abs{X_n}) = 1
\]

\textcolor{red}{escribir sto bien}
\end{proof}
...
\item si $Y \in L^p$ con $1 \leq p \leq \infty$, entonces: $X_n= \mathbb{E}(Y|\algb{A}_n)$ define una martingala.
\item $X_{\infty}$ es $\algb{A}$-medible y $X_n=\mathbb{E}(X_{\infty}|\algb{A}_n)$ \textcolor{red}{caso concreto de lo de antes}
\end{itemize}
...
\[
X_n \stackrel{L^1}{\rightarrow} X_{\infty} \Leftrightarrow \lim_{n \rightarrow \infty} \norm{X_{\infty}-X_n}_1=0 \Leftrightarrow \lim_{n \rightarrow \infty} \int_{\Omega}\abs{X_{\infty}-X_n}dP \geq \lim_{n \rightarrow \infty} \int_B \abs{X_{\infty}-X_n}dP\geq
\]
\[
...
Luego:
\[
\lim_{m \rightarrow \infty} \int_B X_{n+m}=\int_B X_{\infty}  \Rightarrow \mathbb{E}(X_{n+m}) = X_n
\]
\textcolor{red}{no entiendo este ultimo paso}
\end{proof}
