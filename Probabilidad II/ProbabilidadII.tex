...
\textcolor{red}{El segundo signo igual se debe a que como la $\salgb$ $\algb{C}$ está contenida en $\algb{B}$, entonces por ser $\mathbb{E}(X|\algb{B})$ B-medible se puede hacer ese paso}

\textcolor{red}{Si la $\salgb$ $\algb{C}$ está contenida en $\algb{B}$, entonces si X es C-medible, también es B-medible.}
...
Sea $B=\norm{\mathbb{E}(X|\algb{B})}_{\infty} \geq \norm{X}_{\infty}$

\textcolor{red}{Completar con los apuntes de Elena o de alguien.}
\end{proof}


\textcolor{red}{\obs \textbf{CLAVE}: Sea $\algb{C} \subset \algb{B}$, entonces, si X es $\algb{C}$-medible, también es $\algb{B}$-medible.}

\textcolor{red}{\obs \textbf{CLAVE}: Sea $0<r\leq s<\infty$, sabemos que $L^s \subset L^r$, y esto implica que si $X \in L^r$ entonces $X \in L^s$ y además $\norm{f}_r \leq \norm{f}_s$.}
