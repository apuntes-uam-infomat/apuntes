...
\begin{theorem}
Sean $C=C_1,...,C_n$ colecciones de conjuntos, cada una de ellas cerradas por intersecciones finitas, es decir, si $A,B \in C$, entones $A \cap B \in C$. Para cualquier $i\geq1 , i \leq n$, Si $C_1,...,C_n$ son independientes, entonces $\sigma(C_1),...,\sigma(C_n)$ son independientes. La implicación contraria es obviamente cierta.
\end{theorem}

\begin{example}
\textcolor{red}{Preguntar por este ejemplo, las diferencias entre $\sigma(X_1,X_2)$ con $\sigma(X_1,\cdot X_2)$ con $\sigma(\sigma(X_1),\sigma(X_2))$. Supongo que el producto de v.a. da lugar a pares, y poner una coma, solo aumentas el espacio muestral. Pero entre poner sigmas dentro de otros sigmas y no ponerlos, no veo la diferencia.}
Sean $X_1, X_2, X_3, X_4$, variables aleatorias independientes..Entonces $\sigma(X_1)$ y $\sigma(X_2 \cdot X_3 \cdot X_4) \subset \sigma(\sigma(X_2),\sigma(X_3),\sigma(X_4)) \textcolor{red}{=\sigma(X_2,X_3,X_4)}$. Luego por el teorema anterior:

$\sigma(X_1)$ y $\sigma(X_2 \cdot X_3 \cdot X_4)$ son independientes $\Leftrightarrow X_1 y X_2X_3X_4$ son independientes. Y esto implicaría que $\mathbb{E}(X_1X_2X_3X_4)=\mathbb{E}(X_1)\mathbb{E}(X_2X_3X_4)$
\end{example}
...
\end{itemize}
\textcolor{red}{Entender esa implicación...}
...
\begin{theorem}
Sea $\algb{A}$ un álgebra de conjuntos. Entonces todo conjunto con (a) es aproximable. \textcolor{red}{wtf?}
\end{theorem}
...
\]
\textcolor{red}{??? no entiendo esto, una suma de variables aleatorias convergen a un numero?}
...
\textcolor{red}{muy gratis, y no entiendo lo mismo que antes, será algo de notación}
\end{theorem}

\subsection{Tasas de convergencia}
\textcolor{red}{Igual que lo de antes en estas leyes...}
...
\obs Esta observación se basa en que el valor absoluto de la integral de una función es menor que la integral del valor absoluto de la misma función: $\forall t$, $\abs{\phi_X(t)} \leq \mathbb{E}(\abs{e^{itX}})=\textcolor{red}{\phi_X(0)=1}$
...
\begin{proof}
\[
\phi_{\sum_{i=1}^{n}X_i}(t)=\mathbb{E}(e^{it\sum_{i=1}^{n}X_i})=\mathbb{E}(\prod_{i=1}^{n}e^{itX_i})=\textcolor{red}{\prod_{i=1}^{n}\mathbb{E}(e^{itX_i})}=\prod_{i=1}^{n}\phi_{X_i}(t)
\]
\end{proof}
\end{itemize}
...
\textbf{Condición de Lindeberg:} En el TCL o Berry-Esseen se asume implícitamente que \textcolor{red}{$\sigma^2 >0$}. En el TCL de Lindeberg-Levy-Feller, se asume que no todas las varianzas son 0.
\[
\text{ En lugar de } \frac{(\sum_{j=1}^{n}X_j)-n\mu}{\sigma \sqrt{n}} \text{ , tenemos } \frac{\sum_{j=1}^{n}(X_j-\mu_i)}{\sqrt{\sum_{i=1}^{n}\sigma_i^2}}
\]
Cuando no son idénticamente distribuidas. \textcolor{red}{No entiendo muy bien el enunciado de esta condición}

\begin{theorem}[TCL de Lindeberg-Levy-Feller]
Sea $\{X_n\}_{n\geq 1}$ una sucesión de v.a.i en $L^2$. Son equivalentes:
\begin{enumerate}
\item $\forall \epsilon >0$ , $\lim_{n\rightarrow \infty} L(n, \epsilon)=0$.
\item 
\[
...
Abreviaciones: $S^2_n=\sum_{i=1}^{n}\sigma_i^2$ donde N es el primer natural tal que $\sigma^2_N>0$ y $n\geq N$
\[
\text{\~{L}}(n)=\frac{\max_{1\leq k \leq n}\sigma^2_k}{S^2_n}
\]

La condición de Lindeberg aparece expresada en 1). Para $\epsilon>0$: 
$$L(n, \epsilon)=\frac{1}{S_n^2} \sum_{i=1}^{n}\left( \abs{X_i-\mu_i}^2 \ind_{\{\abs{X_i-\mu_i}\geq \epsilon S_n\}} \right)$$

Sea $1\leq k \leq n$, tal que $\sigma^2_k=\max_{1\leq i \leq n}\sigma_i^2$

\[
\text{\~{L}}(n)=\frac{\sigma^2_n}{S^2_n}=\frac{\mathbb{E}(\abs{X_k-\mu_k}^2)}{S^2_n}=\frac{\mathbb{E}\left( \abs{X_k-\mu_k}^2 \ind_{\{\abs{X_k-\mu_k}^2\geq \epsilon^2 S_n^2\}} \right)}{S_n^2}+\frac{\mathbb{E}\left( \abs{X_k-\mu_k}^2 \ind_{\{\abs{X_k-\mu_k}^2\leq \epsilon^2 S_n^2\}} \right)}{S_n^2} \leq
\]
\[
\leq L(n,\epsilon)+\epsilon^2
\]

Si $L(n,\epsilon)\stackrel{n \rightarrow \infty}{\rightarrow}$  $\forall \epsilon$, entonces $L(n)\stackrel{n \rightarrow \infty}{\rightarrow} 0$.
...
\[
L(n, \epsilon)=\frac{1}{S_n^2} \sum_{i=1}^{n}\left( \abs{X_{nm}-\mu_{nm}}^2 \ind_{\{\abs{X_{nm}-\mu_{nm}}\geq \epsilon S_n\}} \right)
\]
