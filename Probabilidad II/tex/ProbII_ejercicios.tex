...
\textcolor{red}{Los ejercicios con 4,5,6 y 8. Aunque tengan cosas en rojo están bien resueltos, eliminaré esos comentarios en cuánto solucione las dudas}
...
SOLUCIÓN DEL PROFESOR:
Para buscar la g, ideita feliz supongo. Definimos $signo(f)=1$ si $f\geq0$ y $signo(f)=-1$ si $f<0$. Y consideramos $g=signo(f)\abs{f}^\frac{p}{q}$. Entonces procedemos a ver que esta g es válida:
...
\]
\item $\norm{f+g}_p \leq \norm{f}_p+\norm{g}_p$.
Demostrado en el ejercicio anterior para $1<p<\infty$.
\end{enumerate}

Para p=1, 1) y 2) son iguales, y comprobamos 3):

$\int \abs{f+g} \leq \int (\abs{f} + \abs{g})=  \int \abs{f} + \int \abs{g}$
...
Dos formas:
\begin{enumerate}
\item Usamos Jensen (aunque en el enunciado se aconseje usar Hölder, Jesús Munarriz tuvo el detalle de hacerlo usando Jensen):
\[
\norm{f}_{L^r(\Omega, \algb{M}, P)} = \norm{f}_r= \left( \int_{\Omega} \abs{f}^r dP \right)^{\frac{1}{r}}
\]

Para simplificar la notación, omitiremos a partir de ahora $\Omega$ y dP. Usamos como función convexa $f(t)=\abs{t}^{\frac{s}{r}}$.
...
\item Usando Hölder... queda pendiente \textcolor{red}{Entender la solución usando Hölder}
\end{enumerate}
...
%%%%%%%%%%%%%%%%%%  PROBLEMA 5.3  %%%%%%%%%%%%%%%%%%%%%%%%%
\begin{problem}[3] Probar que si $X := \{X_n\}_{n=0}^{\infty}$  es una submartingala, y 
 $ \|X\|_s <\infty$, donde 
...
\[
\sigma = \mathbb{E}(X^2)-\mathbb{E}(X)^2 = 1 - 0 = 1 
\]

Y nos queda:
\[
\lim_{n \rightarrow \infty} P(\sqrt{n}S_n \leq t) = N(0,1)(t)
\]

Llamando $P_n$ a la probabilidad en la normal y sabiendo que $z = \frac{S_n - \mu}{\sigma} = S_n$, obtenemos:
\[
P(-c < S_n < c)= P(S_n > -c) - P(S_n > c) = 1 - P_n(z > c) - P_n(z > c) = 1- 2\cdot P_n(z > c)
\]
