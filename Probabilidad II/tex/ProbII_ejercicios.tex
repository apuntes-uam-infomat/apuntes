...
\textcolor{red}{Desde aqui hasta el final del ejercicio trato de demostrar que converge en $L^1$ y tampoco lo consigo}

Pero por la segunda propiedad de la definición de esperanza condicionada ($\forall B \in \algb{B}$, $\int_{B}\mathbb{E}(X|\algb{B})dP=\int_{B}XdP$) tenemos que:

\[
\int \mathbb{E}(\abs{Y}|\mathcal{A}_n) dP = \int \abs{Y} dP  \Leftrightarrow \int \mathbb{E}(\abs{Y}|\mathcal{A}_n) - \abs{Y} dP = 0 
\]
...
Y concluimos:
\[
0 = \int \mathbb{E}(\abs{Y}|\mathcal{A}_n) - \abs{Y} dP \geq \int \abs{\mathbb{E}(Y|\mathcal{A}_n)} - \abs{Y} dP \geq \abs{\int \mathbb{E}(Y|\mathcal{A}_n) dP} -
\]

Y concluimos:
\[
0 = \abs{\int \mathbb{E}(Y|\mathcal{A}_n) - Y dP} \leq \int \abs{\mathbb{E}(Y|\mathcal{A}_n) - Y} dP \geq 0
\]
...
\textcolor{red}{Desde aqui hasta lo proximo en rojo quiero demostrar que $\mathbb{E}(X_i)\mathbb{E}(X_j)=\mathbb{E}(X_i X_j)$, que es lo que uso al final}

Por tanto, dado $X_j$ y $X_k$ con j,k cualesquiera, tenemos que:

$$\mathbb{E}(X_j)=\mathbb{E}(X_k) \Leftrightarrow \mathbb{E}(X_j)\cdot \mathbb{E}(X_k)= \mathbb{E}(X_j)^2 = \left(\int X_j \right)^2$$

Y por Jensen (o por definición de varianza) podemos poner un '$\leq$' y tenemos que:
\[
\mathbb{E}(X_j)\cdot \mathbb{E}(X_k) = \mathbb{E}(X_j)^2 = \left(\int X_j\right)^2 \leq \int X_j^2 = \mathbb{E}(X_j^2)
\]

% = \mathbb{E}(X_j\cdot X_k) = \int X_j \cdot X_k

Idénticamente:
\[
\mathbb{E}(X_j)\cdot \mathbb{E}(X_k) = \mathbb{E}(X_k)^2 = \left(\int X_k\right)^2 \leq \int X_k^2 = \mathbb{E}(X_k^2)
\]

Sumamos:
\[
\mathbb{E}(X_j)^2 + \mathbb{E}(X_k)^2 \leq \int X_j^2 + \int X_k^2
\]

\textcolor{red}{... y no consigo sacarlo }

% = \mathbb{E}(X_j\cdot X_k) = \int X_j \cdot X_k
Aplicado a nuestro problema:
\[
\int Y_j \cdot Y_k = \int (X_j -X_{j-1})(X_k-X_{k-1}) = \int X_jX_k -X_jX_{k-1}-X_{j-1}X_k + X_{k-1}X_{j-1} = 
\]

\[
= \int X_jX_k -\int X_jX_{k-1}- \int X_{j-1}X_k + \int X_{k-1}X_{j-1} = 
\]

\[ 
= \int X_j^2 - \int X_j^2 - \int X_k^2 + \int X_k^2 = 0
\]
