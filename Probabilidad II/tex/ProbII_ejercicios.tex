...
\textcolor{red}{BASTANTE FIABLES TODOS}
...
\textcolor{red}{A LA ESPERA DE SER ESTRICTAMENTE CORREGIDA.}
...
Tenemos que ver que $\int fg = \|fg\|_1 = \|f\|_p\|g\|_q$.

Escogemos g que cumpla:
\[
\frac{\abs{f}}{\norm{f}_p} = \frac{\abs{g}}{\norm{g}_q}
\]

De esta forma obtendremos la igualdad en la desigualdad de Young, y por tanto, punto a punto en la de Hölder, y obtenemos que $\norm{fg}_1 = \norm{f}_p \norm{g}_q$.

Sólo falta ver que: $\int fg = \|fg\|_1$
Según hemos escogido g, tenemos que:
\[
g=\frac{f\cdot\norm{g}_q}{\norm{f}_p}
\] 
...
Por tanto:
\[
\norm{fg}_1 = \int \abs{fg} = \int \abs{f \cdot \frac{f\cdot\norm{g}_q}{\norm{f}_p}} = \int \abs{f^2 \cdot \frac{\norm{g}_q}{\norm{f}_p}} = \int f^2 \cdot \frac{\norm{g}_q}{\norm{f}_p} = \int fg
\]

Ya solo falta concluir que $\|f\|_p= \operatorname{sup}_{\{g\in L^q: \|g\|_q=1\}} \int fg $, por la desigualdad de Young tenemos que:
\[
\norm{f}_p\norm{g}_q \geq \int fg \Rightarrow \norm{f}_p \geq \frac{1}{\norm{g}_q}\int fg
\]

Tomando $\norm{g}_q=1$, entonces:
\[
\norm{f}_p \geq \int fg \Rightarrow \norm{f}_p =  \operatorname{sup}_{\{g\in L^q: \|g\|_q=1\}} \int fg
\]
...
\[
\left( \int \abs{f}^p dP \right)^{\frac{1}{p}} \leq \left( \int \abs{f}^q dP \right)^{\frac{1}{q}}
\]
...
Vamos a desarrollar la primera igualdad, multiplicamos a ambos lados por $\norm{f}_p$.
\[
(\norm{f}_p)^2 \leq \norm{f}_q\norm{f}_p
\]

Si probamos que $(\norm{f}_p)^2 \leq \|f^2\|_1$ ya habríamos terminado.

La desigualdad de Jensen nos dice que $g(EX) \leq E(g(X))$
...
\end{expla}
$Y_n = n$ si sale cara en el lanzamiento n

$Y_n=-n$ si sale cruz en el lanzamiento n

$P(Y_n=2n)=P(Y_n=-n)=\frac{1}{2}$
...
Esto es así porque $$P(T_n=1) = 1-P(T_n\neq 1) = 1 - P(T_1\neq1 \cap T_2\neq1 \cap ... \cap T_{n-1}\neq1) =$$ $$= 1 - P(Y_1=-1 \cap Y_2=-2 \cap ... \cap Y_{n-1}=2^{n-1}) = 1-\left(\frac{1}{2}\right)^n$$
