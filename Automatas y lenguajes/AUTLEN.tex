...
Donde el 0 implica que esa ``a'' no aparece ninguna vez. Según en qué cuerpo nos movamos, podrá tener distinto significado. Técnicamente, sería el elemento nulo de la suma.
\end{defn}
...
SUJETO PREDICADO $\rightarrow$ 

ARTÍCULO NOMBRE PREDICADO $\rightarrow$ 

el NOMBRE PREDICADO $\rightarrow$ 

el coche PREDICADO $\rightarrow$ 

el coche VERBO $\rightarrow$ 
...
%TODO añadir algo de introducción
...
\item \textbf{Análisis sintáctico}. Consiste en construir el árbol de derivación.
...
En el último ejemplo se observa que recorremos el árbol de derivación de una forma un tanto ``rara''. Veamos cómo se explica este recorrido. 

El resultado observado viene de haber realizado un recorrido en \textbf{profundad por la iquierda y con vuelta atrás}. Esta técnica consiste en recorrer siempre el hijo más a la izquierda, cuando no queden hijos a la izquierda, continuamos con el siguiente hijo más a la izquierda. Cuando no queden hijos volvemos al nodo superior y así sucesivamente.

Si suponemos un caso simple en el que un nodo raíz tiene n hijos, podemos ver que al aplicar este procedimiento recorremos el nodo raíz n+1 veces.
...
Como es lógico, la operación de setType representada en la gramática no puede realizarse hasta conocer el tipo de L, por lo que se realiza en el instante 1.
...
Queremos hacer que esta gramática nos devuelva la profundidad de una expresión, es decir, el máximo número de paréntesis contenidos en otros.

Para ello, en primer lugar debemos añadir a los elementos L e I un atributo profundidad.

Vamos a intentar hacerlo propagando el atributo profundidad hacia arriba. Por ello, un nodo terminal como sería λ debería tener profundidad 0. Por lo que la cuarta regla queda de la forma:
...
Para la tercera regla, podemos ver que la profundidad que asciende será el máximo de las profundiades de cada uno de los símbolos en los que deriva. La tercera regla quedaría:
\[I_1 \rightarrow I_2I_3 \{I_1.prof = max(I_2.prof, I_3.prof)\}\]

Si ahora analizamos los instantes en que se realizaría la función de cada regla y agrupamos todo lo indicado anteriormente lleamos a:
\[G=\{\{(,)\}, \{L(prof),I(prof)\}, L, P\}\]
Con P:
\begin{itemize}
...
\textcolor{red}{Completa este ejemplo \textbf{NIGGA}}
...
En este caso es realmente imporante indicar los instantes en que se realiza cad aacción puesto que de lo contrario habría lugar a confusión. (En los casos anteriores los atributos imponían un orden sin ambigüedad).
...
\chapter{Ejercicios 1º Hoja}
\includepdf[pages={1-}]{pdf/HOJA1.pdf}
