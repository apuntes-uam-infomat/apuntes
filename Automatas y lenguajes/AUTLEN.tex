...
Para calcularlo, dada una regla $\appl{}{X}{\alpha A \{\beta_1,\beta_2,...\}}$, con $\alpha, \beta_i \in \sum^*$
\begin{enumerate}
...
\obs $\$ \in $ Siguiente(axioma) siempre.
...
\begin{example}
Definimos la siguiente gramática extendida para aplicarle tabla de análisis LR(0):

\begin{itemize}
\item B' $\rightarrow$ B \$
\item B $\rightarrow$ bD;Ef
\item D $\rightarrow$ d
\item D $\rightarrow$ D;d
\item E $\rightarrow$ e
...
Por tanto el autómata quedará igual a simple vista (sólo que ahora habra más transiciones a estados de error aunque estas no aparezcan dibujadas). Los cambios lo mostraremos en la tabla de análisis.
