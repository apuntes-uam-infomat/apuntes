...
\chapter{Analizador (morfológico y sintáctico}
%TODO Falta clase del lunes

\section{Tablas de análisis}
Las tablas de análisis consisten en analizar una sentencia de forma ascendente, con el fin de saber si se trata de una sentencia válida para una gramática dada o no.
...
\item E $\rightarrow$ E x E
\item E $\rightarrow$ -E
\item E $\rightarrow$ (E)
\item E $\rightarrow$ 1
\end{itemize}

Y vamos a partir de la sentencia:
\begin{center}
(id+id)xid
\end{center}
...
Se aplicará siempre que sea posibles. No hay elección.
...
Tras este ejemplo podemos definir formalmente el algoritmo de análisis ascendnete.
...
Vamos a trabajar con gramáticas sencillas en las que sólo haya una derivación posible en cada paso. En caso de no ser así deberíamos avanzar todo lo posible para despues hacer backtracking, comprobando que no dejamos caminos sin explorar.
...
\end{itemize}
Queremos comprobar si es válida la sentencia:
\begin{center}
i*i
\end{center}
...
Antes de nada vamos a ver qué es esto:
...
\item T $\rightarrow$ i | (E)
\end{itemize}
%TODO Ni guarrada de que estamos haciendo.
