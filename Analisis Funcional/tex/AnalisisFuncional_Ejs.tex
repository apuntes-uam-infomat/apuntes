...
\begin{problem} Construir en $C_{[0,1]}$ un subconjunto acotado de funciones que no esté totalmente acotado.

\solution

Dado que no hemos visto qué es un conjunto totalmente acotado, lo vemos.

\begin{defn}[Conjunto\IS totalmente acotado] Dado un espacio métrico \sdst, un conjunto $K ⊂ X$ es totalmente acotado si y sólo si $∀ε > 0$ existe una colección finita de bolas de radio ε tal que su unión contiene a $K$.
\end{defn}
...
Una vez hecho esto, calculamos la convolución: \[ f_t(x) = \int_ℝ f(x-y) η_t(y) \dif y \] Dado que las $η_t$ tienen un soporte cada vez más pequeño, esto es, $\sop η_t \convs[][t] \set{0}$, podemos considerar la integral anterior en bolas $\bola_δ(0)$. Por ser $f$ continua podremos acotar su valor en esas bolas, esto es, $f(y) ∈ \bola_ε(x)$ si $x - y ∈ \bola_δ()$
