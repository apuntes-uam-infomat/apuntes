...
\begin{itemize}
\item \nref{thm:AcotacionUniforme}: Una colección $\set{T_α}_{α ∈ A} ⊂ \linapp$ con $X$ Banach está o bien uniformemente acotada o bien hay un \nlref{def:ConjuntoGDelta} denso, $B$, para el cual $\sup_α \abs{T_α(x)}_Y = ∞$ cuando $x ∈ B$. El corolario sencillo es que si demostramos que $\sup_α \abs{T_α(x)}_Y$ para $x ∈ X$, entonces la familia está uniformemente acotada.
