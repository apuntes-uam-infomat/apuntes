...
\begin{proof} [\Fref{prop:ConjDensoNoDifs}]
...
Dado $f$ es uniformemente continua, tenemos que $∀ε > 0$ existe un $δ>0$ tal que $\dst(x,y) ≤ δ$, entonces $\abs{f(x) - f(y)} ≤ \sfrac{ε}{3}$. Por otra parte, como $f_n \to f$ uniformemente en $X$; entonces $\norm{f - f_n}_∞ \to 0$. Luego para el $ε>0$ de antes podemos tomar un $n$ tal que si $n ≥ N$ tenemos que $\norm{f - f_n}_∞ ≤ \sfrac{ε}{3}$. Por lo tanto, juntando tenemos que para $n≥N$ y $\dst(x,y) ≤ δ_1$ esto podemos acotar \eqref{eq:LemAscoliArzela:Cota1} y tenemos que \[2\norm{f-f_n}_∞ + \abs{f(x) -f(y)} ≤ 2 · \sfrac{ε}{3} + \sfrac{ε}{3} = ε \]

Por otra parte, si considero las funciones $\set{f_1, \dotsc, f_N}$, son una cantidad finita de funciones uniformemente continuas en $X$ por el \nref{lem:Weierstrass}. Toda familia finita de funciones uniformemente continuas es equicontinua: $∀ε>0$ y $∀n = 1, \dotsc, N$, tenemos que existe un $δ(ε,n)$ tal que si $\dst(x,y) ≤ δ(ε,n)$, entonces $\abs{f_n(x) - f_n(y)} ≤ ε$. Lo que haremos será tomar el mínimo $δ_2 = δ(ε) = \min_{1≤n≤N} δ(ε,n) > 0$.

Lo que hemos hecho ha sido cubrir todos salvo una cantidad finita, cantidad finita que nos controlamos de otra manera. Por lo tanto, sólo tenemos que tomar $δ = \min \set{δ_1, δ_2}$ para tener lo que buscábamos. % TODO: Arreglar esta cosa.
