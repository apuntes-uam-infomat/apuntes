...
Una cosa que querremos ver\footnote{No sé por qué, parece que tiene que ver algo con el Teorema de Representación de Riesz.} es que esa inyección \eqref{eq:InyeccionDualLp} es sobreyectiva si $1 < p < ∞$. Sabemos que si $2 ≤ p < ∞$ entonces $L^p$ es reflexivo por ser convexo. Si $1 < p ≤ 2$, entonces $Λ(L^p)$ es un subespacio cerrado de $L^q$ y como $L^q$ es reflexivo entonces $Λ(L^p)$ es reflexivo y como Λ es isometría entonces $L^p$ también es reflexivo, luego en resumen $L^p$ es reflexivo si $1 < p < ∞$.
