...
\begin{proof}
Si $G(T)$ es cerrado en $X×Y$, entonces será un espacio de Banach con la norma producto restringida a $G(T)$. Si tomamos un $(x,y) ∈ G(T)$, entonces $y = T(x)$ para $x ∈ X$. La norma de ese vector será \[ \norm{(x,y)}_{X×Y} = \norm{x}_X + \norm{T(x)}_Y \]
...
Como $\bola_X$ es compacta en la topología débil de $X$ por el \nref{thm:Kakutani} y $E ⊂ X$ es un conjunto convexo y cerrado, entonces el cierre débil coincide con el cierre fuerte, luego $\bola_X ∩ E = \bola_E$ es un compacto en la topología débil de $E$, luego aplicando Kakutani (vaya nombrecito) de nuevo se sigue que $E$ es reflexivo (pues ok).
\end{proof}
