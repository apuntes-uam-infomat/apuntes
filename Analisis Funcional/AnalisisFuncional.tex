...
\begin{prop}[Identidad\IS del paralelogramo] Sea $(H, \pesc{·,·})$ un espacio de Hilbert y sean $x,y ∈ H$. Entonces \[ \norm{x+y}^2 + \norm{x-y}^2 = 2 \norm{x}^2 + 2 \norm{y}^2 \]
\end{prop}
...
Una nota: al espacio $\linend$ se le puede dar estructura de álgebra (más concretamente, de \concept{{Á}lgebra\IS de Banach}\footnote{Explicar un poco esto.}) y entonces $\comp H$ es un ideal dentro de ese álgebra.
...
Elemental, mi querido Watson. Ejercicio.
\end{proof}
...
\begin{defn}[Espacio\IS de Hilbert separable] \label{def:EspacioHilbertSeparable}
\end{defn}
...
\begin{proof} Fijamos $\set{φ_j}_{j ≥ 1}$ base ortonormal de $R(A)$, y consideramos $A_n = P_n A P_n$ con $P_n = P_{V_n}$ y $V_n = \spn \set{φ_1, \dotsc, φ_n}$. Estos $A_n$ son autoadjuntos y de rango finito, así que aplicamos el \nref{prop:EspectralRangoFinito}, y por cada $n$ existe un $φ_n$ con $\norm{φ_n} = 1$ y $A_n (φ_n) = \pm \norm{A_n} φ_n$. Y el resto de la demostración se hace moviendo las manos y diciendo que es álgebra lineal elemental.
\end{proof}
