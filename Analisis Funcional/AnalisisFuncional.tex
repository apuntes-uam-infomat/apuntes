...
\begin{abstract}
Apuntes de Análisis Funcional del curso 2015/2016 en la UAM. Cubren una base de espacios métricos y topología (\fref{chap:EspaciosMetricos}), espacios de Banach (\fref{chap:Banach}), topologías débiles (\fref{chap:TopologiasDebiles}) y espacios de Hilbert (\fref{chap:Hilbert}). Además, se incluye un pequeño resumen (\fref{app:Resumen}) y algunos ejercicios resueltos (\fref{app:Ejercicios}).

Nota/aviso: ni los apuntes ni los ejercicios están ni mucho menos completos.
\end{abstract}
...
\begin{defn}[Métrica] \label{def:Metrica} Una métrica $\dst$ en un espacio $X$ es una función $\appl{\dst}{X×X}{ℝ^+}$ que cumple las siguientes propiedades: \begin{enumerate}
...
\begin{defn}[Espacio\IS métrico completo] \label{def:EspacioMetricoCompleto} Un espacio métrico \sdst se dice completo si toda sucesión de Cauchy tiene exactamente un punto de acumulación. En otras palabras, si toda sucesión de Cauchy converge en el espacio.
\end{defn}
...
\begin{defn}[Conjunto\IS diseminado] \label{def:ConjuntoDiseminado} Dado \sdst un espacio métrico y $Y⊂ X$, se dice que es residual o diseminado si $\adh{X}^c$ es denso en $X$.
\end{defn}
...
\begin{theorem}[Teorema\IS del punto fijo de Banach] \label{thm:PuntoFijoBanach} Sea \sdst un espacio métrico completo y $\appl{F}{X}{X}$ una \nlref{def:ApplContractiva}. Entonces $F$ tiene a lo más un punto fijo $x_0$ para el cual $F(x_0) = x_0$.
...
\begin{defn}[Espacio\IS dual topológico] \label{def:DualTopologico} Dado un espacio normado $V$ sobre $\kbb$, se define el espacio dual topológico como \[ V^* = \mathcal{L}(V, \kbb) = \set{\appl{T}{V}{\kbb} \tq T\text{ lineal, continua}} \]
\end{defn}
...
\begin{theorem}[Teorema\IS de la aplicación abierta] \label{thm:AppAbierta} Sean $X$ e $Y$ Banach y $T ∈ \mathcal{L}(X,Y)$ sobreyectiva. Entonces:
...
Los dos siguientes corolarios corresponden al \fref{ej:Hoja2:InversaBiyeccion}.
...
\begin{theorem}[Teorema\IS de la gráfica cerrada] \label{thm:GraficaCerrada} Sean $X$ e $Y$ espacios de Banach y $\appl{T}{X}{Y}$ lineal. Supongamos que el gráfico de $T$, dado por \[ G(T) ≝ \set{(x,T(x)) \tq x ∈ X } ⊂ X × Y \] es cerrado con la topología producto\footnote{Hay otro ejercicio para ver que $(X×Y, \norm{·,·}_{X×Y})$ con $\norm{u,v}_{X×Y} = \norm{u}_X + \norm{v}_Y$ es Banach igualmente.}. Entonces $T ∈ \mathcal{L}(X,Y)$.
\end{theorem}

\begin{proof}
Si $G(T)$ es cerrado en $X×Y$, entonces será un espacio de Banach con la norma producto restringida a $G(T)$ (ver \fref{ej:Hoja2:5}). Si tomamos un $(x,y) ∈ G(T)$, entonces $y = T(x)$ para $x ∈ X$. La norma de ese vector será \[ \norm{(x,y)}_{X×Y} = \norm{x}_X + \norm{T(x)}_Y \]
...
\begin{defn}[Espacio\IS reflexivo] \label{def:EspacioReflexivo} Si $\appl{i}{V}{V^{**}}$ es una isometría sobreyectiva, se dice que $X = (V, \norm{·})$ es reflexivo.
\end{defn}
...
\begin{theorem}[Teorema\IS de isomorfía para espacios de Banach] \label{thm:IsomorfiaBanach} Sean $X$ e $Y$ espacios de Banach y $T ∈ \mathcal{L}(X,Y)$. Entonces, $T$ induce una aplicación \begin{align*}
...
\begin{corol} Si $\img T$ es de dimensión finita, entonces $\img T$ es cerrada y entonces $\quot{X}{\ker T} \cong \img T$.
\end{corol}
...
Por suerte, los espacios $L^p$ son duales con $1 < p < ∞$, y de hecho reflexivos. Para $p = ∞$, se puede ver que $(L^1)^* = L^∞$ cuando el espacio de medida es σ-finito. Todo esto lo veremos en la \fref{sec:EspaciosDualesLp}.
...
\begin{defn}[Convergencia\IS débil] \label{def:ConvergenciaDebil} Sea $X$ un espacio de Banach y $\set{x_n}_{n ∈ ℕ}$ una sucesión en $X$. Entonces se dice que $x_n$ converge a $x$ débilmente ($x_n \wconvs[n] x$) si y sólo si $∀λ ∈ X^*$, $\pesc{λ, x_n} \convs \pesc{λ, x}$.
\end{defn}

\begin{defn}[Convergencia\IS débil-$*$] \label{def:ConvergenciaDebilStar} Sea $X = Y^*$ un espacio de Banach dual y $\set{x_n}_{n ∈ ℕ}$ una sucesión en $X$. Entonces se dice que $x_n$ converge a $x$ débilmente-$*$ ($x_n \wsconvs[n] x$) si y sólo si $∀y ∈ Y$, $\pesc{x_n, y} \convs \pesc{x,y}$.
\end{defn}
...
\begin{theorem}[Teorema\IS de Banach-Alaoglu] \label{thm:BanachAlaoglu} Sea $X = Y^*$ un espacio de Banach dual. Entonces \[ \bola_X = \adh{\bola}_1(0) = \{ x \in X \tq \norm{x}_X \leq 1 \} \] es compacto en la topología débil-$*$ de X.
\end{theorem}
...
Entonces \[ \abs{\pesc{f, z_j}} = \abs{\pesc{f, y_{n_j}} + \pesc{f, z_j - y_{n_j}}} ≤ \abs{\sfrac{δ}{2} + \underbracket{\norm{f}_{Y^*}}_{≤ 1 \impliedby f ∈ \bola_X} \underbracket{\norm{z_j - y_{n_j}}}_{≤ \frac{δ}{2}}} ≤  δ \] y con esto $f ∈ V$.
\end{proof}

\begin{prop} Si $\dim X < ∞$, la topología fuerte, la débil y la débil-$*$ coinciden. \label{prop:TopologiasEspacioFinito}
\end{prop}
...
\begin{proof}
	...
	Sólo hace falta ver que la topología débil-$*$ y la fuerte son iguales, ya que son respectivamente las topologías más débil y fuerte de las tres.

	Como estamos con un mundo feliz de dimensión finita, podemos fijar una base $\mathcal{B} = \set{x_1, \dotsc, x_n}$ de $X$ y otra $\mathcal{B'} = \set{x_1', \dotsc, x_n'}$ del dual $X'$.

	Como $X$ es reflexivo, basta ver que la fuerte y la débil son iguales porque la débil y la débil-$*$ coinciden.

	Como $X' = X^*$, entonces $x_j' ∈ X^*$ y ahora sea $\norm{x}_* = \sum_{j=1}^n \abs{\pesc{x_j', x}}$ con $x ∈ X$, que es una norma porque $x_j'$ son base del dual. Como en un espacio de dimensión finita todas las normas son equivalentes pues ya está, entonces inducen la misma topología.
\end{proof}
...
\begin{corol} Si $X^*$ es de Banach y dual con predual X, los únicos funcionales $*$-débil continuos de $X$
\end{corol}

\begin{lemma}[Lema\IS de Goldstine] \label{lem:Goldstine} Si $X$ es un espacio de Banach, entonces $i(\bola_X)$ es densa en $\bola_{X^{**}}$ si en $\bola_{X^{**}}$ se da la topología $*$-débil. Si además $X$ es reflexivo, entonces:
(a) $i(\bola_X) = \bola_{X^{**}}$.
...
\begin{prop}[Proposición\IS Milman-Pettis] \label{prop:MillmanPettis} Todo espacio de Banach \nlref{def:EspacioUnifConvexo} es reflexivo.
\end{prop}

Por ejemplo, los espacios $L^p$ con $1 < p < ∞$ son uniformemente convexos y por lo tanto reflexivos.
\begin{proof} Dado un $ε ∈ (0,1)$, sea $δ$ el módulo de convexidad uniforme. Esto es, si $\norm{x} = \norm{y} = 1$, con $\norm{x -y } < ε$, entonces $\norm{\frac{x + y}{2}} < 1 - δ$.

Sea $i$ la inyección canónica de $X$ en su doble dual. Sabemos que $i(\bola_X)$ es cerrado en $\bola_{X^{**}}$ con la topología fuerte, por ser $i$ isométrica. Además, $i(\bola_X)$ es denso en $\bola_{X^{**}}$ con la topología débil-$*$ de $X^{**}$ (\nref{lem:Goldstine}). Estas dos cosas no dependen de $X$.
...
Recordamos que el \nref{thm:BanachAlaoglu} nos decía que $\bola_X$ es compacta en la topología débil-$*$.
...
Como $\bola_X$ es compacta en la topología débil de $X$ por el \nref{thm:Kakutani} y $E ⊂ X$ es un conjunto convexo y cerrado, entonces el cierre débil coincide con el cierre fuerte (\fref{ej:Hoja3:5}), luego $\bola_X ∩ E = \bola_E$ es un compacto en la topología débil de $E$, luego aplicando Kakutani (vaya nombrecito) de nuevo se sigue que $E$ es reflexivo (pues ok).
\end{proof}
...
A lo largo de esta sección, vamos a estudiar espacios duales específicos y vamos a caracterizarlos, centrándonos en los espacios de funciones integrables $L^p$ (ver \fref{def:EspacioLp}). Por recordar rápidamente, los espacios $L^p$ eran los espacios de funciones tales que $\int_X \abs{f}^p < ∞$, identificando con una relación de equivalencia las funciones que sean iguales en casi todo punto.  Además, la norma se definía como \[ \norm{f}_p = \left(\int_X \abs{f}^p \dif μ\right)^{\frac{1}{p}} \]
...
Si $p = 1$ y además \meas es un espacio de \nlref{def:MedidaSFinita} también tenemos que $L^1\meas^* = L^∞ \meas$.
...
\begin{prop}[Desigualdad\IS de Hölder] \label{prop:DesHolder} Sea $f ∈ L^p$ y $g ∈ L^q$ con $p,q$ exponentes conjugados\footnote{Igual que antes, tales que $\frac{1}{p} + \frac{1}{q} = 1$.}. Entonces, $fg ∈ L^1$ y \[ \norm{fg}_1 ≤ \norm{f}_p \norm{g}_q \]
\end{prop}
...
\begin{prop}[Desigualdad\IS de Minkowsky] \label{prop:DesMinkowsky} Sean $f,g ∈ L^p$ con $1 ≤ p ≤ ∞$. Entonces $f + g ∈ L^p$ y \[ \norm{f + g}_p ≤ \norm{f}_p + \norm{g}_p \]
\end{prop}
...
Para estimar $A$, usamos la \nlref{prop:DesHolder}, sabiendo que $\abs{f+g}^{p-1} ∈ L^q$ con $q$ el exponente conjugado, ya que $(p-1) q = p$. Entonces nos queda que \[ A= \int_X \abs{f+g}^{p-1} \abs{f} \dif μ ≤ \left(\int_X \abs{f+g}^{(p-1)q} \dif μ\right)^{\frac{1}{q}} \norm{f}_p = \norm{f+g}^{p-1} \norm{f}_p \]
...
\begin{proof} El hecho de que sea normado viene directamente de la \nlref{prop:DesMinkowsky}. Vamos a ver que además es un espacio completo según $p$.
...
Con esto podremos aplicar el \nref{thm:ConvDominada}: sabemos que $\abs{f(x) - g_k(x)}^p ≤ G(x)^p ∈ L^1$ y que $f(x) - g_k(x) \to 0$ si $G(x) < ∞$, luego en resumidas cuentas \[ \int_X \abs{f - g_k}^p \dif μ \convs[][k] 0 \implies \abs{f- g_k}^p \convs[L^1][k]\] y entonces $\set{g_k}_{k ≥ 1}$ tiene límite $f$ en $L^p$.
...
\appl{\tilde{λ}}{L^2&}{\kbb} \\
f & \longmapsto λ(Ωf)
\end{align*}, donde $Ωf ∈ L^1$ si $f ∈ L^2$ por la \nlref{prop:DesHolder}. Vamos a ver que este funcional es continuo: \[ \abs{\tilde{λ} (f)} = \abs{λ(Ωf)} ≤ \norm{λ}_{\dual[L^1]} \norm{Ωf}_1  ≤ \norm{λ}_{\dual[L^1]} \norm{Ω}_2 \norm{f}_2 \]
...
Consideramos $f_n = \sign(u) \ind_{A ∩ X_n} ∈ L^1 ∩ L^∞$. Si evaluamos $λ$ sobre $f_n$, tenemos que \[ λ(f_n ) = \int_X u f_n \dif μ = \int_X u \sign(u)\ind_{A∩ X_n} \dif μ = \int_{A ∩ X_n} \abs{u} \dif μ \eqreasonup[≥]{$\abs{u} > c$} c μ(A ∩ X_n)\], luego entonces finalmente \[ cμ(A∩X_n) ≤ λ(f_n) ≤ \norm{λ}_{\dual[L^1]} \norm{f_n}_1  = \norm{λ}_{\dual[L^1]} \int_X \abs{\sign (u)} \ind_{A ∩ X_n} \dif μ\]

Si $x ∈ A ∩ X_n$ entonces $u(x) ≠ 0$ y por lo tanto $\abs{\sign{u(x)}} = 1$ luego al final ahí hay cosas finitas así que al final al final al final al final al final al final nos queda que \[ (c-\norm{λ}_{\dual[L^1]}) μ(A ∩  X_n) ≤ 0 \] pero como $c - \norm{λ}_{\dual[L^1]} > 0$ entonces sólo puede ser $μ(A ∩ X_n) = 0 ∀n$ luego la conclusión es que $u ∈ L^∞$ porque está acotada con $\norm{u}_∞ ≤ \norm{λ}_{\dual[L^1]}$.
\end{proof}

\chapter{Espacios de Hilbert y operadores compactos}
\label{chap:Hilbert}

Hasta ahora hemos estado trabajando con espacios normados, que en cierto sentido se parecen a los espacios vectoriales habituales de álgebra lineal. La cuestión es que nos falta un ingrediente más. Querríamos, por ejemplo, poder dar una base, tener una noción de ``funciones independientes'' o incluso ver si se pueden trasladar conceptos como los de ortogonalidad que sí tenemos en espacios de dimensión finita.

El resultado son los espacios de Hilbert, espacios de Banach que además se les equipa con un producto escalar que, como veremos en este capítulo, nos permitirán definir todas esas nociones extra.

\section{Espacios de Hilbert: propiedades básicas}

\begin{defn}[Espacio\IS de Hilbert] Un espacio de Hilbert $H$ es un espacio de Banach equipado con un producto escalar $\appl{\pesc{·,·}}{H × H}{\kbb = ℝ\text{ ó } ℂ}$ que cumple:
\begin{enumerate}
...
Sobre los espacios de Hilbert se pueden demostrar varias propiedades ya conocidas sobre espacios vectoriales.

\begin{prop}[Desigualdad\IS de Schwarz] Sea $(H, \pesc{·,·})$ un espacio de Hilbert y sean $x,y ∈ H$. Entonces \[ \abs{\pesc{x,y}} ≤ \norm{x} \norm{y} \]
\end{prop}

\begin{proof} Por bilinealidad, dado $λ ∈ \kbb$ entonces \[ 0 ≤ \pesc{x + λy, x+λy} = \pesc{x, x} + 2 \Re (λ\pesc{x,y})+ \pesc{y,y} \] y sale.
\end{proof}

\begin{prop}[Desigualdad\IS de Minkowsky]  Sea $(H, \pesc{·,·})$ un espacio de Hilbert y sean $x,y ∈ H$. Entonces \[ \norm{x+y} ≤ \norm{x} + \norm{y} \]
\end{prop}

\begin{prop}[Identidad\IS del paralelogramo] \label{prop:Paralelogramo} Sea $(H, \pesc{·,·})$ un espacio de Hilbert y sean $x,y ∈ H$. Entonces \[ \norm{x+y}^2 + \norm{x-y}^2 = 2 \norm{x}^2 + 2 \norm{y}^2 \]
\end{prop}

Esta identidad es especialmente importante porque implica directamente que un espacio de Hilbert es un \nlref{def:EspacioUnifConvexo}, lo que dirá que es isomorfo a su doble dual. De hecho, veremos que incluso un espacios de Hilbert es isomorfo a su dual y que esa es una propiedad definitoria de los espacios de Hilbert (esto es, si un espacio de Banach es isomorfo a su dual entonces es un espacio de Hilbert con una norma equivalente apropiada que proviene de un producto escalar).

\begin{prop}[Identidad\IS de polarización] \label{prop:Polarizacion} Sea $(H, \pesc{·,·})$ un espacio de Hilbert sobre $\kbb$ y sean $x,y ∈ H$. Entonces, si $\kbb = ℝ$ tenemos que \[  \norm{x+y}^2 - \norm{x-y}^2 = 4 \pesc{x,y} \] y si $\kbb = ℂ$ entonces \[ \norm{x+y}^2 - \norm{x-y}^2 + i\norm{x+iy}^2 - i \norm{x-iy}^2 = 4\pesc{x,y} \]
\end{prop}

\subsection{Subespacios convexos, proyecciones ortogonales y base del espacio}

Una de las ventajas importantes de los espacios de Hilbert es el concepto de ortogonalidad, que nos facilitará las cosas bastante en algunas demostraciones.

\begin{prop}[Proyección\IS a un convexo cerrado] \label{prop:Proyeccion}  Sea $(H, \pesc{·,·})$ un espacio de Hilbert y sea $E ⊂ X$ un convexo cerrado y no vacío. Entonces existe un único $x ∈ E$ que minimiza la función $\norm{x}$.
...
Sea $0 ≤ L = \inf_{x ∈ E} \norm{x} < ∞ $. Supongamos que esto se cumple para dos puntos $x_0, x_1 ∈ E$ tales que $\norm{x_0} = \norm{x_1} = L$. La \nlref{prop:Paralelogramo} nos dice entonces que \begin{align*}
\norm{x_0 + x_1}^2 + \norm{x_0 - x_1}^2 &= 2\norm{x_0}^2 + 2\norm{x_1}^2 \\
\norm{x_0 - x_1}^2 &= 4L^2 - \underbracket{\norm{x_0 + x_1}^2}_{≤ (\norm{x_0} + \norm{x_1})^2} \\
\norm{x_0 - x_1}^2 &≤ 4L^2 - (L^2 + L^2 + 2L^2) \\
\norm{x_0 - x_1}^2 &≤ 0
\end{align*} luego $x_0 = x_1$ forzosamente.

\proofpart{Existencia}

Sea $\set{x_n}_{n ≥ 1} ⊂ E$ con $\norm{x_n} \to L$. Se puede demostrar usando de nuevo la identidad del paralelogramo que esta sucesión es de Cauchy, y como $H$ es completo existe el límite $x = \lim x_n$. Como $E$ es cerrado pues $x ∈ E$. Como la norma es continua pues entonces es el mínimo.
\end{proof}

\begin{defn}[Subespacio\IS perpendicular] Sea $(H, \pesc{·,·})$ un espacio de Hilbert y $M ⊂ H$ un subespacio cerrado. Entonces se define el subespacio perpendicular $M^\perp$ de la siguiente forma \[ M^\perp = \set{x ∈ H \tq \pesc{x, y} = 0\; ∀y ∈ M} \]
\end{defn}
...
Tomamos $x_2$ el elemento único que minimiza $\norm{·}$ en $x + M$ (\fref{prop:Proyeccion}), que es convexo y cerrado si y sólo si $M$ lo es. Esto así mismo determina unívocamente a $x_1$ tomando $x = x_1 + x_2$. Vamos a ver que $\pesc{x_1, x_2} = 0$ (esto es, que $x_1 \perp x_2$).
...
\begin{defn}[Conjunto\IS ortonormal] Sea $H$ un espacio de Hilbert y $\base = \set{φ_α}_{α ∈ A} ⊂ H$. Diremos que $\base$ es ortonormal si y sólo si $\norm{φ_α} = 1$ $∀α ∈ A$ y $φ_α \perp α_β$ si $α ≠ β$.
\end{defn}
...
\begin{prop} \label{prop:SpanTopologicoBase} Sea $\base = \set{φ_α}_{α ∈ A} ⊂ H$ un conjunto ortornormal (ON). Entonces \[ x ∈ \adh{\spn \base} \iff  \norm{x}^2 = \sum_{α ∈ A} \abs{\pesc{x, φ_α}}^2\] y se cumple la \concept{Desigualdad\IS de Bessel}: $∀x ∈ H$ tenemos que \[ \sum_{α∈A} \abs{\pesc{x, φ_α}}^2 ≤ \norm{x}^2 \]
\end{prop}
...
\begin{defn}[Base\IS hilbertiana] Sea $H$ un espacio de Hilbert y $\base = \set{φ_α}_{α ∈ A}$ un conjunto ortonormal. Entonces, diremos que es una base hilbertiana si es maximal (esto es, que si existe otro conjunto ortonormal $\base' ⊃ \base$ entonces $\base' = \base$).
\end{defn}

\begin{prop} Sea $H$ un espacio de Hilbert. Fijamos $\base$ ortonormal en $H$. Entonces existe un conjunto $\base' ⊃ \base$ ortornomal y maximal, con $\base' = \set{ φ_α}_{α ∈ A}$, que cumple las siguientes propiedades:
\begin{enumerate}
\item $∀x ∈ H$, $\norm{x}^2 = \sum_{α ∈ A} \abs{\pesc{x, φ_α}}^2$.
\item $∀x,y ∈ H$, $\pesc{x,y} = \sum_{α ∈ A} \pesc{x, φ_a}\conj{\pesc{y, φ_α}}$.
\item La aplicación \begin{align*}
\appl{\fourier}{H&}{\ell^2(A, μ_{\mathrm{disc}})} \\
x &\longmapsto \set{\hat{x}(α)}_{α ∈ A}
\end{align*} con $\hat{x}(α) = \pesc{x, φ_a}$ la \concept{Componente\IS de Fourier} respecto de $\base'$, define un isomorfismo de espacios de Hilbert entre $H$ y $\ell^2(A, μ_{\mathrm{disc}})$ donde este último espacio es el de sucesiones cuadrado-sumables en $A$ equipado con la medida de contar.
\end{enumerate}
\end{prop}
...
Si $\mathcal{F} = \set{\base' ⊃ \base \tq \base' \text{es ortonormal}}$, es no vacío, con un orden parcial y cota superior (unión de todos los elementos). Entonces podemos aplicar el Lema de Zorn y nos sale.

En particular, todo espacio de Hilbert admite bases hilbertianas.

\proofpart{a)}

Si $∃x ∈ H$ con $\norm{x}^2 > \sum_{α ∈ A} \abs{\pesc{x, φ_α}}^2$, implicaría por la \fref{prop:SpanTopologicoBase} que $x ∉ \adh{\spn \base'} = V_0$.  Ahora descomponemos $x = x_0 + x_1$ con $x_1 = P_{V_0^\perp} (x)$, que forzosamente es no nulo porque $x ∉ V_0$.

Tomamos entonces $\base'' = \base'  \set{\frac{x_0}{\norm{x_0}}}$, con $\base' \subsetneq \base''$ y además ortonormal. Pero esto contradice la maximalidad de $B'$, luego $x_1 = 0$ y $x ∈ V_0$.

\proofpart{b)}

Consecuencia directa de la identidad anterior y la \nref{prop:Polarizacion}.

\proofpart{c)}

Reformulación de la segunda afirmación. Una vez que sabemos escribir los productos escalares vemos que tienen que ser cuadrado-sumables (si no $x$ tendría norma infinita). Además, vemos que \[ \pesc{x,y} = \sum_{α ∈ A} \hat{x}(α) \conj{\hat{y}(α)} \], lo que implica que $\fourier$ es una isometría sobreyectiva, ya que podemos reconstruir a partir de los coeficientes: \[ x = \sum_{α ∈ A} \hat{x}(α) φ_α ∈ H\]

Sólo puede haber un conjunto numerable $A_1 ⊂ A$ de modo que $\hat{x}(α) ≠ 0 \implies α ∈ A_1$.
\end{proof}

\section{Operadores compactos en espacios de Hilbert}

Esta definición es muy conveniente para poder establecer el teorema espectral.

\begin{defn}[Operador\IS compacto] \label{def:OperadorCompacto} Sea $H$ un espacio de Hilbert y $\appl{k}{H}{H}$ lineal. Se dice que es compacto si, dada una sucesión $x_n \wconvs x$ entonces $k(x_n) \to k(x)$ con convergencia fuerte.

Como notación, diremos que $k ∈ \mop{Comp}\, H$.
\end{defn}

Otra notación: $\linend \equiv \linapp[H][H]$.

\begin{prop} Sea $H$ un espacio de Hilbert. Entonces, se cumplen las siguientes propiedades:

\begin{enumerate}
\item $k ∈ \comp H \implies k ∈ \linend$.
\item \label{prp:OpComp:ProductoAcotado} $k ∈ \comp H,\, A ∈ \linend \implies Ak, kA ∈ \comp H$.
\item Si $\set{k_n}_{n ∈ ℕ} ⊂  \comp H$ y $k_n \to k$ en $\linend$, entonces $k ∈ \comp H$.
\end{enumerate}
\end{prop}
...
Trivial por la definición de convergencias nosequé $x_n \wconvs x \implies k(x_n) \wconvs k(x)$.

\proofpart{b)}

Supongamos $x_n \wconvs x$ y $A ∈ \linend$, de tal forma que $A(x_n) \wconvs A(x)$. Así, $k(A(x_n)) \to k(A(x)$ por ser $k$ compacto, luego $kA$ es compacto.

En el otro sentido sabemos que $k(x_n) \to k(x)$, así que $A(k(x_n)) \to A(k(x))$ por ser $A$ acotado, así que $Ak$ también es compacto.

\proofpart{c)}

Sea $x_j \wconvs x$. Entonces $k_n(x_j) \to k_n(x)$ para todo $n = 1, 2, \dotsc$. Vamos a tratar de evaluar la norma siguiente:
\begin{align*}
\norm{k(x) - k(x_j)} &= \norm{(k(x) - k_n(x)) + (k_n(x) - k_n(x_j)) + (k_n(x_j) - k(x_j))} \\
	&≤
		  \underbracket{\norm{k(x) - k_n(x)}}_{≤ \norm{k - k_n}\norm{x}}
		+ 				\norm{k_n(x) - k_n(x_j)}
		+ \underbracket{\norm{k_n(x_j) - k(x_j)}}_{≤ \norm{k - k_n}\norm{x_j}} \\
	&≤
		\norm{k - k_n}(\norm{x} + \norm{x_j}) + \norm{k_n(x) - k_n(x_j)}
\end{align*} que es una estimación válida para cualesquiera $j,n$.

Como $x_j \wconvs x$, en particular la sucesión es acotada así que $\norm{x_j} ≤ C < ∞$ $∀j$. Eso implica que \[ \norm{k(x) - k(x_j)} ≤ \norm{k - k_n}(\norm{x} + C) + \norm{k_n(x) - k_n(x_j)} \]

Fijando $ε > 0$, como $\norm{k - k_n} \to 0$ en $\linend$ tenemos que existe un $n$ con $\norm{k - k_m}(\norm{x} + C) ≤ \sfrac{ε}{2}$, y así tenemos que \[ \norm{k(x) - k(x_j)} ≤ \frac{ε}{2} + \norm{k_n(x) - k_n(x_j)} ≤ ε\] si $j ≥ J$ por tener que $k_n(x) \to k(x_j)$ así que el limsup es menor que ε y tiende a cero.
\end{proof}

Una nota: al espacio $\linend$ se le puede dar estructura de álgebra (más concretamente, de \concept{{Á}lgebra\IS de Banach}\footnote{Un álgebra de Banach es espacio con una suma, producto asociativo y producto por escalares, y con una norma que cumple $\norm{xy} ≤ \norm{x} \norm{y}$.}) y entonces $\comp H$ es un ideal dentro de ese álgebra.

\begin{defn}[Operador\IS adjunto] Sea $H$ un espacio de Hilbert y $A ∈ \linend$. Entonces definimos su adjunto $A^*$ como el operador lineal tal que para cualesquiera $x,y ∈ H$ tenemos que \[ \pesc{A^*(x), y} = \pesc{x, A(x)} \]
\end{defn}

\begin{prop} Sea $H$ un espacio de Hilbert y $A ∈ \linend$. Entonces se cumple que:

\begin{enumerate}
\item $A^* ∈ \linend$ y además $\norm{A^*} = \norm{A}$.
\item $\ddual[A] = A$.
\item $\dual[A]$ es compacto si y sólo si $A$ lo es.
\item $\dual[AB] = \dual[B] \dual[A]$.
\end{enumerate}
\end{prop}
...
Vemos que $A^*(x)$ está bien definido. Fijamos $x ∈ H$, entonces la aplicación $y \to \pesc{x, A(y)}$ es antilineal y continua.

Ahora aplicamos el \nref{thm:Riesz}, que nos identifica $H^* = H$ y nos dice que existe un único elemento $A^*(x) ∈ H$ con $\pesc{A^*(x), y} = \pesc{x, A(y)}$.

Que $A^*$ es lineal es trivial. En cuanto a la acotación, podemos darla usando el \nref{thm:HahnBanach} por alguna parte como \[ \norm{A^*(x)} = \sup_{y ∈ \bola_H} \abs{\pesc{A^*(x), y}} = \sup_{y ∈ \bola_H} \abs{\pesc{y, A(x)}} ≤ \norm{A}\norm{x} \norm{y} ≤ \norm{A}{\norm{x}}\]

La igualdad de la norma viene de tomar el doble dual y recuperamos el operador, así que $\norm{\ddual[A]} ≤ \norm{A^*}$.

\proofpart{b)}

Sean $x, y ∈ H$ cualesquiera. Entonces \[ \pesc{\ddual[A], y} = \pesc{x, A^*(y)} = \conj{\pesc{A^*(y), x}} = \conj{\pesc{y, A(x)}} = \pesc{A(x), y} \]

Como $x,y$ son arbitrarios, entonces eso implica que $A^** = A$.

\proofpart{c)}

Si $x_n \wconvs x$, entonces \begin{align*}
\norm{A^*(x_n) - A^*(x)}^2
	&= \pesc{A^*(x_n) - A^*(x), A^*(x_n) - A^*(x)} = \\
	&= \pesc{A^*(x_n), A^*(x)} + \pesc{A^*(x), A^*(x)} - 2 \Re \pesc{A^*(x), A^*(x_n)} = \\
	&= \pesc{AA^*(x_n), x_n} + \pesc{AA^*(x), x} - 2 \Re \pesc{AA^*(x), x_n}  \\
\end{align*}

Si $A$ es compacto, entonces $AA^*$ es compacto por la \fref{prp:OpComp:ProductoAcotado} así que $\pesc{AA^*(x_n), x_n} \to \pesc{AA^*(x), x)} = \norm{A^*(x)}^2$, lo mismo con $\pesc{AA^*(x), x_n)} \to \norm{A^*(x)}^2$. De ahí se deduce que si $A$ es compacto $A^*$ lo es, y como $\ddual[A] = A$ pues tenemos la implicación al otro lado.
...
\begin{defn}[Operador\IS autoadjunto] \label{def:OperadorAutoadjunto} Sea $H$ un espacio de Hilbert y $A ∈ \linend$. Se dice que es autoadjunto si $A = A^*$.
\end{defn}

Por ejemplo, si $V ⊂ H$ es un subespacio cerrado, entonces $P_V$ es autoadjunto. Sólo hay que ver que $P_V · P_V = P_V$ y con eso operar en el producto escalar.

\begin{defn}[Operador\IS positivo] Sea $H$ un espacio de Hilbert y $A ∈ \linend$. Diremos que $A$ es positivo si $\pesc{A(x), x} ≥ 0$ para todo $x ∈ H$.
\end{defn}

\begin{prop} Si $A$ es positivo, entonces $A = A^*$.
\end{prop}

\begin{proof} Sale directamente usando la \nref{prop:Polarizacion}.
\end{proof}

De nuevo, la proyección ortogonal es $≥ 0$. Si se hacen las cuentas también sale.

\begin{defn}[Autovalor] Sea $A ∈ \linend$ un operador lineal. Diremos que $λ ∈ ℂ$ es un autovalor de $A$ si y sólo si $∃ x ≠ 0$ tal que $A(x) = λx$.

Igual que en otros casos, $x$ será el \concept{Autovector} correspondiente.
\end{defn}

\begin{prop} Sea $A ∈ \linend$ un operador autoadjunto. Entonces, si $A$ es un \nlref{def:OperadorAutoadjunto}, entonces sus operadores son reales, y los autovectores correspondientes a autovalores distintos son ortogonales.

Si $A$ es un operador positivo y λ es un autovalor, entonces $λ ≥ 0$.
\end{prop}

\begin{proof} Álgebra lineal, se deja como ejercicio para el lector.
\end{proof}

\begin{defn}[Rango\IS de un operador] Sea $A ∈ \linend$. Se define su rango como $R(A) = \img A$.
\end{defn}

\begin{prop} Si $A ∈ \linend H$ y $A$ es de rango finito ($\dim R(A) < ∞$), entonces $A$ es un \nlref{def:OperadorCompacto}.
\end{prop}

\begin{proof}
Sea $x_n \wconvs x$ en $H$. Como $A(x_n) ∈ R(A)$ y $\dim R(A)$ es de dimensión finita, entonces la topología fuerte y la débil coinciden (\fref{prop:TopologiasEspacioFinito}), tenemos que $A(x_n) \wconvs A(x)$ por ser $A$ acotado y eso es equivalente a que $A(x_n) \to x$.
\end{proof}

\begin{defn}[Espacio\IS de Hilbert separable] \label{def:EspacioHilbertSeparable} Se dice que un spacio de Hilbert es separable si y sólo si admite una base ortonormal numerable.
\end{defn}

\begin{prop} Sea $H$ un \nlref{def:EspacioHilbertSeparable} y $k ∈ \comp H$. Entonces existe una sucesión $\set{k_n}_{n ≥ 1}$ de operadores de rango finito con $\norm{k - k_n} \to 0$.
\end{prop}

\begin{proof} Si $\dim H < ∞$ no hay nada que probar, todos los operadores tienen rango finito.

En el otro caso, por ser $H$ separable existe una base $\base = \set{φ_n}_{n ≥ 1}$ ortonormal numerable de $H$. Sea $λ_n = \sup \set{\norm{k(ψ)} \tq ψ ∈ V_n^\perp,\, \norm{ψ}= 1}$ siendo $V_n = \spn \set{φ_1, \dotsc, φ_n}$ y $V_n^\perp = \adh{\spn \set{φ_k}_{k > n}}$.

Es fácil ver que los espacios $V_n^\perp$ son decrecientes, así que $λ_n ≥ λ_{n + 1} ≥ 0$ para cualquier $n$. Eso lleva a la existencia del límite $λ = \lim λ_n$ con $λ_n ≥ λ ≥ 0\; ∀n$.

Vamos a ver que $λ = 0$. Si fuese estrictamente positivo, entonces $\norm{k(φ_n)} ≥ λ\; ∀n$, y como $\set{φ_n}$ es un sistema ortonormal infinito entonces $\sum_{n≥1} \abs{\pesc{x, φ_n}}^2 < ∞$ luego $\pesc{x, φ_n} \to 0$ así que $φ_n \wconvs 0$, luego por ser $k$ compacto tenemos que $k(φ_n) \to k(0) = 0$.

Por tanto, $λ = 0$ si y sólo si $\lim λ_n = 0$. Definimos ahora $k_n = P_{V_n} k$, esto es, \[ k_n(x) = \sum_{j = 1}^n \pesc{x, φ_j} k(φ_j)\]

$K_n$ es de rango finito porque $R(k_n) ⊂ R(P_{V_n}) ⊂ V_n$ que es finito. Y aquí me he periddo completamente.
\end{proof}

\begin{prop} Si $A ∈ \linend$ es autoadjunto, entonces $\adh{R(A)}$ y $\ker A$ son invariantes por $A$ y $H = \ker A \oplus \adh{R(A)}$.
\end{prop}

\begin{prop}[Teorema\IS espectral (rango finito)] \label{prop:EspectralRangoFinito} Sea $A ∈ \linend$ autoadjunto y de rango finito. Entonces podemos escribir $A$ como \[ A(x) = \sum_{j = 1}^n λ_n \pesc{x, φ_j} φ_j\] con $λ_1, \dotsc, λ_n ∈ ℝ$ autovalres de $A$ y $\set{φ_j}_{j=1}^n$ una base ortonormal de $R(A)$.

Además, $\norm{A} = \max_{1 ≤ j ≤ n} \abs{λ_j}$.
\end{prop}

\begin{proof}

Sea $A_1 = A (sombrilla) R(A)$, donde $R(A)$ es cerrado por ser finito-dimensional.

$A_1$ es autoadjunto en $R(A)$ por serlo $A$. Entonces \[ A_1(x) = \sum_{j = 1}^n λ_j \pesc{x, φ_j} φ_j \] por álgebra lineal elemental.

Además, si $x ∈ H$ entonces $x = P_{R(A)} (x) + P_{R(A)^\perp} (x)$, pero $R(A)^\perp = \ker A$ así que $A(x) = A_1(x)$.

Para la parte de la norma, vemos que \[ \norm{A(x)}^2 = \sum_{j = 1}^n \underbracket{λ_j^2}_{≤ M^2} \abs{\pesc{x, φ_j}}^2 ≤ M^2 \norm{x}^2 \] con $M = \max_{1 ≤ j ≤ n} \abs{λ_n}$.
\end{proof}

\begin{theorem}[Teorema\IS espectral] Sea $A ∈ \linend$ autoadjunto y compacto en $H$ espacio de Hilbert separable. Entonces podemos escribir \[ A(x) = \sum_{j ≥ 1} λ_j \pesc{x, φ_j} φ_j \] con $λ_1, \dotsc, λ_n ∈ ℝ$ autovalores de $A$ y $\set{φ_j}_{j ≥ 1}$ una base ortonormal de $\adh{R(A)}$.

Además, $\sup_{j ≥ 1} \abs{λ_j} = \max_{j ≥ 1} \abs{λ_j} = \norm{A}$.

Por último, cada autovalor $λ_j$ tiene multiplicidad finita y $λ_j \convs[][j] 0$.
\end{theorem}

\begin{proof}[Idea] Fijamos $\set{φ_j}_{j ≥ 1}$ base ortonormal de $R(A)$, y consideramos $A_n = P_n A P_n$ con $P_n = P_{V_n}$ y $V_n = \spn \set{φ_1, \dotsc, φ_n}$. Estos $A_n$ son autoadjuntos y de rango finito, así que aplicamos el \nref{prop:EspectralRangoFinito}, y por cada $n$ existe un $φ_n$ con $\norm{φ_n} = 1$ y $A_n (φ_n) = \pm \norm{A_n} φ_n$.
\end{proof}
...
\label{app:Resumen}
\input{tex/AF_Resumen.tex}

\chapter{Otros conceptos}
\label{app:OtrosConceptos}
\input{tex/AF_OtrosConceptos.tex}

\chapter{Ejercicios}
\label{app:Ejercicios}
