...
Así, una vez el usuario introduce su PIN, el cajero calcular $f(ID,PIN)$ y comprueba si coincide el valor.
...
\item La tarjeta envía a la central $(ID,ID\_TRANSACCION)$, es decir, envía un identificador del usuario que está accediendo al cajero y un código ``aleatorio'' que identifica la transacción.

\item La central responde con $f_e(ID.PIN)=G$
