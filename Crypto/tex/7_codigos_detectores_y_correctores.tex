...
En general, empleamos los códigos dectector/correctores de errores para disminuir la probabilidad de leer mal un mensaje.
...
Genralizando a un canal por el que transmitimos $q$ posibles valores con probabilidad $p$ de que haya un error, consideramos que enviando un símbolo cualquiera, podremos recibir cualquier otro con probabilidad $\frac{p}{q}$.
...
puesto que si sólo se produce un error lo corregiremos correctamente empleando la técnica del vecino más próximo.
...
Nuestro objetivo como matemáticos será minimizar la probabilidad de error en el código.

\begin{defn}[($n,M,d$)-código q-ario]

Sea $F_q$ un alfabeto con $|F_q|=q$.

Decimos que un $(n,M,d)$-código q-ario es un código $\algbC$ tal que
...
\[|\algbC| \leq |F_q^{n-d+1}| \implies n \leq q^{n-d+1}\]
...
\[x,y \in F_1^n \ \ F(x)=F(y) \implies d(x,y) \leq d-1\]
Pero la distancia mínima es, por definición $d$ por lo que no es posible que tengamos dos plabras a distancia $d-1$ a no ser que sean iguales.
...
\begin{example}
Consideremos un código con $A_2(5,3)$, que sabemos es menor o igual que 4 por la cota de Singleton.

Para comprobar que se cumple la cota de Singleton, debemos ver que existe un $(5,4,3)$-código binario y que no existe un $(5,5,3)$-código binario.
...
Ahora tenemos que ver que no existe ninguno con 5 términos. Para ello antes debemos ver una definición.
...
\begin{prop}
Dos códigos equivalentes tienen los mismo parámetros y la misma capacidad de detectar/corregir errores, aunque los algoritmos a emplear será distintos en cada caso.
\end{prop}
...
Además
\[\nexists x \in \algbC \tq w(x)=1,2 \text{ puesto que } d=3\]

Por otro lado, si tomamos $w(x)=4$, $w(y)=4$ y $w(z)=4$ tenemos $d(x,y)=1$, $d(y,z)=2$.

Con esto tenemos que en nuestro código de longitud 5 debe haber como mucho 1 palabra de peso 5, otra de peso 0 y ninguna de pesos 2 ni 4. Por tanto, debe haber, como mínimo 3 palabras de peso 3.

Si nos ponemos a buscar estas palabras vemos que sólo hay dos opciones de palabras de peso 3 que disten, al menos 3. Estas palabras son $11100$ y $00111$.

Pero entonces aún nos falta una palabra para nuestro código con lo que acabamos de comprobar que no es posible la existencia del susodicho código.
\end{example}