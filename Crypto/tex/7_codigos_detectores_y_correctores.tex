...
\begin{enumerate}
\item \textbf{EAN}: European Articlle Number
...
\[x_0,x_1,...,x_{12}\]
en el caso EAN y sin el $x_0$ para el caso UPC.

De forma general, la estructura general de un código es:
...
Con lo que hemos estudiado hasta ahora de códigos podemos ver que, para el caso EAN/UPC tenemos:
\[n=13/12, \ \ k=12/11, \ \ M = 10^{12}/10^{11}\]

La distancia mínima de estos códigos es, como mucho, 2, puesto que cualquier combinación de números es válida siempre que coincida en bit de redundancia.

\subsection{Detección de errores}
A priori no sabemos si es posible cambiar un único número sin necesidad de cmabiar el código de redundancia.
...
El sistema no será capaz de detectar el error siempre que $e$ y $3e$ sean distintos de 0 módulo 10.
...
Con esta demostración tenemos claro que el código de barras va a siempre detectar un error. Sin embargo, es posible que se produzcan dos errores sin que el sistema sea capaz de detectarlo.
...
Veamos qué ocurre en función del valor de $A$.
\begin{itemize}
...
\[\left(\sum_{i=1}^5x_{2i} + 3 \sum_{i=0}^5x_{2i+1} \right)\mod 10 = 2 \neq 0\]
