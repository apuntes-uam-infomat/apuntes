...
En cualquier momento puede ocurrir un problema que dificulte la recepción del mensaje por parte del robot, ya sea el ruido del propio canal de comunicación, o la aparición de un rayo cósmico que modifique algún bit.
...
Si empleamos el mismo método que en el caso anterior, tendríamos que el vecino más cercano es $m=S$ pero estamos restringiéndonos a asumir que sólo hay un error. No obstante, el mensaje podría haber sido $m=E$ y encontrarnos con dos errores.
...
Si utilizamos $\algb{C} \subset F_q^n$ con $|G|=M$ para transmitir $M$ mensajes, lo que queremos es que las palabras empleadas estén lo más espaciadas posible, de modo que un pequeño cambio en un elemento de una palabra no nos haga caer en otra palabra válida. Es decir, queremos que la distancia mínima del código, es decir, $d(\algb{C})$ sea grande.
...
Si el número de errores es menor o igual que $d-1$, es imposible que desde un $x\in \algb{C}$ lleguemos a otro $\tilde{x} \in \algb{C}$, puesto que, por definición, $d(x,\tilde{x})\geq d$.
...
\begin{defn}[Código binario de repetición de longitud n]
...
Con el código que acabamos de definir tenemos:
...
En este caso, cuando el código de barras va a comprobar la validez del código leído, calcula:
...
Es evidente que $e=0 \mod 10$ siendo $e \in \ent_{10}$ solamente si $e=0$, caso que no nos preocupa ya que entonces no habría error.
...
Siempre que esta permutación se produzca entre dos cifras en posición par o dos cifras en posición impar el error puede no ser detectable de manera obvia.

Veamos que ocurre cuando se permutan dos cifras en posiciones par e impar respectivamente.
...
De forma general, si cambiamos solamente el parámetro del código de barras, es decir, si tomamos:
...
Detecta el intercambio de cifras en posición par con cifras en posición impar, pero puede cambiar un único dígito sin darnos cuenta, es decir, la distancia mínima pasaría a ser 1.
...
La opción empleada en el ISBN es emplear un ``X'', que representa con un sólo carácter el número 10 en el sistema romano.
...
Por tanto tenemos que la letra de control se utiliza para comprobar la validez del NIF dado atendiendo a la fórmula:
...
Una vez sabemos cuáles son los posibles valores de cada carácter del NIF podemos ver que
...
\subsection{Corrección de errores}
De la misma forma, también será posible recuperar un carácter borrado (sabiendo a qué posición pertenecía) resolviendo la ecuación:
...
Generalizando este problema a un canal por el que transmitimos $q$ posibles valores con probabilidad de error $p$, consideramos que enviando un símbolo cualquiera, podremos recibir cualquier otro con probabilidad $\frac{p}{q}$.
...
Pero la distancia mínima es, por definición $d$ por lo que no es posible que tengamos dos palabras a distancia $d-1$ a no ser que sean iguales, con lo que la distancia entre ellas sería 0.
...
Si escribimos todas las palabras posibles una debajo de otra formando una matriz, esta operación es equivalente a permutar dos columnas de la matriz.
...
\begin{lemma}\label{lemma:todo_codigo_0}
...
Por tanto, como en cada carácter tenemos un total de $q$ posibles valores, si tenemos la palabra 0...0, otra posible palabra será 1...1 y así sucesivamente hasta los $q$ posibles caracteres.
\end{itemize}
...
\end{prop}
\begin{corol}\label{corol:existencia_codigos_binarios}
Si $d$ es par
\[A_2(n,d)=A_2(n-1,d-1)\]
\end{corol}
...
\[ 2 = \frac{2^{2t+1}}{\sum_{i=0}^t {2t+1 \choose i} } \]
...
\begin{align*}
    ...
    &= \sum_{j=0}^{2t+1}{2t+1 \choose j} = 2^{2t+1}
\end{align*}

\end{enumerate}

\subsection{Código de Hamming}
\begin{defn}[Código de Hamming(3,2)]\label{def:codigo_hamming}
...
Si algún círculo no satisface la condición, la regla del vecino más próximo nos que el código original puede obtenerse modificando el bit que se encuentra en la intersección de los círculos que causan el problema.
