...
Vamos a ver algunos ejemplos de códigos con ciertos $A_q$.
...
\begin{lemma}
En $\mathbb{F}_2^n$ se cumple que
...
Cuando hay un 1 en tanto en x como en y, $w(x+y)$ será 0, por lo que en ese caso deberemos restar 2.
...
\[\tilde{\algbC} = \left\{x \in \mathbb{F}_2^(n+1)\tq (x_1,...,x_n) \in \algbC \ \ \sum_{i=1}^{n+1}x_i = 0 \mod 2\right\}\]
...
Por construcción de $\tilde{\algbC}$ ya tenemos que $d+1 \geq d(\tilde{\algbC})\geq 1$ puesto que los primeros $n$ caracteres de las palabras no pueden distar menos de $d$.
...
\[\appl{π}{\mathbb{F}_q^{n+1}}{\mathbb{F}_q^n}\]
