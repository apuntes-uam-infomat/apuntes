...
A lo largo de este capítulo veremos los métodos que han sido empleados a lo largo de la historia para lograr este objetivo.
	...
	\section{Criptografía}
		...
		\section{Criptosistema afín (sobre letras)}
			...
			El recíproco de 1 es cierto: Usando que $f_{a,b}$ es inyectiva y aplicando la definición de inyectiva:
						$$ \exists \alpha \in \mathbb{Z}/N : a \alpha = 1 \Leftrightarrow a \alpha + b = 1 + b $$

			\begin{prop}
				$f_{a,b}$ inyectiva $\iff a \in U(\mathbb{Z}/N)$
				...
				\begin{proof}
					...
					\begin{itemize}
					\item $\Rightarrow$)
						Si la función es inyectiva, como nos estamos moviendo entre cuerpos finitos, la función será también sobreyectiva.

						Por ser sobreyectiva sabemos que, en concreto:
						\[\exists α \tq f_{a,b}(α)=1+b \implies aα+b = 1+b \implies aα=1 \implies α=1^{-1}\]

					\item $\Leftarrow$)

						Si $a\in U(\mathbb{Z}/N)$ sabemos que existe su inverso. Sabiendo esto, es fácil ver que
						\[f_{a,b}(x_1)=f_{a,b}(x_2) \implies ax_1+b=ax_2+b \implies a(x_1-x_2)=0 \implies \atop x_1-x_2=a^{-1}\cdot 0 \implies x_1-x_2=0 \implies x_1=x_2\]
			...
			$$ \#E = \#U(\mathbb{Z}/N) * \#\mathbb{Z}/N = \varphi(N)*N $$
			...
			\subsection{La función de Euler}
				...
				Vamos a demostrar esta fórmula que acabamos de ver por inducción:
				\begin{proof}
				Es evidente que en el caso $n=1$ la fórmula es correcta.

				Suponemos que la hipótesis es válida para un cierto $n$ y tratamos de ver qué ocurre con $n+1$.

				Considerando el número $p^{n+1}$ vamos a contar todos los enteros menores que él y eliminaremos de la cuenta aquellos enteros que sabemos no son coprimos.

				\begin{enumerate}

				\item Empezamos teniendo $p^{n+1}-1$ enteros menores que $p^{n+1}$.

				\item Sabemos que todos los números que no sean coprimos con $p^n$ tampoco lo seρán con $p^{n+1}$, con lo que debemos eliminar de la lista:
				\[\varphi(p^n)=p^n-p^{n-1} \text{(hipótesis de inducción)}\]


				\item Por último debemos eliminar de la lista los múltiplos de $p$, de $p^2$, $p^3$, ... $p^n$ comprendidos entre $p^{n+1}$ y $p^n$.

				Es sencillo ver que en este intervalo tendremos \\$p-1$ múltiplos de $p^n$.

				Así también tendremos $p^2-p$ múltiplos de $p^{n-1}$. En general, tendremos\\
				$p^i-p^{i-1}$ múltiplos de $p^i$.
				\end{enumerate}

				Combinando lo que acabamos de ver tenemos:
				\[\varphi(p^{n+1})=p^{n+1}-1-p^n+p^{n-1}-\sum_{i=1}^{n-1}p^{i}-p^{i-1}\]

				En cuanto nos percatamos de que estamos ante una suma telescópica es sencillo comprobar que:
				\[\varphi(p^{n+1}) = p^{n+1}-p^n\]

				con lo que damos por concluida la demostración por inducción.
				\end{proof}

				Como es una función multiplicativa, siendo $N=\prod p_i^{n_1}$ la factorización de $N$, podemos escribir:

				\[\varphi(N) =\prod \varphi(p_i^{n_i}) = \prod p_i^{n_i} \prod \left(1- \frac{1}{p_i}\right) = N \prod\left(1 - \frac{1}{p_i}\right)\]
				\label{func:Euler}
				\end{defn}
...
Otra posibilidad es acudir a la tabla de frecuencias y ver cuál de los dos sistemas nos permite obtener la siguiente letra más frecuente. Si las cosas funcionan bien, sólo uno de los dos sistemas nos funcionará.
...
Tras estos ejemplos hay una duda que debería surgir directamente en el lector. En ambos ejemplos nos hemos basado en un absoluto conocimiento acerca del sistema alfanumérico empleado en el mensaje original, pero esto no es realista.

Para explicar esto debemos fijarnos en el \textbf{Principio de Kerckhoffs} que dice:
\begin{prop}[Principio de Kerckhoffs]
...
El principal problema del sistema de cifrado César es que sólo existen 26 posibles claves (correctamente hay N posibles claves siendo N el tamaño del alfabeto empleado), que es un número muy pequeño.
...
El problema de los sistemas que hemos visto, y sobre todo del último, es que es necesario que el emisor y el receptor del mensaje se pongan de acuerdo en la relación alfabeto original - alfabeto cifrado que se va a emplear.
...
En esta ocasión emplearemos como reducción el isomorfismo de anillos:
...
Ya tenemos la matriz módulo 13, si conseguimos calcular también esta matriz módulo 2, por el \textbf{Teorema chino del resto}, tendríamos la matriz módulo 26 que buscamos.
...
Recordando que necesitamos que esta matriz sea invertible y que hay sólo 6 matrices invertibles de orden 2 con coeficientes $1$s y $0$s, podemos anticipar que la matriz $A$ tendrá dos formas posibles:
\[A=\left( \begin{array}{cc}
1 & 0 \\
...
\subsubsection{Explicación matemática del índice de coincidencia}

Como ya hemos comentado, la forma práctica de hacer uso del \textbf{índice de coincidencia} consiste en tomar el texto cifrado y escribir debajo de él una copia del mismo. Una vez tenemos las letras dispuestas de esta forma desplazamos el texto inferior una poisición a la derecha y comprobamos con que frecuencia letras colocadas una bajo la otra coinciden. Tras esto desplazamos una vez más y volvemos a calcular este valor y así sucesivamente.
...
\[I.C. = \frac{1}{(n-1)^2}\sum_{i=1}^r\sum_{j=i}^r\ind_{\{a_i=a_j\}}\footnote{La función $\ind_{\{a_i=a_j\}}$ toma el valor 1 únicamente cuando $a_i=a_j$, valiendo 0 en caso contrario.}\]

Por otro lado, el valor que se calcula al aplicar el procedimiento que acabamos de ver es:
\[α = \frac{1}{n-1} \sum_{i=1}^r \ind_{\{a_i=a_{i+1}\}}\]
...
\[\forall j < n \ α \simeq \frac{1}{n-1}\sum_{i=1}^r\ind_{\{a_i=a_{i+j}\}}\]

Teniendo esto en cuenta basta con escribir:
\[I.C. = \frac{1}{(n-1)^2}\sum_{i=1}^r\sum_{j=i}^r\ind_{\{a_i=a_j\}} = \frac{1}{(n-1)^2}\sum_{i=1}^r \left( \ind_{a_i=a_{i+1}} + ... + \ind_{a_i=a_{i+n-1}}\right)\simeq\]
\[\simeq \frac{1}{(n-1)^2}\sum_{i=1}^r(n-1)\ind_{a_i=a_{i+1}} = \frac{1}{n-1}\sum_{i=1}^r\ind_{a_i=a_{i+1}}=α\]
