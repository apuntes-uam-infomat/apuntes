...
Por otro lado, igual que hemos reducido la ecuación inicial a módulo 13, también podemos reducir a módulo 2, obteniendo:
\[A \cdot \left( \begin{array}{cc}
0 & 1 \\
...
\subsection{Criptosistema de Vionère}
...
Este criptosistema sigue siendo un sistema afín y no es más que aplicar $n$ sistemas de cesar simultáneamente, uno sobre cada letra y repitiendo cada $n$ letras, siendo $n$ la longitud de la palabra $e$.

El sistema de César era muy fácil de descifrar, ya que podíamos simplemente comprobar todas las posibilidades. En este caso se complica un poco pero un análisis de frecuencia nos ayudaría a hacerlo fácilmente pero la complejidad del sistema radica en que no conocemos a priori la longitud de la palabra clave.

Si llegamos a conocer esta longitud, el sistema sería fácil de resolver puesto que el problema se reduciría a descifrar varios sistemas César.

Para poder luchar contra esta dificultad, resulta útil el concepto que vimos anteriormente de \textbf{índice de coincidencia} que, recordemos, nos medía la probabilidad de que la misma letra apareciese dos veces seguidas.

% Con analisis de frecuencia saco la longitud de la clave. No entiendo como, lo analizaré
