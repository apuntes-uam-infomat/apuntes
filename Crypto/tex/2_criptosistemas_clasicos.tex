...
En este caso tenemos que tener en cuenta que $\ent_p$ es un cuerpo.
...
\[\appl{\pi}{Mat_{n\times n}(\ent_{p^r})}{Mat_{n\times n}(\ent_{p})}\]
tal que $\pi(a_{ij} \text{ mod } p^r )=a_{ij} \text{ mod } p$
