...
Vamos a buscar cuántas matrices invertibles hay con coeficientes en $\ent_{N}$ por casos:
\begin{itemize}
\item \textbf{N=2}
...
\item \textbf{N=p primo}

En este caso tenemos que tener 
en cuenta que $\ent_p$ es un cuerpo.
...
De forma general, para el vector $c_i$ tendremos un total de $p^n-p^i$ posibilidades.

Finalmente, multiplicando todos estos valores, vemos el número de posibles matrices es:
\[ \phi_n(p) = \prod_{i=0}^{n-1} (p^n-p^i)\]

\item \textbf{N = } $p^r$

En esta ocasión lo que hacemos el reducir la matriz según el homomorfismo de anillos:
\[\appl{Π}{Mat_{n\times n}(\ent_{p^r})}{Mat_{n\times n}(\ent_{p})}\]
tal que $Π(a_{ij} \text{ mod } p^r )=a_{ij} \text{ mod } p$

\textcolor{red}{Me he perdido. Lo completo con los apuntes de Jorge}

Dada $\overline{A} ∈ Mat_{n × n}(ℤ/p)$, sabemos que $\#Π^{-1}(\overline{A})=(p^{r-1})^{n²}$ (pues cada elemento tiene $p^{r-1}$ inversos).

Si elijo $A$ tal que $Π(A)=\overline{A}$ tenemos que $Π^{-1}(A)=A + Ker(Π)$.

Para encontrar la inversa de $A$ nos basta que el el determinante esté en las unidades de $ℤ/p$:
\[A∈GL_n(ℤ/p^r) \iff (det(A), p^r)=1 \iff (det(A), p)=1\]
...
De modo que:
\[\appl{Π}{GL_n(ℤ/p^r)}{GL_n(ℤ/p)}\]
Y por tanto $φ_n(p^r)=φ_n(p)·\#Ker(Π)$. Así que:
\[\phi_n(p^r) = (p^{r-1})^n \prod_{i=0}^{n-1}(p^n-p^i)\]

\item $\phi_n(NM)$ siendo $N$ y $M$ coprimos.

En esta ocasión emplearemos como reducción el ismorfismo de anillos:
\[\appl{\pi}{\ent_{NM}}{\ent_N\times \ent_M}\]

Puesto que el núcleo estará formado por aquellos elementos que son múltiplos de $N$ y de $mM$ simultáneamente (puesto que al calcular su módulo $n$ y $m$ obtendremos 0). Pero, puesto que $N$ y $M$ son coprimos, el único factor común que tienen es su producto, es decir, $NM = 0$.

Por tanto, su núcleo es el trivial y por tanto la aplicación es inyectiva. Además, por ser inyectiva entre anillos de igual cardinal finito, tendremos que la aplicación $\pi$ es biyectiva.

Al ser sobreyectiva tenemos que:
\[U(\ent_{NM}) \approx U(\ent_N)\times U(\ent_M) \implies \phi(NM)=\phi(N)\times \phi(M)\]
...
Puesto que no tenemos más información, no nos queda otra que recurrir a un análisis de frecuencia (por pares). Tras este análisis, obtenemos:
\begin{center}
\begin{tabular}{| c | c |}
\hline
...
A partir de aquí, deberíamos trata de trabajar con la tercera relación para lograr escribir un sistema soluble.
\end{example}