...
para aplicar el algoritmo empezamos probando con $x_0 = [\sqrt{n}]+1$, es decir, el techo de $\sqrt{n}$. En cada iteración comprobamos si $Q(x_i)$ es un cuadrado perfecto y, en caso de no serlo, pasamos a $x_{i+1}=x_i+1$.
...
\[m.c.d.(x-y,n) = \left\{ \begin{array}{lcc}
             n &   si  & \text{Estamos en el caso 1} \\
             1 &  si & \text{Estamos en el caso 2}\\
             p,q &  si  & \text{Estamos en el caso 3 y por tanto hemos factorizado} 
             \end{array}\right.\]
