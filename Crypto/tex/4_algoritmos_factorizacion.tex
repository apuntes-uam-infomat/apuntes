...
\begin{example}
Supongamos que queremos factorizar un número del orden de $10^{75}$ empleando el ordenador mas rápido del mundo.

El ordenador mas rápido del mundo es capaz de realizar 33,86 PETAFLOPS, es decir, realizar un total de $33.86 \cdot 10^{15}$ operaciones por segundo.
...
\[a=a^{-1} \mod n \iff a^2 = 1 \mod n \iff a^2-1 = 0 \mod n\]
...
\begin{proof}
Consideramos el conjunto de valores $\{a_1,...a_r\} \subset \left(\ent_n\right)^*$ tales que
\[a_i^{n-1} = 1 \mod n\]

Entonces $n$ no es pseudoprimo en la bases $\{b_0a_1,...,b_0a_r\}$ ya que
\[(b_0a_i)^{n-1} = b_0^{n-1}a_i^{n-1} = b_0^{n-1} \neq 1 \mod n\]
...
\end{itemize}
\item Compruebo si $b^{n-1} = 1 \mod n$.
\begin{itemize}
...
El origen de este problema, que estudiaremos en la siguiente sección, es que el Teorema de Fermat, en el que nos estamos apoyando, usa una implicación y no un si y sólo si. Es decir, todo número primo cumple cierta propiedad pero cumplirla no garantiza que estemos ante un número primo.
...
\begin{theorem}[Teorema de Korselt (1899)]\label{theorem:Korset}
Sea $n \in \ent$ un número compuesto, son equivalentes:
\begin{enumerate}
...
Además, si $p$ es primo y $p$ divide a $n$ entonces $p-1$ divide a $n-1$
\end{enumerate}
\end{theorem}
...
\begin{proof}
La implicación de izquierda a derecha siempre la tenemos por definición del módulo. Es decir, si la diferencia entre dos números es múltiplo de $n$ y $p|n$, entonces la diferencia será múltiplo de $p$.

Para el otro sentido de la implicación nos apoyamos en el hecho de que $n$ es libre de cuadrados.

Por ser $n$ libre de cuadrados sabemos que $p-1$ divide a $n-1$ por lo que podemos escribir:
\[(b,p)=1 \implies b^n=b\cdot b^{n-1} = b\cdot \left( b^{p-1}\right)^k=b\mod p\]
donde la última igualdad se apoya en el teorema de Fermat, que nos garantiza que $b^{p-1}=b \mod p$ por ser $p$ un primo.
\end{proof}
