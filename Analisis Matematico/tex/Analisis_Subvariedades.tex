...
\subsubsection{Cálculo del plano tangente en $\real^3$}  
  ...
  \paragraph{Gráfica: $z=f(x,y)$}
	...
	\placeholder{Cálculo del plano tangente}  
...
\paragraph{5)}
  \[\appl{F}{\real^2}{\real}\text{ Con } F(x,y) = x^3 - y^6\]
  \[M = \{(x,y) \tq x^3-y^6 = 0\}\]
  \[DF = (3x^2-6y^5)\]
...
Supongamos que es $\displaystyle \dpa{G}{y}(0,0)\neq 0$

Podemos aplicar el teorema de la función implícita y despejar $y=y(x)$.

Tendríamos \[M \cap U = \{x^2-y(x)^2 = 0\}\]

Si fijamos $x = \varepsilon \implies y(x) = \pm \varepsilon$ que no es una función, entonces es imposible que $\dpa{G}{x}\neq 0$

\textbf{Conclusión: } Hemos demostrado que no puede existir una función que defina esto como subvariedad diferencial. Este es el ejemplo de que cualquier objeto que tenga autointersección no puede ser subvariedad.
...
Condición de rango: $\left(\dpa{F}{x},\dpa{F}{y}\right) \neq (0,0)$.
 ...
 \subparagraph{Caso 1} Supongamos $\displaystyle \dpa{G}{x}\neq 0$

 Entonces tenemos $M\cap U = \{y(x)^2 - y^2 = 0\}$

 Si fijamos $x=\varepsilon \implies y(x) = \abs{\varepsilon} \implies G(x,y)\notin C^1$

 \subparagraph{Caso 2} Supongamos $\displaystyle \dpa{G}{y}\neq 0$

 Entonces tenemos $M\cap U = \{x^2 - x(y)^2 = 0\}$

 Si fijamos $x=\varepsilon \implies y(x) = \abs{\varepsilon} \implies G(x,y)\notin C^1$

 Podemos aplicar el teorema de la función implícita y despejar $x = x(y)$. En este caso:
...
Comprobamos que efectivamente $rango \nabla F = max, \forall (x,y)\in \real^2$
