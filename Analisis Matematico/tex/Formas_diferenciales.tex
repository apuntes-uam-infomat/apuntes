...
Entramos en como transformar k-formas de $\real^N$ a $\real^M$. Partimos de la base de que existe una transformación $T$ que va de $\real^N \to \real^M$ y buscamos una $T^{\ast} \tlq \real^M\to\real^N$.
...
tenemos $\gor{s} \in \real^N, \gw_1,\gw_2,..,\gw_k$ vectores en $\real^N$.

Queremos definir $T^{\ast}\omega$ en términos de $T$ y $\omega$.

\[
(T^{\ast}\omega)(\gor{s}) [\underbrace{\gw_1}_{\in\real^N},...,\gw_k] = \omega(T(s)) [DT(s)\gw_j]
\]