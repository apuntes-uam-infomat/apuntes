...
Suponemos que $\exists F(\ga) = \gor{b}$ (sabemos la solución del sistema en al menos 1 punto $\ga$)
...
\[\md{F(\gx)} ≤ \alpha \md{\gx-\ga} + \md{F(\ga)} \]
  ...
  Si tomamos $\ga=0$ (en el caso  $0 \notin C $ solo haría falta una pequeña traslación), y suponemos $ \md{x} < R, \forall x \in C?	$. Esto es razonable de suponer puesto que $x\in K$, compacto y por lo tanto accotado. Tenemos entonces que \[ \md{F(\gx)} \leq \alpha R + \md{F(\gor{0})} < R\]
 ...
 \begin{equation}
  \md{x_n - x_m} = \md{x_n \pm x_{n-1} \pm \dotsb \pm x_{m+1} - x_m} \leq \sum_{i=m+1}^n \md{x_i - x_{i-1}} \label{eqAC_sum}
 \end{equation}
...
\begin{proof}
 ...
 Partimos de $F(\gor{0}=\gor{0} DF(\gor{0}) = Id$.
