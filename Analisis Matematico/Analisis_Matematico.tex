...
\begin{theorem}[Teorema de la aplicación contractiva]
...
\begin{proof}
 Paso 1: Acotación
 $$
  \md{F(\gx) - F(\ga)} \leq \alpha\md{\gx-\ga}$$
  $$\md{F(\gx)} = \md{(F(\gx)-F(\ga)+F(\ga)} \leq \md{F(\gx)-F(\ga)} + \md{F(\ga)}\\
  \leq \alpha \md{\gx-\ga} + \md{F(\ga)}$$
  
  Tomamos $a=0$ (si  $0 \notin C $ solo haría falta una pequeña traslación), y supongamos \[\md{x} < R  \implies \md{F(\gx)} \leq \alpha R + \md{F(\gor{0})} < R\]
  Es decir, F toma un compacto y lo lleva en sí mismo: $\appl{F}{B_R(0)}{B_R(0)}$
 
 Paso 2: Proceso iterativo.
 
 Tenemos que $\appl{F}{K}{K}, K\in \real^N$, compacto.
 
 $\{x_N\}_{n \in \mathbb{N}} \subset K, x_N = F(x_{n-1})$
 \begin{lemma}
  $\{x_N\}_{n \in \mathbb{N}}$ es de Cauchy
 \end{lemma}
 
 \begin{proof}
 Dado $\varepsilon > 0$ hay que hallar $n_0$ tal que si $n,m>n_0$ entonces $\md{x_n-x_m}<\varepsilon$
 
 
  $$\md{x_n - x_m} = \md{x_n \pm x_{n-1} \pm \dots \pm x_{m+1} - x_m} \leq \sum_{i=n}^m \md{x_i - x_{i-1}} N>M!!!$$
 \begin{align*}
  \md{x_n - x_{n-1}} &= \md{F(x_{n-1}) - F(x_{n-2})} \leq \alpha \md{x_{n-1} - x_{n-2}}\\
    & = \alpha \md{F(x_{n-2}) - F(x_{n-3})} \leq \alpha ^2 \md{x_{n-2}-x_{n-3}}\\
    &\quad \quad \vdots\\
    & \leq \alpha^{n-2} \md{x_2-x_1}
 \end{align*}
 
 $$\md{x_n-x_m} \leq * \leq \underbrace{\{\alpha^{n-2} + \alpha^{n-3} + \dots + \alpha^{m-1}\}}_{\displaystyle\sum_{k=m-1}^{n-2} \alpha^k = \alpha^{m-1}\frac{1-\alpha^{n-m}}{1-\alpha}}\leq \frac{\alpha^{m-1}}{1-\alpha} \md{x_2 - x_1} < \varepsilon \text{ tomando } n>m>n_0$$
...
x_n = F(x_{n-1})\\
 ...
 \text{ F continua}          
         \end{matrix}
 \right\} \implies F(x_{n-1} \rightarrow F(x_0)$$
 
 Unicidad del punto fijo:

 Supongamos $\left\{ \begin{matrix}
                      a = F(a)\\
                      b= F(b)
                     \end{matrix}\right.$
 Demostración: $\md{a-b} = \md{F(a)-F(b)}\leq \alpha \md{a-b} < \md{a-b} \implies $ Contradicción!
...
Ejemplo: (¿Cuando ha acabado la demostración??)
