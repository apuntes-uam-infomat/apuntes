...
$$\gor{h} \rightarrow 0 \dimplies \gor{\xi} \rightarrow \gor{0}$$
 ...
 % \begin{gather*}
  ...
  $$\text{El truco es: } \xi = %&
  \underbrace{\left[DF(G(y))\right]^{-1}DF(x)}_{=Id} \cdot \xi$$
%  \end{gather*}
  $$B = \frac{\md{\left[DF(G(y))\right]^{-1} \left[ -\{F(x+\xi)-F(x) - DF(x)\xi\}\right]}}{\md{\xi}}$$
  
  \begin{align*}
  B\leq &C \frac{F(x+\xi) - F(x) - DF(x)\xi}{\md{\xi}}\\
  &\uparrow\\
  \text{Norma de}& \text{ la matriz}\\
  \text{Hay que probar}&\text{ que A está acotado}\\
  \frac{1}{A} = \frac{\md{F(x+\xi)-F(x)}}{\md{x}} &= \frac{\md{F(x+\xi) - F(x) - DF(x)\xi + DF(x)\xi}}{\md{xi}}\\
  \geq \frac{\md{DF(x)\xi}}{\md{\xi}} &- \underbrace{\frac{\md{F(x+\xi) - F(x) - DF(x)\xi}}{\md{\xi}}}_{\rightarrow 0}\\
  %\frac{\md{DF(x)\xi}}{\md{\xi}} = \md{DF(x)\gor{v}}, \md{\gv} = 1\\
  \text{Definimos } M(\gor{v}) = \md{DF(\gx)\gor{v}}, \text{ para } v\in C^1; M(v)\geq \delta \implies \frac{1}{A}\leq \frac{1}{\delta} \leq C  
  \end{align*}
