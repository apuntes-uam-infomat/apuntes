...
Tenemos una $X \sim exp(\theta)$. Queremos contrastar para $\alpha = 0.01, H_0: \theta = 5; H_1: \theta = \theta_1$, siendo $\theta_1>5$ un valor prefijado)

\ppart Test óptimo
\solution

\spart Ejemplo típico de aplicar el lema de Pearson.

\paragraph{1) Ccomprobamos la propiedad de CVM}
(cociente de verosimilitudes monótono)

\[\frac{f_n()}{f_n(;5)} = \]

La región de rechazo del test UMP (por el lema de Neyman-Pearson)
\[R^{\ast} = \left\{ \left(\frac{\theta_1}{5}\right)^n e^(-\theta_1-5)\sum x_i\right\} = \{\sum x_i < c_{\alpha}\}\]
¿Cómo construimos $c_{\alpha}$?

Tiene que cumplir $P_{\theta=5} \{\sum x_i < c_{\alpha}\} = \alpha$

Haciendo esto nos libramos de calcular la $k$.
