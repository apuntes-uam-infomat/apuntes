...
\end{problem}
\subsection{Tema 3 - Estimación puntual paramétrica}
...
\[  \frac{-n\avg{x}}{1-p} + \frac{n}{1-p} = \frac{n}{p} \]

y a saber lo que sale. Pero al final

\[ \hat{p} = \frac{1}{\avg{x}}; \qquad \esp{X} = \frac{1}{p} \]

Pasamos a obtener la distribución asintótica del estimador. Queremos ver que

\[ \sqrt{n}(\hat{p} - p) \convdist  N(0, \hat{sigma}) \]

Usamos el método delta para probarlo. Como $\hat{p} = \frac{1}{\avg{x}} = g(\avg{x})$, donde $g(x) = \frac{1}{x}$, entonces

\[ \sqrt{n}(g(\avg{x}) - g(µ)) \convdist N(0,\abs{g'(µ)}\sigma) = N(0, \frac{\sigma}{µ^2} = N(0, p\sqrt{1-p}) \]

(la desviación típica de la dist. geométrica es $\sigma = 1-p$.
