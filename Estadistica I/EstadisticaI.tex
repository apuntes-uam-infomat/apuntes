...
y \[ \var{X} := \sigma^2 = \int_\real \left(x - µ\right)^2 \dif F(x) \]
\end{theorem}
...
\begin{theorem}[Desigualdad\IS de Chebichev] Sea $X$ v.a. Entonces, $\forall \epsilon > 0$, se cumple que  \[ \prob{\abs{X - \esp{X}} > \epsilon} ≤ \frac{\var {X}}{\epsilon^2} \]
\end{theorem}
...
\label{thmGrandes}
\[ \avg{X} = \frac{1}{n}\displaystyle\sum_{i=1}^n x_i \convcs µ \]
\end{theorem}
...
\[ \frac{1}{\theta^k} = \frac{1}{n}\sum_{i=1}^n x_i^k \dimplies \hat{\theta} = \left(\frac{n}{\sum_{i=1}^n x_i^k}\right)^\frac{1}{k} \]
...
\[ I(\theta) =\esp[\theta]{-\frac{∂^2}{∂\theta^2} \log f(X;\theta)} =\esp[\theta]{\left(\dpa{}{\theta} \log f(X;\theta)\right)^2} \]
...
\begin{theorem}[Lema\IS de Fischer-Cochran] Si \sample son v.a.i.i.d. con distribución $N(μ,σ)$ entonces $\avg{X}$ y $S^2$ (desviación) son estadísticos independientes.
\end{theorem}
