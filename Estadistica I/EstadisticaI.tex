...
\[ \Lambda_n = \frac{\sup_{\theta\in\Theta_0}f_n(x;\theta)}{\sup_{\theta\in\Theta}f_n(x;\theta)} = \frac{\sup_{\theta\in\Theta_0}f_n(x;\theta)}{f_n(x;\hat{\theta}} \]

donde $\hat{\theta}$ es el e.m.v. (\ref{defEMV}) de $\theta$, y 
\end{defn}

Si $H_0$ es cierta y el verdadero valor de $\theta$ están en $\Theta_0$ entonces $A_n\eqsim1$, porque $\hat{\theta}_n$ tiende al verdadero valor del parámetro. Si $H_0$ es falsa, el e.m.v. de $\theta$ tiende a un valor fuera de $\Theta_0$. Entonces $\Lambda_n$ tomará un valor significativamente menor que 1.
...
\[ - 2 \log \Lambda_n > χ_{k-1;\alpha}^2 \]

Consideramos $f(\sample[x];\sample[p][k])$ como la probabilidad de haber observado la muestra $\sample[x]$ con los valores de los parámetros $\sample[p][k]$. Entonces el numerador de $\Lambda_n$ es 
