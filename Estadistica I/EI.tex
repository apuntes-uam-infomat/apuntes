...
\begin{theorem}[Ley de los grandes números] Sea $\{x_k\}$ una sucesión de v.a.i.i.d con media finita $µ$. Se verifica entonces que 
\label{thmGrandes}
...
\begin{proof}
Empezamos demostrando la convergencia del supremo (¿?) \[ \fd_n(t) = \frac{1}{n}\sum_{i=1}^n \ind_{(-\infty, t]} (x_i) \]
...
\[ \esp{Y} = \esp{\ind_{(-\infty, t]}(X)} = \prob{X\in (-\infty, ty]} = F(t) \]

Ahora tenemos que demostrar que el límite por la izquierda converge. Es decir, hay que demostrar que \[ \fd_n(t^-) \convcs F(t^-) \]. Esa convergencia se da si y sólo si en un conjunto de probabilidad $1$ se tiene que $ \fd_n(t^-) \convs F(t^-) $. Según la definición de límite, esto se da si y sólo si \[ \forall \epsilon > 0\; \exists N \tq n ≥ N \implies \abs{\fd_n(t^-) - F(t^-) } < \epsilon \] 
