...
Pero esto es muy poco, vamos a ver el DES Triple
 ...
 \subsection{Definiciones sobre polinomios}
 ...
 \begin{example}
	...
	\begin{center}
		\includegraphics[angle=90 , width=200pt]{DivPol.jpg}
	\end{center}
...
$$P(x) = x^6 + x^4 + x^2 + x + 1 \equiv 01010111_2 = 57_8$$
 ...
 Vamos a distinguir dos casos del xtime:
 \begin{itemize}
 	\item CASO 1: $b_7 =0$
 	Entonces
 ...
 \subsection{Producto de polinomios con coeficientes en GF($2^8$)}
...
\subitem{InvMixColumn}

\begin{enumerate}
	\item
	$$C(x) \cdot C(x)^{-1} \equiv '01' \text{ mod } M(x)$$
	
	$$C^{-1}(x) = '0B'x^3 + '0D'x^2 + '09'x + '0E'$$
	
	\item Transformación inversa de cada columna
	$$\left(\begin{matrix}
	C_0\\C_1\\C_2\\C_3
	\end{matrix} \right) = \left(\begin{matrix}
	0E & 0B & 0D & 09\\
	...
	d_2\\
	d_3
	\end{matrix}\right) = M^{-1} \cdot \left( \begin{matrix}
	d_0\\
	d_1\\
	d_2\\
	d_3
...
BASE DEL DISEÑO DE MIXCOLUMN

$$0B \times d_i \forall d_i \in GF(2^8) \rightarrow 256 bytes$$

$$0D \times d_i \forall d_i \in GF(2^8) \rightarrow 256 bytes$$

$$09 \times d_i \forall d_i \in GF(2^8) \rightarrow 256 bytes$$


Al ser tan pocas posibilidades podemos precalcularlo de forma que el coste quedaría reducido a la velocidad de redireccionamiento de los datos.
...
Si sustituimos recursivamente nos queda que el AES es:
$$\begin{matrix}
	e_{0j}\\
	e_{1j}\\
	e_{2j}\\
