...
Repasemos lo último que vimos sobre el tensor de curvatura:\\ Sea V un campo, entonces $V\longmapsto[\nabla_k,\nabla_l]V$ y habíamos visto que la coordenada i-ésima de esto era $R^i_{jkl}V^j$, que es lineal en V (esto no es nada evidente). $R^i_{jkl}$ reciben el nombre de componentes del tensor de Riemann (\ref{def:tensorRiemann}). Luego teníamos el tensor de Ricci, cuyas componentes eran $R_{jl}=R^i_{jil}$.
 ...
 \begin{defn}[Curvatura\IS escalar]Se define la \textbf{curvatura escalar} como $$R=g^{ij}R_{ij}$$
 \end{defn}
...
\begin{defn}[K-forma diferencial] Una k-forma diferencial en una variedad M es un tensor de tipo (0,k) de la forma $$\omega=\sum a_{i_1\cdots i_k}dx^{i_1}\wedge\cdots\wedge dx^{i_k}$$ donde $(dx^{i_1}\wedge\cdots\wedge dx^{i_k})(\overline{v}_1,\cdots,\overline{v_k})=\text{det(filas }i_1,\cdots,i_k\text{ de la matriz de coordenadas de los vectores }(\overline{v}_1,\cdots,\overline{v_k})\text{)}$
\end{defn}
...
\begin{defn}[Producto\IS exterior]Sean dos formas $\omega\in\Omega^k(M)$ y $\eta\in\Omega^l(M)$ se define su producto exterior como $\omega\wedge\eta$ y consiste en concatenar los $dx^{i_1}\wedge\cdots\wedge dx^{i_k}$ y multiplicar los coeficientes (propiedad distributiva). Entonces $\omega\wedge\eta\in\Omega^{k+l}(M)$
\end{defn}

La motivación de introducir formas diferenciales es que se pueden integrar en cualquier carta sin introducir jacobianos.

\begin{defn}[Derivada\IS exterior] Sea $\omega\in\Omega^k(M)$ con $\omega=\sum_{i_1<\cdots<i_k}a_{i_1\cdots i_k}dx^{i_1}\wedge\cdots\wedge dx^{i_k}$ se define su derivada exterior como $$d\omega=\sum_{j=1}^{n}\sum_{i_1<\cdots<i_k}\pdv{a_{i_1\cdots i_k}}{x^j}dx^j\wedge dx^{i_1}\wedge\cdots\wedge dx^{i_k}\in\Omega^{k+1}(M)$$
\end{defn}
...
Sea $\appl{f}{\mathbb{R}^2}{\mathbb{R}^2}$ y digamos que tiene soporte compacto. Entonces $$\int_{\mathbb{R}^2}f(x,y)dxdy$$ tiene sentido. Pero no se pueden cambiar las coordenadas sin introducir algo más: el \textbf{jacobiano}. Es decir: $$\int_{\mathbb{R}^2}f(x,y)dxdy=\int_{0}^{2\pi}\int_{0}^{\infty}f(rcos\theta,rsen\theta)rdrd\theta$$
 ...
 Sea $M$ una variedad con borde que denotaremos $∂M$. Si $M$ es orientable con dimensión $n$, entonces $∂M$ es orientable con dimensión $n-1$. La restricción de cartas induce una orientación.
 
 \begin{prop}\textbf{(Teorema de Stokes)}\label{teoremaStokes} Sea $M$ una variedad con borde orientable y dimensión $n$. Sea $\omega\in\Omega^{n-1}(M)$ (de soporte compacto). Entonces: $$\int_{∂M}i^*\omega=\int_{M}d\omega$$ donde $\appl{i}{∂M}{M}$ es la inclusión.
 \end{prop}
 
 \begin{prop}Sea M una variedad con borde y compacta. Supongamos que $\omega\in\Omega^{n-1}(∂M)$ con $\int_{∂M}\omega\neq 0$. Entonces, no existe $\appl{f}{M}{∂M}$ tal que deje fijos los puntos del borde.
 \end{prop}

\begin{proof}Si existiera $\restr{f\circ i}{∂M}=Id$ con $\appl{i}{∂M}{M}$ y tenemos $0\neq\int_{∂M}\omega=\int_{∂M}Id^*\omega=\int_{∂M}(f\circ i)^*\omega=\int_{∂M}i^*f^*\omega$. Aplicamos el Teorema de Stokes $$\int_{M}d(f*\omega)=\int_{M}f^*d\omega=0$$ ya que $d\omega$ tendría dimensión n en una variedad de dimensión $n-1$
\end{proof}

\begin{prop}\textbf{(Punto fijo de Brower)} Sea $\overline{B_1}(0)$ la bola unidad cerrada, entonces $\appl{f}{\overline{B_1}(0)}{\overline{B_1}(0)}$ tiene algún punto fijo.
\end{prop}

\begin{proof}Digamos que existe g sin puntos fijos. Supongamos (en dimensión 2) un punto x en la bola unidad y el punto $g(x)$ y consideramos la semirrecta que parte de $g(x)$ y pasa por x y llamamos a esa semirrecta r. Entonces $r\cap\mathbb{S}^{n-1}$ es un punto. Consideramos $\appl{f}{x}{\text{punto }r\cap\mathbb{S}^{n-1}$. Los puntos de la frontera quedan fijos con esta función y contradice el teorema 3.
\end{proof}
