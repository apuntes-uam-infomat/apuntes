...
\begin{defn}[Tensor\IS de Ricci]Se llama \textbf{Tensor de Ricci} al tensor de tipo $(0,2)$ cuyas componentes son $$R_{jl}=R^k_{jkl}$$ Este tensor es simétrico $R_{jl}=R^k_{lj}$ Este tensor es mucho más "accesible" por cálculos: $$R_{ij}=\frac{1}{\sqrt{\abs{g}}}∂_k\left(\sqrt{\abs{g}}\Gamma^k_{ij}\right)-∂_i∂_jlog\left(\sqrt{\abs{g}}\right)-\Gamma^k_{li}\Gamma^l_{jk}$$
\end{defn}
