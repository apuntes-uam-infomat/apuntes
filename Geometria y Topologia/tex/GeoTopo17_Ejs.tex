...
\begin{problem}[1] Se define el volumen  de una variedad riemanniana orientable (con o sin borde) de dimensión $n$ como la integral (si existe) de 
	...
	0&1
	\end{pmatrix};$$$$\abs{g}=cos^2\varphi+4cos\varphi+4\Rightarrow\sqrt{|g|}=\sqrt{cos^2\varphi+4cos\varphi+4}=cos(\varphi)+2$$
	Luego tenemos una variedad 2-dimensional con elemento de volumen\\ $\omega=(\sqrt{cos^2\varphi+4cos\varphi+4})d\theta\wedge d\varphi$, e integrando: $$V=\int_{0}^{2\pi}\int_{0}^{2\pi}(cos(\varphi)+2)d\theta d\varphi=8\pi^2$$ Si comprobamos con la fórmula tradicional $V=2\pi^2rR^2$ vemos que es correcto.
