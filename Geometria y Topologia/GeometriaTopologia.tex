...
0 \ar[r] & Ω^*(M) \ar[r, "r"] & Ω^*(U) \oplus Ω^*(V) \ar[r, "\delta"] & Ω^* (U ∩ V) \ar[r] & 0 \\
	& ω \ar[r, "r"]	& (\left.ω\right|_U, \left.ω\right|_V) & & \\
	&  & (η_1, η_2) \ar[r, "δ"] & \left.η_2\right|_{U∩V} - \left.η_1\right|_{U∩V} &
\end{tikzcd}
\tikzexternalenable
\end{center}
\end{theorem}
...
0 \rar
	& H^0(\crc[2]) \rar & H^0(U) \oplus H^0(V) \rar & H^0(U ∩ V) \arrow[dll, snake] \\
	& H^1(\crc[2]) \rar & \textcolor{red!70!black}{H^1(U) \oplus H^1(V)} \rar & H^1(U ∩ V) \arrow[dll, snake] \\
...
\begin{wrapfigure}[7]{R}[0.1\textwidth]{0.3\textwidth}
\centering
...
La cobertura que cogemos es la de la \fref{fig:CoverTorus}. Dividimos el toro en dos mitades que se solapan, de tal forma que la intersección es una unión disjunta de dos bandas, que podemos deformar continuamente a dos circunferencias. Esto es, $U ∩ V \simeq \crc[1] \sqcup \crc[1]$. Además, cada uno de esos abiertos será homotópicamente equivalente a $\crc[1]$.

La cadena de Mayer-Vietoris que sale es la siguiente:

\tikzexternaldisable
\begin{tikzcd}[row sep = 3pt, column sep = 7pt]
0 \rar
	& H^0(\torus) \rar & H^0(U) \oplus H^0(V) \rar & H^0(U ∩ V) \arrow[dll, snake] \\
	& H^1(\torus) \rar & H^1(U) \oplus H^1(V) \rar & H^1(U ∩ V) \arrow[dll, snake] \\
	& H^2(\torus) \rar & H^2(U) \oplus H^2(V) \rar & H^2(U ∩ V) \rar & 0 \\
\end{tikzcd}
\tikzexternalenable
...
\tikzexternaldisable
\begin{tikzcd}[row sep = 0pt, column sep = 7pt]
H^{k-1}(M ∩ N) \rar & H^k(M \sqcup N) \rar & H^k(M) \oplus H^k(N) \rar & H^k(M ∩ N) \\
0 & h^k(M \sqcup N) & h^k(M) + h^k(N) & 0
\end{tikzcd}
\tikzexternalenable
\end{center}
...
% TODO: Mover esto detrás de la fórmula de Künneth
...
\begin{theorem}[Lema\IS de Poincaré para soportes compactos] \[ H_c^k(ℝ^n) = \begin{cases} ℝ & k = n \\ 0 & k ≠ n\end{cases} \]
\end{theorem}
...
\tikzexternaldisable
\begin{tikzcd}[row sep = 3pt]
0 & Ω^*(M) \ar[l] & Ω^*(U) \oplus Ω^*(V) \ar[l, "r"] & Ω^* (U ∩ V) \ar[l, "\delta"] & \ar[l] 0
\end{tikzcd}
\tikzexternalenable
\end{center}
\end{prop}
...
\tikzexternaldisable
\begin{tikzcd}[row sep = 3pt]
0 	& H^0(M) \lar & \lar H^0(U) \oplus H^0(V) & \lar H^0(U ∩ V)  \\
	& \phantom{H^0(M)} \arrow[urr, snake up] & \dotsb & \phantom{H^1(U ∩ V)} \\
	& H^n(M) \arrow[urr, snake up] & \lar H^n(U) \oplus H^n(V) & \lar H^n(U ∩ V) & \lar 0 \\
\end{tikzcd}
\tikzexternalenable
\end{center}
...
\input{tex/GT_OtrosConceptos.tex}
