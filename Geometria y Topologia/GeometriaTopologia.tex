...
\textit{Nota: Un desarrollo formal de lo del espacio contangente está en \citep[Capítulo I]{ApuntesGeoDif}}.

¿Cómo definimos el espacio tangente? Tomamos un punto $p ∈ M$, y una parametrización $α$ de un entorno de $p$. Por ejemplo, cogemos la esfera y entonces $\tgs_p \crc[2] = \gen{\pd{α}{x}, \pd{α}{y}} = \gen{(1,0, \sfrac{-a}{c}), (0, 1, \sfrac{-b}{c})} ⊆ ℝ^3$.

Ahora bien, para hacer esto estamos apoyándonos en el hecho de que la esfera está en $ℝ^3$. ¿Qué hacemos si cogemos otra variedad abstracta, como $\projp^4$?

Volviendo al ejemplo, podemos expresar el espacio tangente como \[ \tgs_p \crc[2] = \set{α'(0) ∈ ℝ^3 \tq \appl{α}{(-ε, ε)}{\crc[2]}, \, α(0) = p} \]

Si tomamos $f(x,y,z) = x^2 + y^2 + z^2 - 1$, entonces $\crc[2] = \inv{f}(0)$. Por lo tanto, decir que $α(-ε, ε) ⊆ \crc[2]$ es equivalente a decir que $f(α(t)) = 0$ para $t ∈ (-ε, ε)$. Calculando su derivada, \[ 0 = (f ○ α)'(0) = \eval[2]{\Dif f}_{α(0) = p} · \eval[2]{\Dif α}_{0} = (2a, 2b, 2c) · α'(0) \], luego $α'(0)$ es ortogonal a $(a,b,c)$, y precisamente el espacio de los vectores ortogonales a $(a,b,c)$ es lo que habíamos definido antes como $\tgs_p \crc[2]$. Esto sí nos permitirá definir correctamente el espacio tangente a una variedad, aunque tendremos que sortear de alguna forma el hecho de que no sabemos derivar en espacios abstractos. Para ello, nos basaremos en las cartas y en los entornos de cada punto homeomorfos a $ℝ^n$, donde sí sabemos derivar.
...
Con esta definición, se puede probar que $\tgs_p M$ es un espacio vectorial. La cuestión es ver de qué dimensión es. Daremos una definición alternativa de espacio tangente que nos permitirá entender mejor la dimensión.

Primero, definimos $θ_{p,M} = \set{\appl{f}{U}{ℝ} \tq p ∈ U, f\text{ diferenciable}}$. Esto no es un espacio vectorial porque los entornos no son equivalentes. Construimos una relación de equivalencia dada por \[ (f_1, U_1) \sim (f_2, U_2) \iff f_1 = f_2\text{ en }W ⊂ U_1 ∩ U_2 \], y denotaremos $Θ_{p,M} = \quot{θ_{p,M}}{\sim}$. Lo bueno es que aquí sí podemos sumar, tomando el entorno abierto más pequeño en el que estén definidas ambas. De hecho, es un anillo local (ver \citep{ApuntesAlgebraConmutativa}). Entonces, definiremos el vector tangente que tiene sabor a derivada\footnote{¿A qué saben las derivadas? ¿De qué color es el cielo? ¿Qué estoy haciendo con mi vida?.}
s

\begin{defn}[Vector\IS tangente] Un vector tangente $v$ en $p ∈ M$ es una derivación lineal de $Θ_{p,M}$, es decir:
\begin{enumerate}
\item $\appl{v}{Θ_{p,M}}{ℝ}$ es lineal.
\item \label{prp:LeibnizVectorTg} $v(f·g) = f(p) · v(g) + g(p)·v(f)$.
\end{enumerate}
\end{defn}

Dejamos como ejercicio ver que $v(c) = 0$ si $c$ es constante (la idea es que $7 = 1 · 7$) y aplicamos lo del análogo de la regla de Leibniz (\fref{prp:LeibnizVectorTg}). También querremos ver un ejemplo de un $v$ en libertad. Queremos fotos! Cogemos cartas y cosas y esta clase está siendo un tanto surrealista. Pero bueno, que sí, que sale, que me lo creo. Mañana (lo dudo porque no tenemos clase) hará nosequé con nosequécosa.

Como ejercicio, tenemos que ver que $\eval[2]{\pd{}{x_i}}_p$ es una derivación, y además

\begin{prop} $\set{\eval[2]{\pd{}{x_1}}_p, \dotsc,\eval[2]{\pd{}{x_n}}_p}$ es una base de $\tgs_p M$. En particular, $\dim \tgs_p M = \dim M$.
\end{prop}

\[ \eval{\dpd{}{x_i}}_p (λ) ≝ \pd{λ○\inv{φ}}{t_i}_{φ(p)}\]
...
¿Qué pasa con el cambio de coordenadas? Bueno, pues que hay un cambio de coordenadas y parece que el infierno se abre ante nosotros y lo que nos queda es que \[ \eval{\dpd{}{y_j}}_p = \sum_{i=1}^n \eval{\dpd{x_i}{y_j}}_p · \eval{\dpd{}{x_i}}_p \]

El espacio tangente es equivalente si lo definimos mediante curvas. Esto tengo que mejorarlo muchísísísísimo porque está siendo un caos absoluto.
