...
El plano proyectivo complejo es interesante. Por ejemplo, podemos ver que $\projcp^1 \simeq ℂ ∪ \set{∞} \simeq \crc[2]$. Otro aspecto interesante de $\projcp^n$ es que no sólo es diferenciable como comentábamos antes: también es holomorfa. Y lo relevante de las variedades holomorfas es que son todas orientables. Esto nos sirve como excusa para introducir otro tipo de variedades.
...
\begin{defn}[Submersión] Si $F$ es suprayectiva y la diferencial también es suprayectiva, entones es una submersión.
\end{defn}
...
\begin{problem}
Sea $\appl{f}{\real^n}{\real^m}$, entonces $f^*(\dif t_1 ∧ ... ∧ \dif t_n) = f^*\dif t_1 ∧ ... ∧ \dif t_n$.
\solution
...
\end{align*}, donde el $(-1)^{\deg ω}$ aparece al mover $\deg ω$ diferenciales (las del multíndice $\dif x_I$) para ordenarlo todo bien.
...
Otro ejemplo interesante es calcular \[ H^k(ℝ) \simeq \begin{cases} ℝ & k = 0 \\ \set{0} & k = 1 \\ \set{0} & k > 1 \end{cases}\]
...
Definimos el anilo de cohomología como \[ H^*(M) = \bigoplus_{k ≥ 0} H^k (M) \], que es un anillo con las operaciones $(+, ∧)$.
...
\begin{corol} $π^*$ y $s^*$ son mutuamemente inversas a nivel de cohomología, luego $H^k(ℝ^{n+1}) = H^k(ℝ^n)$.
\end{corol}
