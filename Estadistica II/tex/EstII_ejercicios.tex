...
\begin{problem}[11]
Se desea estudiar la esperanza de vida Y en una serie de países como función de la tasa de
natalidad nat, la tasa de mortalidad infantil mortinf y el logaritmo del producto nacional bruto
lpnb. Para ajustar el modelo

\[Y_i = β_0 + β_1·nat_i + β_2·mortinf_i + β_3·lpnb_i + ε_i \]

donde los errores $ε_i$ son v.a.i.i.d. $N(0,σ^2)$. Se ha utilizado $R$ con los siguientes resultados:

\begin{center}
\includegraphics[scale=0.5]{img/ej3_11.png}
\end{center}

\ppart ¿De cuántos países consta la muestra utilizada?
\ppart ¿Cuál es la suma de cuadrados de la regresión (SCR) que se utiliza para medir la variabilidad
explicada por las tres variables regresoras?
\ppart ¿Cuánto vale la cuasivarianza muestral de la variable respuesta $\sum (Y_i - \gor{Y})^2/(n-1) $?
\ppart Contrasta a nivel α = 0,05 la hipótesis nula $H_0 : β_1 = β_2 = β_3 = 0$
\ppart Determina cuál es la hipótesis nula y la alternativa correspondiente a cada uno de los tres
estadísticos F que aparecen en la tabla de análisis de la varianza anterior.

\solution

\doneby{Dejuan}

\spart Sabemos $n-k-1 = 87$ y $k=3$, con lo que $n=91$

\spart SCE=676.5 y $R^2 = 0.93$. Utilizando $R^2 = 1 - \frac{SCE}{SCT}$  despejamos $SCT = 9705.88$ y con este, obtenemos $SCR = 9029.38$

\spart \[\frac{SCT}{n-1} = 99.224\]

\spart \[R = \left\{385.9 > F_{3,18} \right\}\]
Podemos observar que el p-valor del contraste de la regresión (en que nos piden) es cercano a 0, con lo que rechazamos $H_0$.

\spart El primero corresponde al contraste de la regresión, el segundo corresponde al cpntraste $β_2,β_3 = $ y el tercero corresponde a $β_3 = 0$.
...
Con el fin de evaluar el trabajo de los directores de los 30 departamentos de una gran empresa,
se llevó a cabo una encuesta a los empleados a su cargo en la que se les pidió que valoraran
varias afirmaciones con una nota de 1 (máximo acuerdo) a 5 (máximo desacuerdo). Algunas de
las variables eran: Y , el trabajo del director es en general satisfactorio; x 1 , el director gestiona
correctamente las quejas de los empleados; x 2 , el director trata equitativamente a los empleados;
x 3 , la asignación del trabajo es tal que los empleados pueden aprender cosas nuevas con frecuencia.
El vector (Y i , x i1 , x i2 , x i3 ) contiene la suma de puntos de las respuestas en el departamento i, donde
i = 1, . . . , 30. Con estos datos se ajustó con R el modelo:




\includegraphics[scale=0.5]{img/3_13.png}

\ppart Calcula un intervalo de confianza de nivel 0.95 para el parámetro $β_3$ . Contrasta la hipótesis
$H_0 : β 3 ≤ 0$.

\ppart Determina el valor de AAA, BBB y CCC en la última línea de la salida anterior. ¿A qué hipótesis nula corresponde el p-valor que aparece en esta última línea?

\solution

\spart  \[ IC(β_3) = \left[ 0.238 \mp 0.1394·t_{28;0.025}\right] = \left[0.238 \mp 0.2855\right]\]
Y la región de rechazo correspondiente es:

\[
R = \left\{ \frac{β_3}{e.t.(β_3)} > t_{n-2;α} \right\} = \left\{ 1.707 > 1.701 \right\}
\]
Entonces rechazamos la hipótesis $H_0$. 

\obs Es curioso que rechazamos la hipótesis de que $β_3$ pueda ser negativo, pero uno de los extremos del intervalo de confianza es negativo.

\spart Corresponde al contraste de la regresión que es $β_1=β_2=β_3 = 0$

Tenemos BBB=3, CCC=26 con lo que $AAA = \frac{R^2}{1-R^2}·\frac{26}{3} = 21.74$
