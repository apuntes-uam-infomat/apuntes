...
% % % Dibujo
\textcolor{red}{Si Jorge se hace un dibujito, se agradece xD}

La idea es que $SCE_0 > SCE$. Geométricamente, vemos que $SCE$ es la hipotenusa y $SCE_0$ es un cateto. Por otro lado, la variabilidad no explicada de un modelo simple siempre va a ser menor que la $SCE$ de un modelo complejo para los mismos datos.
...
\begin{center}
\includegraphics[scale=0.6]{img/DiagramaRatasR}
\end{center}
...
Este es un tema que da para libros enteros. También se llama ``diseñode experimentos''. Nosotros vamos a ver el \concept{Modelo\IS unifactorial}, un caso concreto del tema para entender el fundamento matemático que hay detrás. Todo lo demás en lo que se profundiza en los libros son casos concretos.
