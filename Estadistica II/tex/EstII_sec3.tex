...
\begin{itemize}
	...
	Donde
		...
		\obs El \concept{Estimador\IS de la covarianza insesgado}  entre 2 variables $X,Y$ es $S_{xy}$. Utilizando $\cov{X,X} = \var{X}$, sabemos que $S_{xx}$ es el estimador insesgado de la varianza. \footnote{Para más información \href{https://es.wikipedia.org/wiki/Covarianza}{Wikipedia}, as usual.}
...
Ahora que ya tenemos estimada la varianza, vamos a manipular algebraicamente $\frac{\hat{β_1}-β_1}{\frac{S_R}{\sqrt{S_{xx}}}}$
...
En el numerador tenemos una $N(0,1)$ y en el denominador la raíz de una $\chi^2$ dividida por sus grados de libertad (por la proposición anterior). Esto es por definición una $\mathcal{T}$ (T-Student) con $n-2$ grados de libertad. (\href{https://en.wikipedia.org/wiki/Student%27s\_t-distribution#Characterization}{Wikipedia}})

\begin{prop}
Ahora que conocemos la distribución, podemos calcular el intervalo de confianza  para la pendiente de la recta, utilizando la fórmula de intervalo de confianza (y utilizando que es una $\mathcal{T}$)

\[
IC_{1-α}(β_1) \equiv \left[ \hat{β_1} \mp \mathcal{T}_{n-2,\frac{α}{2}}\frac{S_R}{\sqrt{S_{xx}}}\right] \equiv \left[ \gor{Y} \mp \mathcal{T}_{n-1,\frac{α}{2}}\frac{S_R}{\sqrt{n}} \right]
\]
\end{prop}
