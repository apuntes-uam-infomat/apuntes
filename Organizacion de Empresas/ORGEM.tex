...
Es decir, a menudo tendremos que comparar diferentes métodos de pago (diferentes hipotecas o diferentes cuotas en la devolución de un préstamo). 
Si los comparamos sumando de manera aritmética todos los pagos, tendríamos una comparación falseada puesto que una misma cantidad tiene diferente \textbf{valor} según el momento en el que se paga/recibe esa cantidad.


Por tanto, lo que tendremos que hacer es \substudy{mover todos los capitales hasta un mismo instante de tiempo} de modo que podamos \substudy{comparar teniendo en cuenta los intereses}, que marcan el cambio de valor del dinero a lo largo del tiempo.

\paragraph{Modelo simple de interés}

El primer modelo, el simple, es en el que ponemos una inversión que nos va generando intereses que se acumulan. Por ejemplo, un depósito con un interés $i ∈ (0,1)$ anual, en el que si tenemos depositado $x$ nos pagan $(i·x)$ al final del año. 
Entonces, está claro que si depositamos $C_n$ en un período, al siguiente tendremos $C_{n+1} = C_n(1+i)$. Desarrollando esto nos queda simplemente que, con $C_0$ la inversión inicial, \( C_n = C_0 (1+i)^n \iff C_0 = C_n (1+i)^{-n} \label{eq:InteresCapFijo} \)

Se entiende por \concept{capitalizar} el proceso de ``\substudy{mover el capital}'' hacia el futuro, es decir, calcular $C_n=C_0·(1+i)^n$ conociendo $C_0$. Del mismo modo se entiende por \concept{descontar} al proceso contrario, es decir, ``mover el capital'' hacia el pasado calculando $C_0=C_n·(1+i)^{-n}$ a partir de un $C_n$ conocido.

Este es el caso más sencillo y \study{viene marcado porque capital no cambia más que por los intereses} percibidos en cada período.

\paragraph{Modelo compuesto}
Hay otra posibilidad, que se da cuando vamos poniendo un cierto dinero en cada período. 
