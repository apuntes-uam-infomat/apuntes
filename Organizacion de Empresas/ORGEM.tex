...
Vamos a centarnos en el análisis del caso más ``clásico'', la estructura jerárquica. Esta estructura queda representada con más detalle en la \fref{fig:organizacionEmpresa}.

\begin{figure}[hbtp]
\centering
\includegraphics[width=0.9\textwidth]{img/organizacionEmpresa.png}
\caption{Esquema de la organización de una empresa.}
...
\caption{Ejemplo de pedidos óptimos, vía maestremiranda.com.}
...
\item En imprevistos imagina unos 100 \texteuro.
...
Procedemos a estudiar la previabilidad estática (con los datos aportados por las transparencias) \footnote{Lo que esta subrayado es lo que había que completar}. En la \fref{tab:CuotaMensualAmortizacion} vemos cuál sería la cuota mensual de amortización.
...
Total &21.000  & - & 361 \\
\end{tabular}
\caption{Cálculo de la cuota mensual de amortización.}
\label{tab:CuotaMensualAmortizacion}
\end{table}
...
Así, al final llegaríamos a completar la tabla entera, que podemos encontrar en el apéndice \ref{sec:Prestamos}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=0.8\textwidth]{img/apalancamiento.png}
...
Para responder a la pregunta debemos atender a la fórmula que mide la \concept{palanca financiera}:
...
\begin{figure}[hbtp]
\centering
\includegraphics[width = \textwidth]{pdf/_ProyectoInversion.pdf}
\caption{Criterios para la evaluación de proyectos de inversión}
\label{fig:CriteriosInversion}
\end{figure}

A la hora de evaluar si llevar a cabo un proyecto o no, necesitaremos ciertos indicadores (ver \fref{fig:CriteriosInversion} para un resumen) que nos permitan saber si ese proyecto puede ser rentable o no. No vale simplemente con sumar los ingresos previstos y restarle los gastos: como vimos en la sección anterior, el valor del dinero cambia con el tiempo, así que nos interesará saber además en cuánto tiempo vamos a recuperar la inversión.
...
El apéndice \ref{sec:ProyectoInversion} contiene un ejemplo de proyecto de inversión completado, extraído de maestremiranda.com.
...
\begin{figure}[hbtp]
\centering
\includegraphics[width = 0.8\textwidth]{img/CicloVidaProducto.png}
\caption{Ciclo de vida de un producto, vía maestremiranda.com.}
\label{fig:CicloVidaProducto}
\end{figure}

La primera es el \textbf{producto}: lo que vendemos al cliente. Tenemos que ajustarlo para que sirva a los clientes, y tener en cuenta también su ciclo de vida (\fref{fig:CicloVidaProducto}). Además, es interesante conocer la curva de adopción de la tecnología: al principio tenemos unos pocos innovadores y \textit{early adopters}, muy informados y que buscan tecnología y rendimiento. Después se da el salto a las mayorías: primero la precoz, luego los conservadores y por último llegando a los escépticos. Estos usuarios buscan más bien comodidad y buena experiencia de usuario, y no se preocupan tanto por estar a la última.
...
\includepdf[landscape=true, frame=true, noautoscale=true, delta=10 10]{pdf/_AnalisisInversion.pdf}
