...
\begin{theorem} Es posible identificar el retículo de subgrupos de $G_2$ con los subgrupos de $G_1$ que contienen al núcleo $N(f)$

\todo[inline]{¿Qué significa "identificar"?}
\end{theorem}
...
entonces $\{ 1, i, -1, -i \}$ es isomorfo al grupo $(\mathbb{Z} /4\mathbb{Z}, +)$
\end{example}

\begin{defn}[Biyección]
 Un homomorfismo $\appl{f}{G_1}{G_2}$ es una biyección si 
 $\exists \ \appl{h}{G_2}{G_1}$ de tal forma que $f\circ h$ es la función identidad en $G_1$ y $h \circ g$ es la identidad en $G_2$.
\end{defn}
...
\begin{theorem}[Teoría de isomorfismos]
 Siendo $\ncl f \lhd G$, con $\appl{f}{G_1}{G_2}$ y $\appl{\pi}{G_1}{G_1/N(f)}$se cumple:
 \begin{enumerate}
...
\begin{itemize}
 ...
 Recordemos que $\times H=yH \Leftrightarrow \ y^{-1}x\in H$.  Por tanto\\
...
\begin{example}[Clave de las claves para este examen]
 ...
 \begin{lemma}
  ...
  Si $|H_3\times H_{11}|=|G| \Rightarrow G\simeq H_3\times H_{11}$
 \end{lemma}
 \begin{proof}
 ...
 Por tanto podemos definir el homomorfismo $\appl{f}{H_1\times H_2}{H_1H_2}$. La condición necesaria y suficiente para que sea un homomorfismo
...
\begin{itemize}
 \item Cerrado por las operaciones, es decir: $A\cdot A \subset A$. 
 \item Todo elemento tiene inverso respecto de la suma
 \item Asociativo respecto de ambas operaciones.
 \item Existe el elemento neutro de ambas operaciones. Es único para cada operación, pero no tiene por qué ser el mismo para ambas.
 \item Conmutativo respecto de la suma.
\end{itemize}
Estas propiedades pueden resumirse en la principal propiedad que define los anillos: \textbf{La propiedad distributiva}. \todo{Estaría bien probar esto}

En los anillos, se cumple la propiedad cancelativa respecto de la suma ya que tenemos elemento inverso. No obstante, no tenemos propiedad cancelativa respecto del producto, ya que esta operación no tiene por qué tener inverso.

\begin{example}
 $(\mathbb(M)_{2\times 2}(\mathbb{R}), +, \cdot)$ es un anillo conmutativo.
\end{example}

\begin{defn}[Unidad] Una unidad de un anillo es un elemento con inverso multiplicativo. Denotamos por $U(A)$ el conjunto de todas las unidades de un anillo $A$.
\end{defn}

\begin{example}
 Dado el anillo conmutativo $(\mathbb{Z}/n\mathbb{Z},+,\cdot)$, su conjunto de unidades es \[U(A)=\{\bar{a}\in\mathbb{Z}/n\mathbb{Z} \tq m.c.d(a,n)=1\} \]
\end{example}

\begin{example}
 Dado $A=\{f:[0,1] \rightarrow \mathbb{R}\}$  siendo $f$ una función, definimos las siguientes operaciones para dos elementos $f,g∈A$:
 ...
 Entonces, el conjunto de sus unidades es
...
\begin{defn}[Cuerpo]
 Un cuerpo es un anillo conmutativo con la propiedad: $U(A)=A \setminus \{0\}$
\end{defn}

\begin{remark} Sea -1 el inverso aditivo del neutro multiplicativo, entonces  $\forall a \in A \ a^{-1} = (-1)a$. Es decir, el inverso aditivo de cualquier 
...
\begin{itemize}
 ...
 \item $1 \in U(A);\, 1a=a1=a\; \forall a \in A$.
 \item $a \in U(A) \Rightarrow \exists a^{-1} \tq a*a^{-1}=a^{-1}*a=1$
\end{itemize}
...
\begin{defn}[Dominio]
 Anillo conmutativo que tiene la propiedad: $a\neq 0 \wedge b\neq 0 \ \Rightarrow ab \neq 0$. Es decir, que el producto de dos elementos no nulos da un resultado no nulo.
...
\begin{example}
 ...
 Se cumple siempre que si A es un anillo, $U(A[x])=U(A)$. Un caso concreto: $\mathbb{R}[x]$  es un dominio.
\end{example}
...
\begin{defn}[Dominio\IS de integridad] Un dominio de integridad es un anillo que no tiene divisores de 0.

 Por definición, si $A$ es un anillo finito, son equivalentes el hecho de que $A$ sea un cuerpo y un dominio de integridad. \todo{Esto también molaría demostrarlo}
\end{defn}

\begin{lemma} Sea $A$ un anillo finito. Entonces

\[ a \in U(A) \dimplies a \;\text{no es divisor de 0} \]
\end{lemma}

\begin{proof} 

\paragraph{Implicación a la derecha} Para que $a$ sea divisor de cero, tiene que existir $b≠0 \tq ab = 0$. Como $a∈U(A)$, existe su inverso y entonces podemos multiplicar a la izquierda por él en ambos lados:
...
lo que nos llevaría a una contradicción, por lo tanto $a$ no puede ser divisor de 0.

\paragraph{Implicación a la izquierda} Defimimos la siguiente aplicación
...
de tal forma que $f_a$ es inyectiva. Además, por ser un grupo finito, $f_a$ es biyectiva, lo que implica que llega a todo el conjunto de destino. Por tanto, $\exists b \tq f_a(b)=ab=1$, de tal forma que existe un inverso de $a$ y por lo tanto $a \in U(A)$.

\todo[inline]{Eso de que sea biyectiva por ser grupo finito no me convence nada.}

\end{proof}

Resumamos algunas propiedades interesantes de los anillos:
\begin{lemma}[Propiedades de los anillos][]
\begin{enumerate}
 \item $a\in U(A) \Leftrightarrow \gen{a} = A$
 \item Si $u \in U(A)$ y además $a \in A$, entonces $\gen{a}=\gen{ua}$
 \item Si A es un dominio, $\gen a= \gen b \Leftrightarrow \exists u \in U(A) \tq a=ub$
\end{enumerate}
\end{lemma}

\begin{proof}
\paragraph{Propiedad 1}
\subparagraph{Implicación a la izquierda}
 Si $\gen{a} = A$, entonces $1 \in A$ y por lo tanto $\exists \alpha \in A \tq 1=\alpha a$. Es decir, $a$ tiene inverso multiplicativo y por lo tanto es una unidad de $A$.
 
 \subparagraph{Implicación a la derecha}
 Si a $\in$ U(A) $\Rightarrow$  <a>=$\{\alpha a \tq \alpha \in A\}$. Por otro lado 1 $\in$<a> $\Rightarrow \ \forall b \in A \Rightarrow b1=b \in$<a>=A  
 
 \todo[inline]{Hay que arreglar esto.}
\end{proof}

\subsection{Ideales}
Los ideales son estructuras algebraicas que jugarían un papel "equivalente" al de los subgrupos en el mundo de los anillos
\begin{defn}[Ideal]\label{defIdeal}
 I $\subset$ A es un ideal si:
 \begin{enumerate}
  \item $I$ es un subgrupo de $(A, +)$
  \item $a \in A,\, b\in I \Rightarrow  a*b \in I$.
 \end{enumerate}
\end{defn}
...
por ejemplo, con $\mathbb{R}$, ya que si tomo el subgrupo $\mathbb{Z}$  se cumple la primera pero no la segunda de las propiedades: se ve fácil observando qué ocurre si se multiplica por un racional.
...
\begin{enumerate}
 ...
 \item En un cuerpo, los únicos ideales son: $\{0_A\}$  y A.
\end{enumerate}

\begin{lemma}\label{thmIdeal1}
Sea $I$ un ideal de $A$, entonces

\[ I = A \dimplies I \cap U(A) \neq \varnothing \]
...
\end{corol}
\begin{proof}
Tenemos $I \subset A$. Por la propia definición de cuerpo, $U(A)=A/\{0\}$. 

Supongamos ahora que $I≠\{0\}$. En este caso, tendríamos que $I\cap U(A) ≠ \emptyset$, y por lo tanto y según el lema (\ref{thmIdeal1}) tendríamos que $I=A$. Es decir, que no hay ningún ideal propio distinto de $\{0\}$.
\end{proof}


\begin{theorem}
 $(A,+,*)$ es un cuerpo si y sólo si sus únicos ideales son $\{0\}$  y A
\end{theorem}
\begin{proof}
 Primero demostramos la implicación a la derecha. Tomamos $j \in J$.
 Si $A$ es un cuerpo, pueden darse dos casos: 
 \begin{itemize}
  \item $j \in U(A) \Rightarrow \gen{j}=A$
  \item $j=0 \Rightarrow \gen{j}=\{0\}$
 \end{itemize}
 
Vamos a probar ahora la implicación a la izquierda. Si $A$ no es un cuerpo, $\exists a \in A \tq a \notin U(A) \Rightarrow \gen{a} \subset A$ pero $\gen{a}≠A$.
\end{proof}
...
\begin{example}
 Tomemos $A = \mathbb{Z},\, f_1=6, f_2=21$. Entonces, construimos el ideal como 
\[  I=\gen{f_1,f_2}=\{6a+21b \tq a,b\in \mathbb{Z}\} \]
 $6\mathbb{Z}\subset I \ \wedge \ 21\mathbb{Z}\subset I \Rightarrow 3\mathbb{Z}$ \todo{what?}
\end{example}

\begin{defn}[Ideal\IS principal]\label{defIdealPrincipal}
...
\subsection{Particiones}
Al igual que hicimos particiones de un grupo a partir de un subgrupo, haremos lo mismo: particiones de un anillo a partir de un ideal. Antes de hablar de particiones, es necesario definir una relación de equivalencia:
...
Una vez definida esta relación, vamos a \textit{partir A en cajas} obteniendo las diferentes clases de equivalencia.
...
\begin{proof}
\paragraph{Suma} Tenemos que probar que $a\rel a' \wedge b\rel b' \Rightarrow (a+b)R(a'+b')$.
  Para ello basta con ver que $a+b-a'-b'=a-a'+b-b'=i_1+i_2 \in I$. Siendo $a-a'=i_1 \wedge b-b'=i_2$
 
\paragraph{Producto} Tenemos que probar que $a\rel a' \wedge b\rel b' \Rightarrow (ab)\rel (a'b')$
...
\begin{defn}[Maximal]
 ...
 De esta forma, $A/I$ sería equivalente al número de ideales que "se interponen" entre I y A.
 \end{defn}
...
\begin{defn}[Núcleo\IS de un anillo]
...
\begin{proof}
 \begin{itemize}
  \item $\ncl f=\{a \in A \tq f(a)=0_A\}$. Como $f$ es, en particular, un homomorfismo de grupos, sabemos que $\ncl f< A'$. 
...
\begin{lemma}
 Un homomorfismo de anillos $\appl{f}{A}{A'}$  es un isomorfismo si y sólo si $f$ es una biyección
\end{lemma}
...
Además, dada  $\appl{f}{A}{A'}$, $\exists \appl{\bar{f}}{A/N(f)}{A}{A'} \tq f=\pi \circ \bar{f}$  siendo f inyectiva.
\end{theorem}

\begin{proof}
 Vamos a definir la aplicación $\bar{f}(\bar{a})=f(a)$  que cumple las condiciones impuestas por el teorema. Ahora vamos a comprobar que está bien definida, es decir, que no dependa del representante de la clase escogido.
...
\begin{theorem}[Teorema\IS de isomorfía en anillos (II)]
 Si  $\appl{f}{A}{A'}$  es un homomorfismo de anillos, entonces $\img f$ es isomorfa a $A/\ncl f$.
 
 Por otro lado, si $f$ es sobreyectiva, entonces A' es isomorfo a $ A/\ncl f$.
\end{theorem}

\subsection{Anillos de polinomios}

Vamos a estudiar los anillos de polinomios sobre un cuerpo $\mathbb{K}$. Por tanto $\mathbb{K}[X]$ denota el anillo de polinomios con coeficientes en $\mathbb{K}$.

\subsubsection{Definiciones}
\begin{defn}[Polinomio\IS constante] Un polinomio constante es aquel que se puede identificar con un elemento de $\mathbb{K}$. Es decir, que no tiene ningún término con $x$.
\end{defn}
...
\begin{defn}[Polinomio\IS mónico] Un polinomio no nulo es mónico de grado r si y sólo si $a_r=1$.
\end{defn}

Para tener un anillo correctamente definido, tenemos que describir las operaciones que consideramos sobre él. En este caso definimos la suma y el producto de polinomios de la forma habitual. Como se puede deducir, el 0 y el 1 son los elementos neutros respecto del producto y la suma respectivamente.
...
Primero empezamos con un lema básico.
...
\begin{defn}[Múltiplo\IS de un polinomio]
 Sean $f(x),g(x)$ polinomios, ambos contenidos en un dominio de integridad $\mathbb{K}[X]$. Decimos que $g(x)$ es un múltiplo de $f(x)$ cuando existe $h(x)\in \mathbb{K}[X] \tq g(x)=f(x)h(x)$.
 
 Dicho de otra forma, $g(x)$ es múltiplo de $f(x)$ si y sólo si el resto de dividir $g(x)$ entre $f(x)$ es 0.
\end{defn}

Vamos a ver una aplicación del algoritmo de división. Vamos a utilizarlo para demostrar que si $\mathbb{K}$  es un cuerpo, todo ideal es principal (definición \ref{defIdealPrincipal}). Recordemos que siendo $A$ un anillo, un ideal $I$ es un principal si está generado por un único elemento.
...
\begin{proof}
  Si I es un ideal y es igual al 0, entonces I es principal. Si no es 0, entonces contiene un elemento distinto del 0, es decir, contiene un polinomio no nulo $f(x)$.
  
 Sea $g(x)$ un polinomio no nulo en $I$ de grado mínimo. Como $g(x)\in I$ todo múltiplo de $g(x)$ se contiene en I. Por tanto $\gen{g(x)} \subset I$.
 
 Afirmo que $I=\gen{g(x)}$. Vamos a probarlo. Sea $h(x)\in I$, lo divido por $g(x)$ y obtenemos
 \[ h(x)=g(x)q(x)+r(x) \] 
 
 con $\mathrm{gr}(r(x)) < \mathrm{gr}(g(x))$ ó $r(x)=0$.
 ...
 Por tanto $r(x)\in I$ y $gr(r(x))<gr(g(x))$. Pero esto no puede ser porque tomamos $g(x)$ con el menor grado posible. Por tanto $r(x)$ tiene que ser igual a 0.
\end{proof}
...
\begin{lemma}
 Todo ideal principal no nulo en $\mathbb{K}[X]$  es de la forma $\gen{g(x)}$ donde $g(x)$ es mónico.
\end{lemma}
...
\begin{remark} Dados dos ideales en $\mathbb{K}[X]$, $\gen{f(x)} \subset \gen{g(x)} \dimplies f(x)∈\gen{g(x)} \dimplies f(x)=g(x)h(x)$ con $\mathrm{gr}(h(x))>1$.
...
\begin{proof} Tal y como hemos planteado la división de polinomios, tenemos que 

\begin{gather*}
 g(x)=(x-\alpha)q(x)+r(x) \\ g(\alpha)=(\alpha-\alpha)q(\alpha)+r(\alpha)=r(\alpha)
 \end{gather*}
\end{proof}
...
\begin{proof}
 Dados dos polinomios primitivos f(x) y g(x), suponto que existe un primo p $\in \mathbb{Z}$  que divide a todos los coeficientes del producto
 f(x)g(x).\\
...
\begin{theorem}[Criterio de irreducibilidad de Eisenstein]
 Tomo un polinomo f(x)=$a_0 \ + \ a_1\cdot x + \cdots + x^{n} \ \in \mathbb{Z}[X]$\\
 Si existe un primo p que divide a todos los coeficientes salvo al del término de mayor grado y $p^2$  no divide a $a_0$\\
 Entonces el polinomio f(x) no se puede expresar como producto de polinomios.
\end{theorem}
\begin{proof}
 Vamos a suponer f(x) mónico y a probarlo por reducción al absurdo\\
 Tomamos f(x) de grado n y factorizable de la forma f(x)=g(x)h(x)\\
 
 Supongamos g(x)=$b_0+\cdots b_sx^{x}$  y h(x)=$c_0 +\cdots c_{n-s}x^{n-s}$\\
 
 Al multiplicar g(x) y h(x) tenemos que $b_s\cdots c_{n-s} = 1 \Rightarrow b_s,c_{m-s} = \pm 1$. Puedo suponer además, que ambos son 1.\\
 Vamos a pasar de $\mathbb{Z}[X] \ a \ \mathbb{Z}[X]/p\mathbb{Z}[X]$  mediante el homomorfismo de anillos $\pi$\\
 Al aplicar el homomorfismo $\pi$  a el polinomio f(x) nos quedamos sólo con el coeficiente del término de mayor grado. Por ser un homomorfismo
 de anillos la imagen de f(x) será el producto de las imágenes, por tanto al aplicar el homomorfismo $\pi$  sobre los polinomios g(x) y h(x)
 deben reducirse a 0 también todos aquellos términos salvo el de mayor grado. \\
 
 Por tanto, el primo p también divide a los coeficientes de g(x) y de h(x). En particular, p divide a $b_0$  y a $c_0$, por tanto $p^2$  divide
 a $a_0$.
\end{proof}
