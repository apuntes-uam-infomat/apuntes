...
\begin{itemize}
 \item Cerrado por las operaciones, es decir: AxA$\Rightarrow$ A. 
...
\end{itemize}
Estas propiedades puede resumirse en la principal propiedad que define los anillos: \textbf{La propiedad conmutativa}\\
En un anillo se da la unicad de los elementos neutros.
\begin{example}
...
Tenemos la propiedad cancelativa respecto de la suma ya que tenemos elemento inverso. No obstante, no tenemos elemento inverso respecto del producto, 
ya que esta operación no tiene por qué tener inverso multiplicativo.\\
En general, vamos a suponer que el elemento nuetro de la suma y del producto son distintos.

\begin{defn}[Cuerpo]
 Anillo cnmutativo con la propiedad: $U(A)=A \setminus \{0\}$
\end{defn}
\obs Sea -1 el inverso aditivo del neutro aditivo, $\forall a \in A \ a^{-1}$ = (-1)a. Es decir, su producto con el inverso aditivo de la unidad
...
\begin{itemize}
 ...
 \item Si a $\in U(A) \Leftarrow \exists a^{-1} \tq a*a^{-1}=a^{-1}*a=1$
\end{itemize}
...
\begin{enumerate}
 \item Si a $\in$U(A) $ \Leftarrow$  a no es divisor de 0
 \item Si a no es divisor de 0, podemos aplicar la propiedad cancelativa
 \item Si A es un anillo finito $\Leftarrow$  a $\in$ A no es divisor de 0 $\Leftrightarrow$  a $\in$ U(A)
\end{enumerate}

Vamos a demostrarlas:
\begin{proof}
 \begin{enumerate}
  \item \[ab=0 \Leftarrow a^{-1}ab=a^{-1}0=0 \Leftarrow b=0 !¡\]
  \item \[ab=c \Leftrightarrow a(b-c)=0. \ Si \ a \ no \ es \ divisor \ de \ 0 \Leftarrow b-c=0 \Leftrightarrow b=c\]
  \item La implicación hacia la izquierda se deduce de la segunda propiedad aquí citada.\\
  Para la implicación a la drecha tenemos:
  \[Si \ a \ no \ es \ divisor \ de \ 0, \ defino \appl{f_a}{A}{A} \tq f_a(b)=ab \Leftarrow f_a \ es \ inyectiva\]
  Por ser un grupo finito, $f_a$  es biyectiva, lo que implica que llega a todo el conjunto de destino. Por tanto, $\exists b \tq f_a(b)=1 \Leftarrow b=a^{-1}
  \Leftarrow a \in U(A)$.
 \end{enumerate}
...
\begin{defn}[Ideal]
 ...
 \begin{enumerate}
  ...
  \item a $\in$ A y b$\in$ B $\Leftarrow$  a*b$\in$I
 \end{enumerate}
\end{defn}
...
\begin{enumerate}
 ...
 \item $I_1, I_2$  ideales de A $\Leftarrow I_1 \cap I_2$  es un ideal.
...
Vamos a probar los dos sentidos de la implicación
 \begin{itemize}
  \item $\Leftarrow$\\
  Sea I=A, se tiene que 1 $\in$ I $\Leftarrow$ 1$\in I\cap U(A)$
  \item $\Rightarrow$\\
  Sea $r \in I \cap U(A) \Leftarrow r \in I \wedge r*r^{-1}\in I \Leftarrow 1\in I$\\
  Por tanto $\forall a \in A \Leftarrow 1*a=a\in I \Leftarrow A=I$
 \end{itemize}
...
Si I=$\{0\} \Leftarrow I \cap U(A) \neq \varnothing \Leftarrow I=A$
\end{proof}
...
\begin{example}
 Tomemos A =$\mathbb{Z}$\\
 $f_1=6 \wedge f_2=21$\\
 I=$<f_1,f_2>={6a+11b \tq a,b\in \mathbb{Z}}$\\
...
\begin{proof}
 ...
 \begin{enumerate}
  ...
  \item Si i $\in\pi^{-1}(\bar{J}) \wedge a \in A \Leftarrow ai\in \pi^{-1}(\bar{J})$\\
...
\begin{lemma}
 Un homomorfismo de anillos $\appl{f}{A}{A'}$  es un isomorfismo sii f es una biyección
\end{lemma}
...
\begin{proof}
 ...
 $\bar{a}\in N(\bar{f}) \Leftrightarrow \bar{f}(\bar{a})=0_{A'} \ \equiv \bar{f}(\bar{a})=f(a)=0 \Leftarrow a \in N(f) \bar{a}=\bar{0}$
\end{proof}
...
\begin{itemize}
 \item La suma de dos polinomios es, o bien un polinomio nulo, u otro polinomio de grado menor que el máximo de los grados de los sumandos
 \item Si el producto de dos polinomios es nulo, entonces al menos uno de los factores también lo era
\end{itemize}
...
\end{theorem}
\begin{proof}
 Si (x-$\alpha$) divide a g(x) $\Rightarrow \ g(x)=(x-\alpha)q(x)+r(x) \ \Leftarrow g(\alpha)=(\alpha-\alpha)q(\alpha)+r(\alpha)=r(\alpha)$
\end{proof}

\obs U($\mathbb{K}[X]$)=U($\mathbb{K}$)=polinomios no nulos de grado 0.
...
Fijamos un ideal $I\subset K[x]$, siendo $K$ un cuertp. 
