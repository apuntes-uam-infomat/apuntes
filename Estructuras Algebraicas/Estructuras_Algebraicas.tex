...
\begin{theorem}[Teorema\IS de isomorfía en anillos (II)]
 Si  $\appl{f}{A}{A'}$  es un homomorfismo de anillos, entonces $\img f$ es isomorfa a $A/N(f)$.
 
 Por lo tanto, si $f$ es sobreyectiva $A'$ es isomorfo a $A/N(f)$.
\end{theorem}
...
\begin{theorem}[Criterio\IS de irreducibilidad de Eisenstein]
 Tomo un polinomo $f(x)=a_0  +  a_1\cdot x + \cdots + x^{n} \in \mathbb{Z}[X]$.
 
 Si existe un primo $p$ que divide a todos los coeficientes salvo al del término de mayor grado y $p^2$  no divide a $a_0$, entonces el polinomio $f(x)$ no se puede expresar como producto de polinomios.
\end{theorem}
