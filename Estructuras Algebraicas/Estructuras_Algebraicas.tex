...
Si cogemos la expresión  $e=a\ast a'$ y multiplicamos  $a''$ en los dos lados obtenemos  $a''= a'' \ast a\ast a'$. Pero como $ a'' $ es inverso de a, tenemos que $a''= (a'' \ast a)\ast a' = e\ast a' = a'$ por la propiedad asociativa. Así pues, obtenemos $a'=a''$.
\end{proof}
...
\begin{defn}[Subgrupo]
  ...
  \begin{enumerate}
    \item $S$ es cerrado por la operación ( $x,y \in S \implies \varphi(x,y) \in S$ )
    \item El elemento neutro pertenece al subgrupo ($e \in S$)
    \item Todo elemento del subgrupo tiene a su vez su inverso en el mismo ($s \in S \implies s^{-1}\in S$).
  \end{enumerate}
...
\begin{defn}[Homomorfismo]
 Sea $\appl{f}{(G_1, \cdot )}{(G_2, \ast )}$. $f$ es un homomorfismo de grupos si
...
\begin{defn}[Trasposición] Una trasposición es un ciclo de longitud 2 de la forma $(ab)$, es decir, que lleva $a$ a $b$ y $b$ a $a$.
\end{defn}
...
\begin{defn}[Conjugado]
 Decimos que dos grupos A, B son conjugados si $\exists g \in G \tq gAg^{-1}=B$. Por definición, dos grupos conjugados son isomorfos
\end{defn}
...
\begin{defn}[Polinomio\IS constante] Un polinomio constante es aquél que podemos identificar con los elementos del cuerpo $\mathbb{K}$.
\end{defn}
