...
El procedimiento habitual podría ser derivar e igualar a 0, pero en este caso no es derivable. Vamos a ver que la solución es la mediana y vamos a demostrarlo de 2 maneras distintas.
...
Tomando una muestra de tamaño $n$, vamos a verlo.

Tomamos una serie de intervalos contenidos de la siguiente manera:

\[ [y_{(1)},y_{(n)}] \subset  [y_{(2)},y_{(n-1)}] \subset [y_{(3)},y_{(n-2)}] \subset ... \subset [y_{(m)},y_{(n-m+1)}]\]
