...
Sea $S\subset\mathbb{R}^n$ un convexo cuyo cierre afín es $\mbox{afin}(S)$. El \concept{interior relativo} de $S$ se define como el conjunto de puntos $x\in S$ tales que existe $r>0$ con $B(x,r)\cap \mbox{afin}(S) \subset S$.

\begin{itemize}
\item ¿Cuál es el interior relativo de $S = \{x\in \mathbb{R}^3:\, -1\leq x_1\leq 1,\, -1\leq x_2\leq 1,\, x_3=0\}$?

$\afin{S} = \{(x_1,x_2,0) : x_i\in\real\}$.
%
$∀x\in \mbox{int}\left(S\right)$, podemos construir una bola con un radio suficientemente pequeño para que $B_{\varepsilon}(x)\cap \afin{S} \subset S$. 
%
Los puntos $x\in \partial S$ no se encontrarán en el interior relativo, ya que $\nexists r \tlq B_{r}(x)\cap \afin{S} \subset S$

Es decir, $intrel(S) = \{x\in \mathbb{R}^3:\, -1< x_1< 1,\, -1< x_2< 1,\, x_3=0\}$

El \textbf{interior relativo es interesante porque} define el interior de un conjunto si estuviéramos en menor dimensión. 
%
Es decir, el interior de un cuadrado en $\real^3$ es Ø, pero el interior de un cuadrado en $\real^2$ no es Ø. 
%
La idea de interior relativo es "preservar" que el interior en $\real^3$ no es vacío aunque según la definición lo sea.
%
Otro ejemplo, los segmentos no tienen interior (según la definición topológica) si los miramos en $\real^3$, pero el punto medio del segmento está en lo que intuitivamente se podría considerar el interior. 
%
El interior relativo define esa idea "intuitiva". 
...
\begin{proof}
En la figura \ref{II.14:counterex} vemos un contraejemplo por el que es necesaria la condición de compacidad. 
%
El conjunto tiene 3 puntos extremos, cuyo cierre convexo no es igual al conjunto entero.
...
\fill[pattern color = green!70!white, pattern = north east lines] (0,0) -- (0,1) -- (2,2)  -- (4,2) -- (4,0) -- (0,0);
\draw (-0.5,0) -- (2,0);
\draw (0,-0.5) -- (0,2);
\draw (0,0) -- (0,1) -- (2,2)  -- (4,2);
\draw (0,0) -- (4,0);
\fill[pattern color = red!70!white, pattern = vertical lines] (0,0) -- (0,1) -- (2,2) -- cycle;
\end{tikzpicture}
\caption{En rojo el cierre convexo del conjunto sombreado en verde.}
\label{II.14:counterex}
\end{figure}
...
\obs La notación es realmente importante entenderla bien porque el algoritmo de Simplex la utiliza mucho.
...
\begin{proof}
%% TODO:
La demostración se deja como ejercicio ya que es parecida a la demostración del teorema de caracterización de puntos extremos.
\end{proof}
...
\[
B = \{a_1,a_2\}, a_j = a_3 \to B^{-1}α_j = \begin{pmatrix}-1&1\\0&1\end{pmatrix}\begin{pmatrix}1\\0\end{pmatrix} \to d = \begin{pmatrix}-1\\0\\\hline1\\0\end{pmatrix}
\]
Encontramos la dirección extrema.

Podemos tomar otra columna no básica, y ver qué obtenemos:
\[
B = \{a_1,a_2\}, a_j = a_4 \to B^{-1}α_j = \begin{pmatrix}-1&1\\0&1\end{pmatrix}\begin{pmatrix}0\\1\end{pmatrix} \to d = \begin{pmatrix}1\\1\\\hline0\\1\end{pmatrix}
\]

En este caso, no tenemos una dirección extrema.

\obs Esta base no ha sido elegida arbitrariamente. Aunque no tiene porqué ser la única base posible.
\end{example}
...
En este caso \[A=\begin{pmatrix}1&1&0\end{pmatrix}\;b=\begin{pmatrix}1\end{pmatrix}\]
...
\subsection{La tabla del simplex.}
...
\subsection{Fórmulas de actualización de la tabla}
...
\begin{itemize}
	\item (1) : Conjunto no acotado de soluciones óptimas.
	\item (2) : Hay un segmento de soluciones óptimas.
	\item (3) : Degeneración y no sabemos. Tendríamos que pivotear y ver qué pasa.
\end{itemize}
