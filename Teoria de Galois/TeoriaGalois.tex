...
\begin{defn}[Elemento\IS primitivo]
...
\item $\gal(E/F)=\{\appl{\sigma}{E}{E} \text{automorfismo} | \sigma_{|F}=id\}$. Por tanto, si la extensión es separable sobre F, tendremos $[E:F]$ extensiones de la inmersión $\appl{\sigma}{F}{E}$ a $\appl{\sigma}{E}{E}$, que son los elementos de $\gal(E/F)$. Por tanto, queda claro que:
\[\abs{\gal(E/F)}=[E:F]\]
\end{itemize}
...
\begin{theorem}[Teorema\IS fundamental de la teoría de Galois]
...
nterior, la antiimagen del subgrupo serán aquellos 
lementos que permanecen fijos.
\end{verbatim}
...
\begin{example} Sean $K = ℚ(\sqrt{2}, i)$, $F = ℚ$. Consideramos \[ G = \mop{\gal}(K/F) = \gen{σ,τ} = \{id, σ, τ, στ\} \] con $σ^2 = Id$.
	...
	\begin{align*}
	i\sqrt{2} &∈ E \\ 
	i\sqrt{2} (\sqrt{2} + 7i) &∈ E \\ 
	2i - 7\sqrt{2} &∈ E = ℚ(\sqrt{2} + 7i) \\
	2i - 7\sqrt{2} + 7\sqrt{2} + 7²i &∈ E \\
	(7²+1)i ∈ E &\Rightarrow i ∈ E \Rightarrow E = K
	\end{align*}
...
\begin{proof}[1 afirmación del teorema fundamental de la teoría de Galois]
	\[E \longrightarrow \gal(K/E) \longrightarrow K^{\gal(K/E)} = E\]

	\begin{enumerate}
		\item Veamos que $E \subset K^{\gal(K/E)}$\\
		Sea $x ∈ E$, y sea $σ ∈ \gal(K/E) \Rightarrow$\\
		$\Rightarrow σ(x) = x$ ya que queda fijo por todos los elementos de $\gal(K/E) \Rightarrow x~∈~K^{\gal(K/E)}$

		\item Veamos que $K^{\gal(K/E)} \subset E$ mediante reducción al absurdo.
		...
		$α ≠ σ_0(α) = $ otra raíz del polinomio irreducible de $α$ sobre $E$.
		
		$σ = \tilde{σ_0}$, es una extensión de $σ_0 $ a $ σ ∈ \gal(K/E)$
		
		Ahora bien, $σ(α) = σ_0(α)$ por ser una extensión y sabemos que $σ_0(α) ≠ α \Rightarrow σ$ no fija a $α \Rightarrow α ∉ K^{\gal(K/E)}$.
		
		Y llegamos a una contradicción, pues teníamos como hipótesis que $α~∈~K^{\gal(K/E)}~\setminus~E$
	\end{enumerate}
\end{proof}


\begin{proof}[2 afirmación del teorema fundamental de la teoría de Galois]\\
	Dada una extensión $K/K^H$ galoisiana, sabemos que $|\gal(K/K^H)| = [K:K^H]$. Usando esto demostraremos que $[K:K^H] = |H|$ comprobando que $[K~:~K^H]~≤~|H|$ y que $[K~:~K^H]~≥~|H|$.

	\begin{enumerate}
		\item Veamos que $H \subset \gal(K/K^H)$.
		...
		\item Vamos a ver que $r = |H| ≥ |\gal(K/K^H)|$
		
		Sea $H = \left\{σ_1, σ_2, \ldots, σ_r\right\}$.
		
		Gracias al resultado del teorema del elemento primitivo podemos suponer que $K=F(α)$. Consideramos $f(t)~=~(t - σ_1(α))~\ldots~(t - σ_r(α)) = b_0~+~b_1t~+~\ldots…~+b_{r-1}t^{r-1}~+~t^r ∈ K[t]$
		...
		Esto se debe a que $∀σ ∈ H, σ$ manda raíces de $F$ a otras raíces de $F$.
		
		Y viendo las igualdades, deducimos que $b_i = σ(b_i), ∀σ ∈ H \Rightarrow b_i ∈ K^H \Rightarrow f(t) ∈ K^H[t] \Rightarrow |\gal(K/K^H)| = [K:K^H] = [K^H(α):K] ≤ r$

		\item Por último, vamos a comprobar ahora la inclusión al otro lado: \[ \mop{\gal}(K/K^H) ⊆ H \] 

		Escribimos $K = F(α)$, $H= \{ σ_1, σ_2, \dotsc, σ_r\}$. Construimos el polinomio \begin{multline*} f(t) = \prod_{σ∈H} (t-σ(α)) = (t-σ_1(α)) \dotsb  (t - σ_r(α)) = \\ = b_0 + b_1 t + \dotsb + b_{r-1} t^{r-1} + t^r ∈ K[t] \end{multline*}

		Me gustaría ver que $f(t) ∈ K^H[t]$, ya que en ese caso tendríamos que $[K : K^H] ≤ r$. Curiosamente, $K= K^H(α)$, así que sabemos calcular el grado de esta extensión: es el grado del polinomio mínimo de α en $K^H$. 
...
\[ σ_2(x) = a_0 - a_1α + a_2 α^2 -a_3 α^3 + a_4i - a_5 iα + a_6 iα^2 - a_7iα^3\]. Igualando coeficientes, tenemos que si $σ_2(x) = x$, entonces $a_1 = a_3 = a_5 = a_7 = 0$. Luego \[ x = a_0 + a_2 \sqrt{2} + a_4 i + a_6i\sqrt{2}\], es decir, $K^{\gen{σ_2}} = ℚ(i, \sqrt{2})$. Esta extensión es galoisiana, luego debe tenerse $\gen{σ_2} \lhd G$.
