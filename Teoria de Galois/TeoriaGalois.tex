...
\begin{itemize}
\item[a)] $F_{i+i} = F_i(α_{i+1}) \tq (α_{i+1})^{d_i} = a_i ∈F_i$, donde $d_i$
...
Ya tenemos definida la cadena que nos da que $\gal(E/F)$ es soluble.
...
Observamos también que $\gal(F_{m-1}/F_{m-2})$ es abeliano. Entonces, podemos probar que $\gal(E/F)$ es soluble.
...
\item n=4 Este, obviamente, lleva un poco más de trabajo.Definimos la cadena de subgrupos normales:
...
Ahora podemos construir $K$ y obtenemos:
\[K = \{id,(1,2)(3,4),(1,3),(2,4),(1,4)(2,3)\}\]
en el que todos los elementos son de orden 2 por lo que $K \approx C_2×C_2$

\item n=5

La demostracion de que no son solubles depende del siguiente cálculo: $α=(i,j,k), β = (k,r,s)$ con $i,j,k,r,s$ distintos dos a dos. Observamos que son 5 elementos distintos (aquí utilizamos $n≥ 5$).

Vamos a calcular $αβα^{-1}\inv{β} = ... = (i,r,k)$.

De esta cuenta vamos a deducir que si $N\lhd K$, entonces $H/N$ abeliano. y con la propiedad de que $H$ contiene todos los 3-ciclos $\implies$ $N$ también contiene todos los 3-ciclos.
...
Para acabar, supongamos que existiera una cadena de subrgupos normales. Como $S_5$ contiene a todos los 3-ciclos y $S_n/H_{m-1}$ abeliano $\implies H_{m-1}$ también los contiene. Siguiendo este proceso, llegamos a la congradicción $H_0 = \{1\}$ contiene a todos los 3-ciclos.
...
Sea $K$ su cuerpo de descomposición y $G = \gal(K/ℚ)$. \textbf{Entonces} $$G\approx S_5$$ Y por lo tanto $f(t) \text{ irresoluble por radicales}$.
...
Ya vimos que $\gal(F_1/F)$ era abeliano, pero podemos ser más precisos, para ello:
...
$$\begin{array}{ccl} ℚ(ω_5)& \longleftrightarrow& \gen{σ_1}\\
ℚ(ω_5)^{\gen{σ_4}}& \longleftrightarrow & \gen{σ_2^2=σ_4} \\
\vdots & \longleftrightarrow & \gen{\dots}\end{array}$$
...
\item [n=13] $G = \gal(ℚ(ω_{13}) /ℚ) \approx\units{13}$

$$|G| = [ℚ(ω_{13}) : ℚ] = φ(13) = 12$$

Vamos a calcular el polinomio irreducible tomando $ω=ω_{13}$.


$$t^{13}-1 = \sum_{i=0}^{12} t^i$$.

Como $\units{13}$ es cíclico ya que tiene un elementos de orden 12, tenemos que ver si $∃x∈G, \tq \ord(x) = 12 \dimplies \ord(x) ≥ 6 \dimplies \ord(x) ≠ 1,2,3,6$.

Estudiamos $\ord(2)$. $\ord(2) ≠ 1,2,3,6$ ya que $2^i ≠ 1, ∀ i=1,2,3,6 \implies \ord(2) = 12 \implies$ cíclico.

\subparagraph{Subgrupos de $G \equiv \units{13}$}
Con cuidado podemos ver que los subgrupos de $G$ son:
$$\gen{σ_{12}} = C_2 = \gen{σ_2^6}$$$$ \gen{σ_3} = C_3 = \gen{σ_2^3}$$$$ \gen{σ_8} = C_4 = \gen{σ_2^3}$$$$ \gen{σ_4} = C_6 = \gen{σ_2^2}$$$$ \gen{σ_2} = C_{12} = G$$



\subparagraph{Cuerpos fijos}
Como todos los subgrupos son cíclicos (en particular, son abelianos y más concretamente  normales).


\textbf{Primer cuerpo fijo:}
$$K^{\gen{σ_2}} = K^{G} = ℚ$$

\textbf{Segundo cuerpo fijo}
$$K^{\gen{σ_4}} = K^{\gen{σ_2^2}} = ?$$ Para calcularo, tomamos $x = a₁ω + a₂ ω^2 + ... + a_{12} ω^{12}$ y resolvemos el sistema $σ_2^2(x) = x$.

$$σ_2^2(x) = \left\{ \begin{array}{c}a_1 = a_4 = a_3 = a_{12} = a_9 = a_{10}\\
a_2 = a_8 = a_6 = a_{11} = a_5 = a_7 \end{array}\right.$$

Esto nos dice:\\ $x∈K^{\gen{σ_4}} \dimplies a_1(\underbrace{ ω + ω^3 + ω^4 + ω^9 + ω^{10} + ω^{12}}_{β_1}) + a_2 (\underbrace{ ω^2 + ω5 + ω^6 + ω^7 + ω^8 + ω^{11}}_{β_2})$, es decir $K^{\gen{σ_4}} = ℚ(β_1)$ ¿Y qué pasa con $ β_2$? Pasa que ya se encuentra en el cuerpo, ya que $β_2 = 1 - β_1$ ($β_1 + β_2 = 0$ debido a que $\sum_{i=0}^{12} ω^i = 0$).
...
Como el grado tiene que ser 2 tiene que tener 2 raíces. Una tiene que ser $β_1$. Por lo que $p(t) = (t-β_1) · q(t)$, siendo $q(t)= t - λ$, donde $λ$ de momento no sabemos que es. Como tiene que ser raíz, lo lógico es pensar que sea $σ_2(β_1)$, ya que si está una raíz tiene que estar su transformada \footnote{Si $α$ es ráiz de $f(t)$, entonces $σ(α)$ es raíz de $f^σ(t)$. En este caso, como estamos calculando cuerpos fijos, tenemos $f^σ(t) = f(t)$} quedando:
...
\textbf{Posibles dudas:} ¿Porqué,si estamos calculando el cuerpo fijo por $σ_2^2$ tomamos $σ_2(β_1)$ y no $σ_2^2(β_1)$? Es importante darse cuenta que en este cuerpo fijo, $σ_2^2 = id$ con lo que no tendría mucho sentido $σ_2^2(β_1)$. Además, si recordamos el teorema de Galois \ref{TmaFundGalois}  tenemos que $\gal(K^{\gen{σ_2^2}} / ℚ) = \gen{σ_{| K^{\gen{σ_2^2}}}}$


\textbf{Tercer cuerpo fijo}
...
Entonces $x∈K^{\gen{σ_2^3}} \dimplies x = a_1 (\underbrace{ω + ω^5 + ω^8 + ω^{12}}_{β_1}) + a_2 (\underbrace{ω^2 + ω^3 + ω^{10} + ω^{11}}_{β_2}) + a_3 (\underbrace{ω^4 + ω^6 + ω^7 + ω^9}_{β_3}) $

Aquí se nos complica el mismo cálculo que antes, ya que el polinomio tiene un grado más, asique vamos a buscar otro camino, Como antes, $\gal(K^{\gen{σ_2^3}} / ℚ) = \gen{{σ_2}_{|K^{\gen{σ_2}}}}$. Pero, $β_2$ y $β_3$ son imágenes de $β_1$ por $σ_2$ y $σ_2^2$ respectivamente.

$K^{σ_2^3} = ℚ(β_1)$

\subparagraph{Cuarto cuerpo fijo}
$$K^{id} = K = ℚ$$
\subparagraph{Quinto cuerpo fijo}
$$ K^{\gen{σ_{12}}} = ℚ(ω+ω^{12}) = ℚ(ω^2+ω^{11}) = ...$$

Sabemos que $σ_{12}$ es la conjugación compleja como habíamos visto antes. Si no,  podemos razonar que la conjugación compleja tiene que estar en el grupo y dado que el único elemento con orden 2 es $σ_{12}$, entonces $σ_{12}$ tiene que ser la conjugación compleja. Es por ello que el cuerpo fijo es $K^{\gen{σ_{12}}} = K∩ℝ = ℚ(ω+ω^12)$.

\subparagraph{Sexto cuerpo fijo}

$$K^{σ_3} = ?$$

Resolvemos el mismo sistema de ecuaciones que sale de igualar $σ_3(x) = x \implies σ_3(x) = a_1ω^3 + a_2 ω^6 + a_3 ω^9 + a_4 ω^12 + a_5ω^2 + a_6 ω^5 + a_7ω^8 + a_8ω^{11} + a_9ω + a_{10}ω^4 + a_{11}ω^7 + a_{12}ω^{10}$
...
Tenemos $x∈K^{\gen{σ_3}} \dimplies x=a_1(ω+ω^3+ω^9) + a_2 (ω^2 + ω^5 + ω^6) + a_3(ω^4+ω^{10}+ω^{12}) + a_7(ω^7 + ω^8 + ω^12)$

Entonces $K^{\gen{σ_3}}$ como espacio vectorial sobre $ℚ$ tiene grado 4, y su base es $\{ω+ω^3+ω^9,ω+ω^3+ω^9,ω^4+ω^{10}+ω^{12},ω^7 + ω^8 + ω^12\}$

Pero como cuerpo, está generado por un único elemento (esto dice el teorma del elemento primitivo, ya que estamos con característica 0). Además, los elementos de la base son conjugados por $σ_3$ (no conjugados complejos): podemos comprobar $σ(ω+ω^3+ω^9) = ω^2 + ω^5 + ω^6$ y así sucesivamente con el resto.

Concluimos que el cuerpo fijo $K^{\gen{σ_3}} = ℚ(ω+ω^3+ω^9)$


\textit{Mismo ejercicio resuelto de otra manera}

Vamos a utilizar otra base para calcular el cuerpo fijo. La base a utilizar es: $x = b_0 + b_1ω + ... + b_{11}ω^{11}$
