...
\begin{proof}[1 afirmación del teorema fundamental de la teoría de Galois]
	...
	\begin{enumerate}
		...
		$σ = \tilde{σ_0}$, es una extensión de $σ_0 \Rightarrow σ ∈ Gal(K/E)$\\
		Ahora bien, $σ(α) = σ_0(α) ≠ α \Rightarrow σ$ no fija a $α \Rightarrow α ∉ K^{Gal(K/E)}$.\\
...
\begin{proof}[2 afirmación del teorema fundamental de la teoría de Galois]\\
	Con $K/K^H$ galoisiana, sabemos que $|Gal(K/K^H)| = [K:K^H]$. Usando esto demostraremos $[K:K^H] = |H|$ comprobando que $[K~:~K^H]~≤~|H|$ y que $[K~:~K^H]~≥~|H|$.

	\begin{enumerate}
		\item Veamos que $H \subset Gal(K/K^H)$.\\
		Tomo $σ ∈ H, x ∈ K^H \Rightarrow σ(x) = x \Rightarrow$\\
		$\Rightarrow σ ∈ Gal(K/K^H)$

		\item Vamos a ver que $r = |H| ≥ |Gal(K/K^H)|$\\
		Sea $H = \left\{σ_1, σ_2, \ldots, σ_r\right\}$.\\
		Voy a suponer que $K = F(α)$ gracias al resultado del teorema del elemento primitivo. Consideramos $f(t)~=~(t - σ_1(α))~\ldots~(t - σ_r(α)) = b_0~+~b_1t~+~\ldots…~+b_{r-1}t^{r-1}~+~t^r ∈ K[t]$
		...
		Esto es así porque $∀σ ∈ H, σ$ manda raíces de $F$ a otras raíces de $F$.\\
		Y viendo las igualdades, deducimos que $b_i = σ(b_i), ∀σ ∈ H \Rightarrow b_i ∈ K^H \Rightarrow f(t) ∈ K^H[t] \Rightarrow$\\
		$\Rightarrow |Gal(K/K^H)| = [K:K^H] = [K^H(α):K] ≤ r$
	\end{enumerate} 
\end{proof}
