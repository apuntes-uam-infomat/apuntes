...
Una extensión $E/F$ se llama separable si toda inmersión $\appl{σ}{F}{A}$, $A$ es algebraicamente cerrado, tiene exactamente $[E:F]$ extensiones.
\end{defn}
...
$σ_{1k}(\sqrt[3]{2})$ = una reíz del polinomio $(t^3 - 2)^{σ_1} = t^3-2$

$σ_{1k}(\sqrt[3]{2})$ = una reíz del polinomio $(t^3 - 2)^{σ_2} = t^3-2$
\end{itemize}
\end{example}

\begin{defn}[Elementos primitivos]

Supongamos una extensión finita $F \subset F(α,β)=E$

¿Habrá siempre un r∈E tal que E=F(γ)?$\dimplies$ si $E=F(α₁,...,α_n)$, ¿∃ siempre r tal que E=F(γ)?
\end{defn}
...
Luego, si $[E:F] = n$, entonces existen $n$ extensiones $\appl{σ₁,...,σ_n}{E}{A}$ de la inmersión $j:F\hookrightarrow A$, A es algebraicamente cerrado.
...
Aplicamos inducción al polinomio $g(t) ∃∈K[t]$ definido por $f(t) = (t-α_1) \cdot g(t)$. Como $deg(g(t)) = def(f(g)) + 1$, ya podemos aplicar la inducción.
...
De nuevo, escribimos $f(t) = p₁(t) · ... · p_r(t) ∈F_1[t]$ la  descomposición en factores irreducibles en $F_1[t]$.

Tomamos una raíz $α_1∈K_1$ de $p_1(t)$. De este modo, $σ(α₁)$ será una raíz de $p_1^σ(t)$ y por tanto de $f^σ(t)$.

Savemos que existe una extensión $\tilde{σ}$ de $σ$ del tipo:

$$
\begin{array}{ccc}
F_1&\to&F_1(α_1)\\
σ\;\vline&&\vline\;\tilde{σ}\\
F_2&\to&F_2(σ(α₁))
\end{array}
$$
