...
\begin{enumerate}
\item En primer lugar vamos a ver que $H \subset \gal(K/K^H)$.
...
Escribimos $K = F(α)$, $H= \{ σ_1, σ_2, \dotsc, σ_r\}$ y tomamos de nuevo el polinomio anterior:
...
Para comprobar que los $σ_i$ forman un grupo, vamos a ver:

$σ_5σ_3σ_5^{-1} = σ_5σ_3σ_5$ y vamos a buscar $$\begin{array}{cc}
σ_5σ_3σ_5(i) = σ_5σ_3(-i) = σ_5(-i) &= i\\
σ_5σ_3σ_5(α) = ... &=-iα 
\end{array}$$

Es decir, $σ_5σ_3σ_5 = σ_4 = σ_3^3$.
...
Seguimos con el mismo ejemplo viendo otras cosas interesantes:
...
¿Qué información provechosa sacamos de esto? Veamos qué coeficientes necesitamos para que se cumpla $σ_3(x) = x$
...
Tenemos que $K^H = \{x∈K | x=a₀ + 0α + 0α^2 + 0α^3 + a_4i + 0iα + 0iα^2 + 0iα^3\}$, es decir: $K^H = \{
x = a₀ + a_4i | a_0,a_4 ∈ℚ\} \implies K^H = ℚ(i)$

¿Es esta extensión de Galois? Sí, tiene característica 0 y porque tiene índice 2 (la mitad de ...).
...
\begin{gather*}
K{\gen{σ_8}} = \{x∈K | σ_8(x) = x\} = a₀ + a₁i + a_2 α^2 + a_3α^3 + a_4i + a_5iα + a_6iα^2 + a_7iα^3\\
σ_8(x) = ... = ... = a₀ + a₁iα - a₂α^2 + a_3iα^3 - a_4i -a_5α + a_6iα^2 + a_7α^3
\end{gather*}
...
No tenemos mucha suerte porque no se nos cancelan muchos términos. Vamos a obtener información por otro camino. Sabemos $\ord σ_8 = 2$ (calculado en \ref{tblGaloisT4-2}), entonces $[K^{\gen{σ_8}} : ℚ] = \frac{|G|}{|σ_8|} = 4$
...
Elegimos un posible generador de los 3 posibles. Nos la jugamos con $γ=α-iα$, y obtenemos:
...
Para comprobar por ejemplo si $iα ∈ K^{\gen{σ_8}}$ (si pertenece al cuerpo fijo) vemos que $σ_8(iα) ≠ iα$

Vamos a estudiar la siguiente implicación.
$$\gen{σ_8} \lhd G \dimplies K^{\gen{σ_8}} / ℚ \text{  es galoisiana}$$

Decir que es un subgrupo normal, quiere decir que $σ_i^{-1}σ_8σ_i = σ_8\quad i=1,2,...,7$. Ya hemos visto que $G = \gen{σ_3,σ_5}$, luego basta comprobarlo para $i=3,5$.
...
Viendo que $σ_3^{-1}σ_8σ_3(α) ≠ σ_8(α)$ concluimos que $\gen{σ_8}$ no es normal. \footnote{Revisar el cálculo, porque el profesor no estamos seguros de que no haya errores.}
