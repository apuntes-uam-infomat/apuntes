...
\begin{theorem}[Teorema fundamental de la teoría de Galois]
	...
	\begin{enumerate}
		\item Se tiene la siguiente biyección:
			\[\left\{E\ |\ F \subset E \subset K\right\} \longrightarrow \text{ subgrupos de }G = Gal(K/F)\]
...
\begin{verbatim}
Dado un subrupo de los automorfismos de K que fijan F,
...
\begin{example}
	...
	$G \neq C_4$ ya que en G no tenemos un elemento de orden 4. Sin embargo $G = C_2 \times C_2$
	...
	Siguiendo con el ejemplo $E = ℚ(\sqrt{2} + 7i) = ?$.\\
	$E ≠ ℚ$, ¿$E = ℚ(\sqrt{2})$?\\
	Si $E = ℚ(\sqrt{2})$, tendríamos que $\sqrt{2} ∈ E = ℚ(\sqrt{2} + 7i) \Rightarrow$\\
	$\Rightarrow i ∈ E = ℚ(\sqrt{2} + 7i) \Rightarrow$\\
	$ \Rightarrow i ∈ E \Rightarrow E = K$\\
	Por tanto $E ≠ ℚ(\sqrt{2})$


	¿$E = ℚ(i)$?. No, por lo mismo que antes.


	¿$E = ℚ(i\sqrt{2})$?\\
	Si fuera así, $i\sqrt{2} ∈ E \Rightarrow$\\ 
