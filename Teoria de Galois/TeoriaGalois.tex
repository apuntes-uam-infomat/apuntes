...
\begin{prop}
Un homomorfismo de cuerpos $\appl{σ}{F}{K}$ es siempre inyectivo.
\end{prop}
\begin{proof}
Basta ver que $ker(σ) = {0}$.

Sea $x∈F,x≠0 \implies σ(1) = 1 = σ(x · x^{-1}) = σ(x) · σ(x^{-1}) \implies σ(x)=0$

Esto demuestra que si $x≠0$ entonces $x∉Ker(σ)$, lo que significa que es inyectivo.
\end{proof}

Los homomorfismos de cuerpos se llaman \concept{inmersiones.}

Se dice qe $\appl{σ}{E}{K}$ es automorfismo cuando además es suprayectiva.


\obs $\appl{σ}{F}{K}$ induce un homomorfismo entre los correspondientes anillos de polinomios.

\[\begin{array}{c}
\appl{σ}{F[t]}{K[t]}\\
h(t) = \sum a_it^i \to h^σ(t) = \sum σ(a_i)t^i
\end{array}
\]
...
\begin{proof}
\[
0 = f(α) = \sum a_iα_i \implies 0=σ(0) = \sum σ(a_i)σ(α)^i = f^σ(σ(α)) = 0
\]
\end{proof}
...
Notación: $τ_{|F}$ significa $τ$ restringido a $F$.
\end{defn}

\begin{example}
Tomamos $F=ℚ,E=ℚ(i),L=ℂ$. $\appl{σ}{ℚ(i)}{ℂ}$.

Tenemos 2 posibilidades de extender, con la identidad o con la conjugación.
\end{example}
...
¿Cómo construir τ?. Si es un homomorfismo, la única posibilidad es:

$τ(x) = τ(\sum a_iα^i) = \sum τ(a_i)τ(α)^i = \sum σ(a_i)β^i = h^σ(β)$.

Por las propiedadades de anillos, es clar que τ es un homomorfismo \textbf{siempre que esté bien definido.}

Supongamos que $x=h(α) = f(α)$, donde $f(t)$ es otro polinomio en $F[t]$.

La pregunta es: $h^σ(β) \overset{?}{=} f^σ(β)$.
