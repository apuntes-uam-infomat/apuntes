...
\begin{problem}[3]
\ppart Calcular el grado de la extensión $\mathcal{F}_3(x, y)/\mathcal{F}_3(x^3, y^3)$.

\ppart Encontrar todas la extensiones de la inclusión $\mathcal{F}_3 (x^3 , y^3) → \mathcal{F}_3(x, y)$.
\ppart Probar que esta extensión no tiene elementos primitivos.

\solution

Pequeño anexo interesante: $\mathcal{F}_9 ≠ ℤ_9$, $\frac{\mathcal{F}_3[x]}{x^2+1}$



\spart 
$$[\mathcal{F}_3(x,y) : \mathcal{F}_3(x^3,y^3)] = 9 \implies \mathcal{F}_3 = \frac{ℤ}{(3)}$$


Lo calculamos haciendo cadenas de inclusiones:

$$\begin{array}{ccccc}\mathcal{F}_3(x^3,y^3) &\subset& \mathcal{F}_3(x^3,y^3)(x) &\subset& \mathcal{F}_3(x,y^3)(y)\\
&\downarrow& & \downarrow&\\
&p_1(t) = t^3-x^3& & p_2(t) = t^3-y^3&\end{array}$$

$p_1$, $p_2$ serán irreducibles sii no tienen raíces en $\mathcal{F}_3(x^3,y^3)$.

Esto es sencillo de comprobar puesto que sabemos que $t^3-x^3 = (t-x)^3$ en $\mathcal{F}_3(x,y)$, y este polinomio solo tiene una raíz, que es $x$.

Ahora la única posibilidad de que no fuese irreducible es que resulte que $x\in \mathcal{F}_3(x^3,y^3)$. En ese caso resultaría que la extensión tendría grado 1 pues estaríamos 'añadiendo' un elemento que ya está en el cuerpo.

Si $x$ perteneciera a $\mathcal{F}_3(x^3,y^3)$, entonces podríamos escribir:
\[x = \frac{\sum a_{ij}(x^3)^i(y^3)^j}{\sum b_{ij}(x^3)^i(y^3)^j}; a_{ij},b_{ij} ∈ \mathcal{F}_3\]
...
\[γ∈\mathcal{F}_3(x,y) \implies γ^3 ∈\mathcal{F}_3(x^3,y^3)\]

Supongamos que esto es verdad. Entonces: 

\[[\mathcal{F}_3(x^3,y^3)(γ) : \mathcal{F}_3(x^3,y^3)] ≤ 3\] 

porque $γ$ satisface el polinomio $t^3 -γ^3 ∈ \mathcal{F}_3(x^3,y^3)[t]$

\[γ = \frac{\sum a_{ij}(x^3)^i(y^3)^j}{\sum b_{ij}(x^3)^i(y^3)^j}; a_{ij},b_{ij} ∈ \mathcal{F}_3\]

Vamos a demostrar la pista que, en el examen, nos daba el enunciado.

Para ello, fjémonos en el valor de $γ^3$. Como el cuerpo en el que nos movemos tiene $Ch(\mathcal{F}) = 3$ se cumple que $(a+b)^{3} = a^3 + b^3$.

Por tanto: 
\[γ^3 = \frac{\sum a_{ij}^3(x^3)^i(y^3)^i}{\sum b_{ij}^3(x^3)^i(y^3)^i}; a_{ij},b_{ij} ∈ \mathcal{F}_3 \implies γ^3 ∈ \mathcal{F}_3 \implies [\mathcal{F}_3(x^3,y^3)(γ) : \mathcal{F}_3(x^3,y^3)] ≤ 3\]

\paragraph{Nota:} Esta respuesta estaría también bien:
$$γ^3 = \frac{\sum a_{ij}(x^3)^i(y^3)^i}{\sum b_{ij}(x^3)^i(y^3)^i}; a_{ij},b_{ij} ∈ \mathcal{F}_3 \implies γ^3 ∈ \mathcal{F}_3 \implies [... : ...] ≤ 3$$

Es exactamente la misma respuesta solo que aplicamos el pequeño teorema de Fermat (que dice que en $Ch(\mathcal{F}_p) \implies a^p = a, ∀a∈\mathcal{F}_p$)
