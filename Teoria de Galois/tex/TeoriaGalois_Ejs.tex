...
Tenemos $H=\gen{σ^3} \quad K^{\gen{σ_3}} = ℚ(ω + ω^{-1})$.

$$|H| = 2 \implies \underbrace{\subset}{3} \underbrace{\subset}{2} K = ℚ(ω)$$.

El polinomio irrducible de $ω$ sobre $Ke^H$ será de forma $q(t) = (t-ω) (t-ω^{-1})  = t^2 - ωtω^{-1}t + 1$.

Vamos a calcular el polinomio irreducible de $γ=ω+ω^{-1}$ sobre $K^H$ y lo llamamos $f(t)$. Buscamos un polinomio $f(t) = a₀+a₁t + a₂t^2 + t^3$ con $f(γ) = 0$. Este polinomio sabemos que s de grado 3 porque es el grado de la extensión.
...
\textbf{Conclusión: } El polinomio irreducible del elemento $ω+ω^{-1}$ sobre ??????? es $f(t) = 1-2t+t^2+t^3$
\end{problem}
...
Nos piden demostrar que el grado de la extensión es $4$.moommom
...
Vemos que $∀i, t_i∉ℚ \implies p(t)$ irreducible en $ℚ$.

Por definición, el cuerpo de descomposición $\mathcal{K} = ℚ(++,+-,-+,--)$.

$\mathcal{K} = ℚ(++,+-)$, porque si $a∈ \mathcal{K} \dimplies -a ∈ \mathcal{K}$



\paragraph{[ℚ(++):ℚ]}
El grado de esta extensión tiene que ser $≤4$, por ser de grado 4 el polinomio $p(t)$.

Tenemos que demostrar: $[K:ℚ] = 4\dimplies K =ℚ(++) \dimplies +-∈ℚ(++)$


Vamos a clarificar la notación utilizando $γ_1 = ++$ y $γ_2 = +-$.


\textbf{Sugerencia: } Multiplicar $γ_1 · γ_2$ (nos lo sugiere el enunciado)

$$γ_1γ_2 = ... = 3\sqrt{5} \implies γ_1·γ_2 ∈ℚ(\sqrt{5})\underbrace{\subset}{?} ℚ(γ_1)$$
...
\textbf{Conclusión:} $K = ℚ(γ_1,γ_2) = ℚ(γ_1) = ℚ(γ_2)$ (es decir, las 4 raíces son elementos primitivos.)
