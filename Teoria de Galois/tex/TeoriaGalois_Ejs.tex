...
Sea $α = \sqrt[4]{5}$ y $ω = e^{\frac{2πi}{4}} = e^{\frac{πi}{2}} = i$

$σ_k(α) = i^{k-1}α; k=1,2,3,4$ y $σ_k(i) = i$. Las otras 4, lo mismo cambiando de signo la $i$.

Los generadores son $σ_5$ el de orden 2 (conjugación compleja) y $σ_2(α) = iα$.

¿Se trata de $S_4$?

\spart Se deja como ejercicio para el lector
...
Otro autor distinto al anterior que escribió esa frase, comenta que el apartado de extensiones ciclotómicas (que son justamente estas) hay muchos más ejemplos.
...
Si $Δ∉ℚ$, tenemos $ℚ\overset{≥2}{\subset} ℚ(Δ) \overset{a}{\subset}  ℚ(Δ,γ₁) \subseteq K$. Además, $ℚ\overset{3}{\subset}  ℚ(γ₁) \overset{b}{\subseteq} ℚ(Δ,γ₁)$.
...
Ahora vamos a demostrar la implicación hacia la derecha. $G=S_3 \implies Δ∉ℚ$. Esto se debe a que $Δ$ no queda fijo por todos los elementos de $S_3$ (basta tomar la permutación del 1 con el 2 y entonces $Δ$ cambia de signo), entonces no queda fijo por $G$, es decir $Δ∉K^G$, es decir $Δ∉ℚ$.
...
El teorema del que disponemos (en su versión general) dice que todo polinomio de grado ≥ 5 que tenga solamente 2 raíces complejas es irresoluble por radicales. En clase lo hemos visto para n=5, pero al parecer es general (\href{http://stealcode.com/misc/algebra/Hungerford\_grad.pdf}{Todo el álgebra del mundo según Hungerford}, capítulo 4).

Suponiendo que efectivamente tiene 2 raíces reales, podríamos utilizar el teorema. Si al lector requejoso le parece poco justificado, estos son los pasos que más o menos lo aclaran:

\begin{itemize}
\item El grupo de Galois va a ser un subgrupo de $S_5$ (opción 1: Teorema 4.2 del libro de Hungerford, opción 2: por número de elementos: tengo 5 raíces que llevar a 5 sitios distintos $\implies 5!$  Y ya sabemos que $|S_5| = 5!$\footnote{Creo})

\item $S_5 = \{<(1,2),(1,2,3,4,5)>\}$. Si el grupo de Galois del polinomio tiene 2 elementos, uno de orden 2 y el otro de orden 5 entonces será isomorfo a $S_5$, porque el único subgrupo de un grupo que contiene los generadores del grupo es el grupo entero

\item $σ$ como la conjugación compleja es el elemento de grado 2 que andamos buscando.

\item No vamos a encontrar explícitamente el elemento de orden 5, pero podemos demostrar que existe. 

Sea $K$ el cuerpo de descomposición de $p(x)$. Tenemos que $5 \| [K:ℚ]$ y el teorema de Cauchy nos asegura que tiene que existir un elemento de orden 5. 

¿Porqué 5 divide al orden de la extensión? Sea $α$ una raíz del polinomio. $α$ es algebraico en $ℚ$ y tenemos que $[ℚ(α):ℚ] = 5$ (porque es raíz de un polinomio de grado 5 irreducible en $ℚ$). Si añadimos el resto de raíces a $ℚ$ (obteniendo así el cuerpo de descomposición) vemos que el grado va a ser múltiplo de 5.

Este elemento que sabemos que existe será idenficable con un isomorfismo a $(1,2,3,4,5)$ (o alguna potencia suya).
\end{itemize}

Ya hemos encontrado 2 elementos identificables a los generadores de $S_5$ $\implies$ el grupo de Galois será isomorfo $S_5$. 
