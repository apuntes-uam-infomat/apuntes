...
Este polinomio $p(t)$ ya es irreducible sobre $ℚ$ porque $\appl{σ}{ℚ[t]}{ℚ[t]}, σ(p(t)) = p(t+1)$ es un isomorfismo de anillos, y $p(t+1)$ es irreducible por Einsenstein. Ya hemos hecho en otros ejercicios anteriores este argumentos.
...
\textbf{Útil:} Solo hay 4 posibilidades para el orden del elemento: $1,2,3,6$, porque tienen que dividir al orden del grupo. Es interesante también darnos cuenta de que $1 = σ_3^6 = (\underbrace{σ_3^2}_{σ_2})^3 = (\underbrace{σ_3^3}_{σ_6})^2 = 1 \implies ord(σ_6) = 2$



\paragraph{Subgrupos/cuerpos fijos} Hay cuerpos fijos como subgrupos. 

El cuerpo fijo correspondiente a $σ_1=id$ es $K$.

El cuerpo fijo correspondiente a $K^{\gen{σ_3}}$ es $ℚ$ (creo que por el teorema de Galois). \\Vamos a comprobarlo (por puro amor a las cuentas): $$K^{\gen{σ_3}} = \{x∈K \tq σ_3(x) = x\}$$
...
Si $γ∈ℚ(γ) \implies γ^3 ∈ℚ(γ)$. En este caso, $γ^3 = ω^6 + 3ω^2 + 3ω^5 + ω \imples (ω+ω^6)∈ℚ(γ)$.
