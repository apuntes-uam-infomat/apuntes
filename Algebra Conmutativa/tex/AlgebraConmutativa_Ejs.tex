...
\begin{itemize}
	\item $\Sigma \neq \emptyset$ ya que $p \subset p$.
	\item Definimos la siguiente relación: $p \leq p' \Leftrightarrow p' \subset p$.
	\item Sea $L=\{p_i\}$ una cadena creciente de ideales primos, es decir $p_n \leq p_m \forall m \geq n$, que dada la relación de equivalencia definida es lo mismo que decir que $p_n \supset p_m \forall m \geq n$ queremos ver que la cadena está acotada en $\Sigma$.
	
	Podemos coger como cota superior $p_0=\bigcap_{p_i\in L}$. Esta claro que $p_0 \geq p_i \forall i$, ya que $p_0 \subset p_i  \forall i$.
	
	Sabemos que la intersección de ideales es un ideal, por tanto, nos falta ver que $p_0$ es primo, es decir, que si tomamos $a,b \in R$ tal que $a\cdot b \in p_0$, entonces $a$ ó $b$ tiene que pertenecer a $p_0$, es decir o $a$ o $b$ pertenecen a todos los $p_i$ de L. Supongamos que no, tenemos dos posibilidades:
	\begin{enumerate}
		\item $a$ y $b$ no pertenecen a ningún $p_i \in L$, entonces ningún $p_i$ es primo, por lo que llegamos a contradicción.
		\item Suponemos que existen $a,b\in R$, tal que $a \in p_i, b \notin p_i$ y $a \notin p_i, b \in p_j$ con $j \neq i$. Que también nos llevaría a contradicción ya que, asumimos sin pérdida de generalidad que $p_i \leq p_j$, por tanto $p_j \subset p_i$, por tanto, $a,b \in p_i$.  
	\end{enumerate} 
...
\solution
\doneby{Parra}
\spart En $\ent_12$ tenemos 4 ideales distintos del total o del $\{\zero \}$ que son: $\gen{2}$, $\gen{3}$, $\gen{4}$ y $\gen{6}$. Los dos últimos no son primos ya que tenemos que $2 \cdot 2 \in \gen{4}$ y $2 \notin \gen{4}$. Y por otro lado $2 \cdot 3 \in \gen{6}$ y $2,3 \notin \gen{6}$. 
...
$$f:\real[y] \longmapsto \quot{ℝ[x,y]}{\gen{x^2+x-y}}$$

Es sobreyectiva ya que sea un $p(x,y) \in \real[x,y]$ cualquiera, se puede escribir como un $p(y) in \real[y]$.

Además también es inyectiva ya que $\ker(f)=\{p(y):\cls{p(y)}=0 \}$. Esos son los $p(y) \in \gen{x^2+x-y}$. Sea $p(y) \in \gen{x^2+x-y}$ en $\real[x,y]$, entonces $\exists q(x,y) \in \real[x,y]$ tal que $p(y)=q(x,y)(x^2+x-y)$. Lo cual solo es posible si $q(x,y)=0$, que implica que $p(y)=0$ y por tanto $\ker(f)=0$.

Así nos queda que:

$$\quot{ℝ[x,y]}{\gen{x^2+x-y}} \simeq \real[y]$$

Como $\real[y]$ es un dominio de integridad, entonces $\gen{x^2+x-y}$ es primo. Pero $\real[y]$ no es un cuerpo, ya que $y$ no tiene inverso multiplicativo porque $\frac{1}{y} \notin \real[y]$, por tanto, no es maximal.
