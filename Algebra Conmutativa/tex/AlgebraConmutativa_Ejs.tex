...
Sabemos que $I ⊂ \sqrt{I}$, y en el apartado anterior hemos visto que $\sqrt{I}$ es intersección finita de primos minimales, y creo que con eso me basta.

\spart

Tomamos $\sqrt{0}$ y está contenido en la intersección de todos los minimales, que es una cantidad finita por el apartado anterior.
...
\textcolor{blue}{Este ejercicio consideradlo todo en azul porque había bastantes cosas mal}
...
Calculamos la intersección: para ello sustituimos en la ecuación de la curva Y: $y_2^2 = y_1^2 + y_1^3$ nos queda:

\begin{align*}
(\lambda d)^2 = (\lambda c)^2 + (\lambda c)^3 \\
(\lambda c)^3 + (\lambda c)^2 - (\lambda d)^2 = 0 \\
\lambda^2(\lambda c^3+c^2-d^2) = 0
\end{align*}

De aquí sacamos $\lambda = 0$ y:

\begin{align*}
	\lambda c^3 = d^2-c^2 \\
	\lambda = \frac{d^2-c^2}{c^3}
\end{align*}

Por tanto nos queda:
$$ x = \frac{d^2-c^2}{c^3} c = \left( \frac{d}{c} \right)^2-1 $$

$$ y = \frac{d^2-c^2}{c^3} d = \left( \frac{d}{c} \right) \left(\left( \frac{d}{c} \right)^2-1 \right) $$

Tomamos $a= \frac{d}{c}$ y acabamos de demostrar que todo punto de $Y$ tiene una preimagen en $X$.  Por tanto nuestra función es sobreyectiva.
...
\textcolor{blue}{Ana dijo que hay que decir que es un morfismo porque son funciones regulares, vaya que son polinómicas normales y corrientes.}
...
\begin{problem}[7]
	...
	\begin{align*}
		...
		r & \rightarrow t\quad \forall r \in \real
	\end{align*}
	...
	$(c,d) \in \img(\varphi)$, entonces $\#\{ \varphi{-1}(c,d) \}=1$ (es la cuenta del apartado b).

	\spart

	$\varphi^{-1}(\{ \alpha y_1+ \beta y_2=0 \})$
...
\ppart $\dk ℤ = 1$.
\ppart Para todo $n ≥ 2$, $\dk ℤ_n = 0$.

\solution

\spart

Como se ha visto en teoría, $\dk ℤ = 1$ porque la cadena más larga que podemos montar de primos es $\set{0} \subsetneq \gen{p}$ con $p$ primo.

\spart

Si $n$ es primo, $ℤ_n$ es un cuerpo con dimensión 0. Si no es primo, entonces hay divisores de cero, luego $\set{0}$ no es ideal primo y los únicos ideales primos serán $\gen{p}$ con $p$ primo dividiendo a $n$, luego $ℤ_n$ tiene $\dk ℤ_n = 0$.

\end{problem}

\begin{problem} Sea \K un cuerpo. Demuestra que:

\ppart $\dk \K[x] = 1$.
\ppart Si $I ⊂ \K[x]$ no es el ideal \zerogen, entonces $\dk \quot{\K[x]}{I} = 0$.

\solution

\spart

Como \K es cuerpo, \zerogen es primo. Los otros ideales serán $\gen{p(x)}$ con $p(x)$ irreducible, y además estos son maximales, luego la cadena que podemos montar es $\zerogen \subsetneq \gen{p(x)}$ así que $\dk K[x] = 1$.

\spart

Si $I$ es primo, es maximal en $\K[x]$ así que $\quot{\K[x]}{I}$ es un cuerpo y por lo tanto tiene dimensión $0$. Si no lo es, hay divisores de cero así que \zerogen no es primo, luego sólo tendremos los ideales primos $\pideal ⊂ \quot{\K[x]}{I}$ dados por los primos en $\K[x]$ que contienen a $I$, y estos son maximales luego no podremos montar ninguna cadena. En definitiva, que la dimensión es efectivamente cero.
\end{problem}

\begin{problem} Sea $R$ un anillo con $\dk R < ∞$ y sea $I ⊂ R$ un ideal.

\ppart Demuestra que $\dk \quot{R}{I} ≤ \dk R$.
\ppart Da un ejemplo en el que se dé la desigualdad.
\ppart Demuestra, dando un ejemplo, que se puede dar la igualdad. ¿Cómo debes escoger $I$?

\solution

\spart

Dado que hay correspondencia biyectiva entre ideales primos con el paso al cociente, no vamos a poder crear una cadena más larga en $\quot{R}{I}$ de la que ya tenemos en $R$.

\spart

Como hemos visto en los dos ejercicios anteriores, $\quot{ℤ}{nℤ}$ o $\quot{\K[x]}{I}$.

\spart

El ejemplo estúpido que se me ocurre es $I = \gen{0}$, y entonces la igualdad se da bastante trivialmente.

$\ent_4$ tiene $\dk=0$. Si cojo $\quot{\ent_4}{\gen{2}}=\ent_2$ también tiene $\dk = 0$.
...
\begin{problem}[4] Sea $X= \V(x \gen{y,z}) \subset \A^3_{\real}$, y sea $R = \quot{\real[x,y,z]}{x\gen{y,z}}$. Encuentra en $R$:

	\ppart Una cadena de ideales primos de longitud $2$ que termine en el maximal $\gen{x,y,z-1}$. ¿Con qué cadena de variedades irreducibles contenida en $X$ se corresponde?

	\ppart Una cadena de ideales primos de longitud $1$ que termine en el maximal $\gen{x-1,y,z}$. ¿Con qué cadena de variedades irreducibles contenida en $X$ se corresponde?

	\solution

	X es la unión de una recta $\{ y=0, z=0 \}$ y un plano $\{ x=0 \}$, por tanto $X$ tiene dimensión 2.
...
\begin{problem}[5] Sea $X = \V(x\gen{y,z}) ⊂ \afesp[\K][4]$ y sea $R = \quot{\K[x,y,z,t]}{x\gen{y,z}}$ su anillo de coordenadas. Encuentra en $R$ una cadena de ideales primos de longitud $3$.

\solution

\inclass

Tenemos que $X$ es la unión de un plano $\set{y = 0, z = 0}$ y un hiperplano (o volumen o como queráis llamarlo) $\set{x = 0}$. Para encontrar la cadena miramos variedades irreducibles en $X$. La primera será un punto (el origen, por simplificar), luego podemos tomar la recta $x = y = z = 0$, después el plano $x = y = 0$ y por último el hiperplano $x = 0$.

Así, la cadena de ideales asociados es $\gen{x} \subsetneq \gen{x,y} \subsetneq \gen{x,y,z} \subsetneq \gen{x,y,z,t}$.

\end{problem}

\begin{problem}[6]
	Considera la extensión entera \[ A = \cplex[x,y] \subset B = \quot{\cplex[x,y,z]}{\gen{z^3-xy^2}} \] de dominios de integridad (no hay que probarlo). Encuentra

	\ppart Todos los ideales maximales $\mideal ⊂ B$ tales que $\mideal ∩ A = \gen{x-1, y-2}$.
	\ppart Todos los ideales primos $\pideal ⊂ B$ tales que $\pideal ∩ A = \gen{x-y}$.
	\ppart Todos los ideales primos $\pideal ⊂ B$ tales que $\pideal ∩ A = \zerogen$.

	\hint{Sea $X = \set{z^3 - xy^2 = 0} ⊂ \afesp[ℂ][3]$, sea $Y = \afesp[ℂ][2]$ y sea $\appl{f}{X}{Y}$ el morfismo inducido por la inclusión $A ⊂ B$. Ahora observa que en el apartado a) se te pide la fibra sobre el punto $(1,2)$, en el b) la fibra sobre la recta $x = y$ y en el c) la fibra sobre el plano.}
	...
	Procedemos del siguiente procedimiento procedimental.  Consideramos el primo $\pideal = \gen{x-y}$, y vemos su extendido en el cociente ($\gen{\cls{x} - \cls{y}}$). Buscaremos entonces los primos que contengan a ese extendido.

	Ahora bien, tenemos que tener cuidado: $\gen{x - y}$ no es maximal, así que los primos que encontremos no pueden ser maximales en el cociente.

	Buscamos primos cual niño aburrido en una comunión. Como viene siendo habitual, cocientamos de nuevo por $\gen{\cls{x} - \cls{y}}$ y buscamos los primos ahí. Simplificando, \[ \quot{ℂ[x,y,z]}{\gen{z^3 - xy^2,x-y}} \simeq \quot{ℂ[x,z]}{\gen{z^3 - x^3}} \]

	Lo que nos queda es una curva en $\afesp[ℂ][2]$. Los maximales ahí son puntos, así que esos no los queremos. Buscaremos otros primos no maximales (componentes irreducibles).

	Podemos factorizar y ver que \[ z^3 - x^3 = (z-x) \left(z-xe^{\frac{2π}{3}}\right) \left(z - x e^{\frac{4π}{3}}\right) \]

	Así, los primos que nos salen son $\mathfrak{q}_1 = \gen{x-y, z-x}, \,\mathfrak{q}_2 = \gen{z-y, z - x e^{\frac{2π}{3}}},\, \mathfrak{q}_3 = \gen{x-y, z - x e^{\frac{4π}{3}}}$. Ahora bien, tenemos que tener cuidado porque haciendo la intersección nos puede salir algo más grande que $\gen{x-y}$ (cuando lo hacíamos con maximales no hacía falta verificarlo porque no podíamos encontrar algo más grande que un maximal).

	Vamos a hacer el argumento con el primer ideal (es análogo para el resto). Sabemos que $\pideal ⊂ \mathfrak{q}_1$. Queremos ver que son iguales, así que que hacemos lo siguiente:
	\begin{center}
	\begin{tikzcd}[column sep = 50pt]
	\mathbb{C}[x,y] \arrow[hookrightarrow]{r}{\text{finita}} \arrow{d}{}
		& \quot{\mathbb{C}[x,y,z]}{\gen{z^3 - xy^2}} \arrow{d}{} \\
	\quot{\mathbb{C}[x,y]}{\mathfrak{q}_1 \cap A} \arrow[hookrightarrow]{r}{\text{finita}}
		& \quot{\mathbb{C}[x,y,z]}{\gen{z-xy^2, z-x, x-y}} \simeq \mathbb{C}[x]
	\end{tikzcd}
	\end{center}

	$ℂ[x]$ tiene dimensión de Krull $1$ y $\quot{ℂ[x,y]}{\gen{x-y}}$ también, así que la intersección no puede ser más grande.

\end{problem}

\begin{problem} Considera la extensión de anillos \[ A = \quot{ℂ[x,y]}{\gen{y^2 - x^3} ⊂ B = \quot{ℂ[x,y,z]}{\gen{y^2 - x^3, z^3 - xz^2 - y^3 + 512}}} \]

\ppart Demuestra que $\dk A = \dk B$.
\ppart Sea $Y = \set{y^2 = x^3} ⊂ \afesp[ℂ][2]$ y sea $X = \set{y^2 = x^3, z^3 - xz^2 - y^3 = -512} ⊂ \afesp[ℂ][3]$.  Demuestra que la extensión $A ⊂ B$ induce un morfismo de variedades $\appl{φ}{X}{Y}$ y descríbelo.
\ppart Calcula la fibra de $φ$ sobre el punto $(4, 8) ∈ Y$.
\ppart Encuentra todos los maximales $\mideal ⊂ B$ tales que $\mideal ∩ A = \gen{x - 4, y-8}$ y compara tu respuesta con el resultado del apartado anterior.
...
LA forma de probarlo será ver que la dimensión es entera. Es claro que es una extensión, y además es fácil ver que $z$ es un cero del polinomio \[ p(T) = T^3 - \cls{x} T^2 + \cls{y}^2 + 512 ∈ A[T]\]

Como el polinomio es entero, la extensión es entera y la dimensión de Krull es la misma.

\spart

Para ver el morfismo vemos que el homomorfismo de anillos de coordenadas ($A = K[Y]$, $B = K[X]$) viene dado por
\begin{align*}
\appl{φ^*}{ℂ[Y]&}{ℂ[X]} \\
\cls{x} & \longmapsto \cls{x} \\
\cls{y} & \longmapsto \cls{y} \\
\end{align*}
luego el morfismo quedará simplemente
\begin{align*}
\appl{φ}{X&}{Y} \\
(a,b,c) &\longmapsto (a,b)
\end{align*}

De lo que se trata es de la superficie ``cúspide'' ($Y$ es la cúspide en el plano $xy$), que luego en el espacio de 3-dimensiones hacemos superficie y la cortamos con otra superficie (la del polinomio infernal ese), y hacemos la proyección.

\spart

A la vista de lo que hemos dado, la fibra no debería de ser vacía (el punto está en la variedad). Además, nos deberían de salir 3 puntos porque la extensión es entera con un polinomio de grado 3.

Para calcular la fibra, miramos el maximal asociado al punto, que es $\mideal = \gen{x-4, y-8}$, y calculamos el extendido de este ideal a través del homomorfismo $φ^*$.

Lo que hacemos es resolver las ecuaciones, añadiendo $x = 4$ y $y = 8$. Sustituyendo, nos queda la ecuación $z^2(z-4) = 0$, que nos da $z = 0$ (dos soluciones que han colapsado) y $z = 4$, luego la fibra es \[ \inv{φ}(\set{4, 8}) = \set{(4,8,0), (4, 8, 4)} \]

Nota: en teoría tendríamos que haber tenido en cuenta que el ideal lo tenemos que encontrar dentro de $A$, y aquí lo hemos sacado en $ℂ[x,y]$. La cuestión es que como $(4,8) ∈ Y$, el ideal maximal de definición está en correspondencia biyectiva con un ideal maximal en $A$, que sería $\gen{\cls{x} - 4, \cls{y} - 8}$ (con las clases de equivalencia).

\spart

Lo que nos dice el apartado es que calculemos los maximales que nos dan los puntos de la fibra de $(4,8)$. Según la teoría (\fref{prop:ExtensionMaximal}), lo que tendríamos que hacer sería buscar los ideales maximales en $B$ que contengan a $\gen{x-4, y-8}$, que es equivalente a buscar los maximales en el cociente \[ \quot{ℂ[x,y,z]}{\gen{y^2 - x^3, z^3 - xz^2 -y^3 + 512, x - 4, y- 8}} \]

Simplificando todas esas ecuaciones del cociente (la misma cuenta que en el apartado anterior) tenemos que eso es isomorfo a $\quot{ℂ[z]}{\gen{z^3 - 4z^2}}$ y tendremos que los maximales son las componentes irreducibles de ese polinomio $z^3 - 4z^2$, que es precisamente lo que nos ha salido antes (yuju, no estamos con teoría que no funciona).

\seprule

Duda de Álvaro: ¿Qué ocurriría si estuviésemos en $ℝ$?

Consideramos las dos siguientes extensiones: \[ ℝ[x] \hookrightarrow \quot{ℝ[x,y]}{\gen{y^2 - x}} \qquad ℝ[x] \hookrightarrow \quot{ℝ[x,y]}{\gen{xy - 1}} \]

La primera es una parábola y la segunda una hipérbola. La forma de ver que la segunda no es una extensión entera es ver que no hay una fibra sobre el $0$. Si vemos la imagen extendida de $\gen{x} \to \gen{x}^e = 1$, porque $x$ nos sale invertible y por lo tanto definitivamente no puede ser entera.

La cuestión es que el segundo caso sí es entera, pero por ejemplo la fibra sobre $-1$ no parece que vaya a tener ningún punto. Pero si hacemos las cuentas, tenemos que el ideal de definición de $x = -1$ es $\gen{x + 1}$. Haciendo el extendido de la imagen, tenemos que resolver las ecuaciones \[ \begin{cases} x + 1 = 0 \\ y^2 = x \end{cases} \] que simplifica a $y^2 + 1$. Luego el maximal es $\gen{y^2 + 1}$, que es maximal pero no es el total. Esa es la diferencia: si no genera el total, es posible que no estemos ante un cuerpo algebraicamente cerrado y que la extensión siga siendo entera.
\end{problem}

\begin{problem} Sea $X = \V(\gen{xy + zx + yz})⊂ \afesp[ℂ][3]$.

\ppart Demuestra que el anillo $\quot{ℂ[x,y,z]}{\gen{xy + zx + yz}}$ es finito sobre un anillo de polinomios en dos variables.

\ppart Demuestra que existe un plano afín $π ⊂ \afesp[ℂ][3]$ y una proyección finita $\appl{p}{X}{π}$; describe el plano y la dirección de la proyección.
...
\textit{Nota: Vamos a pensar en que el ideal de definición es el que nos dicen porque si no es un pifostio.}

Hacemos el siguiente cambio: \[ \begin{cases}
x_1 = x - az \\
y_1 = y - bz \\
z_1 = z \end{cases} \] y nos queda el polinomio \[ (ab + a + b) z_1^2 + (ay_1 + bx_1 + x_1 + y_1)z_1 + x_1 y_1\]

Para que el polinomio sea mónico buscamos $a,b$ para que el coeficiente de $z_1^2$ no sea cero. En ese caso tendremos que la extensión $ℂ[x',y'] \hookrightarrow K[X]$ es finita.

\spart

Lo que queremos hacer es encontrar un nuevo sistema de referencia tal que la proyección es simplemente en la dirección vertical. Es decir, que proyectamos al plano $z_1 = 0$ en la dirección $x_1 = 0, y_1 = 0$. Sustituyendo, tenemos que en el sistema de referencia habitual la proyección viene dada por la dirección $x = az, y = bz$ al plano $z = 0$.
