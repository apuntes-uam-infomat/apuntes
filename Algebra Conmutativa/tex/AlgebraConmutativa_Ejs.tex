...
\subsection{Problemas para entregar}
\begin{problem}[1]
	Sea $M$ un $R$-módulo y sea $Ann_R(M) = \{a \in R: am=0, \forall m \in M \}$
	\ppart Demuestra que $Ann_R(M)$ es un ideal de R.
	\ppart Demuestra que $M$ tiene estructura de $\quot{R}{Ann_R(M)}$-módulo y que ésta coincide con la que tiene como $R$-módulo.
	\ppart Considera el ideal $J=\gen{\cls{3}} \subset \ent_{12}$. Demuestra que $J$ es un $\ent_4$-módulo.
	\ppart Decide de manera razonada para qué valores de $n$, el ideal $J$ del apartado anterior es de manera natural un $\ent_n$-módulo.
	\solution

	\spart
	\begin{itemize}
		\item $Ann_R(M) \neq \emptyset$, ya que $0 \in Ann_R(M)$, porque $0\cdot m = 0 \quad \forall m \in M$.
		\item Sea $a \in Ann_R(M)$ y $b \in Ann_R(M)$ entonces $a+b \in Ann_R(M)$.
		...
		\item Sea $a \in Ann_R(M)$ y $r\in R$ entonces $(r\cdot a)m=0$ $\forall m \in M$.

		Cierto ya que si $a \in Ann_R(M)$ entonces $am=0$, y por las propiedades sensatas de los $R$-módulos tenemos que:
		$r(r'm)=(rr')m \; \forall r,r' \in R, \; \forall m \in M$. Por tanto: $(ra)m=r(am)=r0=0$.
	\end{itemize}

	\spart

	Queremos ver primero que $M$ es un $\quot{R}{Ann_R(M)}$-módulo:
	\begin{enumerate}
		\item $(M,+)$ es un grupo abeliano por ser $M$ un $R$-módulo.
		\item  Definimos la operación externa:
		\begin{align*}
			\quot{R}{Ann_R(M)} × M & \longmapsto  M \\
			(\cls{r},m) & \longmapsto  \cls{r}\cdot m = rm \\
		\end{align*}
		Tenemos que ver que esta operacion está bien definida. Cogemos un representante de $\quot{R}{Ann_R(M)}$, por ejemplo $\cls{r}=r+a$ con $a\in Ann_R(M)$. Ahora definimos el producto:

		$$\cls{r} m = (r+a)m= rm+am \underbrace{=}_{a \in Ann_R(M)} rm + 0 = rm$$
	...
	Por tanto $M$ tiene estructura de  $\quot{R}{Ann_R(M)}$-módulo y también de $R$-módulo, y estas coinciden.
	...
	\begin{itemize}
		...
		$$\cls{a}\cdot \cls{m}=a3n+12ak+12ln+48lk \underbrace{\equiv}_{mod 12} a3n=\cls{a3n}=\cls{a}\cls{3n}$$
	...
	\ppart Para $n=4t \quad \forall t \in \nat$.
...
\begin{problem}
	...
	\ppart Utiliza los apartados anteriores y la propiedad universal de la localización para demostrar que ${R_{\gen{x}} \cong K(y)}$.

	\solution

	\spart

	$R \setminus \gen{x}$ son los elementos de la forma $p(x,y) = \sum_{k=0}^N a_k (y^k + b_k x^k)$ con $a_k, b_k ∈ K$.

	\spart

	Los elementos invertibles será los $p(x,y) ∈ R\setminus \gen{x}$, es decir, los polinomios cuyo denominador cumpla que, si tiene una potencia $x^k$, también tiene una potencia $x^k$.

	\spart

	Sabemos que, dado $a ∈ R$, $f(a) = 0 \iff ∃s ∈ R \setminus \gen{x}$ tal que $s · a = 0$. Si $a ∈ \gen{x}$, entonces podemos encontrar un polinomio $p$ con variables sólo en $y$ de tal forma que $a·p ∈ \gen{xy}$.
...
\begin{problem}
	...
	\ppart Demuestra que da lo mismo primero localizar y después cocientar, que primero cocientar y luego localizar. Es decir, demuestra que $S^{-1}R/IS^{-1}R \cong \pi(S)^{-1}(R/I)$.
	...
	La \nlref{thm:PropUniversalLoc} nos dice que si $\appl{φ}{R}{B}$ es un homomorfismo de anillos que lleva elementos de $S ⊂ R$ a unidades en $B$, entonces existe un único homomorfismo de anillos $\appl{g}{S^{-1}R}{B}$ tal que $φ = g ○ f$, con $f$ el homomorfismo de paso al localizado $f(a) = \frac{a}{1}$.
	...
	Tenemos que demostrar que $g$ es sobreyectivo.
	...
	Por otra parte, tenemos que ver que si $(r,s) ∉ IS^{-1}R$ entonces $g(\cls{(r,s)}) ≠ 0$.
