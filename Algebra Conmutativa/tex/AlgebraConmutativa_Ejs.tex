...
\begin{defToUse}
	\begin{itemize}
		\item \textbf{R anillo de polinomios}: $R[x] = \set{ \sum_{i=0}^n a_i x^i \tq a_i \in R, n \in \nat}$. Dos expresiones en $R[x]$ son iguales $\iff$ los coeficientes son iguales uno a uno\ie $a_i = b_i \ \forall i$.
	\end{itemize}
\end{defToUse}

Por tanto, para demostrar que $K[x]$ es un dominio euclídeo, tenemos que definir una aplicación $\phi$ que sea normal euclídea.

Sea $\phi$ una aplicación que asigna a cada polinomio su grado. vamos a probar que $\phi: K \setminus \{0\} \rightarrow \nat$ es una aplicación normal euclídea:
\begin{enumerate}
	\item Si tenemos $p(x) | q(x)$, entonces quiere decir que $q(x)=p(x)\cdot r(x)$, con $r(x) \in K[x] /\{0\}$. Es obvio que el grado de $p(x)$ es menor o igual que el grado de $p(x)\cdot r(x)$.
	\item Sea $a(x), b(x) \in K[x]$, con $b(x) \neq 0$, tenemos que ver que existe $r(x), q(x) \in K[x]$ tal que $a(x)=b(x)q(x)+r(x)$ con $r(x)=0$ ó $gr(r(x)) < gr(b(x))$. 
	
	Eso también es cierto. Utilizando el algoritmo de la división, existen $q,r \in K[x]$ tales que $c=bq+r$ con $r=0$ ó $\phi(r) < \phi(b)$.
\end{enumerate}
...
\begin{defToUse}
	...
	\end{itemize}
\end{defToUse}
Sea $K[x]$ dominio euclídeo. Sea $I$ un ideal de $K[x]$, por definición, $I$ es subgrupo normal de $K[x]$. Si $I=\set{0}$ es claro que es un ideal principal. Supongamos entonces que I contiene al menos a un elemento distinto de 0.

Sea $a \in I$, tal que $\phi(a) \leq \phi(c)$ para cualquier otro $c\in I$. Entonces, utilizando el algoritmo de la división, existen $q,r \in R$ tales que $c=qa+r$ con $r=0$ ó $\phi(r) < \phi(a)$.

Pero $r=c-qa \in I$, $r \neq 0$, implicaría $\phi(r) \geq \phi(a)$, por la definición de $a$, en contra de las hipótesis del algoritmo de la división. Por tanto, tendríamos que tener $r=0$, y todo elemento $c\in I$ es de la forma $c=qa$, por tanto, $I=(a)$.



\spart
\textcolor{red}{Que pesadez...}

\spart
Este ejercicio se resuelve cogiendo ejemplos y proposiciones vistos en teoría.

Consideramos el ideal $I=\{p(x,y)\in K[x,y]:p(0,0)=0 \}$. Primero vamos a ver que es un ideal:
\begin{enumerate}
...
Ahora usamos la proposición \ref{prop:def_idealGeneradoxy}, para probar que ese conjunto esta generado por dos elementos, $'x'$ e $'y'$.
...
Por tanto hemos encontrado un ideal que no es principal en $K[x,y]$ porque esta generado por dos elementos.
\end{problem}
...
El anillo $\ent_n$ es un anillo local para $n=p^m$, para p primo y $m \in \nat$. El único ideal maximal de esos anillos es el ideal $<\cls{p}>$.
 
Vamos a probarlo:
Sea $\ent_{p^m}$. Vamos a ver que $<\cls{p}>$ es su único ideal maximal. Sabemos que $a\in <\cls{p}>$ si $a=kp$ para algún $k\in \nat$. 

Sea $b \in \ent_{p^m}$ tal que $\cls{b} \neq k\cls{p}$, $\forall k \in \nat$. Entonces $b \nmid p \implies b \nmid p^m \implies m.c.d(b,p^m)=1 \implies <b> = \ent_{p^m}$. 

Hemos probado que cualquier elemento que no pertenezca a ese ideal, genera todo el conjunto. Por tanto $<\cls{p}>$ es maximal, y para $n=p^m$ tenemos que $\ent_n$ es local.

Falta probar que si $n \neq p^m$, entonces $\ent_n$ no es local. Pero $\ent$ es un dominio de factorización única, por tanto, si $n \neq p^m$, nos queda como única opción que $n=p_1^{m_1}p_2^{m_2}...p_s^{m_s}$ con $s > 1$, $p_1,...,p_n$ primos. Y en ese caso, $<p_1>$, $<p_2>$,...,$<p_s>$ serían ideales maximales distintos los unos de los otros.
...
Para ver si $m$ es un ideal vamos a comprobar si cumple las 3 propiedades de los ideales:

\begin{enumerate}
	\item $m \neq \emptyset$, es cierto ya que el elemento $\frac{x}{1} \in m$.
	\item $(m,+)$ es un subgrupo. Como hemos visto esto es equivalente a probar que $\forall \frac{p_1(x)}{q_1(x)}, \frac{p_2(x)}{q_2(x)} \in m$, entonces $\frac{p_1(x)}{q_1(x)}- \frac{p_2(x)}{q_2(x)} \in m$.
	...
	\item Si escogemos $\frac{p_1(x)}{q_1(x)} \in R$ y $\frac{p_2(x)}{q_2(x)} \in m$, tenemos que ver que $\frac{p_1(x)}{q_1(x)}\cdot \frac{p_2(x)}{q_2(x)} = \frac{p_1(x)p_2(x)}{q_1(x)q_2(x)} \in m$. Obvio ya que $p_2(0)=0 \implies p_1(0)p_2(0) = 0$
\end{enumerate}

\spart
Basándonos en la sugerencia, vamos a probar que todo $ \frac{p(x)}{q(x)} \in R \setminus m $ es una unidad.

Así, sea $a=\frac{p(x)}{q(x)}$ tal que $q(0) \neq 0$ y $p(0) \neq 0$. Entonces, el elemento $\frac{q(x)}{p(x)}$ es el inverso multiplicativo de $a$, ya que $\frac{p(x)}{q(x)}\cdot \frac{q(x)}{p(x)}=1$. Por tanto si $a \in R \setminus m$ entonces $a$ es unidad. Y por el ejercicio 7 tenemos que:

$R$ es un anillo local con ideal maximal $\mathfrak m$ $\Leftrightarrow$ $R$ contiene un ideal  $\mathfrak m$ tal que todo elemento de $R\setminus \mathfrak m$ es una unidad.
...
\begin{enumerate}
	...
	Cierto ya que si $a \in (I:J)$, entonces $aJ=\{a_1,...,a_j\} \subset I$, y si $b \in (I:J)$, entonces $bJ=\{b_1,...,b_j\} \subset I$. Por tanto, $a_1,...,a_j,b_1,...,b_j \in I$ y $a_i-b_k \in I$ $\forall i,k \in [1,j]$ porque $I$ es un ideal y por tanto $(I,+)$ es un subgrupo. Por tanto $(a-b)J \subset I$ y $a-b \in (I:J)$.
...
Tenemos que:
 ...
 $$(\langle 12\rangle : \langle n \rangle) = \{ a \in R: a<n> \subset \gen{12} \} =  \gen{\frac{12}{n}} $$
 
 Ya que $\gen{\frac{12}{n}}$ son todos los elementos $a = m \frac{12}{n}$ con $m \in \ent$, y $<n>$ son todos los elementos $b = t n$ con $t \in \ent$. Esta claro que $ab=m\frac{12}{n} t n = 12\cdot m \cdot t \subset \gen{12}>$.
 
 \spart 
 
 $$(I: \langle a\rangle) =  \{ b \in R: b \gen{a} \subset I \} $$
 
 \textcolor{red}{Carezco de la intuición femenina de Ana para intuir esta cosa tan poco intuitiva}
 ...
 $(p: I) = \{ a \in R: a I \subset p \} $
 
 \begin{itemize}
 	\item Si $I \subset p$, entonces $(p:I)=R$.
 	\item Si $I$ no esta contenido en $p$, entonces $(p:I)=p$
 \end{itemize}
 
 \textcolor{red}{Lo de por qué crees, que alguien tire de inventiva}
 
 \spart 
 
  $(I:\langle a_1,\ldots, a_r\rangle)= \{ b\in R : b \langle a_1,\ldots, a_r\rangle \subset I \} = \{ b\in R : b\langle a_1\rangle, b\langle a_2\rangle,...,b\langle a_r\rangle \subset I \} = \{ b\in R : b_1\langle a_1\rangle \subset I\} \cup \{ b_2\in R : b\langle a_2\rangle \subset I\} \cup ... \cup \{ b_r\in R : b\langle a_r\rangle \subset I\} = \cap_{i=1}^r(I: \langle a_i\rangle)$
  
  \textcolor{red}{Si alguien se anima a ponerlo más bonito 	que lo haga}
