...
\begin{itemize}
	...
	$$X_1 \cup X_2=\V(I_1 \cap I_2)=\V(I_1 \cdot I_2)=\V(\gen{f_i, g_j})$$
...
\begin{example}
	...
	Es un número finito de puntos porque primero fijamos los puntos que anulan los polinomios de la forma $\gen{y^2-x}=p(x,y)\cdot(y^2-x)$, que son infinitos puntos de una parábola.
	...
	Por tanto llegamos a una contradicción y $\I(X)=\gen{y^2-x}$, es un ideal primo y por lo tanto $X$ es irreducible. \noteby{Guille}{No veo muy clara la contradicción.} \textcolor{blue}{Parra: Pues hamijo, hemos visto que SUPONIENDO que $\gen{y^2-x} \subsetneqq \I(X)$ entonces $\V(\I(X))$ esta contenido en una cantidad finita de puntos, pero esto es una mentira incredibol porque la parábola tiene infinitos puntos. Así que, CONTRADICCIÓN!! Luego nos queda que  $\gen{y^2-x} = \I(X)$ que es un ideal primo. Ver que $\gen{y^2-x}$ es primo yo lo mirario viendo que el cociente $\quot{\real[x,y]}{\gen{y^2-x}}=\real[y]$ que es un dominio de integridad}
\end{example}
