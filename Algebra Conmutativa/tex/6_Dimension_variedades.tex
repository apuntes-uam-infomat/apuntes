...
\begin{defn}[Dimensión\IS de Krull] \label{def:DimKrull}
...
\begin{prop} \label{prop:ExtensionEnteraPrimos}
...
\begin{prop} \label{prop:ExtensionPrimosUnico}
...
\begin{corol} \label{crl:DimKrullExtension}
...
\begin{theorem}[Teorema\IS Going-up] \label{thm:GoingUp}
...
\begin{defn}[Morfismo\IS finito] \label{def:MorfismoFinito}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\begin{tikzcd}
			\K[Y] \arrow[hookrightarrow]{r}{entera}
			\arrow[rightarrow]{d}{\varphi^*}
			&  \K[X]
			\arrow[rightarrow]{d}{g}\\
...
\begin{defn}[Polinomio\IS mónico en la variable $x_i$]
...
Este cambio de variables funciona siempre que el cuerpo sea infinito.
...
\begin{theorem}[Lema\IS de normalización de Noether]
