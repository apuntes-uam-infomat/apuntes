...
\begin{example} En $\ent$
	Sean $<2>$ (los pares) y $<3>$ (múltiplos del 3) son ideales, sin embargo $J=<2> \cup <3>$ no es un ideal.

	Esto es así porque $3\in <3>$, $2 \in <2>$ y sin embargo $3-2=1$, que no pertenece a $<2> \cup <3>$, por tanto, $(J,+)$no es subgrupo de $\ent$, y por tanto $J$ no es ideal de $\ent$
\end{example}
...
\begin{defn}[Producto de ideales]
	Sean $I$ y $J$ dos ideales, entonces $J\cdot I = <a\cdot b: a \in I, b \in J>$

	En general, sea $I=<f_1,...,f_s>$ y $J=<g_1,...,g_t>$ entonces:
	$$I\cdot J=<f_i, g_j: i \in \{1,...,s\}, j \in \{1,...,t\}$$
\end{defn}

\begin{example} Sea $<x,y> \subset \real[x,y]$
	$$<x,y>^2 = <x^2,xy,y^2>$$
\end{example}
...
\begin{example}
	Sea $I=J=<x,y>$, entonces $I\cdot J = <x^2,xy,y^2>$.
	...
	\obs En los complejos si se podría ya que $(x+iy)(x-iy)=x^2 + y^2$
\end{example}
...
\begin{example}
	\begin{itemize}
\item Inclusión estricta: en $\ent$, cogemos $I=<2>$, $J=<4>$. Tenemos que $I\cdot J=<8>$ pero $I\cap J=<4>$.
\item Inclusión no estricta: en $\ent$, cogemos $I=<2>$, $J=<3>$. Tenemos que $I\cdot J=<6>$ pero $I\cap J=<6>$.
	\end{itemize}
\end{example}

\begin{defn}[Radical de un ideal]
...
\begin{prop}
	Sea K un cuerpo, y sea $K[x]$, entonces $<x>$ es primo.
\end{prop}
\begin{proof}
	...
	Supongamos que $p(x)\cdot q(x) \in <x>$. Entonces, por ser un ideal, por la propiedad de absorción $\exists r(x) \in K[x]$ tal que $p(x) q(x)=r(x) x$. Por tanto, necesariamente $x|p(x)$, y en ese caso $p(x) \in <x>$, o $x|q(x)$, y en ese caso $q(x) \in <x>$.
\end{proof}

Generalizamos la proposición anterior:
\begin{prop}
	Sea K un cuerpo, y sea $K[x_1,...,x_n]$, entonces $<x_i>$ es primo.
\end{prop}
...
\begin{prop}
	Sea $K$ un cuerpo (\textcolor{red}{Cierto también si K es un anillo que no sea cuerpo no?}), y sea el ideal $<x,y>$ en $K[x,y]$, entonces:
