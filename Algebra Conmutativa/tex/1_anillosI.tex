...
\begin{theorem}[Tercer teorema de isomorfía][Teorema\IS 3º de isomorfía]
...
\textcolor{blue}{Demostracion a saco (cuentas tontas):
	...
	\begin{theorem}[Propiedad universal de la localización]
		...
		\path[-stealth]
		(m-1-1) edge node [above] {$f$} (m-1-3)
		(m-1-1) edge node [below] {$\phi$} (m-2-2)
		(m-2-2) edge node [right] {$g$} (m-1-3);
		\end{tikzpicture}
	\end{theorem}
	...
	\begin{theorem}
		Sea $S \subset R$ una parte multiplicativa. El homomorfismo de paso al localizado $\phi:R \longmapsto S^{-1}R$ tiene las siguientes propiedades:
		\begin{enumerate}
			\item Si $s\in S$, $\phi(s)$ es invertible en $S^{-1}R$.
			\item $\phi(a)=0 \Leftrightarrow \exists \in S$ tal que $s\cdot a=0$.
			\item Cada elemento de $S^{-1}R$ es de la forma $\phi(r)\phi(s)^{-1}$
		\end{enumerate}
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			$$ = \{ \frac{r}{s}: f(r)=0_B \} \implies r \in \ker(f) \implies r \in \ker(\phi)\textcolor{red}{???}$$ 
		\end{itemize}
	\end{proof}
