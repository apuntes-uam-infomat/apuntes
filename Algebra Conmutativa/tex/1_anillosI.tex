...
\begin{enumerate}
	...
	\begin{proof}
		Basta probar que $\pi^{-1}(\pi(L)) \subseteq L+I$. Sea $a \in \pi^{-1}(\pi(L)) \implies \pi(a) \in \pi(L) \implies \exists b\in L \text{ tal que } \pi(a)=\pi(b) (\text{ b pertenece a la clase de equivalencia de a) } \implies \pi(a)-\pi(b)=\pi(a-b)=0 \implies a-b \in Ker(\pi) = I  \implies a=b+\alpha$, con $b \in L$ y $\alpha \in I$.
	\end{proof}
...
Podemos complicarnos la vida un poco más estudiando $\gen{2, x-1}$, cuyas propiedades serán más difíciles de calcular manualmente. Ahora bien, las cosas se simplifican si estudiamos $\quot{ℤ[x]}{\gen{2, x-1}}$. Tenemos una parte igual que antes (los coeficientes viven en $ℤ_2$), pero si además hacemos el cociente por $x-1$, tenemos que $\cls{x} = 1$ (El resto de dividir x entre x-1) y por lo tanto $\quot{ℤ[x]}{\gen{2, x-1}} = \fd_2$, así que es maximal, primo y radical.
...
Para probar que $\quot{R}{Ker(f)} \sim Im(f)$, basta probar que si nos restringimos a las $f \subset S$, entonces $\cls{f}:\quot{R}{Ker(f)} \sim Im(f)$ es biyectiva.
...
$$ ker(\cls{f})=\{ \cls{a} \in \quot{R}{Ker(f)}: \cls{f}(\cls{a})=0 \} = \{ \cls{a}\in \quot{R}{Ker(f)}: f(a)=0 \} = \{0\} $$
\end{proof}

\begin{prop}
Sea $I \subset Ker(f)$, el siguiente diagrama conmuta:
...
En tal caso, se dice que $f$ factoriza por el homomorfismo $f_1: \quot{R}{I} \rightarrow S$. Esta proposición no es más que una generalización del Primer Teorema de Isomorfía.
\end{prop}
...
\begin{theorem}[Teorema\IS 2º de isomorfía] \label{thm:Isomorfia2} Sean $I ⊂ J ⊂ R$ ideales de un anillo $R$. Entonces \[ \quot{R}{J} \simeq \frac{\quot{R}{I}}{\quot{J}{I}} \]
\end{theorem}
...
\matrix (m) [matrix of math nodes,row sep=4em,column sep=2em,minimum width=2em]
{
	R &  & \quot{\quot{R}{I}}{\quot{J}{I}} = S\\
	& \quot{R}{I} = \quot{R}{\ker(\pi)} &  \\};
\path[-stealth]
...
%Mirar como se aplica aqui el primer teorema de isomorfía y comtrastarlo con lo de los apuntes.
\end{proof}
