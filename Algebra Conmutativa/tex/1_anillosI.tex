...
Por definición un \nlref{def:IdealPrimo} cumple que, cada vez que $ab ∈ I$, o bien $a ∈ I$ o bien $b ∈ I$. En nuestro caso, esto será equivalente a que si $\cls{ab} = \cls{0}$, entonces $\cls{a} = 0$ o $\cls{b} = 0$, luego $\quot{R}{I}$ es un dominio de integridad.
...
Vamos a ver un poco más en detalle la construcción de espacios cocientados con polinomios, como por ejemplo $\quot{ℝ[x]}{\gen{x^2+1}} \simeq ℂ$, o $\quot{ℝ[x,y]}{\gen{xy}} \simeq ?$, donde $\cls{x}\cls{y} = \cls{0}$.

También podemos fabricar $\quot{ℝ[x,y]}{\gen{xy-1}}$ que hace que tanto $\cls{x}$ como $\cls{y}$ sean invertibles, ya que $\cls{xy} = 1$.

% Me estoy perdiendo mucho aquí
...
La otra aplicación importante es la siguiente: tomamos $I \subsetneq R$ un ideal. Entonces podremos demostrar que $\sqrt{I} = \bigcap_{\substack{p ⊂ R\text{ primo}\\ I ⊂ p}} p$.
...
\begin{enumerate}
	\item f induce un homomorfismo de anillos:$$ f^': R/Ker(f) \rightarrow S$$
	\item $f'$ es un isomorfismo de:
	$R /Ker(f)$ con $Im(f) \subset S$: $R/Ker(f) \sim Im(f)$
\end{enumerate}
