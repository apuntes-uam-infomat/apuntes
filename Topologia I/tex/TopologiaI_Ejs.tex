...
\begin{prop} Si $\appl{f}{X}{Y}$ es continua e $Y$ es Hausdorff (\ref{defHausdorff}) entonces \[ G_f = \{ (x, f(x))\tq x∈X \} \] es cerrado en $X×Y$.
\end{prop}
...
\begin{problem}[7]
\solution
\spart Hacemos una observación: si $V_1, V_2⊆X$, entonces $V_1 ∩ V_2 = ∅$ es equivalente a que $Δ∩(V_1×V_2)=∅$.
...
\begin{problem}[16] En el plano con la topología usual, sea \[ S = \left\{ (r \cos t,r \sin t ) \tq r = 1 − \frac{1}{t} , t ≥ 1 \right\} \]. Probar que $X = S ∪ \mathbb{S}^1$ es conexo pero no es conexo por caminos. \solution
\end{problem}
