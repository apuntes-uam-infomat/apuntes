...
La función $R(λh)$ suele denominarse \concept{función de amplificación}
...
La parte inicial de este ejercicio, que nos muestra la idea a seguir, ya se ha visto en los apuntes. No obstante, repetimos aquí las cuentas para poder encauzar el ejercicio.
...
\[G(h)=y(x_n)+h\sum_{i=1}^sb_if() + h^2\sum_{i=1}^sb_ic_if_x()+h^2\sum_{i,j=1}^sb_ia_{ij}f_y()+\varphi(x_n,y(x_n),h)h^3\]
...
\[R_n=h\left(f-\sum_{i=1}^sb_if\right)+h^2\left(\frac{1}{2}f_x+\frac{1}{2}f_y\cdot f - \sum_{i=1}^sb_ic_if_x()-\sum_{i,j=1}^sb_ia_{ij}f_y()f()\right)+O(h^3)\]
...
\item $\sum_{i,j=1}^sb_ia_{ij} = \frac{1}{2}$
\end{itemize}

Donde la última propiedad es trivial por la asumción de la condición suma.
\spart

Para forzar a que el orden de consistencia sea 3 tenemos que forzar que el orden del residuo sea 4, para lo que necesitamos desarrollar un paso más los desarrollos de Taylor anteriores. Así tenemos:

\[F(h)=y(x_n)+hy'(x_n)+\frac{h}{2}y''(x_n)+\frac{h^3}{3!}y'''(ε_n) = \]
\[= y(x_n)+hf+\frac{h^2}{2}\left(f_x+f_y\cdot f\right)+\frac{h^3}{3!}\left(f_{xx}+ f_{xy}\cdot f + f_{yx}\cdot f+f_{yy}\cdot f^2+ f_y\cdot f_x+f_y^2\cdot f \right) + \frac{h^4}{4!}y^{iv}(ε_n)\]

Por otro lado:
\small
\[\frac{\partial^2 K_i}{\partial h^2}(x_n,y(x_n);h) =\frac{\partial}{\partial h} \left(c_if_x\left(x_n+c_ih,y(x_n) + h\sum_{j=1}^sa_{ij}K_j(x_j,y(x_j);h\right)+\atop f_y\left(x_n+c_ih,y(x_n)+h\sum_{j=1}^sa_{ij}K_j(x_j,y(x_j);h)\right)\sum_{j=1}^sa_{ij}K_j(x_j,y(x_j);h)\right)=\]
\normalsize
Para simplificar la notación vamos a realizar la derivada y evaluar en $h=0$ directamente, pues es lo que necesitamos para el desarrollo de Taylor.

\[\frac{\partial^2 K_i}{\partial h^2}(x_n,y(x_n);0) = c_i^2f_{xx}+c_if_{xy}\sum_{j=1}^sa_{ij}f+\left(f_{yx}c_i+f_{yy}\sum_{j=1}^sa_{ij}f()\right)\sum_{j=1}^sa_{ij}f() + \atop f_y()\sum_{j=1}^sa_{ij}\left( c_if_x+\sum_{j=1}^sa_{ij}f_y\cdot f\right)\]

Una vez tenemos esta derivada podemos escribir

\[G(h)=y(x_n)+h\sum_{i=1}^sb_if() + h^2\sum_{i=1}^sb_ic_if_x()+h^2\sum_{i,j=1}^sb_ic_{ij}f_y()+ \frac{h^3}{2} \sum_{i=1}^sb_i c_i^2f_{xx}+\]
\[+ \frac{h^3}{2} \sum_{i=1}^sb_i c_if_{xy}\sum_{j=1}^sa_{ij}f+\frac{h^3}{2} \sum_{i=1}^sb_i \left(f_{yx}c_i+ f_{yy}\sum_{j=1}^sa_{ij}f()\right)\sum_{j=1}^sa_{ij}f() + \atop \frac{h^3}{2} \sum_{i=1}^sb_i f_y()\sum_{j=1}^sa_{ij}\left( c_if_x+\sum_{j=1}^sa_{ij}f_y\cdot f\right) +\varphi(x_n,y(x_n),h)h^4\]

Si ahora forzamos que los términos de $F(h)$ y $G(h)$ con $h^i$ para $i\neq 4$ deben ser iguales, obtenemos las relaciones pedidas en el enunciado.
\begin{itemize}
\item $\sum_{i=1}^sb_ic_i^2 = \frac{1}{3}$
\item $\sum_{i,j=1}^sb_ic_ia_{ij}=\frac{1}{3!}$
\end{itemize}
además de las calculadas en el apartado anterior.

También se obtienen otras relaciones, que deberán poder ser deducidas a partir de estas.

\spart

Este apartado requiere demasiadas cuentas.
\end{problem}
...
Este ejercicio se deduce inmediatamente de las cuentas realizadas en el ejercicio anterior ya que las hemos realizado sin simplificar en base a la condicicón suma. Basta con sacar el resto de ecuaciones necesarias.\footnote{En el ejercicio anterior no eran necesarias pues podían deducirse de las ya mostradas y la conidición de suma. Al no tener esta última condición, las relaciones dejan de ser triviales.}
\end{problem}
...
\[\left(\begin{array}{c} y_{n+2} \\ y_{n+1}\end{array} \right) = \underbrace{\left(\begin{array}{cc} a(z) & b(z) \\ 1 & 0\end{array} \right)}_{A}\left(\begin{array}{c} y_{n+1} \\ y_{n}\end{array} \right)\]
...
Podemos simplificar la matriz escribiendo:
\[A = \frac{1}{12-5z}\left(\begin{array}{cc} 12+8z & z \\ 12-5z & 0\end{array} \right)\]

Al calcular los autovalores (restando λ a los elementos de la diagonal y calculando el determinante obtenemos:
\[(12+8z-λ)(-λ)-12z+5z^2 = λ^2+λ(-12-8z)-12z+5z^2 = 0\]
Resolviendo la ecuación tenemos:
\[λ = \frac{12+8z \pm \sqrt{144+64z^2+88z+48z-20z^2}}{2}=\frac{12-8z \pm \sqrt{44z^2+136z+144}}{2}\]

Repitiendo las cuentas que hemos hecho en diversos ejercicios, podemos ver que el método será $A$-estable si estos autovalores tienen módulo menor que 1.
\end{problem}
...
Puesto que estamos dividiendo un número entre su conjugado, es sencillo ver que el módulo será siempre $1$. Por tanto el método es $A$-estable.
\end{problem}
