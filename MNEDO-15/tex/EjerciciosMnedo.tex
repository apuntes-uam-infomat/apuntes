...
\doneby{Pedro} (Se que hizo este ejercicio en clase pero no lo copié y me toca hacerlo de nuevo en casa. En algún momento alguien (el malvado) lo corregirá y podrá borrar esta línea)
...
\[\norm{1+z+\frac{z^2}{2}} = \norm{1+a+a^2-b^2 + i(b+2ab)}=\sqrt{(1+a+a^2-b^2)^2+(b+2ab)^2}\]
que será menor que 1 siempre y cuando
\[(1+a+a^2-b^2)^2+(b+2ab)^2<1 \iff (1+a+a^2-b^2)^2 +b^2+4a^2b^2+2ab^2 < 1\]
...
Si calculamos simétricos respecto a la recta $Re(z)=-1$, es decir, estamos cambiando $a$ por $-2-a$, que puede hacer que nos salgamos del dominio de estabilidad.

Si giramos 180 grados respecto a $z=-1$ la parte real sufre el cambio $a \to -2-a$ y la parte imaginaria cambia de signo. El cambio de la parte imaginaria nos es indiferente pero no el de la parte real, como acabamos de ver.

Por tanto, el dominio de estabilidad del método de Euler mejorado es simétrico respecto a la recta $Im(z)=0$ pero no respecto al punto $z=-1$ ni a la recta $Re(z)=-1$.
\end{problem}