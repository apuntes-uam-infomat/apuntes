...
\[p(J) = \sum_{j=0}^kα_jJ^j\]
decimos que un método numérico satisface el \concept{criterio de la raíz} si todas las raíces del primer poliniomio característico tienen módulo menor o igual que 1 y las de módulo 1 son simples.
...
\[P(J) = J^2+J-2 \implies J_+=1 \y J_-=-2\]

También observar que, puesto que nuestro problema tiene $f=0$, se cumple:
\[\sum_{j=0}^2α_jy_{n+j} = y_{n+2}+y_{n+1}-2y_n = 0\]

Consideramos ahora la sucesión $u_n = J_-^n$ con lo que obtenemos:
\[\sum_{j=0}^2α_ju_{n+j}=J_-^n\sum_{j=0}^2α_jJ_-^j=0\]

Y definimos la sucesión $v_n=0$. 

Si volvemos a la definición de 0-estabilidad con estas dos sucesiones, vemos que las sucesiones $δ_n$ y $γ_n$ han de ser 0. Por tanto, si queremos que se cumpla la condición de 0-estabilidad necesitamos que:
\[\max_{k \leq n \leq N}\norm{u_n - v_n} \leq C\left( \max_{0 \leq n \leq k-1}\norm{v_n - u_n}\right)\]
...
\[2^N \leq 2C\]
Podemos observar que no hay ninguna $C$ que satisfaga esta desigualdad, por lo que podemos concluir que el método no es 0-estable.
\end{example}
...
\[\left\{ \begin{array}{l}y_n = ε^n \implies \sum_{j=0}^ku_{n+j} = 0 \implies δ_n = 0 \\
v_n = 0 \implies \sum_{j=1}^kv_{n+j}=0 \implies γ_n=0 \end{array}\right.\]
...
Tomamos ahora las sucesiones:
\[\left\{ \begin{array}{l}y_n = nε^n \implies \sum_{j=0}^ku_{n+j} = nε^n\sum_{j=0}^kα_jε^j = +ε^n\sum_{j=1}^kα_jjε^j = 0 \implies δ_n = 0 \\
v_n = 0 \implies \sum_{j=1}^kv_{n+j}=0 \implies γ_n=0 \end{array}\right.\]
...
Para no complicar la notación, vamos a considerar problemas del tipo $y'(x)=f(x,y(x))$ con $\appl{y}{\real}{\real}$ y $\appl{f}{\real\times\real}{\real}$, aunque la demostración puede ser ``fácilmente'' extendida a problemas con $\appl{y}{\real^i}{\real^j}$
...
siendo $A$ una matriz con 1s por debajo de la diagonal y el resto de elementos 0s excepto la primera fila donde tendremos los elementos $(-α_{k-a},...α_0)$ y donde $F$ es un vector columna con $\phi_f(x_n,y_x,...,y_{n+k-1},h)$ en su primera coordenada y el resto 0s.
...
\[Y_{n+1} = A^{n+1}Y_0 + h\sum_{l=0}^nA^{n-l}F(x_l,Y_l) = A\cdot A^nY_0+hF(x_n,Y_n) +h\sum_{l=0}^{n-1}A^{n-1-l}F(x_l,Y_l)=\]
...
Vamos a probarlo.
\begin{proof}
El primer polinomio característico coincide con el polinomio característico de la matriz $A$ y, por tanto, las soluciones de $P(J)$ son los autovalores de $A$.
...
\begin{enumerate}
\item Convergencia $\implies$ criterio de la raíz $\implies$ 0-estabilidad

Vamos a demostrar que si no se cumple el criterio de la raiz entonces no tendríamos convergencia. Como ya hemos visto anteriormente, la negación del criterio de la raíz se divide en dos partes:
...
Pero en nuestro caso, tenemos la igualdad de la izquierda pero no se cumple la igualdad asociada al límite de la derecha, puesto que $|ε|^N$ crece más rápido que el denominado $N$.
...
\item Convergencia $\implies$ consistencia
\end{enumerate}
\end{itemize}
