...
\[\lim_{h \to 0} \max_{0 \leq n \leq Nk} \frac{\norm{R_n}}{h}=0\]
\end{defn}
...
Un método numérico es 0-estable si para cada PVI existe una constante $C>0$ tal que para cada dos sucesiones $u_n,v_n$ que satisfacen\footnote{Es decir, que no satisfacen la descipción del método como función incremento pero casi}
...
Podemos restarlas y tomar valres absolutos obteniendo:
...
\[\max_{1\leq k \leq N}e_n \leq (1+Lh)^{(b-a)h}e_0 + \frac{K}{L}(b-a)e^{L(b-a)}\]

Con esto podemos concluir la demostación puesto que $e_n=|u_n-v_n|$ es el término general de la diferencia de las sucesiones, $e_0=|u_0-v_0|$ es la diferencia entre los valores de arranque en un método de un sólo paso, y la constante $C$ sería $C=(b-a)e^{L(b-a)}$\footnote{Para poder deducir esta $C$ tenemos en cuenta que $(1+Lh)^{(b-a)h}$ es de la forma $e^{L(b-a)}$}
\end{proof}
...
Ahora, con las dos sucesiones que tenemos, $y_n, y(x_n)$, sabiendo que el método es 0-estable, entonces tenemos que $R_n=hδ_n$ y podemos escribir:
...
Y AQUI ENTRA EN ACCION EL GRAN MALVADO COMPLETANDO LA DEMOSTRACION
