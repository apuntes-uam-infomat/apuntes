...
Teniendo en cuenta que $y(x_n)=0$ podemos vemos que:
...
Hasta aquí ya hemos probado que si el método no es 0-esable entonces tampoco será convergente.
...
Para ello consideramos el PVI $y'(x)=0$ con dato inicial $y(0)=1$ del que sabemos la solución es $y=1$.
...
\[\lim_{h \to 0} \max_{k \leq n \leq N} \norm{y_n-y(x_n)} = 0 \implies \lim{h\to o}y_n = 1\]

Por tanto tendríamos:
\[\lim{h \to 0} \sum_{j=0}^kα_jy_{n+j} = 0 = \sum_{j=0}^kα_j\]
...
Por tanto tenemos que $g(x)=0$, es decir, el método ha de ser consistente.
\end{enumerate}
