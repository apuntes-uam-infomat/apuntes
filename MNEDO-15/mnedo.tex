...
En este capítulo explicaremos conceptos como la consistencia, la estabilidad o el residuo, útilis a la hora de valorar la calidad de un método numérico.

A partir de ahora vamos a trabajar con métodos numéricos que se pueden escribir de la forma:
\[\sum_{j=0}^k α_jy_{n+j} = h \phi_f(x_n,y_n,y_{n+1},y_{n+k-1},h) \text{ con } n=0,...,N\cdot k\]
...
En este caso podemos pasar $y_n$ restando para que, junto con $y_{n+1}$ nos de el sumatorio y a la derecha del igual nos queda $hf()$ con lo que tendríamos que $\phi_f=f$.
...
No obstante, en métodos implícitos como el del trapecio es algo más complicado
...
que, para $h$ suficientemente pequeño, es decir,
\[\left(\norm{F(\phi)-F(\tilde{\phi})} \leq \frac{Lh}{2}\norm{\phi- \tilde{\phi}}\right)\]
la aplicación será contractiva y, por tanto, el problema del punto fijo tendrá solución.
...
La idea de estos convenios es que no consideremos como métodos distintos algunos que trivialmente son iguales como:
