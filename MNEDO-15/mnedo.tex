...
\[\abs{y(x) - \tilde{y}(x)} \overbrace{≤}^{\hyperref[ineq:dif_picard]{1}} \int_a^x\abs{f(t,y(t)) - f(t,\tilde{y}(t))} dt + \int_a^x \abs{r(t, \tilde{y}(t))} dt + \abs{y(a) - \tilde{y}(a)} ≤\]

\[≤ L\abs{y(t) - \tilde{y}(t)} + \int_0^xM dt + \abs{y(a) - \tilde{y}(a)}\]

Tomaremos $g'(x) = L\abs{y(x) - \tilde{y}(x)} + M$. Y obtenemos:

\[\frac{g'(x) - M}{L} = \abs{y(x) - \tilde{y}(x)} ≤ g(x)\]
...
\section{Sesión 2 de Laboratorio}
...
La primera gráfica obtenida, que dibuja la función resultado y la obtenida con RK4 queda como sigue:
\begin{center}
\includegraphics[width=0.6\textwidth]{img/RK4_explicito_grafica.png}
\end{center}
...
Al igual que hicimos en la sesión anterior, en esta ocasión también estamos dibujando la pendiente de la función que relaciona $h$ con el error en escala logarítmica con el fin de conocer el orden del método. Así, obtenemos:

\begin{center}
\includegraphics[width=0.6\textwidth]{img/RK4_explicito_pendiente.png}
\end{center}
...
Nuevamente, pintamos las gráficas de las funciones (la solución y la obtenida por el método numérico) para comprobar que coinciden.

\begin{center}
\includegraphics[width=0.6\textwidth]{img/trapecio_grafica.png}
\end{center}

Por último, estudiamos la pendiente del error respecto a $h$ en escala logarítmica para conocer el orden del método con lo que obtenemos:
\begin{center}
\includegraphics[width=0.6\textwidth]{img/trapecio_pendiente.png}
\end{center}
