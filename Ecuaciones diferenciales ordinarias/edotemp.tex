...
\begin{theorem}
Para todo compacto $K\subset A$ tal que $(x_0,y_0)∈K$ podemos encontrar $x^\ast ∈[x_0,x_0 + α_{max}]$ tal que $(x^\ast, y(x^\ast)) ∉ K$.
\end{theorem}

\begin{theorem}
\[\left\{\begin{array}{cc}
y' &=f(x,y)\\ y(x_0)&=y_0\end{array}
\right.\]

f es continua y localmente Lipschitz respecto y en un abierto $A\subset \real^2$ con $(x_0,y_0)\in A$. 

\textbf{Entonces} para cualquier compacto K con $(x_0,y_0)\in K\subset A$, podemos encontrar $x^{\ast}\in[x_0,x_0+\alpha_{max})$ tal que $(x^{\ast},y(x^{\ast}))\in A-K$.
