...
\begin{lemma}[Lema\IS de Gronwall] Dada una función 

\[ w(x)≤C+ \int_{x_0}^x k(s)w(s)\dif s \]

entonces 

\[ w(x) ≤ C\exp{\int_{x_0}^xk(s)\dif s} \]

\end{lemma}

Aplicamos a nuestro querido lema:
...
\end{gather*}
\todo[inline]{Completar. Completar mucho}
Después de un chorrón de cuentas enorme llegamos a la expresión
...
y aplicando el lema de Gronwall tenemos que

\[ \abs{y(x)-z(x)} ≤ \abs{y_0-z_0}\exp \int_{x_0}^{x}L_R\dif S = \abs{y_0-z_0} M \]

y tenemos la continuidad.
\end{proof}
