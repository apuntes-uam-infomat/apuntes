...
$$\cos(\alpha) = \frac{\dotproduct{(1,0)}{(1,y^\prime)}}{\sqrt{1+(y^\prime)^2}}$$
$$\cos(\beta) = \frac{\dotproduct{(1,y^\prime)}{(x,y)}}{\sqrt{1+(y^\prime)^2}\cdot\sqrt{x^2+y^2}}$$

Igualando $\cos(\alpha)$ y $\cos(\beta)$ obtenemos
\begin{equation*}
  \left\lbrace
  \begin{array}{l}
     x+yy^\prime = \sqrt{x^2 + y ^2} \\
     y(-d) = 0  \\
  \end{array}
  \right.
\end{equation*}
