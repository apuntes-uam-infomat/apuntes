...
Supongamos que esta matriz nos da autovalores $1$ y $-1$ con autovectores $(1,1)$ y $(1,-1)$. Automáticamente vemos que es un punto de silla y que las soluciones alrededor de ese punto tendran la forma de la figura \ref{imgML_Silla}
...
\foreach \x in {1,2,3} 
	\draw[very thin, blue,dense directed] (4,{2.5 - 0.1*\x }) to [looseness=\x,out=210, in=240] ({2.5 - 0.1*\x},2);

\foreach \x in {0.5,1,1.5,2} 
	\draw[very thin, blue,dense directed] (4,{1.7 + 0.1*\x }) to [looseness=\x,out=210, in=60] ({-1.5 - 0.2*\x},-2);

\foreach \x in {1,2,3,4} 
	\draw[very thin, blue,dense directed] (-4,{-2.5 + 0.1*\x }) to [looseness=\x,out=30, in=60] ({-2.5 + 0.1*\x},-2);

\foreach \x in {0.5,1,1.5,2} 
	\draw[very thin, blue,dense directed] (-4,{-1.7 - 0.1*\x }) to [looseness=\x,out=30, in=240] ({1.5 + 0.2*\x},2);


\draw[very thick, red!40!white, directed] (0,0) -- (2,2);
\draw[very thick, red!80!black, directed] (0,0) -- (-2,2);

\draw[very thick, orange!40!white, directed] (2,2) -- (0,0);
\draw[very thick, orange!80!black, directed] (-2,-2) -- (0,0);

\draw[->,thick,red] (0,0) -- (0.7,0.7) node[left] {$\vv$};
\draw[->,thick,orange] (0,0) -- (0.7,0.7) node[above] {$\vu$};
\end{scope}
\end{tikzpicture}
\caption{Distribución de las soluciones alrededor del punto crítico $(0,0)$.}
\label{imgML_Silla}
\end{figure}

Si linealizamos en $(1,1)$, nos saldría una matriz similar a \eqref{eqML_MatSistLin}, pero con las derivadas evaluadas en $(1,1)$. Si suponemos que tenemos autovalores $-1+\imath$ y $-1+\imath$, lo que nos daría espirales asintóticamente estables. ¿Cómo juntamos estas dos cosas? 
...
\draw[red!60!white, thick, directed] (-4,-4) -- (4,4);
\draw[orange!60!white, thick, directed] (-4,4) -- (4,-4);

\foreach \x in {1,2,3} 
	\draw[blue, very thin, dense directed] ({-3.4 - 0.1 * \x}, 3) to[looseness=\x, out=315, in=225] ({3 - 0.1 * \x}, 3);
\end{tikzpicture}
\end{figure}

Si quisiésemos analizar este punto crítico, nos interesaría la recta separatriz (en naranja en la figura \ref{imgmierdas}), que divide el plano en dos regiones, una en la que las soluciones se van a una espiral y otra en la que las soluciones se van a infinito. 
