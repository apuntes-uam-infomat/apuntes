...
\begin{multline*}
 \abs{f(x) - f(x_0)} = \abs{f(x) \pm f_n(x) \pm f_n(x_0) - f(x_0)} ≤ \\
  \abs{f(x) - f_n(x)} + \abs{f_n(x) - f_n(x_0)} + \abs{f_n(x_0) - f(x_0)}
\end{multline*}
...
pero ese uno entre dos raíz de equis se va a menos infinito, y como tenemos hun balor havzolutoh me ahorro las dos caras de cuentas de derivadas de Azorero. Pero dice que no, que comprobemos en $(1/2, + ∞)$. %TODO
...
y(x_0)&=y_0
\end{cases} \]
con $f$ continua y localmente Lipschitz respecto de $y$ en un abierto $A\subset ℝ^2$ con $(x_0,y_0)\in A$.
...
\begin{proof}
Llamamos \[ S\equiv \{\alpha\in ℝ \st \text{ la solución y(x) está definida en } [x_0,x_0+\alpha] \} \] y \[ \alpha_{max} = \sup S \].
...
Poniéndolo en notación integral:
...
\textbf{Respuesta:} Todavía no. Si tuviéramos una solución que oscile al acercarse a la frontera, el límite dependería de la sucesión que eligiéramos. Necesitamos demostrar que $y^{\ast}$ no depende de la sucesión $\{x_n\}$ de puntos de la curva que hayamos elegido.

Para ello: sea $\{z_n\}$ otra sucesión con $z_n$ que se aproxima a $x_0+\alpha_{max}$. ¿Qué pasa con $|y(x_n) - y(z_n)| \to 0$?
...
\end{array}\]
Por ser Lipschitz tenemos el teorema de existencia y unicidad local (referencia) entonces tendríamos una solución que sobrepase el punto $x_0+\alpha_{max}$ contradiciendo la hipótesis. %TODO
...
El dibujo de los triángulos te lo dejo a ti Rual xD %TODO
...
\footnote{Quince primeros minutos de clase faltan aquí} %TODO
