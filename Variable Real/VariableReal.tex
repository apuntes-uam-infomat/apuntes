...
\begin{defn}[Espacio\IS $L^1_{loc}$] Se dice que una función $f$ está en el espacio $L^1_{loc}$ (notación: $f ∈ L^1_{loc} (ℝ^N)$) si y sólo si $\int_K \abs{f} \dif x < ∞$ para todo $K ⊂ ℝ^N$ compacto.

En general, tendremos que $f ∈ L^1 (ℝ^N)$ si y sólo si $\int_{ℝ^N} \abs{f} \dif x < ∞$.
\end{defn}

A modo de curiosidad, existe una inmersión continua $L^1 (ℝ^N) \hookrightarrow L^1_{loc} (ℝ^N)$, donde este último es el más grande espacio de funciones.

Esta definición de funciones nos permite definir la media local de una función:

\begin{defn}[Media\IS local de una función] Dada una función $f ∈ L^1(ℝ^N)$ o $f ∈ L^1_{loc} (ℝ^N)$, se define su media local como  \[ A_r f(x) ≝ \frac{1}{m(\bola_r(x))} \int_{\bola_r (x)} f(y) \dif y ≝ \fint_{\bola_r(x)} f(y) \dif y \]
\end{defn}

\begin{defn}[Función\IS maximal Hardy-Littlewood] \[ H(f)(x)\equiv Mf(x) ≝ \sup_{r > 0} A_r(\abs{f})(x) \]
\end{defn}
