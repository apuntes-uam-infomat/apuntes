...
La \fref{sec:TeoriaBasicaIntegral} se dedica a un repaso de los conceptos básicos de cursos anteriores de teoría de la integral y la medida (en \citep{ApuntesTIM} están los apuntes completos de esta asignatura). Estos conceptos se amplían con otros más avanzados en la \fref{sec:ComplementosTIM}.
...
\label{sec:EspaciosBanachHilbert}
...
Algunas propiedades de espacios de Hilbert:

\begin{enumerate}
\item Si $H$ es Hilbert y de dimensión finita, entonces es isomorfo a $ℝ^N$ o $ℂ^N$.
\item Todo espacio vectorial real se puede complexificar: pasamos de $V_{ℝ}$ con producto escalar $(·,·)$ a otro $V_{ℂ}$ con los vectores $\vec{x} + \imath \vec{y}$ con $\vec{x}, \vec{y} ∈ V$ y producto escalar \[ \pesc{\va + \imath \vb, \vx + \imath \vy} = (\va,\vy) + \imath(\vb,\vx) - \imath (\va, \vy) + (\vb, \vy) \]
\item Los subespacios vectoriales de un Hilbert son Hilbert si y sólo si son cerrados, esto es, si el \concept{Span} del subconjunto $E$ (que es el conjunto de combinaciones lineales finitas de elementos de $E$) es $E$ mismo.
\end{enumerate}

Los espacios de Hilbert tienen también propiedade sgoemétricas, ya que son espacios muy similares a los euclídeos con los que estamos acostumbrados a trabajar.

\begin{prop} \label{prop:PropsHilbert} Sea $V$ un espacio pre-Hilbertiano, real o complejo. Entonces:

\begin{enumerate}
\item Se cumple la \concept{Desigualdad\IS de Cauchy-Schwartz}: $∀x,y ∈ V$, se tiene que $\abs{\pesc{x,y}} ≤ \norm{x} \norm{y}$.
\item El mapa $x \mapsto \norm{x} = \pesc{x,x}$ es una norma en $V$.
\item Dos vectores son ortogonales si y sólo si $\pesc{\vx,\vy} = 0$.
\item \concept{Teorema\IS de Pitágoras}: si $\vx_1, \dotsc, \vx_n$ son ortogonales entonces \[ \norm{\vx_1 +\dotsb + \vx_n}^2 = \norm{\vx_1}^2 + \dotsb + \norm{\vx_n}^2 \]
% A partir de aquí edbería sacarlo del enumerate. TODO.
\item Una familia $\set{e_α}_{α∈A}$ con $A$ no necesariamente numerable es una \concept{Familia\IS ortogonal} si y sólo si $\pesc{e_α,e_β} ≠ 0$ sólo cuando $α = β$. Análogamente, será una \concept{Familia\IS ortonormal} si y sólo si $\pesc{e_α,e_β} = δ_{α,β}$ con \[ δ_{α,β} = \begin{cases} 1 & α = β \\ 0 & α ≠ β \end{cases} \] la \concept{Delta\IS de Kronecker}
\item \concept{Ortogonalización\IS de Gram-Schmidt}: Dado un sistema finito de elementos ortogonales $\set{e_1, \dotsc, e_n}$, entonces $∀v ∈ V$ existe un $e_{n+1}$ tal que $v ∈ \mop{span}\set{e_1, \dotsc, e_{n+1}}$.
\item Sea $\set{e_α}_{α∈A}$ un sistema ortogonal finito. Si $x ∈ \mop{span}\set{e_α}$ entonces $x = \sum_{α∈A} \pesc{x, e_α} e_α$ (\concept{Fórmula\IS de representación}). Además, $\norm{x}^2 = \sum_{α∈A} \abs{\pesc{x,e_α}}^2$ (\concept{Identidad\IS de Plancharel}).
\item \concept{Identidad\IS del paralelogramo}: \[ \norm{\vx + \vy}^2 + \norm{\vx - \vy}^2 = 2\norm{\vx}^2 + 2 \norm{\vy}^2 \]
\item Los espacios pre-Hilbert se pueden completar a espacios de Hilbert tomando su cierre.
\item \concept{Espacio\IS complejo-conjugado} Dado un espacio de Hilbert $H$, podemos definir el espacio $\conj{H}$ como el dado porlos vectores $\conj{x} + \conj{y} = \conj{x+y}$. y algo. No sé, tengo sueño.
\end{enumerate}
\end{prop}

\begin{proof}
\paragraph{1: Desigualdad Cauchy-Schwartz} Lo hacemos para el caso complejo. Es obvio que \[ 0 ≤ \norm{λ \frac{x}{\norm{x}} - \frac{y}{\norm{y}}}^2\] para todo $λ ∈ ℂ$. Desarrollando eso y tomando $λ = \frac{\pesc{x,y}}{\abs{\pesc{x,y}}}$ sale.
\end{proof}

\begin{prop} Sea $H$ un espacio de Hilbert, y $K$ un conjunto no vacío, cerrado y convexo\footnote{Esto es, que $∀x,y ∈ K$, se tenga que $λx + (1-λ)y ∈ K$ para todo $λ ∈ [0,1]$. Es decir, que el segmento entre $x$ e $y$ esté contenido en $K$.}, y sea $x ∈ H$. Entonces existe un único $y ∈ K$ tal que \[ \dst (x,K) ≝ \min_{z∈K} \norm{z-y} = \norm{y-x} \], y además $∀z ∈ K$ tenemos que $\Re \pesc{z-y, y-x} ≥ 0$ (esta es la llamada \concept{Desigualdad\IS variacional}).
\end{prop}

Lo que nos dice el teorema es que podemos hacer una proyección ortogonal en espacios de dimensión infinita igual que si tuviese dimensión finita. Es una propiedad importante y muy potente, también porque nos da un mínimo único en la distancia, que habitualmente está definida con el ínfimo.

\begin{proof}
La unicidad sigue de la regla del paralelogramo: Si tenemos dos puntos $y, y'$ su punto medio estará a menos distancia de $x$ que cualquiera de los dos.

Probamos ahora la existencia de $y$. Ponemos $D= \inf_{y∈K} \norm{x-y}$, y tomamos una sucesión minimizante $y_n∈ K$ tal que $\norm{x-y_n} \convs D$. Vamos a demostrar que esa sucesión $y_n$ converge a un $\bar{y} ∈ K$.

Primero, notamos que los puntos medios están en $K$: por convexidad, sabemos que $\frac{y_n + y_m}{2} ∈ K$. Por la definición de $D$ como el ínfimo, es obvio que \[ \norm{x - \frac{y_n + y_m}{2}} ≥ 2\]

Usaremos la \nlref{prop:IdParal} y escribiremos lo siguiente: \[ 0 ≤ D^2 ≤ \norm{\frac{x-y_n}{2} + \frac{x-y_m}{2}}^2 \eqreason{I.Paral.} 2\norm{\frac{x-y_n}{2}}^2 + 2\norm{\frac{x-y_m}{2}}^2 - \norm{\frac{x-y_m}{2} - \frac{x-y_n}{2}}^2 \], que despejando nos dice que \[ \norm{\frac{y_n - y_m}{2}}^2 ≤ \underbracket{2\norm{\frac{x-y_n}{2}}^2 + 2\norm{\frac{x-y_m}{2}}^2}_{\to D^2}- D^2 \convs 0 \]

Por sándwich, tenemos entonces que $\norm{y_n - y_m} \convs[][n,m] 0$, luego $y_n$ es Cauchy y, por ser $K$ cerrado y $H$ completo sabemos que existe el límite $\bar{y}$ que es el mínimo que buscábamos.

Por último, probaremos la desigualdad variacional. Sea $y$ un elemento de mínima distancia. Entonces $∀z ∈ K$ y $∀θ ∈ (0,1)$ tenemos que $\dst ((1-θ)y + θz, x) ≥ \dst ((1-θ)y + θz,y)$., y desarrollando los cuadrados nos queda que \[ 2 \Re \pesc{z-y, y-x} ≥ - θ \norm{z-y}^2\], cuando movamos $θ \to 0$ nos quedará la desigualdad variacional.
\end{proof}


Recordamos teorema Riesz.
