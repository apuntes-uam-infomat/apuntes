...
Ahora vamos con la prueba del \fref{thm:LiebLoss}.

\begin{proof}
Para el primer paso, recordamos la desigualdad numérica: para todo $ε>0$ y $p > 0$, existe una constante $C_{ε,p} >0$ tal que $∀a,b∈ℂ$ se tiene que \( \abs{\abs{a+b}^p - \abs{b}^p} ≤ ε\abs{b}^p C_{ε,p} \abs{a}^p \label{eq:ProofLiebLossDesNum} \).

Para el segundo paso necesitaremos una función especial para usar en ella el teorema de convergencia dominada (\fref{thm:ConvDominada}). Entonces escribiremos la serie como $f_n ≝ f + g_n$, por lo que $g_n \convs 0$.
