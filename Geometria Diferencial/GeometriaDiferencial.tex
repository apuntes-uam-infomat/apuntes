...
\subsubsection{Ejemplo: estudio del espacio tangente con funciones polinomiales}
...
Vamos a la teoría ahora.

\subsubsection{No sé qué ha pasado aquí}

\begin{theorem} Sea $p ∈ U ⊆ ℝ^n$, con $p = (x_1, x_2, \dotsc, x_n)$. Entonces el espacio $\quot{m_p}{m_p^2}$ es un R-espacio vectorial de dimensión $n$.\end{theorem}
...
Vamos ahora con la demostración más rigurosa. Para una función $f ∈ A_p$, tenemos que
...
Sabemos que $∀D$ $D(F) = 0$. Entonces, elegimos un conjunto de derivaciones $D_j$ tales que $D_j ( x_i - p_i) = δ_{ij}$ con $δ$ la delta de Kroenecker (ver \ref{defDeltaKroenecker}). En ese caso, \[ D_j(F) = \sum λ_i D_j (x_i - p_i) = λ_j\], pero hemos dicho que $D(F) = 0$, contradicción.
...
Con esta demostración hemos llegado a algo interesante, que es ver qué es un diferencial: \[ (\dif x_i)_p ≝ [x_i - p_i]\]
...
\begin{defn}[Delta\IS de Kroenecker] \label{defDeltaKroenecker} La delta de Kroenecker se define como \[ δ_{ij} = \begin{cases} 0 & i ≠ j \\ 1 & i = j \end{cases} \] \end{defn}
