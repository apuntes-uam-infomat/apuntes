...
Para llegar a esa notación, vamos a empezar estudiando la imagen $Φ(\vx,1, \dotsc, \vx_b)$.Sabemos que cada $\vx_j$ se puede expresar como combinación lineal de elementos de la base de $E$, es decir, \[ \vx_j = \sum_{i = 1}^n x_i^j e_i\] donde $x_i^j$ es la coordenada $i$ del vector $j$.
...
\paragraph{El determinante como tensor $(0,b)$} Un ejemplo más concreto: antes decíamos que el determinante es un tensor, así que vamos a ir a ello. Sea $Φ_b$ la operación que nos da el determinante de una matrix $b × b$, es decir, \[ \appl{Φ_b}{M_{b×b}}{ℝ} \]. ¿Cómo lo expresamos como un tensor?
