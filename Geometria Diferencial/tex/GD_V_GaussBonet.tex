...
\begin{proof} Consideramos $p$ y una circunferencia estándar $S_r$ en la carta. Suponemos $p = (0,0)$ en la carta. La circunferencia encerrará una bola de radio $r$ $\bola_r$. Calculamos el límite siguiente \[ \lim_{r\to 0} \frac{1}{2π} \int_{S_r} \gor{ω}_{12} ≝ \gor{I} \], si existe demostraremos también que $I = \gor{I}$. Es claro\footnote{Como siempre, no te jode.} que esto demostrará que no depende de la referencia.
...
Ahora hay que ver que el límite es igual al índice. Para eso fijamos un $r_1$ y hacemos tender $r_2$ a 0. Entonces, la diferencia de antes cambia y tenemos que \[ \lim_{r_2 \to 0}  \int_{S_{r_1}} \gor{ω}_{12} - \int_{S_{r_2}} \gor{ω}_{12} = \int_{S_{r_1}} \gor{ω}_{12} - 2π\gor{I} \], pero por Stokes esa diferencia es también \[ \lim_{r_2 \to 0}  \int_{S_{r_1}} \gor{ω}_{12} - \int_{S_{r_2}} \gor{ω}_{12} = \int_{\bola_{r_1}} \dif \gor{ω}_{12} = - \int_{\bola_{r_1}} K \gor{ω}_1 \y \gor{ω}_2\]

Usando ahora la fórmula del lema \ref{thmOmegaTau}, tenemos que \[ \int_{S_{r_1}} \gor{ω}_{12} = \int_{S_{r_1}} ω_{12} + \int_{S_{r_1}} τ \eqexpl{Stokes} \int_{\bola_{r_1}} \dif ω_{12} + \int_{S_{r_1}} τ = - \int_{\bola_{r_1}} K ω_1 \y ω_2  + 2πI \] Luego parece que sí que funciona, pero vamos, lo llega a hacer con un poco menos de orden y me pego un tiro.
\end{proof}
