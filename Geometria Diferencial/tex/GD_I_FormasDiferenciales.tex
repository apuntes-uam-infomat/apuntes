...
\label{chap:FormasDiferenciales}
...
\begin{defn}[Diferencial\IS exterior] Dada $\appl{f}{U⊆ℝ^n}{ℝ}$, con $f∈C^∞(U)$ y $x_0 ∈ U$, entonces la diferencial en un punto es \[ (\dif f)_{x_0} = \sum \dpa{f}{x_i} (\dif x_i)_{x_0} \] En general, la diferencial será \[ \dif f = \sum \dpa{f}{x_i} \dif x_i \] \label{def:Diferenciald}
\end{defn}

En realidad, la única diferencia entre esta definición y la dada en \fref{def:DiferencialD} es que cambiamos los $λ_i$, los componentes del vector tangente, por $\dif x_i$. ¿Qué significa eso?
...
Dado que queremos que exista $h$, vamos a suponer que efectivamente existe $h$. Entonces va a ocurrir que $\dpa{h}{x_i} = f_i$, y si volvemos a derivar tendremos que $\frac{∂h}{∂x_j∂x_i} = \dpa{f_i}{x_j}$. Si derivamos en orden contrario, como el Teorema de Schwarz\index{Teorema!de Schwarz} nos dice que las derivadas cruzadas son iguales, tendremos que tener \[ \dpa{f_i}{x_j} = \dpa{f_j}{x_i} \]

Hay una manera natural de poner toda esta información. Nosotros tenemos la matriz Hessiana\index{Matriz!Hessiana} de derivadas parciales segundas dada por \[ H = \left(\frac{∂h}{∂x_j∂x_i}\right)_{ij}\] que es simétrica por el Teorema de Schwarz.
...
Para demostrarlo, lo que vamos a hacer es construir esa función $h$. Es trivial ver que \[ h(x) = \int_γ \dif h \] para un cierto camino $\appl{γ}{I}{U}$. En este caso, aplicando la regla de Barrow\index{Regla!de Barrow}
...
\begin{defn}[Germen\IS de función] Un germen de una función en un punto $p$ es un par $(V,f)$ de un abierto $p ∈ V ⊆ U$ y una función $\appl{f}{V}{ℝ}$ con $f∈C^∞$. \label{def:GermenFuncion}
\end{defn}
...
\end{enumerate}\label{def:Derivacion}
\end{defn}
...
Consideramos\footnote{Aviso para navegantes: todo esto es una idea feliz que no sé de dónde sale.} ahora $\mathfrak{m}_p^2$, el ideal generado por productos de elementos de $\mathfrak{m}_p$, esto es, \( \label{eq:Mp2} \mathfrak{m}_p^2 ≝ \set{\sum g_{ij} f_i f_j \tq f_i, f_j ∈ \mathfrak{m}_p; \; g_{ij} ∈ \mathcal{A}_p }\). Es fácil ver que esto es un subconjunto de $\mathfrak{m}_p$.
...
Es decir, que sólo con saber lo que vale $\adh{D}$ nos vale ya que simplemente trasladamos las funciones $f$ con valores desconocidos $f ∉ \mathfrak{m}_p$ a una clase de equivalencia de la cual sí sabemos el valor. Sólo tenemos que ver que efectivamente cumple la propiedades de derivación que habíamos pedido (\fref{def:Derivacion}). Es trivial ver que se cumple la linealidad, pero Leibniz es más interesante. La demostración de eso se deja como ejercicio para el lector.
...
\begin{theorem} \label{thm:DimQuotMp} Sea $p ∈ U ⊆ ℝ^n$, con $p = (p_1, p_2, \dotsc, p_n)$. Entonces el espacio $\quot{\mathfrak{m}_p}{\mathfrak{m}_p^2}$ es un R-espacio vectorial de dimensión $n$.\end{theorem}
...
Sabemos que $∀D$ $D(F) = 0$. Entonces, elegimos un conjunto de derivaciones $D_j$ tales que $D_j ( x_i - p_i) = δ_{ij}$ con $δ$ la delta de Kronecker (ver \fref{def:DeltaKronecker}). En ese caso, \[ D_j(F) = \sum λ_i D_j (x_i - p_i) = λ_j\] pero hemos dicho que $D(F) = 0$, contradicción.
...
De esta notación y del \fref{thm:DimQuotMp} podemos llegar a otra notación:

\( (\dif f)_p = \sum \dpa{f}{x_i}(p) (\dif x_i)_p \label{eq:DiferencialNotacion} \)
...
Definiremos entonces $F_{*,p}(D_p) ∈ \tgs_p V$ como un vector tangente a $F(p)$ en $V$. Este elemento existe, se podrá definir con una fórmula, y se podrá aplicar a funciones $g ∈ \mathcal{A}_{F(p)}$ donde $\mathcal{A}_{F(p)}$ es el conjunto de gérmenes de funciones en $F_{(p)}$ (ver \fref{def:GermenFuncion}).
...
Con esto, nos queda que podemos usar los teoremas de análisis que ya conocemos: regla de la cadena, teorema de la función inversa (\fref{thm:FuncionInversa}) y teorema de la función implícita (\fref{thm:FuncionImplicita}). Son teoremas demostrados por definición de límite pero que usaremos libremente.
...
\label{fig:EspiralDifeomorfismo}
\end{wrapfigure}

¿Es esta condición también suficiente? Aquí podemos usar el teorema de la función inversa (\fref{thm:FuncionInversa}) nos dice que es ``casi'' suficiente, ya que nos dice que en un entorno $V$ de $p$ quizás más pequeño que $U$ la función $F$ del cambio de coordenadas (la formada por las $f_i$) es un difeomorfismo (\fref{def:Difeomorfismo}) con su imagen.
...
En este caso, tenemos que $Φ_1 \otimes Φ_2 ∈ \tens_{a+c}^{b+d}(E)$. \label{def:ProdTensorial}
\end{defn}
...
Parece que los elementos de la base del dual son lo que necesitamos, lo que actuará de ``selector de coordenadas''. ¿Cómo los componemos para poder aplicarlo a $b$ vectores? Usando lo que vimos antes del producto tensorial (\fref{def:ProdTensorial}). El truco es que podemos considerar un elemento del dual como un tensor $(0,1)$, y entonces el producto tensorial de $b$ tensores es un tensor $(0,b)$.
...
Expresando esto con los elementos de la base, tendremos que \( Ψ_2 = 0 · e_1^* \otimes e_1^* + 1 · e_1^* \otimes e_2^* - 1 · e_2^* \otimes e_1^* + 0 · e_2^* \otimes e_2^* \label{eq:Det2Tensor} \)
...
Sólo tendremos que especificar estas coordenadas cuando hablemos un vector. Por ejemplo, siguiendo con el ejemplo que habíamos descrito en la ecuación \eqref{eq:Det2Tensor} del determinante de matrices de orden 2 como tensor, tendríamos que dar las cuatro coordenadas como $T_{1,1} = 0,\, T_{1,2} = 1,\, T_{2,1} = -1,\, T_{2,2} = 0$ (no ponemos superíndices porque las entradas del determinante son vectores del espacio base, no hay nada de dual).
...
\paragraph{Algo de magia} Si tenemos un tensor $(1,1)$, un endomorfismo, su contracción es un tensor $(0,0)$, un número, y vemos, escribiendo su definición, que es exactamente la traza del endomorfismo.
...
Vamos a recuperar un poco la perspectiva: queremos buscar una función $f$ tal que $\dif f = ω$, donde ω es una 1-forma. Buscábamos también el significado de la diferencial de una variable $(\dif x_i)_{x_0}$ o de una función $(\dif f)_{x_0}$, que veíamos en la \fref{sec:DimTangente}. Ahora, con las definiciones del álgebra multilineal, lo que estamos haciendo es darle sentido a la expresión del producto exterior $\dfl{x_{i_1}}{x_{i_a}}$.
...
\begin{defn}[{Á}lgebra\IS exterior] Se define el álgebra exterior como el conjunto de todos los tensoriales antisimétricos
\end{defn}
...
Tenemos un conjunto $X$ y una aplicación $\appl{f}{X}{F}$. Tomando una cierta aplicación $\appl{u}{X}{\kbb^X}$,\footnote{$\kbb^X$ es el espacio vectorial libre, ver \fref{sec:EspacioVectorialLibre}.} demostrar que existe una aplicación $\appl{\hat{f}}{\kbb^X}{F}$ tal que $f = u ○ \hat{f}$ (ver \fref{fig:VecLibre}).
...
Las propiedades de esta imagen inversa o pullback son las siguientes (por definición):
...
\begin{defn}[Derivada\IS exterior] Dada $ω_p = \sum a_I \dif x_I$, entonces se define la derivada exterior como \[ \dif ω_p ≝ \sum \dif a_I \y \dif x_I \] teniendo en cuenta que \[ \dif f ≝ \sum \dpa{f}{x_i} \dif x_i \]\label{def:DerivadaExterior}
\end{defn}
...
\begin{defn}[p-forma\IS cerrada] \label{def:FormaCerrada} Se dice que $ω$ es una p-forma cerrada si $\dif ω = 0$.\end{defn}

\begin{defn}[p-forma\IS exacta] \label{def:FormaExacta} Se dice que $ω$ es una p-forma exacta si existe una $(p-1)$-forma $α$ tal que $ω = \dif α$.\end{defn}
...
\begin{lemma}[Lema\IS de Poincaré] \label{lem:Poincare}
...
Si $U$ es contractible (\fref{def:Contractible}), entonces existe una $(p-1)$-forma α en $U$ tal que \[ ω = \dif α\]. Esto se denota como que $ω$ es una $p$-forma exacta.
\end{lemma}

Para la demostración necesitaremos una definición previa

\begin{defn}[Abierto\IS contractible] Un abierto $U ⊂ ℝ^n$ es contractible si existe una función continua $\appl{Φ}{I×U}{U}$ con $Φ(1,x) = x\; ∀ x ∈ U$ y $Φ(0,x) = x_0 ∈ U$.\label{def:Contractible}
\end{defn}
...
Una propiedad clave es que \( \label{eq:PropKdif} K(\dif ω) + \dif (K(ω)) = j_1^* ω - j_0^* ω \), y vamos a demostrarlo.
...
Nos queda ahora demostrar \fref{eq:PropKdif} para $ω = a_I(t,x) \dif t \y \dif x_I$. Por la construcción de los $j_i$, el lado de la derecha de la igualdad es $0$ ya que ambas aplicaciones nos llevaban $\dif t$ $0$. Así, tenemos que demostrar que $K(\dif ω ) = \dif (K(ω))$.
...
Cogemos ahora la contracción $Φ$ de la \fref{def:Contractible}. Entonces, \[ K(\dif Φ^*ω) + \dif (K(Φ^*ω) = j_1^* ω - j_0^*ω = ω - 0 = ω \] luego el α que buscamos es algo.
\end{proof}
...
\label{sec:IntegralesLinea}

Ya mencionamos que si teníamos $ω = \sum a_i \dif x_i$ una 1-forma en $U ⊂ ℝ^n$, definíamos su integral sobre una curva $\appl{γ}{I = [a,b]}{U}$ diferenciable como \[ \int_γ ω ≝ \int_a^b γ^* ω \] que se puede escribir con más detalle como \( \int_a^b \left(\sum_i a_i(t) \deriv{x_i}{t}\right) \dif t \label{eq:IntLinea} \)
...
Lo que hacemos es seguir reescribiendo la \fref{eq:IntLinea} como \[ \int_γ ω = \int_a^b \left(\sum_i a_i(t) \deriv{x_i}{τ}\deriv{τ}{t}\right) \dif t = \int_c^d \left(\sum_i a_i(τ) \deriv{x_i}{τ}\right) \dif τ = \int_{γ'} ω \]
...
Acabamos de ver en la sección anterior que, de alguna manera, las formas diferenciales dan ciertas ideas sobre la topología del espacio en el que están definidas. Ahora bien, ¿qué significa esto exactamente? ¿Podemos formalizarlo?

La respuesta es que sí, por supuesto. La herramienta que se usa para ello es la cohomología de De Rham. No nos vamos a extender mucho en esta parte y sólo daremos una intuición. Para un estudio más extenso (y aviso, complicado) es recomendable echarle un ojo a \cite[Capítulo 6]{barden03}.

La idea es bucear en el álgebra que hay por detrás de las formas diferenciales. Partimos de una variedad $X$ genérica, y vamos a definir $Ω^p_X$ como el espacio vectorial de todas las $p$-formas en $X$. Por ejemplo, $Ω^0_X$ sería el espacio de todas las $0$-formas (funciones de $X$ a $ℝ$), y $Ω^1_X$ sería el espacio de las $1$-formas dadas como $f(x_1, \dotsc, x_n) \dif x_i$.

Por otra parte, nos fijamos en la operación de diferencial exterior. Diremos que $\appl{\dif_p}{Ω^p_X}{Ω^{p+1}_X}$ es la diferencial exterior definida sobre las $p$-formas de $X$, que es una aplicación lineal. Es fácil ver que podemos montar una secuencia de espacios vectoriales y aplicaciones lineales \[ \dotsb \rightarrow Ω^{p-1}_X \xrightarrow{\dif_{p-1}} Ω^p_X \xrightarrow{\dif_{p}} Ω^{p+1}_X \rightarrow \dotsb  \] que nos lleva de cada espacio de formas al siguiente.

La ventaja de ver la diferencial como una aplicación lineal es que podemos establecer una equivalencia entre conceptos de álgebra lineal e ideas sobre las formas diferenciales que ya habíamos establecido. El núcleo de la diferencial $\ker \dif_p ⊆ Ω^p_X$ es el conjunto de todas las $p$-formas tales que su diferencial es $0$. Es decir, que $\ker \dif_p$ es el espacio vectorial de todas las $p$-formas cerradas (\fref{def:FormaCerrada}). De forma similar, la imagen de la diferencial $\img \dif_{p-1} ⊆ Ω^p_X$ es el conjunto de todas las $p$-formas que son la diferencial de una $(p-1)$-forma, esto es, las $p$-formas exactas (\fref{def:FormaExacta}).

La idea feliz de la cohomología de De Rham es estudiar un grupo cociente en particular, un grupo cociente que nos describirá la relación entre las $p$-formas cerradas y exactas y que, como veremos más adelante, nos da resultados muy curiosos.

\begin{defn}[Espacio\IS de cohomología de De Rham] \label{def:CohomologiaDeRham} Se define el espacio (también llamado grupo) de cohomología de De Rham de una variedad $X$ como el cociente de espacios vectoriales dado por \[ H^p(X) ≝ \quot{\ker \dif_p}{\img \dif_{p-1}} \], esto es, el grupo cociente de las $p$-formas cerradas y las $p$-formas exactas.
\end{defn}

La primera observación que sale rápidamente es que $H^p(X) = 0$ si y sólo si toda $p$-forma cerrada es exacta. Que, casualidad, resulta ser lo mismo que expone el Lema de Poincaré (\fref{lem:Poincare}). Es decir, que podemos reducir el estudio del problema de Poincaré a un estudio puramente algebraico sin tener en cuenta el cálculo diferencial. De hecho, la solución de ese problema nos llevará también a un resultado topológico, que es de decir si la variedad es una abierto contractible.

Otros resultados interesantes sobre los grupos de cohomología que revelan cosas sobre la topología subyacente son dos cálculos sencillitos:

\begin{prop} Si $X$ es conexo, entonces $H^0(X) = ℝ$.
\end{prop}

\begin{proof} Dado que no hay $-1$-formas, no hay ninguna $0$-forma que sea exacta. Esto es, $\img \dif_{-1} = ∅$. Por lo tanto, el cociente que nos da el grupo es perfectamente isomorfo al espacio de $p$-formas cerradas, formas $ω$ tales que $\dif ω = 0$.

La cuestión es que las $0$-formas no son más que funciones $f$ con $\dif f = 0$, y para que eso se cumpla todas sus derivadas tienen que ser nulas. Esto es, tiene que ser constante en todo entorno local.

Como hemos dicho que $X$ es una variedad conexa, no hay ``saltos'' ni ``huecos'' que permitan que $f$ cambie de valor sin dejar de ser diferenciable. Es decir, que lo que tiene que ocurrir es que $f(x) = c ∈ ℝ$ para todo $x ∈ X$, esto es, que sea constante en toda la variedad. Ese espacio de funciones constantes es obviamente isomorfo a $ℝ$ y ya tenemos la demostración.
\end{proof}

De forma similar, podríamos demostrar que si $X$ tiene dimensión $n$ y es compacto, conexo y orientable, su espacio de cohomología de grado $n$ sería $H^n(X) = ℝ$.
