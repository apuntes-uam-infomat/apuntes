...
Bajo ciertas hipótesis topológicas, existe $x$ una $(p-1)$-forma en $U$ tal que \[ ω = \dif x\]. Esto se denota como que $ω$ es una p-forma\IS exacta.
\end{theorem}

Para la demostración necesitaremos una definición previa

\begin{defn}[Cosa\IS contractible] $U ⊂ ℝ^n$ es un algo contractible si existe una función continua $\appl{Φ}{I×U}{U}$ con $Φ(1,x) = x\; ∀ x ∈ U$ y $Φ(0,x) = x_0 ∈ U$.\label{defContractible}
\end{defn}
...
El pullback de ambas se haría cambiando $t = 1$ en $ω$ y $\dif t = 0$ con $j_1^*(ω)$, y cambiando $t = 0$ en $ω$ y $\dif t = 0$ con $j_0^*(ω)$. El truco es ahora usar las dos copias de $U$ (cada tapa del cilindro) y usar estas $j_i^*$ de una forma que vamos a ver ahora \footnote{NPI de que es esto, lo copió Pablo.}.
...
Cuando $ω = a_I(t_Ix) \dif t \y dif x_I$, entonces $K(ω) ≝ \left(\int_0^1 a_I(t_1x) \dif t\right) \dif x_I $. Los elementos son suma de estos dos tipos y como es lineal con definirlo en los ya se queda definido.\footnote{En serio, NPI de qué es esto.}

Una propiedad clave\footnote{Que demostraremos luego.} es que $K(\dif ω) + \dif (K(ω)) = j_1^* ω - j_0^* ω$. Con esto y usando la $Φ$ de \ref{defContractible}, que se puede reescribir como $Φ j_1 = id$ y $Φj_0 = x_0$ concluimos $K(\dif ω) + \dif (Kω) = ω - 0 = ω$, así que $Kω = x$ y ya lo tenemos.

PD: siento que os haya tocado a vosotros leer esta mierda.
\end{proof}
