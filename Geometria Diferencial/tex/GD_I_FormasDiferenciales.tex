...
\begin{defn}[p-forma\IS cerrada] \label{defFormaCerrada} Se dice que $ω$ es una p-forma cerrada si $\dif ω = 0$.\end{defn}

\begin{defn}[p-forma\IS exacta] \label{defFormaExacta} Se dice que $ω$ es una p-forma exacta si existe una $(p-1)$-forma $α$ tal que $ω = \dif α$.\end{defn}
...
\begin{theorem}[Lema\IS de Poincaré] \label{thmPoincare}
...
Si $U$ es contractible (definición \ref{defContractible}), entonces existe una $(p-1)$-forma α en $U$ tal que \[ ω = \dif α\]. Esto se denota como que $ω$ es una $p$-forma exacta.
\end{theorem}
...
\begin{defn}[Espacio\IS de cohomología de De Rham] \label{defCohomologiaDeRham} Se define el espacio (también llamado grupo) de cohomología de De Rham de una variedad $X$ como el cociente de espacios vectoriales dado por \[ H^p(X) ≝ \quot{\ker \dif_p}{\img \dif_{p-1}} \], esto es, el grupo cociente de las $p$-formas cerradas y las $p$-formas exactas.
\end{defn}

La primera observación que sale rápidamente es que $H^p(X) = 0$ si y sólo si toda $p$-forma cerrada es exacta. Que, casualidad, resulta ser lo mismo que expone el Lema de Poincaré (\ref{thmPoincare}). Es decir, que podemos reducir el estudio del problema de Poincaré a un estudio puramente algebraico sin tener en cuenta el cálculo diferencial. De hecho, la solución de ese problema nos llevará también a un resultado topológico, que es de decir si la variedad es una abierto contractible.
