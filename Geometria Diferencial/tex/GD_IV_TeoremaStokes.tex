...
\begin{defn}[Orientable] $X$ es orientable si y sólo si existe una $n$-forma $ω_n$ en $X$ tal que $ω_n(x) ≠ 0\;∀x∈X$.
\end{defn}

A la $n$-forma $ω_n$ se le llama la \concept[Forma\IS de volumen]{forma de volumen}, así que definiremos el \concept[Volumen]{volumen de una variedad}\footnote{Por supuesto, si tenemos una superficie será el área.} como \[ \mop{Vol(X)} ≝ \int_X ω_n\]
