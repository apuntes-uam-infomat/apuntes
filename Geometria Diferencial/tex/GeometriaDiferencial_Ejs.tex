...
\doneby{Pedro} \textcolor{green}{Dejuan approves}
...
\begin{gather*}
φ ∧ φ = \sum_{I,J} a_I · a_J \; dx_I ∧ dx_J = \\
\sum_{i_1,...,i_k} \sum_{j_i,...,j_k} a_{i_1,...,i_k} · a_{j_1,...,j_k} \; dx_{i_1} ∧ ... ∧ dx_{i_k} ∧ dx_{j_1} ∧ ... ∧ dx_{j_k} \overset{*}{=} \\
\sum_{i_1,...,i_k}\quad \sum_{j_i<i_1,...,j_k<i_k} (a_{i_1,...,i_k}a_{j_1,...,j_k} + (-1)^k) a_{j_1,...,j_k}a_{i_1,...,i_k}) dx_{i_1} ∧ ... ∧ dx_{i_k} ∧ dx_{j_1} ∧ ... ∧ dx_{j_k} =\\
 \sum_{I,J<I} (a_Ia_J + (-1)^ka_Ja_I) dx_I∧dx_J  \overset{k \text{ impar}}{=}0
 \end{gather*}
