...
\[f_x \dif x + f_y \dif y = a g \dif x + b \dif y \implies \left\{ \begin{align*}f_x &= a g \\ f_y &= b g\end{align*}\right.\]
...
\ppart Demostrar que $\{(U_α×V_β, h_{αβ})\}$ es una estructura diferenciable para $M×N$ llamada \concept{Producto de variedades}.
...
Sea $S^2$ la esfera unidad y $\app{π}{S^2}{P^2(\real)}$ la proyección canónica de $S^2$ en el plano proyectivo real. Puesto que $\phi(p)=\phi(-p)$ definimos una aplicación $\appl{\theta}{P^2(\real^2)}{\real^4}$ como:
\[\theta(\{p,-p\})=\phi(p)\]
