...
\begin{problem}[12]
Un campo de planoes en un abierto $U\subset\real^3$ es una correspondencia $P$ que asocia a cada punto $p∈U$ un plano $P(p)$ tal que $p∈P(p)$.

El campo $P$ es diferenciable si los coeficientes de la ecuación $P(p)$ son funciones diferenciables en $p$.


Una \textit{integral surface} de $P$ es una superficie $S\subset\real^3$ tal que $∀q∈S$, $T_qS = P(p)$, es decir, $S$ es tagente en cada uno de sus puntos al plano del campo que pasa por ahí.

Sea $ω$ una 1-forma diferencial en $U\subset \real^3$ con $ω(q) ≠ 0,q∈U$. Demuestra que

\ppart $ω$ determina un campo de planos diferenciables con la condición \[v∈P(p)\subset\real^3 \dimplies ω_p(v) = 0\]
\ppart
\ppart
\ppart
...
Sea $v=(v_1,v_2,v_3)$, entonces $ω_p (v) = a_1(p) · v_1 + a_2(p) · v_2 + a_3(p) · v_3$. La condición $ω_p (v) = 0$ equivale a la condición de definir un plano como \textit{el conjunto de vectores que son perpendiculares a uno dado} \footnote{$π \equiv Ax + By + Cz = 0$ tiene como vector normal $(A,B,C)$ entonces podemos escribir $π = \{(v_1,v_2,v_3) \tq A·v_1 + B·v_2 + C·v_3 = 0\}$}.  En este caso, $ω_p = α_1 dx + α_2 dy + α_3 dz$ con $α_i ∈ ℝ$ podríamos tomarlo como $ω_p=(A,B,C)$ y listo. Dado un $p$, tenemos definido un plano (a partir de su vector normal).

Además, como $ω$ es forma diferencial, sus coeficientes son formas diferenciales, con lo que el campo de planos será direrencial ($a_i$ son diferenciables en $p$).
