...
\begin{theorem}[Enderezamiento\IS de campos] Sea $D$ un campo en $X$ y $x_0$ un punto en $X$ donde el campo no se anula ($D_{x_0} ≠ 0$). Entonces existe un sistema $y_1, \dotsc, y_n$ de coordenadas locales en un entorno de $x_0$ en el cual $D = \dpa{}{y_1}$. \label{thm:EnderezamientoCampos}
\end{theorem}
...
El resto del ejemplo es demostrar que el determinante de la matriz jacobiana es no nulo en todo punto y entonces las coordenadas locales que enderezan el campo son coordenadas globales en este caso. Se comprueba de hecho de que es una carta local calculando la inversa.
...
¿Qué es, visualmente hablando, el corchete de Lie? Lo podemos ver en la \fref{fig:CorcheteLie}. El corchete $[D, D']$ de dos campos es una forma de medir la variación del campo $D'$ cuando nos movemos a lo largo de las líneas de flujo del campo $D$.
...
\begin{enumerate}
\item $[D, D'] = - [D', D]$. Esta se puede ver geométricamente de forma muy sencilla en la \fref{fig:CorcheteLie}: si hacemos el camino al revés, el vector que sale es el inverso.
...
\begin{theorem}[Teorema\IS de Frobenius] Sea $Δ$ una distribución en una variedad $M$ de dimensión $m$. Entonces, Δ es involutiva si y sólo si es completamente integrable.
\end{theorem}
...
\begin{example} Nos dan $f(t) = (t,t^2,t^3)$ y nos piden que digamos si es una inmersión, siendo $X = \img f ⊂ ℝ^3$ %y nosequé.
...
Recordamos que las $p$-formas pertenecen al espacio $\bigwedge^p(\tgsd_x X)$, ya que son de la forma \[ω = \sum_{i_1<i_2,\dotsc,i_p} a_{i_1, \dotsc, i_p} \, \dfl{x_{i_1}}{x_{i_p}} \]
...
\item Hay otras operaciones con formas interesantes (como la derivada de Lie) que no da tiempo a ver y que no utilizaremos durante el curso.
\end{itemize}
