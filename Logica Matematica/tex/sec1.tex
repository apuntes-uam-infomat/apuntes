...
\begin{defn}[Consecuencia tautológica]
	Decimos que $p$ es una consecuencia tautológica de $T$, y escribimos $T\vDash p$, si para toda interpretación $\sigma$ de $T$, $\sigma(p) = 1$. En este contexto, decimos que $p$ es una \textbf{tautología} y escribimos  $\vDash p$.
\end{defn}
...
\begin{theorem}[Teorema de completitud]	Sea $T = \emptyset$.
	\begin{itemize}
		\item \textbf{Validez}:
...
\begin{proof}
	...
	Si no, es decir, si $M\nvdash p$ y $M\nvdash \neg p$, tanto $M\cup\{p\}$ como $M\cup\{\neg p\}$ son consistentes, por el teorema \ref{thm:pconsnegp} y, puesto que $M$ es maximal y  $M\cup\{p\}$ extiende a $T$ y es consistente tendremos que $p \in M$. Del mismo modo, $\neg p \in M$.

	Pero si ocurre esto, de $M$ se podrían deducir $p$ y $\neg p$ con lo que dejaría de ser consistente llegando a una contradicción.

	Por tanto $M$ es completa.
