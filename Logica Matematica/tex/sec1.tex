...
\begin{mdframed}
	\textbf{Problema de las tautologías (TAUT)}: dada $F\in FBF(L)$, determinar si $F$ es una tautología, es decir, si para toda interpretación $\sigma$, $\sigma (F)=1$. ¿Cuánto tiempo se tarda en averiguar si $F$ es una tautología.
\end{mdframed}
...
\begin{itemize}
	\item \textbf{P:} clase de problemas para los que existe un algoritmo que los resuelve en tiempo polinómicoen función del número de datos de entrada.
	...
	\item No se sabe si P$=$NP. Clay Mathematics Institute ha ofrecido un premio de un millón de dólares estadounidenses para quién desarrolle la primera demostración correcta (si lo haces tú el profesor te pone una matrícula además).
\end{itemize}
