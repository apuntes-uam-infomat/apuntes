...
\end{enumerate}
\end{defn}
Si rescatamos las siguientes definiciones:
...
\begin{itemize}
\item el filtro no puede estar vacío (al menos contiene al total),
\item Si $A_i, A_j \in F$, $i \leq j$ , $A_i \cap A_j = A_i \in F$
\item Si $A_i, A_j \in F$, $i \leq j$ , $A_i \cup A_j = A_j \in F$
\end{itemize}
