...
Vamos a ver un pequeño resumen de lo que hemos hecho hasta ahora a fin de poder obtener una visión global de lo que hemos estudiado hasta la fecha.

\begin{enumerate}
\item Hemos empleado la teoría elemental de $\nat$, que no es más que la inducción, para probar validez, es decir:
\[\vdash \varphi \implies \vDash \varphi\]

\item Hemos empleado lenguages de primer orden para enunciar los axiomas de ZFC.

\item ZFZ (Zorn $\iff$ C) $\implies$ adecuaci'on, es decir:
\[Σ \models σ \implies Σ \vdash σ\]

\obs Esto nos llevo al teorema de completitud 2: Si Σ es consistente entonces tiene un modelo.

\item Llegados a este punto nos encontramos con un problema:

Al hablar de la consistencia, en $a$, $\forall σ$ tenemos que ver si $a \models σ$ ó $a \models \neg σ$ pero, ¿Cómo construimos $a$?
...
Si $Σ\models σ = \exists x \psi(x)$, decimos que $c_σ$ es un testigo de σ si $Σ \models \psi(c_σ)$.
\end{defn}

En general, dados $L$ y $Σ$ por cada $σ = \exists x \psi(x)$ tal que $Σ \vdash σ$ añadimos una nueva constante $c_σ$ a $L$.

Así obtenemos una extensión $L_1 = L \cup \{c_σ : \  Σ \vdash σ\}$ donde todas las constantes $c_σ$ son distintas entre si y distintas de las constantes en $L$.

Ahora extendemos Σ, primero añadiendo $\psi (c_σ)$ para cada σ, y a continuación, zornicando para obtener una teoría maximal consistente $Σ_1$.

Aunque puede parecer que ya hemos terminado aún podemos volver a tener el mismo problema de la \textbf{falta de testigos}, es decir, puede haber sentencias $σ_1$ en $L_1$ que no estén en $L$ tales que $Σ \vdash σ=\exists x \psi_1(x)$.

La solución obvia a este problema es repetir el procedimiento obteniendo $Σ_2, L_2$ siendo $Σ_2$ una teoría maximal consistente.
