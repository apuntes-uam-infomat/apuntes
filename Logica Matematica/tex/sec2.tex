...
En general, dados $L$ y $Σ$, por cada $σ = \exists x \varphi(x)$ tal que $Σ \vdash σ$, añadimos una nueva constante $c_σ$ a $L$.
...
Ahora extendemos Σ, primero añadiendo $\varphi (c_σ)$ para cada σ, y a continuación, zornicando para obtener una teoría maximal consistente $Σ_1$.

Extendemos $\Sigma$ a $\Sigma \cup \{\varphi(c_\sigma)\} = \{\exists x(x\neq c), c_\sigma \neq c\}$. Sea $\Sigma_1$ una extensión maximal consistente de $\Sigma\cup \{\varphi(c_\sigma)\}$.

\begin{obs}
	Todas las sentencias de $\Sigma$ tienen testigos.
\end{obs}

Tenemos que $L_1=\{c, c_\sigma\}$, y $\Sigma_1$ maximal consistente. 

Ahora bien $$\exists z((x\neq c)\y z\neq c_1) \in \Sigma_1$$ o bien $$\neg \exists z((x\neq c)\y z\neq c_1) \in \Sigma_1$$ En el primer caso tenemos una sentencia $\sigma_1$ sin testigo.
...
{\color{red}{Pedro arregla tu esto que ha empezado por aquí y como yo no vine la última clase he copiado la continuación donde me ha parecido pero seguro que no queda claro.}}
