	...
	\setcounter{enumi}{4}
		\item \textbf{Axioma de la unión.} Dada cualquier colección de conjuntos $\algb{C}$, existe un conjunto representado por $\bigcup \algb{C}$ llamado \textit{unión de }$\algb{C}$, que contiene todos los elementos de cada conjunto $\algb{C}$. Esto es:
		 ...
		 \begin{obs}
		 	...
		 	Pude demostrarse que este axioma prohíbe todas las cadenas infinitas descendientes y los bucles tales como $z_1\ni z_2\ni \hdots \ni z_n \ni z_1$.
		 \end{obs}
...
\begin{theorem}[Teorema\IS de completitud]
...
Es trivial y se deja como ejercicio para el lector.
\end{enumerate}
\end{proof}
...
\begin{theorem}[Teorema\IS de Presburger y Skolem]
...
\begin{theorem}[Teorema\IS de Cantor]
Cantor probó que
\[card(\nat) < card(\real)\]
por diagonalización.
...
\begin{defn}[Fórmula\IS bien formada]
...
En caso contrario, es decir, si $x$ aparece en $\phi$ pero no está cuantificada, decimos que $x$ es una \concept{variable\IS libre}
\end{defn}

\begin{defn}[Fórmula\IS cerrada]
...
\begin{defn}[Consecuencia\IS lógica]
...
La demostración de esta proposición se basa en el empleo del Teorema de Lindenbaum usando Zorn.
...
\begin{defn}[Término\IS constante]
...
\begin{theorem}[Teorema\IS de Lowenheinn-Skolem]
...
\begin{theorem}[Teorema\IS de Löwenheim-Skolem generalizado]
