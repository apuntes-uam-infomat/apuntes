...
\begin{tikzpicture}[scale=0.5]
	\draw[->] (-1,0) -- (3*pi,0) node[right] {$\gamma$};
	\draw[->] (0,-3) -- (0,3);
	\draw[-, dashed] (pi/2,-3) -- (pi/2,3);
	\draw[-, dashed] (3*pi/2,-3) -- (3*pi/2,3);
	\draw[-, dashed] (5*pi/2,-3) -- (5*pi/2,3);
	\draw[scale=1,domain=0:(pi/2-0.3),smooth,variable=\x,blue] 
		plot ({\x},{tan(\x r)});
	\draw[scale=1,domain=(pi/2+0.3):(3*pi/2-0.3),smooth,variable=\x,blue] 
		plot ({\x},{tan(\x r)});
	\draw[scale=1,domain=(3*pi/2+0.3):(5*pi/2-0.3),smooth,variable=\x,blue] 
		plot ({\x},{tan(\x r)});
	\draw[scale=1,domain=0:3.7,smooth,variable=\x,red] 
		plot ({\x},{1/(\x-4)});
	\draw[scale=1,domain=4.3:8,smooth,variable=\x,red] 
		plot ({\x},{1/(\x-4)});
\end{tikzpicture}
\caption{$\alpha, \beta > 0$}
\label{fig:ej611}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
\centering
\begin{tikzpicture}[scale=0.5]
	...
	\draw[scale=1,domain=(3*pi/2+0.3):(5*pi/2-0.3),smooth,variable=\x,blue] 
		plot ({\x},{tan(\x r)});
	\draw[scale=1,domain=0:8,smooth,variable=\x,red] 
		plot ({\x},{3*\x/(\x*\x+0.5)});
\end{tikzpicture}
...
\subsection{Condiciones de contorno simétricas}
