	...
	\subsubsection{Propiedad de la media}
		...
		\begin{prop}[Fórmula\IS de integración por partes] \label{prop:IdGreen2}$ $\\ % hack
			Sean $u \in C^1$, $v \in C^2$, entonces
				\( \int_\Omega u \Delta v \dif \gor{x} = \int_{\partial \Omega} u \pd{v}{\vn} dS - \int_\Omega \pesc{\grad u, \grad v} \dif \gor{x} \label{eq:IntegPartesNDim} \)
		\end{prop}
		\begin{proof}
			...
			despejando
			\[ \int_\Omega u \Delta v \dif \gor{x} = \int_{\partial \Omega} u \pd{v}{\vn} dS - \int_\Omega \pesc{\grad u, \grad v} \dif \gor{x} \]
		\end{proof}
		...
		% Es especialmente interesante porque nos dará la \concept[Fórmula\IS de integración por partes]{fórmula de integración por partes} en dimensiones superiores, ya que podemos reescribir $\int v Δu$ quitándonos el laplaciano en $u$: \( \label{eq:IntegPartesNDim} \int_\Omega v · Δ u \dif \gor{x} = \int_{∂\Omega} \left(v \dpd{u}{\vn} - u \dpd{v}{\vn}\right) \dif S + \int_\Omega u·Δv \dif \gor{x}\)
		...
		\begin{itemize}
			\item Dirichlet:
				...
				\[ 0 = - \int_{\Omega} w \Delta w \dif x = - \left\{  \int_{\partial \Omega} \eqreason[w]{= 0} \frac{\partial w}{\partial u} d\sigma = \int_{\omega}|\grad w|^2 dx \right\}  \]
			...
			\item Newman:
				...
				\[ \left. \begin{array}{r}
					-\Delta u = F \text{ en } \Omega \\
					\frac{\partial u}{\partial u}|_{\partial \Omega} = g
					\end{array} \right\} … \left\{ \begin{array}{l}
						- \Delta w = 0 \\
						\frac{\partial w}{\partial u}|_{\partial \Omega} = 0
					\end{array}  \right.  \]
