...
\right\} x - F(s) -s \equiv x - ky = s_1 \text{ con } k = F(s)
	...
	\begin{example}{5}
		...
		En este caso podemos despejar explicitamente:
	...
	\begin{example}{6}
		...
		Este último 0 nos indica que que el vector normal horizontal es el plano tangente, y es vertical por lo que la deriada se hará infinito en algún punto $\Rightarrow u(x,y) \notin C^1$
	...
	\begin{example}{7}
		...
		Tomamos un dato alternativo que satisfaga la transversalidad. Por ejemplo $u(x,0) = Cx$. Si se resuelve para estos datos podremos hasta despejar explicitamente y llegaremos a que:
	...
	\begin{example}{8}
		...
		Si la compatibilidad se cupliera habría solución pero si no, solo se puede obtener solución en regiones aisladas. En este caso no lo vamos a comprobar pero no son compatibles.
