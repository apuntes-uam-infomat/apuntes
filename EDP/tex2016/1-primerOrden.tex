...
\section{Series de Fourier}
	...
	Al empezar el curso ya vimos un ejemplo: la ecuación de laclor en una dimensión con datos de contorno Dirichlet homogéneos.

	\[
	\begin{cases}
	u_t + u_xx = 0 & x \in (0,L), t > 0
	u(0,t) = u(L,t) = 0 & t > 0
	u(x,0) = f(x) & x \in [0,L]
	\end{cases}
	\]

	La ecuación del medio es el dato de contorno de Dirichlet, es decir que especifica el dato en los extremos.


	Llegamos con separación de variables a que la solución del problema podía ser escrita como: \[ u(x,t) = \sum^{\infty}_{k=1} a_k e^{-(\frac{k\pi}{L})^2 t} \sin{\frac{k\pi}{L}} x \] donde \[ f(x) \eqexpl{?} = \sum^{\infty}_{k=1} a_k \sin{\frac{k\pi}{L}x} \]


	\begin{example}{2}

		Condiciones de la exuación de calor:
		...
		Esperamos que al final, cuando el tiempo tienda a infinito el calor se haya disipado, distribuido a lo largo de la varilla y la temperatura sea constante a lo largo de esta.

		Empecemos con el métodode separación de variables. Buscamos $u(x,t) = X(t) T(t)$ que sea solución de la ecuación con el contorno (el dato inicial se tratará después).
		...
		\[ \frac{T'(t)}{T(t)} = \frac{X''(x)}{X'(x)} \forall x, \forall t \]
		...
		\begin{itemize}
			\item $\lambda = 0$
				...
				\[ \left. \begin{array}{l} X(x) = a + bx \\
				\left. \begin{array}{r}
					X'(x) = b \\
					X'(0) = X'(L) = 0 \end{array} \right\} \Rightarrow b = 0
			...
			\item $\lamda < 0$ Con $\lambda = - \mu^2$

			 	\[
			 	X(x) = a \cos \mu x + b \sin \mu x
			 	\]
			 	\[ X'(x) = -a \mu \sin \mu x + b \mu \cos \mu x \]
			 	\[ 0 = X'(0) = b \mu \Rightarrow b = 0 \]
			 	\[X(x) = a \cos \mu x\]
			 	\[X'(x) = -a \mu \sin \mu x\]

			 	\[0 = X'(L) = -a \mu \sin \mu L \Rightarrow \mu L = k \pi , k = 1,2,…\]
		...
		Conclusión:
			 	...
			 	\[ \lambda_0 = 0, X_0 = a_0 \]
			 	\[\lambda_k = - (\frac{k\pi}{L})^2, X_k(x) = a_k \cos \frac{k \pi}{L}x\]

			 	EDP àra Y (para las $\lambda$ encontradas antes)

			 	\[\lambda_0 = 0 \Rightarrow T'_0 = 0 \Rightarrow T_0 \equiv \lambda_0\]
			 	\[\lambda_k = - (\frac{k\pi}{L})^2 \Rightarrow T'_k = (\frac{k\pi}{L})^2 T_k \Rightarrow T_k (t) = \lambda_k e^{-(\frac{k\pi}{L})^2 t} \]

			 	Soluciones particulares:

			 	\[u_0(x,t) = A_0 , u_k (x,t) = A_k \] (FALTA UN POQUITO)


			 	Dato inicial: $u(x,0) = f(x)$

			 	Idea: $u(x,t) \eqexpl{?} A_0 + \sum\limits_{k=1}^{}$
		...
		\begin{example}{3}
			...
			Veamos una cuerda de guitarra en tensión. La guitarra está atada en los extremos y en su punto medio se levanta $u$ sobre el eje horizontal.
			...
			\[\lambda_k = - (\frac{k\pi}{L})^2, X_k(x) = b_k \sin \frac{k\pi}{L} x\]
			...
			\[T''_k = - (\frac{k\pi}{L})^2 T_k \Rightarrow T_k (t) = \alpha_k \cos \frac{k \pi}{L} t + \beta_k \sin \frac{k \pi}{L}t\]

			Con lo que llegamos a las soluciones particulares:

			\[u_k(x,t) = A_k \cos (\frac{k\pi}{L})t \sin (\frac{k\pi}{L}) x + B_k \sin (\frac{k\pi}{L}) t \sin (\frac{k\pi}{L}) x\]

			Idea: Buscar

			\[u(x,t) \eqexpl{?} = \sum_{k=1}^{\infty} A_k \cos (\frac{k\pi}{L}) t \in (\frac{k\pi}{L}) x + B_k \sin (\frac{k\pi}{L})t \sin (\frac{k\pi}{L}) x\]

			Datos iniciales:

			\[ f(x) = u(x,0) \eqexpl{?} \sum^{\infty}_{k=1} A_k \sin (\frac{k\pi}{L}) x \]
