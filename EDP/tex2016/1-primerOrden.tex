...
\chapter{Ecuaciones de primer orden}
	...
	\label{sec:ModeloGeneral}
		...
		\label{sec:PrincipioDuhamel}
			...
			Una segunda solución es aplicar el \concept{Principio\IS de Duhamel}, (utilizado por prácticamente todos los físicos) que se basa un poco en la misma idea que antes: que el valor de $u$ en un punto depende de cómo ha ido evolucionando a lo largo del tiempo, y a ese resultado se le puede sumar el dato inicial (la suma de soluciones es solución).
			...
			Lo que ocurre es que hay una función, que llamaremos ξ y que, por así decirlo, ``transporta'' las aportaciones externas. Dependerá de $x$ y de $t$, el momento en el espacio y tiempo en el que queremos saber la aportación externa, pero también de otro parámetro $s$, que será el ``momento'' en el que aparece la aportación externa que estamos considerando. Es inútil dar la respuesta al impulso $F(x,s)$ en $t=0$, (antes de la aplicación de los impulsos), luego tomamos $t = s$ , y la aportación externa viene dada por $f(x,t)$, luego ha de ser $ξ(x,s; s) = f(x,s)$.
	...
	\label{sec:ModeloTraficoRealista}
		...
		En los ejemplos anteriores de tráfico no hemos tenido en cuenta que la velocidad de los coches no es totalmente proporcional a la densidad. Si ésta baja mucho, los coches llegan a pararse. Hay una densidad máxima a partir de la cual el tráfico quedará totalmente estancado y el flujo es 0 (los coches están muy pegados).
		...
		¿Por qué una parábola? Porque si cogemos otra función cualquiera haríamos Taylor y nos quedaríamos con el polinomio interpolador de orden 2 (velocidad constante $\equiv$ Taylor de grado 1; velocidad no constante $\equiv$ Taylor grado 2).
		...
		Obtengamos las características, las curvas a lo largo de las cuales la densidad es constante ${u(x(t),t) = k}$, luego derivando tenemos que ${u_x x' + u_t = 0}$. Viendo el sistema anterior, tiene que ser
...
\newline
		Nuevamente son rectas, pero esta vez su pendiente es variable dependiendo del dato (lo que hace que pueda haber más casos que veremos ahora mismo, como que se corten las características).
		...
		por lo que la onda de choque sigue $x = s(t) = \alpha t$.
			...
			\begin{example}[Semáforo - Pendiente de resolución]
				...
				\begin{figure}[hbtp]
					\centering
					\inputtikz{caracteristicasSemaforo}
					\caption{Rectas características con una región en la que no sabemos qué ocurre (Hay una región del plano no cubierta por características).}
				...
				Para calcular la región intermedia usaremos esta \textbf{idea} (Ondas de Rarefacción): Una aproximación y paso al límite. Imaginemos que $F_\epsilon$ es continua y decreciente, como en la \fref{fig:FEpsilon}. Entonces en la región intermedia:
