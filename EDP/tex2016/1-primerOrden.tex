...
\section{Modelo de un atasco}
	...
	\subsubsection{Modelo general}
		...
		\paragraph{Solución 2}
			...
			Para hallar la solución en este caso, lo que vamos a hacer va a ser separarla: por un lado tendremos una solución que nos dará la dependencia del dato inicial, y por otro la depedencia de la aportación externa. Tendremos entonces que \[ u = \varphi + \psi \], lo que nos crea dos sistemas:
			\begin{align*}
			\varphi_t + v\varphi_x &= 0 	& \psi_t + v\psi_x &= f(x,t) \\
			\varphi(x,0) &= F(x)		& \psi(x,0) & = 0
			\end{align*}

			El primero ya lo hemos resuelto en la \fref{sec:ModeloBasicoFlujoProporcional}, y la solución era $φ(x,t) = F(x - vt)$.

			El segundo problema se resuelve con el \concept{Principio\IS de Duhamel} (la fuente externa se interpreta como una fuente de datos iniciales, se cambia el cero de la segunda ecuación de las anteriores) se puede interpretar como que tenemos una función que transporta el sistema.
			...
			\noteby{Guille}{¿Es cosa mía o ambas soluciones llevan a lo mismo, a integrar la $f$ de $0$ a $t$?}
	...
	\subsection{Modelo de tráfico más realista}
		...
		Me fijo en un punto en el instante inicial. Con el valor de ese punto calculamos la pendiente. A lo largo de las características el valor del dato se mantiene. En este caso las características no son paralelas porque la pendiente es $1-2k$, es decir, depende de $k$, como en la \fref{fig:rectasDivergentes}.
		...
		\begin{example}[Frenazo]
			...
			Si ibujamos las características con \eqref{eq:caracteristicas_realistas} nos saldrá algo como lo de la \fref{fig:ondaChoque}. Ahí vemos casos en los que cuando recorremos una recta característica con valor $\frac{1}{5}$ nos encontramos con otra de valor $\frac{2}{3}$. Momento en el cual se produce el frenazo.
		...
		En otras palabras, que la variación de la curva de choque $s$ (llamada \concept{Ecuación\IS de Rankine - Hugoniot}) es el cociente entre el salto en el flujo $q$ y el salto en la densidad $u$. Esta curva es precisamente la queque es precisamente la que vimos en la figura \ref{fig:ondaChoque}.
