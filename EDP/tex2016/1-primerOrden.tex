...
\chapter{Ecuaciones de primer orden}
	...
	\label{sec:ModeloGeneral}
		...
		\label{sec:PrincipioDuhamel}
			...
			Una segunda solución es aplicar el \concept{Principio de Duhamel}, que se basa un poco en la misma idea que antes: que el valor de $u$ en un punto depende de cómo ha ido evolucionando a lo largo del tiempo, y a ese resultado se le puede sumar el dato inicial (la suma de soluciones es solución).
	...
	\label{sec:EcuacionBurgers}
		...
		\subsubsection{Características en la ecuación de Burgers}
			...
			Por lo que las características serán: \[ x-kt = x_0 \text{, con }F(x_0) = k \], y con $W(x,0) = F(x)$ nuestro dato inicial.
		...
		El contrario, por supuesto, si que tiene más sentido físico\footnote{Si no recuerdo mal, el sentido físico era coches estrellándose contra un muro. No es un caso que estemos especialmente interesados en estudiar, creo.}.
