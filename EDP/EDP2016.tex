\documentclass[bibnumbers, palatino]{apuntes}

\title{Ecuaciones en Derivadas Parciales}
\author{G. Guridi, E. Miravalls, G. Julián}
\date{15/16 C2}

% Paquetes adicionales
\usepackage{tikztools}
\usepackage{fancysprefs}
\usepackage{enumitem}
\usepackage{fastbuild}
\usepackage{titlesec}
\usepackage{xfrac}
\usepackage{tikz-3dplot}
\usepackage{relsize}
% --------------------





\titleclass{\subsubsubsection}{straight}[\subsection]

\newcounter{subsubsubsection}[subsubsection]
\renewcommand\thesubsubsubsection{}
\renewcommand\theparagraph{\thesubsubsubsection.\arabic{paragraph}} % optional; useful if paragraphs are to be numbered

\titleformat{\subsubsubsection}
  {\normalfont\normalsize\bfseries}{\thesubsubsubsection}{1em}{}
\titlespacing*{\subsubsubsection}
{0pt}{3.25ex plus 1ex minus .2ex}{1.5ex plus .2ex}

\makeatletter
\renewcommand\paragraph{\@startsection{paragraph}{5}{\z@}%
  {3.25ex \@plus1ex \@minus.2ex}%
  {-1em}%
  {\normalfont\normalsize\bfseries}}
\renewcommand\subparagraph{\@startsection{subparagraph}{6}{\parindent}%
  {3.25ex \@plus1ex \@minus .2ex}%
  {-1em}%
  {\normalfont\normalsize\bfseries}}
\def\toclevel@subsubsubsection{4}
\def\toclevel@paragraph{5}
\def\toclevel@paragraph{6}
\def\l@subsubsubsection{\@dottedtocline{4}{7em}{4em}}
\def\l@paragraph{\@dottedtocline{5}{10em}{5em}}
\def\l@subparagraph{\@dottedtocline{6}{14em}{6em}}

\precompileTikz

\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{4}

\bibliographystyle{plainnat}

\begin{document}
\pagestyle{plain}

% http://tex.stackexchange.com/a/14243
%\relpenalty=10000
%\binoppenalty=10000

\begin{abstract}
Estos son los apuntes del curso de Ecuaciones en Derivadas Parciales (2016), del profesor Jesús García Azorero. Es recomendable también mirar los apuntes del año 2015 \cite{ApuntesEDPRual}, que son del mismo curso aunque con un temario organizado de forma distinta.
\end{abstract}

\maketitle

\tableofcontents
\newpage
% Contenido.

\input{tex2016/0-introduccion.tex}

\input{tex2016/1-primerOrden.tex}

\input{tex2016/2-segundoOrden.tex}

\input{tex2016/3-comportamientoCualitativo.tex}

%% Apéndices (ejercicios, exámenes)
\appendix

\input{tex2016/A-Resumen.tex}

\chapter{Ejercicios}

Últimamente nos llegan comentarios de gente que dice que ``los apuntes están muy bien, pero me daba vergüenza deciros que el ejercicio $X.Y$ estaba mal''. Gracias.

Pero si tú te beneficias de estos ejercicios y crees que están mal, por favor háznoslo saber por email o en persona.

El profesor no va a tener ningún reparo en ponernos un 0 en el examen.

\input{ejs2016/hoja1.tex}
\input{ejs2016/hoja2.tex}
\input{ejs2016/hoja3.tex}
\input{ejs2016/examenes.tex}


\chapter{Ejemplos - Ecuaciones cuasilineales de primer orden}\label{apx:JGA-cuasilinear}
El profesor Jesús García Azorero confeccionó el siguiente documento en el que explica los ejemplos más complicados vistos en el tema 1.
\tikzexternaldisable
\includepdf[pages=-]{Ejemplos-primer-orden.pdf}
\tikzexternalenable

\bibliography{../Apuntes}{}
\printindex
\end{document}

