...
\begin{problem}[4] Un modelo bastante realista del tráfico en un túnel muy largo es el siguiente:
	...
	Con esto, lo que queda es un dibujo como en la \fref{fig:Hoja1:E4}. Hay un problema, y es que hay una zona en la que no sabemos qué ocurre. Para la zona dada por $t(v_m - λ) ≤ x ≤ -λt$ no hemos visto la ecuación de la densidad, así que tenemos que hacer un poco de magia. Lo que hacemos es fijarnos en $t = t_0$.
	...
	Sabemos que $x_0 = v_m t_0$, y que $x_1 = t_0(v_m - λ)$. Con eso operamos:
	\begin{align*}
	e^{1 + \frac{x_0}{λt_0}} &= e^{1 + \frac{v_m}{λ}}
	\end{align*}
