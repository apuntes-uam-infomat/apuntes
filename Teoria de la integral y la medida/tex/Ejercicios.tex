...
Puesto que se trata de una unión disjunta, si tomamos la medida de todo ello obtenemos:
...
%TODO completar: estos conjuntos los obtenemos numerando los elementosde X
\end{enumerate}
...
\[µ(\lim (\inf (E_j)))<\lim(\inf (µ(E_j)))\]
\[µ(\lim (\sup (E_j)))>\lim(\sup (µ(E_j)))\]
