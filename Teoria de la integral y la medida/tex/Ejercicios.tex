...
\begin{problem}
Sea µ la medida de LEbesque-Stieldjes asocaida a la función:
...
\solution
Puesto que la función está definida a trozos vamos a estudar las medidas de los diferentes intervalos:
\begin{itemize}
\item $µ(\{1\}) =1 = F(1)-\lim_{x \to 1^-} F(x)$
\item $µ(\{2\}) = 0$
...
\begin{enumerate}
\item Sea $A=\{1,2,...,n,...\}$ ¿Es $µ_A$ una medida de Lebesque-Stieldjes?. En caso afirmativo halla F tal que $µ_A=µ_F$
\item Sea $A=\{\frac{1}{1},\frac{1}{2},...,\frac{1}{n},...\}$ ¿Es $µ_A$ una medida de Lebesque-Stieldjes?. En caso afirmativo halla F tal que $µ_A=µ_F$
\end{enumerate}
...
Por tanto, simplemente tenemos que buscar una función que realice esa misma función, tomamos para ello la $F$ que toma la parte entera de cada número.

\item Esta medida cuenta el número de racionales de la forma $\frac{1}{n}$ que hay en un conjunto.

Esta medida no es de Lebesque-Stieldjes ya que no puede existir una función F como en el paratado anterior. Dicha función debería saltar en una unidad en cada uno de los puntos indicados y ser creciente a la par que continua por la derecha, lo cual es imposible.
...
\solution
\begin{itemize}
%TODO completar el resto en casa
\item $µ(\{x \in \real \tq |x|+2x^2 > 1\})$=$µ((-\infty, -\frac{1}{2})) + µ((\frac{1}{2}, \infty))$
\end{itemize}
\end{problem}
...
Prueba que $F(x)=\int_{-\infty}^x f(y) dy$ es una función de distribución de probabilidad y que, además, F es continua.
...
Observando que $F$ es una integral y que integrar una función equivale a calcular el área encerrada bajo ella, vemos que a medida que avanzamos la y, cada vez estamos calculando un área mayor.

Para ver que es continua por la derecha observamos que:
\[\lim_{h \to 0^+}F(x+h)=\lim_{h \to 0^+} \int_{-\infty}^{\x+h}f(y)dy = \int_{-\infty}^xf(y)dy\]
