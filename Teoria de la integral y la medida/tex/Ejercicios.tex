...
\[\limsup µ(\bigcup_{k=0}^{\infty}\bigcap_{j=k}^{\infty}E_j)=\]
...
\[\limsup µ(\bigcup_{k=0}^{\infty}\bigcap_{j=k}^{\infty}E_j)=\limsup µ(E_k)\]
\end{problem}
...
\solution 
Lo apaño yo. Vaya mierda

La idea es tomar el supremo. Ver que si hay supremo se alcanza y que si se alcanza se puede superar.

Por tanto el supremo es infinito
\end{problem}

\begin{problem}
En un espacio de meidda $(X, \algbM, µ)$ sea $\{A_i\}$ una familida de conjuntos medibles tales que $\sum µ(A_i) < \infty$.
...
La última convergencia aquí indicada se deduce de la finitud de la serie. Si una serie es finita, su cola converge a 0.
\end{problem}

\begin{problem}
Sea $X$ un conjunto no numerable. Sea $\algbM$ la $\salgb$ formada por los conjuntos finitos o numerables y los conjuntos con complementario finito o numerable. Sea µ la medida que me da el cardinal del conjunto si éste es finito e infinito si el conjunto es infinito:
\begin{enumerate}
...
\solution
Aunque el enunciado no dice mucho al respecto, Patricio insiste en dejar claro que  $\algbM$ es realmente una $\salgb$ lo cual se ve de manera sencilla.
...
\item La medida exterior a partir de µ y de $\algbM$.

En este caso, $µ^*(E)=card(E)$ si $E$ es finito y $µ^*(E)=\infty$ en caso contrario.

Habría que ver ahora qué ocurre con los conjuntos $\notin \algbM$. Para ellos todo recubrimiento contiene algún conjunto ... bla bla bla
%TODO...
\end{enumerate}
\end{problem}
...
\[µ^*(E) = \infty \text{ si } E \text{ es infinito}\]
...
Vemos que en todos los casos posibles para estos $E$ se verifica la desigualdad: los dos finitos, infinitos, uno y uno
%TODO completar

\item $µ^*(\bigcup A_n) \leq \sum µ^*(A_n)$
Como en el apartado anterior, podemos verlo por casos.

%TODO por casos otra vez, exactamente como lo hice en casa
\end{itemize}

Vamos a ver ahora cuál es la $\salgb$ de los conjuntos medibles.
\[\algbM^*=\{E \subset \nat \ \forall A \subset \nat \ µ^*(A)\geq µ^*(A \cap E) + µ^*(A \cap E^c)\}\]
Tomamos la desigualdad porque la desigualdad contraria está garantizada siempre, de modo que forzar que se cumpla esta desigualdad implica el cumplimiento de la igualdad.

Para que un conjunto $E$ pertenece a esta $\salgb$ necesitamos ver que la parte de la derecha de la desigualdad es menor que 1.

Sin embargo, tanto tomando $E$ finito como infinito nos tomamos siempre con contradicción. Al final este algebra solo tiene el vacío y el total.
\end{problem}
