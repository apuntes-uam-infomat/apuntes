...
Queda claro que esta construcción genera la mínima $\salgb$ generada por el conjunto ε y que tiene el cardinal buscado.
\end{proof}
...
Vamos a definir:
\[\forall E \in \algb{C} \ \algb{C}(E)=\{F \in \algb{C}: \ F \in \algb{C}, E\cap F, \ E\setminus F, \ F\setminus E \in \algb{C}\} \subset \algb{C}\]

Vemos que claramente, $\algb{C}(E)$ es clase monótona. 
%TODO Completar si eso que el profe ha gastado tiempo considerable en escribirlo.
Además esta claro que:
\[\forall E,F \in \algb{C} \implies E\in \algb{F} \iff F \algb{E}\]
