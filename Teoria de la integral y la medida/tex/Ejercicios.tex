...
%TODO copiar como queda la función
...
$≥$) la da el lema de Fatou, reescribiendo $f_n(t)\ind_{(-∞,x)} \to f(t)\ind_{(-∞,x)}$, entonces:

\[\int_{-∞}^{x} f_n = \int_{ℝ} f_n(t)\ind_{(-∞,x)}\]

\[\lim_n \int_{-∞}^{x} f_n dm =\lim_n \int f_n(t)\ind_{(-∞,x)} = \lim inf \int f_n(t)1_{(-∞,x)}dm \overset{Fatou}{≥} \int f\ind_{(-∞,x)} dm\]

Por hipótesis $\lim f_n = f \implies \lim \int f_n(t)\ind_{(-∞,x)} = \lim inf \int f_n(t)\ind_{(-∞,x)}$
...
Como el límite inferior y superior coinciden (por hipótesis) y el valor de la izquierda del todo y el de la derecha del todo son el mismo, obtenemos la igualdad buscada
...
$h(y) = \int_X \ind_V(x,y) = \int0=0$. \textbf{Razón:} $h(y)$ depende de $y$, y para cada valor de $y$ hay un único punto $(x,y)$ en el que la indicatriz toma valor, que es $(y,y)$. Es por esto, y por ser $m(x)$ la medida de Lebesgue, que la integral vale 0.
...
$g(x) = \int_Y \ind_V(x,y) dν(y) = 1$, por ser $ν$ la medida de contar y para cada $x$ hay un punto en $V(x,y)$, con lo que $\ind_V$ sólo toma valor en un único punto ($(x,x)$)
