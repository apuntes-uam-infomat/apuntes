...
Vamos ahora a buscar un ejemplo de $\salgb$ de infinitos elementos, como pide el enunciado.
 TODO
\end{problem}
...
Supongamos ahora que  la función $f$ es suprayectiva. En este caso seguríamos teniendo problemas si $f$ no es inyectiva.
...
Podemos comprobar fácilmente que es una $\salgb$ observando que cumple las propiedades necesarias siguiendo el mismo razonamiento que el realizado para construirla (la unión de numerables o finitos siguie siendo numerable o finita y su cumplementario cumple la propiedad de tener complementario numerable o finito).
...
Si queremos probar que $µ_E$ es una medida deberemos comprobar que se satisfacen esas dos conciciones, es decir:
\[µ_E(\emptyset) = 0\]
...
Si fuese $\sfin$,
\[\exists \{E_n\}_{n \in \nat} \subset \algbM \tq X = \bigcup E_n \text{ con } µ(E_n)< \infty \forall n\]
es decir, el cardinal de $E_n$ sería finito  y $X= \bigcup_{n \in \nat }E_n$ lo que implicaría que $X$ es numerable, en contradición la hipótesis inicial.
...
$µ(A)=0$ si A es finito y $µ(A)=\infty$ si A infinito.
...
Para obtener elegamentemente estos conjuntos podemos tomar una numeración de los elementos de X (ya que es numerable). Es decir, consideramos:
\[X= \bigcup_{n=0}^{\infty}A_n \text{ donde } A_n=\{a_n\}\subset X\]
En este caso µ(X) es finita pero a la izquierda tendríamos una suma infinita de valores positivos ($µ(A_n)$) que nos daría infinito
...
Ahora bien, ¿puede ser esa desigualdad estrictamente mayor cuando $n\to∞$? Si \[ μ(A) > μ\left(\bigcup_{j=0}^∞ A_j\right) \], entonces $\bigcup_{j=0}^∞ A_j \subsetneq A$, contradicción.
\end{problem}
...
Ahora bien, tenemos una sucesión de conjuntos de la que caculamos el límite superior y el límite inferior mediante las dos definiciones proporcionadas al inicio del ejercicio.
...
Una vez queda claro que el límite superior y el inferior no dependen de los n primeros elementos de la familia, está claro que, sin pérdida de generalidad, podemos demotrar lo que pide el enunciado suponiendo que k=0.
...
Tomando el incio y el final de esta cadena de desigualdades llegamos a lo que queríamos demostrar:
...
\begin{problem}[9]
Sea µ una medida semifinta y sea $E$ tal que $µ(E) < \infty$. Prueba que si c es un número real mayor que 0, existe un conjunto $F \subset E$ tal que c < $µ(F)$ < $\infty$
...
\begin{enumerate}
\item Para ver que se una medida completa debemos ver que la medida del vacío es 0, cosa que es obvia pues el cardinal del vacío es 0.

Además debemos comprobar que todo subconjunto de un conjunto de medida 0 se contiene en $\algbM$.

Puesto que en $\algbM$ el único conjunto de medida 0 es $\emptyset$, no hay nada que demostrar.

\item La medida exterior a partir de µ y de $\algbM$ se define como:
\[µ ^*(E)=\inf\{µ(A) \tq E \subset A \text { con } A \in \algbM\}\]

En este caso, $µ^*(E)=card(E)$ si $E$ es finito y $µ^*(E)=\infty$ en caso contrario.

Habría que ver ahora qué ocurre con los conjuntos que no pertenecen a  $\algbM$.

Se deja como ejercicio para casa.
%TODO completar esto
...
Tomamos la desigualdad porque la desigualdad contraria está garantizada siempre, de modo que forzar que se cumpla esta desigualdad implica el cumplimiento de la igualdad (que es como se define la $\salgb$ de los conjuntos medibles).
...
\begin{problem}
Sea µ la medida de Lebesgue-Stieldjes asociada a la función:
...
\begin{enumerate}
\item Sea $A=\{1,2,...,n,...\}$ ¿Es $µ_A$ una medida de Lebesgue-Stieldjes?. En caso afirmativo halla F tal que $µ_A=µ_F$
\item Sea $A=\{\frac{1}{1},\frac{1}{2},...,\frac{1}{n},...\}$ ¿Es $µ_A$ una medida de Lebesgue-Stieldjes?. En caso afirmativo halla F tal que $µ_A=µ_F$
\end{enumerate}
...
Esta medida no es de Lebesgue-Stieldjes ya que no puede existir una función F como en el paratado anterior. Esta función debería alcanzar el valor infinito para cualquier intervalo $(0, ε)$, ya que a partir de cierto n todos los siguientes elementos de $A$ se contienen en $(0, ε)$.
...
Prueba que $F(x)=\int_{-\infty}^x f(y) dy$ es una función de distribución de probabilidad y que, además, F es continua (f es la función de denisdad de F).
...
Suponemos ahora $f(x)=\ind_{[0,1]}$, para responder a la segunda preguna del enunciado. Entonces
...
\begin{problem}
Halla el valor de $k$ para que $f= kx(1-x)\ind_{[0, 1)}$ sea la función de denisdad de una medida de probabilidad. Calcula su función de distribución.
...
Vamos ahora a por la parte jodidilla del ejercicio.
...
\begin{enumerate}
\item Prueba que el conjunto de puntos de discontinueidad de $F$ es numerable
...
\solution
\begin{enumerate}
\item Si tuviesemos una cantidad no numerable de discontinuidades tendríamos un intervalo cerrado que contiene una cantidad no numerable de discontinuidades. Puesto que cada una de esas discontinuidades tenemos un salto, resulta que tendríamos un número no numerable de saltos en un intervalo cerrado.
...
\end{enumerate}
\obs Se recomienta construir una función $F(x)$ que sea constante en un intervalo
\solution
TODO
\end{problem}
