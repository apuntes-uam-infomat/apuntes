...
\solution
Sabemos que en un conjunto finito toda álgebra es una $\salgb$, ya que la diferencia entre estos dos conjuntos era el cierre por uniones infinitas numerables. Si un conjunto es finito, $\algb{P}(X)$ será finito.
...
\begin{enumerate}
\item
\[x\in g^{-1}(E_1 \cup E_2) \iff g(x) \in E_1 \cup E_2 \iff  \exists i=1,2 \ g(x) \in E_i \iff \exists i=1,2 \ x\in g^{-1}(E_i) \iff\]
\[\iff x\in g^{-1}(E_1) \cup g^{-1}(E_2) \]
\item
\[x\in g^{-1}(Y \setminus E) \iff g(x) \in Y \setminus E \iff g(x) \notin E \iff x \notin g^{-1}(E) \iff x \in X\setminus g^{-1}(E)\]
\end{enumerate}

\end{proof}
Así, por la primera propiedad queda claro que $\algb{B}$ es cerrado por uniones y con la segunda propiedad vemos qu es cerrado por complementación. 

Además, queda claro que tanto el vacío como el total se contienen en $\algb{B}$ por lo que se trata de una $\salgb$
\end{problem}
...
\item $\Leftarrow$
Este caso tiene mas chicha.
\textcolor{red}{Ni guarra}
\end{itemize}
\end{problem}
...
Podemos comprobar fácilmente que es una $\salgb$ observando que cumple las propiedades necesarias siguiendo el mismo razonamiento que el realizado para construirla. Es la mínima por construcción.
...
\solution
Si la $\salgb$ es infinita podemos encontrar una colección numerable y disjunta de elementos de la misma. (Lo podemos hacer tomando una colección numerable y haciéndola disjunta mediante la eliminación de la unión de los anteriores).

Denotamos a esta colección como: $\{A_n:n\in \nat\}$. 

Ahora vamos a construir una colección infinita de $A_x$ como sigue:
\[\forall x \in (0,1) \text{ obtenemos el desarrollo en base 2 de x } x=0,x_1,x_2,x_3,...\]
\[A_x=\bigcup_{n=1}^{\infty}A'_n \text{ donde } A'_n=A_n \ si \x_n=1\]
...
La condición que le falta a un $\sigma$-anillo para ser una $\salgb$ le falta contener al total. Por tanto queda clara la última demostración.
\end{problem}
...
No hay nada que demostrar por que esta primera propiedad es parte de la definición de $\salgb$.

Ahora vamos a buscar el ejemplo de clase monótona que no sea $\salgb$.
...
La clase monótona sería una subclase de $\algb{P}(\nat)$ formada por todos los conjuntos aquí descritos y no es una $\salgb$.
\end{problem}
...
\begin{enumerate}
\item Si una clase de conjuntos es álgebra y clase monótona, entonceses $\salgb$.
