...
\[m^*\left(I \smallsetminus K\right) \leq m^*\left(I \smallsetminus O\right) + \left(O \smallsetminus K\right)\]

Si usamos que $m^*\left(K\right) \leq m^*\left(O\right)$, a partir de la desigualdad anterior obtenemos:
\[m^*\left(K\right) + m^*\left(I \smallsetminus K\right) \leq m^*\left(O\right) + m^*\left(I \smallsetminus O\right) + \left(O \smallsetminus K\right) \leq \abs{I} + \epsilon\]
...
\end{lemma}
\begin{proof}
%TODO Jorge, apaña esto
Dado $\epsilon \geq 0, \exists K \subset C$ tal que $m^*(K) \geq m_*(C) - \epsilon \Rightarrow m_*(C)$
\end{proof}
