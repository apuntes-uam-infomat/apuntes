...
Dado un espacio de medida ($X, \algb{M}, µ$), se dice que µ es semifinita si todo $E\in \algb{M}$ tiene un subconjunto $F\subset F$ con $E\in \algb{M}$ tal que 0<µ($F$) < $\infty$
...
\[\forall \epsilon > 0 \ µ_0(\bigcup_{i=1}^{\infty}(a_i, b_i]) \leq \sum_{i=1}^{\infty} (F(b_i= - F(a_i))+ \epsilon\]
...
\[a_{i_1}<a_{i_2} < b_{i_1}+\delta_{i_1} < a_{i_3} < ... < a_{i_N} < b_{i_{N-1}} + \delta_{i_{N-1}} < b_{i_N}+\delta_{i_N}\]
...
Por ser F creciente y basándonos en la ordenación de los intervalos indicada anteriormente llegamos a:
\[F(b)-F(a) \leq F(b_{i_M}+\delta_{i_N}) - F(a_{i_N}-F(a_{i_{N-1}} + ... + F(a_{i_1}-F(a+\delta)+\frac{\epsilon}{2}\]
