...
\pagestyle{plain}
\begin{abstract}
Apuntes de Teoría de la Integral y la Medida, tomados en la clase del profesor Patricio Cifuentes. Contienen un primer repaso a la integral de Riemann, una introducción a la teoría de la medida y, por último, su aplicación a las funciones medibles y a la integral de Lebesgue incluyendo ciertas nociones sobre convergencia.
\end{abstract}
...
\[\alpha f \in R([a,b])\] \[ J(\alpha f) = \alpha J(f)\]
...
Dado un conjunto $M \subset \mathbb{R}$, $M$ es medible si y sólo si $\forall n \in \mathbb{Z}, \ M \cap(n, n+1)$ es medible en $(n, n+1)$ y entonces:
...
Entonces definiremos nuestro producto cartesiano como \[ X=\prod_{\alpha\in A}X_\alpha=\{\rho: A \rightarrow \bigcup_{\alpha\in A} X_\alpha : \rho(\alpha)\in X_\alpha\} \]


Podemos ver\footnote{jajajajajajaja} que coincide con la definición de producto cartesiano habitual: \[X=X_1\times X_2=\{\rho: \{1,2\} \rightarrow X_1 \bigcup X_2 : \rho(1)\in X_1 \wedge \rho(2) \in X_2\}\]
...
\[\varepsilon_2 =\set{\prod_{\alpha\in A}E_\alpha:E_\alpha \in \algb{M}_\alpha}\]
...
Sea $X \neq \emptyset$ y sea $µ^*$ una medida exterior en $X$. Entonces \[ \algbM = \{A \subset X: \ \forall E \subset X \ µ^*(E)=µ^*(E \cap A) + µ^*(E \cap A^c)\}\] es una $\salgb$ y $µ^*_{|\algbM}$ es una medida completa.
\end{theorem}
\begin{proof}
...
\begin{itemize}
\item La medida del vacío es cero: $µ_0(\emptyset)=0$.
\item Si $\{A_j\}_{j \in \nat} \subset \algb{A}, A_i \cap A_j = \emptyset$ y $\bigcup_{j=0}^{\infty}A_j = \algb{A}$ entonces \[ µ_0\left( \bigcup_{j=0}^{\infty} A_j \right) = \sum_{j=0}^{n}µ_0(A_j) \]

Es decir, si podemos descomponer $\algbA$ como unión de disjuntos, entonces la medida de la unión es la suma de las medidas.
\end{itemize}
\end{defn}
...
\begin{enumerate}
\item La medida exterior extiende la premedida: $µ^*_{|\algb{A}}=µ_0$.
...
\[\forall a \in \real \quad \exists \lim_{x \to a_+}F(x)=F(a)\]
\end{defn}

La idea es definir $µ_0(a,b]=F(b)-F(a)$.

\begin{prop}
Sea $\appl{F}{\real}{\real}$ una función creciente y continua por la derecha. Entonces, la función de conjunto en $\algbA$ definida por \[ µ_0\left(\bigcup_{i=1}^{\infty}(a_i, b_i]\right) = \sum_{i=1}^{\infty}\left(F(b_i)-F(a_i)\right) \] para una unión disjunta de intervalos abierto-cerrados, es una premedida en $\algbA$.
\end{prop}

\begin{proof}
\begin{enumerate}
\item Lo primero que debemos hacer es comprobar que $µ_0$ está bien definida. Es decir, que si una unión de este estilo puede escribirse de dos formas diferentes, la unión resulte ser la misma.

Puesto que trabajamos con uniones finitas, basta con comprobar que esta propiedad es cierta para dos intervalos y extenderla por inducción. Vamos entonces a por el caso complicado. Sea
\[(a,b]=(a_1,b_1]\cup (a_2, b_2] \] con $(a_1,b_1]\cap (a_2, b_2] = \emptyset$ y siendo $a=a_1, \ b_1 = a_2, \ b=b_2$. Entonces \[F(b)-F(a)=F(b_1)-F(a_1)+F(b_2)-F(a_2)\] y la igualdad se cumple para dos conjuntos $(a_1,b_1], (a_2, b_2]$ cualesquiera tales que su unión sea $(a,b]$.
...
\begin{enumerate}
\item $µ(E)=\inf \{µ(A) \tq E \subset A, \ A \text{ abierto}\}$.
\item $µ(E)=\sup \{µ(K) \tq K \subset E, \ K \text{ compacto}\}$.
\end{enumerate}
\end{theorem}
\begin{proof}
...
\begin{enumerate}
\item $E$ es medible, es decir, $E \in \algbM$.
\item $E$ se puede expresar como intersección numerable de abiertos menos un conjunto de medida cero (que puede ser el vacío). Esto es, que $\exists V \in G_{\delta}, \ \exists N \in \algb{N} \tq E=V\setminus N$.

\item $E$ se puede expresar como unión numerable de cerrados menos un conjunto de medida cero (que puede ser el vacío). Esto es, que $\exists H \in F_σ, \ \exists N \in \algb{N} \tq E=H \cup N$.
\end{enumerate}
\end{theorem}
...
\begin{prop}
Si $µ(E) < \infty$ entonces para todo $ε>0$ podemos encontrar una unión de abiertos que lo aproxima tanto como queramos. Es decir, que
\[ \exists A=\bigcup_{i=1}^{n}(a_i, b_i) \tq µ(E \vartriangle A) < ε\]
...
\begin{defn}[Integral\IS positiva]
...
\section{Extensión de la integral a funciones de la forma $\appl{f}{X}{\overline{ℝ}}$ medibles}
...
\begin{defn}[Integrable\IS Lebesgue]
...
\begin{theorem}[Teorema\IS de la convergencia dominada]
...
Primero reduciremos el conjunto de funciones %TODO completar
...
\section{Tipos de convergencia (sucesiones de funciones)}
Vamos a trabajar siempre con sucesiones de funciones $(f_n)$ medibles.

% Parecía que íbamos a dar definiciones de cada una
\subsection{Uniforme}
\subsection{Puntual}
\subsection{en casi todo punto}
\subsection{en $L^1$}


\begin{example}
\begin{itemize}
\item $f_n = \frac{1}{n}1_{[0,n]}$.

\textbf{Límite puntual} es 0.
\item $f_n = 1_{[n,n+1]}$

\textbf{Límite puntual} también es 0. Cuando vamos aumentando el $n$, la parte distinta de 0 se va escapando hacia el infinito.
\item $f_n = n 1_{[0,\frac{1}{n}]}$

\textbf{Límite puntual: } Tenemos que distinguir, ya que $x=0,x≠0$ son casos distintos. $0$ pertenece a todos los intervalos, $∀n$, con lo que $$\lim_{n\to\inf}f_n(0) = \infty$$ pero, $$\lim_{n\to\infty}f_n(x) = 0 \dimplies x≠0$$

Ayuda mucho ver el dibujo de esta sucesión, que son rectángulos cada uno el doble de alto y la mitad de fino que el anterior, todos empezado en $0$.

El límite puntual entonces se escribe: $$\lim_{n\to\infty}f_n(x) =\left\{\begin{array}{cc}f(x) = \infty & x = 0\\f(x) = 0 & x≠0 \end{array}\right.$$
\item $f_n = 1_{[j2^{-k},(j+1)2^{-k}]}, 0≤j≤2^k, n=j+2^k, k = 0,1,2,...$

Para $k=2$, tenemos el intervalo dividido en cuartos, tomando $k=3$, tenemos dividido en octavos. En estas divisiones, la función vale 1 en una de esas divisiones y 0 en todas las demás.


\textbf{Límite puntual: } No existe el límite, ya que tiene infinitos $1$ en sitios demasiado dispersos como para poder aplicar la definición. Una forma de escribirlo sería $1,10,1000,01000000,...$ que representa de una manera más fácil la situación.
...
Entre los ejemplos, encontramos una sucesión de funciones que converge uniformemente y no converge en $L^1$. Vamos a verlo con el primer ejemplo

$f_n \to 0$ uniformemente.

$$\left.\begin{array}{c}\int |f_n| = 1\\\int 0 = 0\end{array}\right\}\implies f_n \text{ no converge} $$


Otro ejemplo interesante es ver que la cuarta sucesión converge a $0$ en $L^1$, ya que $\int \lim_{n\to\infty} f_n(x) = \int 0 = 0$ y por otro lado $\lim_{n\to\infty} \int f_n(x) < \lim_{n\to\infty} 2^-n$.

Tenemos que $f_n$ no converge a 0 a.e. (een casi todo punto) pero me he perdido el porque.

Recordando el teorema de la convergencia dominada, que es ampliable ya que la condición de convergencia a.e. es más débil, es decir, que la hipótesis de que la sucesión converja a.e. puede ser sustituida por convergencia de otro tipo y el teorema sigue siendo cierto.


Estos ejemplos son interesantes y dan bastante juego viendo más casos de convergencias de un tipo que sí son o no del otro, pero no he podido captar todos.
\end{example}

\subsection{Convergencia en medida}
\begin{defn}[Convergencia\IS en medida]
$f_n \to f$ en medida si $$∀ε>0, \lim_n µ\left(\{x∈X, |f_n(x) - f(x) | < ε \}\right) = 0$$.
\end{defn}

\begin{defn}[Sucesión\IS de Cauchy en medida]
$$∀ε>0,∃N ∀n,m≤N \tq µ\left( \{|f_n(x) - f_m(x)| > ε\}\right)$$
\end{defn}


Vamos a ver con los ejemplos anteriores las convergencias en medida de cada caso.

En el primero, hay convergencia en medida, porque $µ(|f_n(x)| > ε) =0$ si $n>\frac{1}{ε}$.

En el segundo ejemplo, $µ(|f_n(x)|>ε) = 1$, si $ε<1$. Siempre hay un intervalo en el que la función vale 1 y $1>ε$, con lo que no hay convergencia en medida.

En el tercer ejemplo, $µ(|f_n(x)| > ε)< \frac{1}{n}$, entonces, tomando límites tenemos demostrada la convergencia en medida.

En el cuarto ejemplo, $µ(|f_n(x)| > ε)< \frac{1}{2^k}$. Estamos en un caso muy parecido al anterior. Basta tomar límites para demostrar la convergencia en medida.


Vamos a ver las relaciones entre convergencia en medida y el resto de las ya vistas.

\begin{prop}
$f_n \to f$ en $L^1 \implies f_n \to f$ en medida.
\end{prop}
\begin{proof}
Sea $E_{ε,n} = \{|f_n - f| > ε\} = E$.

$$\int|f_n - f| ≥ \int_{E} |f_n - f| ≥ ε·µ(E)$$

$$µ(E) ≤ \frac{1}{ε} \int|f_n-n| \implies µ(E) \convs 0$$
...
1) Si $f_n$ converge en medida, existe una subsucesión que converge en a.e.

2) convergencia $L^1$ implica en medida implica existe subsucesión, con lo que $L^1$ implica que existe una subsucesión.
\begin{theorem} %debería ser el 34.
$(f_n)$ es de Cauchy en medida. Entonces $∃f$ medible tal que $f_n \to f$ en medida.

Además, $∃$ una subsucesión $(f_{n_k})$ que converge a.e. (en casi todo punto).

Si $g$ es otra función tal que $f_n \to g$ en medida, entonces $f=g$ a.e.

\end{theorem}
\begin{proof}
No vamos a demostrarlo, simplemente comentamos que a través de una sucesión de Cauchy en medida, yo puedo escribir una subsucesión de Cauchy en casi todo punto. Al ser una sucesión de número reales converge y por lo tanto se puede definir el límite en casi todo punto de la subsucesión.
\end{proof}

\begin{corol} %debería ser el 35.
$f_n \to f$ en $L^1$ entontes existe una subsucesión $(f_{n_k})\to f$ a.e.
\end{corol}

\begin{proof}
= (Teorema) + ($L^1\implies$ medida.)
\end{proof}

\begin{theorem}[Teorema\IS de Egoroff]
$µ(X) < ∞$ y $f_n\to f$ a.e., entonces $∀ε> 0, ∃E\subset X, µ(E) < ε$ tal que $f_n \to f$ uniformemente en $X\backslash E$

\end{theorem}

\chapter{Medidas en espacios productos}

Dados dos espacios de medida $(X,\algbM, µ), (Y, \algb{N}, \upsilon$), buscamos una medida $\pi$ en $\algbM \otimes \algb{N}$ de forma que
\[\forall M \in \algbM, N \in \algb{N}, \pi(M \times N) = µ(M) \upsilon(N)\]

\obs Las uniones finitas de rectángulos $M \times N, \ M \in \algbM, N \in \algb{N}$ forman un álgebra.

$\pi$ está definida de forma aditiva sobre esta álgebra en una premedida.

Su extensión a la $\salgb$ generada por $\algbA$ es una medida.

Si µ, $\upsilon$ son $\sfin$s esta extensión es única

\section{Integrales sobre funciones en el espacio producto}
\begin{defn}[Sección]
Si tenemos $E \subset X \times Y$ definimos las secciones:
\[E_x = \{y \in Y \tq (x,y) \in E\}\subset Y \ x \in X\]
\[E^y = \{x \in X \tq (x,y( \in E\}\subset X \ y \in Y\]
\end{defn}

Dada una función $\appl{f}{X \times Y}{(\overline{\real} \ | \ \cplex)}$, definimos:
\[\appl{f_x}{Y}{(\overline{\real} \ | \ \cplex)} \tq f_x(y)=f(x,y)\ \forall x\in X\]
\[\appl{f^y}{X}{(\overline{\real} \ | \ \cplex)} \tq f_y(x)=f(x,y)\ \forall y\in Y\]

Veamos ahora una serie de propiedades al estilo \textbf{Patricio}
\begin{prop}
Si $E \in \algbM \otimes \algb{N}$ entonces
\[\forall x \in X \ E_x \in \algb{N} \text{ y }  \forall y \in Y \ E^y \in \algbM\]
\end{prop}

\begin{prop}
Si $f$ es $\algbM\otimes\algb{N}$-medible, entonces
\[\forall x \in X \ f_x \text{ es } \algb{N}-medible\]
\[\forall y \in Y \ f^y \text{ es } \algbM-medible\]
\end{prop}

Con estas propiedades podemos escribir un resultado que constituye una primera aproximación al Teorema de Fubini

\begin{theorem}
Dados dos espacios de medida $(X,\algbM, µ), (Y, \algb{N}, \upsilon$) $\sfin$, y sea $E \in \algbM \otimes \algb{N}$ entonces las funciones $\appl{}{x}{\upsilon(E_x)}$ y $\appl{}{y}{µ(E^y)}$ son medibles y, además
\[(µ\times \upsilon) (E) = \int \upsilon(E_x) dµ(x) = \int µ(E^y)d\upsilon(y)\]

Básicamente este resultado nos dice que el área del conjunto $E$ es la integral de las longitudes de las secciones (\textit{Principio de Cavallieri})
\end{theorem}
\begin{proof}
Símplemente vamos a indicar los pasos a segir, dejando para el lector el trabajo de seguirlos
\begin{enumerate}
\item \textbf{Espacios finitos}
\begin{enumerate}
\item Los rectángulos $E$ verifican la propiedad
\item Las uniones finitas de rectángulos disjuntos verifican la propiedad
\item Las uniones numerables crecientes de conjuntos que verifican la propiedad, también verifican la propiedad (se utiliza el teorema de la convergencia monótona: TCM).
\item Las intersecciones decrecientes de conjuntos que verifican la propiedad, también verifican la propiedad (se utiliza el teorema de la convergencia dominada: TCD).
\item Los conjuntos $E$ que verifican la propiedad constituyen una clase monótona que contiene a las uniones finitas disjuntas de rectángulos. Por tanto contiene a $\algbM \otimes \algb{N}$
\end{enumerate}
\item \textbf{Espacios de medida $\sfin$}

Extendemos el resultado de espacios finitos
\end{enumerate}
\end{proof}

Ahora ya estamos en condiciones de escribir el teorema de Fubini completo. Dice así:
\begin{theorem}[Teorema\IS de Fubini-Fonelli]
Dados dos espacios de medida $\sfin$ $(X,\algbM, µ), (Y, \algb{N}, \upsilon)$:
\begin{enumerate}
\item \textbf{[Tonelli]}
\[f \in L^+(X \times Y) \implies g(x)=\int f_x(y)d\upsilon(y), \ h(y)=\int f^y(x)dµ(x)\]
\[ \text{ están en } L⁺(X) \text{ y } L^-(Y) \text{ respectivamente}\]
y además
\[\int f d(µ\times \upsilon) = \int g(x) dµ(x) = \int h(y)d\upsilon(y)\]

\item \textbf{[Fubini]}
\[f \in L^1(µ \times \upsilon) \implies f_x \in L^1(\upsilon) \text{ a.e. }x \text{ y } f^y \in L^1(µ)\text{ a.e. }y\]
por tanto, las funciones $g,h$ están bien definidas en casi todo punto y $g \in L^1(µ)$ y $h \in L^1(\upsilon)$ y se verifica la última propiedad de la parte de Tonelli.
\end{enumerate}
\end{theorem}
