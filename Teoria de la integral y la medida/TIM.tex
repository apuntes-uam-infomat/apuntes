...
\begin{example}
Supongamos que $\Omega$ es un espacio de probabilidad de finito, es decir: $(\Ometa, \algbP(\Omega), P)$
...
\[= \sum_{i=1}^n a_i \sum_{j=1}^{\infty}\ind_{E_i \cap A_j} = \sum_{j=1}^\infty}\sum_{i=1}^n a_i \ind_{E_i \cap A_j} = \sum_{j=1}^{\infty} \int \Phi \ind_{A_j} = \sum_{j=1}^{\infty} \upsilon(A_j)\]
\end{proof}
