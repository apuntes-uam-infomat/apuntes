...
\begin{theorem}[Teorema de la integral monótona]
...
%TODO cambiar todas las flechas por flechas inclinadas hacia arriba (continuidad creciente)
\end{proof}
...
Obviamente la desigualdad se da en cada punto de $f_j$ y por tanto, podemos integrar a ambos lados obteniendo:
...
\section{Extensión de la integral a $\appl{f}{X}{\overline{R}}$ medible}
Para esta sección vamos a definir:
\[\int f = \int f⁺ - \int f^-\]
siempre que al menos una de las dos últimas integrales sea finita.
...
\begin{defn}[Integrable Lebesgue]
Diremos que $f$ es integrable Lebesgue si $\int f^+ < \infty \text{ y } \int f^- < \infty$ es decir:
\[f \text{ es integrable } \iff \int |f| < \infty\]
\end{defn}

\begin{prop}
El conjutno de las funciones integrables forma un espacio vectorial y la integral es funcional lineal en ese espacio.
\end{prop}
\begin{proof}
%TODO completar esto
Debemos ver que:
\[\forall α , \beta \int \real \ |αf + \beta g| \leq |α||f| + |\beta||g|\]
...
\section{Extensión de la integral a funciones $\appl{f}{X}{\cplex}$}
...
Con este resultado hemos probado que la integral de una función no depende de los valores que tome la función en un conjunto de puntos de medida 0.
