...
\item $\algb{M}$ es cerrado por la unión
\item $\algb{M}$ es cerrado por complementación
\end{enumerate}
\end{defn}
...
Lo que realmente nos interesa es la definición de $\salgb$

\begin{defn}[.$\salgb$]
Se dice que $\algb{M}$ es una $\salgb$ de conjuntos si:
\begin{enumerate}
\item $\algb{M}$ es un álgebra de conjuntos
\item $A_1,A_2,...,A_n \in \algb{M} \Rightarrow \bigcup_{n=1}^{\infty}\in \algb{M}$
\end{enumerate}
Es decir, tenemos un álgebra de conjuntos cerrada para uniones infinitas
\end{defn}

De esta definición se deduce que la intersección numerable de conjuntos queda dentro de la $\salgb$
...
\[\algb{E}=\lbrace A \subset X \tq A \ numerable \ o \ A^c \ numerable \rbrace\]
...
\[\algb{F}=\lbrace B \subset X \tq B \ finito \ o \ B^c \ finito \rbrace\]
...
\item $\Leftarrow$:
Dados $A_1,A_2... \in \algb{M}$ construimos una sucesión de conjuntos: 

$B_1=A_1, \ B_2 = A_2\setminus A_1, \ B_3=A_3\setminus (A_1 \bigcap A_2)...$.
...
\item Si $\algb{M}$ es una $\salgb$ y $E \subset \algb{M} \Rightarrow \algb{M}(E) \subset \algb{M}$
\end{itemize}
...
De la construcción de la medida de Lebesgue en un intervalo, podemos extraer que todos los conjuntos que se contienen en la $\salgb$ de Borel son medibles. 

\obs Otros conjuntos que están en $\algb{B}_X$ son: los cerrados, la intersección numerable de abiertos: $G_\delta$, la unión numerable de cerrados: $F_\sigma$, unión numerable de $G_\delta$ y la intersección numerable de $F_\sigma$.
...
\begin{proof}
Supongamos que $\rac$ es una $G_\delta$, entonces $\exists(A^k)$ sucesión de abiertos densos tales que $A^{k+1} \subset A^k$ y $\bigcap A^k=\rac$.
...
Dada $\{X_α\}_{α\in A}$, una colección de conjuntos no vacíos, consideramos el conjunto $\{\algb{M}_α\}_{α\in A}$, siendo $\algb{M}_α$ una $\salgb$ de $X_α$.

Sea $X=\prod_{α\in A}X_α=\{\rho: A \rightarrow \bigcup_{α\in A} X_α : \rho(α)\in X_α\}$.
...
%TODO apañar esto
Esta definición de producto cartesiano representa lo que queremos pero tiene el problema de que una función se define como un subconjunto de un producto cartesiano, lo que implica recursividad infinita. Para resolver este problema se define el producto cartesiano de dos elementos como es habitual. PAra más de dos conjutnos es totalmente válida esta definición

\begin{defn}[.$\salgb$ producto: $\otimes_{α\in A}\algb{M}_α$]
Se define la $\salgb$ producto como la $\salgb$ generada por:
\[\varepsilon_1 = \{\pi_α^{-1}(E):E \in \algb{M}_α, α\in A\}\]
donde $\pi: X \rightarrow X_α$
\end{defn}

\begin{prop}
Con la notación anterior, si $A$ es numerable, entonces $\otimes_{α\in A}\algb{M}_α$ está generada por:
\[\varepsilon_2 = \{\prod_{α\in A}E_α: \forall α \in A, E_α \in \algb{M}_α\}\]
La necesidad de que $A$ sea numerable viene de que ya vimos que las $\salgb a$ son cerradas por uniones e intersecciones numerables.
...
\begin{itemize}
\item $\subset$
Todo elemento de $\varepsilon_1$ está en $ε_2$ ya que:
\[\pi^{1}(E_n)=\prod_{k \in \nat}E_k : \forall k\neq n, E_k=X_k\]
\[\algb{M}(ε_1)=\otimes_{α \in A}\algb{M}_α \subset \algb{M}(e_2)\]
\item $\supset$
Todo elemento de $ε_2$ se genera con elementos de $ε_1$:
\[\prod_{α\in A}E_α = \bigcap_{α\in A} \pi^{-1}(E_α)\]
\end{itemize}
\end{proof}

\begin{prop}
Con la notación anterior, si $\algb{M}_α$ es la $\salgb$ generada por un conjunto $e_α$, es decir: $\algb{M}_α = \algb{M}(ε_α)$ entonces:
\[\algb{F}_1= \{\pi^{-1}_α(E_α): α \in A, E_α \in ε_α\}\]
genera $\otimes_{α\in A}\algb{M}_α$
\end{prop}
\begin{proof}
\begin{itemize}
\item $\subset$
Esta inclusión es trivial
$αlgb{F}_1 \subset e_1 \Rightarrow \algb{M}(\algb{F}_1 \subset \otimes_{α\in A}\algb{M}_α$
\item $\supset$
Observemos que:
\[\{E \subset X_α: \pi^{-1}_α(E)\in \algb{M}(\algb{F}_1)\}\]
es una $\salgb$, (lo que probaremos al hacer el ejercicio 4 de la hoja 2) que contiene a $ε_α$, por ser el conjunto de generadores, y por tanto contiene a $\algb{M}_α$ puesto que contiene a todos los generadores.

Entonces
\[\forall E  \in \algb{M}_α, \pi^{-1}_α(E) \in \algb{M}(\algb{F}_1)\]
es decir, $ε_1 \subset \algb{M}(\algb{F}_1)$ y por tanto:
\[\algb{M}(ε_1)=\otimes_{α\in A}\algb{M}_α \subset \algb{M}(\algb{F}_1)\]
\end{itemize}
\end{proof}

\begin{corol}
Si $A$ es numerable y cada $\algb{M}_α$ está generada por $ε_α$ entonces $\otimes_{α\in A}\algb{M}_α$ está generado por:
\[\algb{F}_1 = \{\prod_{α\in A}E_α, E_α \in ε_α\}\]
\end{corol}
