...
%TODO Así queda horrible. Cuando tengas pensado como escribir tu pregunta en rojo y una respuesta en azul lo cambias, que eso queda medio elegante.
%TODO Al cambiarlo revisa el resto del documentos para que los newpage no descojonen todo.
...
\textcolor{red}{Parra: por tocar los culons. Abuso de notación, al inicio de la sección nos referíamos a $\algb{M}$ como subconjunto que es una $\salgb$, y ahora usamos $\algb{M}(E)$ como el minimo subconjunto que es una $\salgb$ y que contiene al subconjunto E.  }
...
\textcolor{yellow}{Aquí falta algo}
...
\begin{defn}[Continuidad por la derecha]
...
\begin{defn}[Medible en un subconjunto]
...
y es la mínima $\salgb$ de X que hace que todas las $f_α$ sean medibles.
...
\item Si $X= \prod_{α \in A} Y_α$ y $\appl{f}{X}{(Y, \algb{N})}$, f es ($\otimes \algb{N}_α, \algb{N}$)-medible $\iff \forall α \in A, \appl{\pi f}{X}{Y_α}$ es medible.
...
\begin{proof}
Consideramos la función $\appl{F}{X}{\cplex \times \cplex}$ tal que $F(x)=(f(x), g(x))$ que es medible puesto que sus componentes son medibles ($π_1 · F = f$, $π_2 · F = g$).
...
\item Si $forall j, \appl{f_j}{X}{\overline{\real}}$ es medible, entonces osn medibles:
\begin{enumerate}
...
\[\iff \exists j \tq x \in f^{-1}((a, \infty]) \iff x \in \bigcup_j f^{-1}_j((a, \infty])\]
Por tanto 
...
\[sgn(f)=\] %TODO completar. Función a trozos que asocia el signo
\end{defn}
...
\begin{proof}
La función signo es emdible ya que es continua fuera del 0. Por tanto, la imagen inversa de un abierto que no contiene el 0 es un abierto medible y si contiene el 0, la imagen inversa  es un abierto $\cup$ $\{0\}$ que también es medible.
\end{proof}

\item La función característica de un conjunto es medible si y sólo si el conjunto es medible.

\begin{defn}[Función simple]
Una función $s(x)$ es simple si es una combinación linea finita de funciones indicatrices de conjuntos medibles (no admitimos valores infinitos).
...
\item $\Rightarrow$ Sea $\{a_0, a_1, \cdots a_n\}$ el conjunto de valores que toma la función $s(x)$, entonces
...
%TODO limite de fi sub n es f de x y el moduflo al cuadrado de fi su n es la suma de lso cuadrados de las otras dos y es creciente
\end{proof}
\end{enumerate}

\item Sea $(x, \algbM, µ)$ un espacio de medida completo:
\begin{enumerate}
\item $f$ medible y $f=g$ para casi todo (a.e.) µ, entonces $g$ es medible
\begin{proof}
Como no podía ser de otra forma, para comprobar esto necesitamos ver que la imagen inversa de un boreliano es medible (Se que es obvio y todos los estábamos pensando ya, pero hay que escribir hasta las trivialidades).
...
Ahora nos consideramos que $f$ es simple (combinación lineal finita de funciones características).
