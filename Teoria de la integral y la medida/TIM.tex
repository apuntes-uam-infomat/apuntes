...
una función f:$[ a,b] \rightarrow \textbf{R}$ acotada es integrable Riemann ($f \in R([a,b])$) si el supremo y el ínfimo anteriores coinciden, en cuyo caso se escribe:
$J(f) = \int_a^b f = \underline{J}(f) = \overline{J}(f) = \int_a^b f(x) dx$
\end{defn}

\subsection{Propiedades}
$f,g \in R([a,b]), \alpha \in \textbf{R}$
\begin{enumerate}
...
\[
 1_{\textbf{Q}  \cap  [0,1]} (x) =
  \begin{cases}
   1 &  x \in \textbf{Q} \cap [0,1] \\
   0       &  x \in [0,1] \backslash \textbf{Q}
  \end{cases}
\]
...
\[\forall P, \overline{J_P}(1)_{\textbf{Q} \cap [0,1]} = 1\]
\[\forall P, \underline{J_P}(1)_{\textbf{Q} \cap [0,1]} = 0\]
