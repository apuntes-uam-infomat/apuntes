...
%
% Propiedad 4
\item \[m_*(\bigcup_{n=1}^{\infty}K_n) \geq m_*(\bigcup_{n=1}^{N}K_n)= m(\sum_{n=1}^{N}K_n) \geq \sum_{n=1}^{\infty}m(K_n)-\epsilon = m^*(\bigcup_{n=1}^{\infty}K_n) - \epsilon\]
...
De donde podemos deducir:
\[m_*(C) = m^*(C) \Rightarrow C \ es \ medible\]
\end{proof}

Vamos a ver ahora el buen comportamiento que los conjuntos medibles tienen por medio de la unión y la intersección, tanto finitas como numerables. 
...
Ya sabemos que la unión numerable de medibles disjuntos es medible. Vamos a extenderlo ahora para una unión cualquiera de meidibles:

\newpage
\begin{prop}
\[\forall n \in \mathbb{N}, C_n \subset I \ es \ medibles \Rightarrow \bigcup_{n=0}^{\infty} C_n, \bigcap_{n=0}^{\infty} C_n  son medibles\]
\end{prop}
\begin{proof}
Si consigo escribir esta unión numerable como unión disjunta de medibles, ya lo tendríamos hecho.

Para conseguir esto tomo el primer conjunto; del segundo, lo que no está en el primero; del tercero, lo que no está en el primero ni el segundo.
Es decir, le resto a cada conjunto la unión de los anteriores. Esta unión, por ser finita será numerable y por tanto lo será la diferencia.

Así, acabamos obteniendo una unión disjunta de numerables, que ya sabemos que es numerable, y que representa exactamente el mismo conjunto.
\end{proof}

\begin{prop}[Carathedony]
...
Obvio tomando $C=(a,b)$
%TODO Jorge, completa con lo que has preguntado aquí
...
Dado un conjunto $M \subset \mathbb{R}$, M es medible sii $\forall n \in \mathbb{Z} M \cup(n, n+1)$ es medible en (n, n+1) y entonces:
\[m(M)= \sum_{n \in \mathbb{Z}}m(M \cup (n, n+1))\]

El siguiente paso sería demostrar que esto es cierto para cualquier división de M en intervalos
