...
Sobre estas integrales (de Lebesque) podremos aplicar el cálculo de Lebesque para obtener el valor numérico de la integral.
...
Como ya vimos en el ejercicio 5 de la hoja 2 %TODO comprobar esto
$\algb{N}^{-1}=\{f^{-1}(E) \tq E \in \algbM\}$ es una $\salgb$
...
\item  Supongamos que la $\salgb$ $\algbM$ está fromada por un conjunto de elementos: $\algbM = \algbM(ε)$, entonces
...
\begin{defn}[Medible Borel]
 Si $\appl{f}{(X, \algbM)}{\real | \cplex}$ diremos que es medible si es medible Borel, es decir, es ($\algbM, \algbB_{\real}$)-medible ó ($\algbM, \algbB_{\cplex}$)-medible
\end{defn}

\begin{defn}[Medible Lebesque]
Si $\appl{f}{\real}{\real | \cplex}$ diremos que es medible Borel si es medible ($\algbB_{\real}, \algbB_{\real}$) ó ($\algbB_{\real},\algbB_{\cplex}$) y diremos que es medible Lebesque si es medible
...
Sea $\{(Y_α, \algb{N}_α)\}_{α \in A}$ una colección de espacios medibles tales que $\forall α \in A, \appl{f_α}{X}{Y_α}$, la $\salgb$
...
Si $X$ es el producto cartesiano de las $Y_α$ ($X= \prod Y_α$) y $\pi_α$ la proyección: $\appl{\pi_α}{X}{Y_α}$ entonces la $\salgb$ producto $cosoraroporducto \algb{N}_α$ es la misma que la $\salgb$ generada por $\{\pi_α\}_{α \in A}$

\item Si $X= \prod_{α \in A} Y_α$ y $\appl{f}{X}{(Y, \algb{N})}$, f es (cosorado producto Nalfa, N)-medible $\iff \forall α \in A, \appl{\pi f}{X}{Y_α}$ es medible.

\item Esta propiedad es más bien un corolario sobre los complejos.

Una función $\appl{f}{X}{\cplex}$ es medible si y solo si su parte real y su parte imaginaria son medibles.
...
\begin{proof}
Consideramos la función $\appl{F}{X}{\cplex \times \cplex}$ tal que $F(x)=(f(x), g(x))$ que es medible puesto que sus componentes son medibles.
...
%TODO JORGE, mira si apañas esto
\end{defn}
