...
Sea f:[a,b]\rightarrow \textbb{R} acotada definimos:
\begin{itemize}
\item \begin{denf}[Suma superior]
$\bar{J_p(f)}=\sum_{i=1}^[N]sup(f(x))|I_i|$
\end{defn}

\item \begin{denf}[Suma inferior]
$J_p(f)=\sum_{i=1}^[N]inf(f(x))|I_i|$
\end{defn}
\end{itemize}
...
Si P$\prec$Q entonces $bar{J_P(f)} < \bar{J_Q(f)}$  y $J_Q(f) < J_P(f)$.
...
Entonces $J_{P_1(f)} \leq J_P(f) \leq \bar{J_P(f)} \leq \bar{J_{P_2}$. Es decir, cualquier suma inferior de la función f es menor o igual que cualquier suma superior de dicha función
...
una función f:$\left[a,b\right[ \rightarrow \textbb{R}$ acotada es integrable Riemann si el supremo y el ínfimo anteriores coinciden, en cuyo caso se escribe:
\end{defn}

\subsection{Propiedades}
Dadas dos funciones f,g integrables Riemann y seal $\alpha \in \textbb{R}$
\begin{enumerate}
...
\begin{example}
Veamos un ejemplo de función acotada en $\left[ 0,1\right[$ que NO ES integrable Riemann.
...
Existe una sucesión de funciones $f_n$, tales que $f_n\in R(\lef[ 0, 1 \right[)$, creciente y uniformemente acotada que converge a $f(x)$ tal que:
