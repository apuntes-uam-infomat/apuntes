...
Vamos a estudiar, sucesivamente, cómo medir funciones:
\begin{enumerate}
...
\[s(x) = \ind_{[0, \frac{1}{2})} + \ind_{[\frac{21}{2}, 1]}\]
...
\begin{example}
Supongamos que $\Omega$ es un espacio de probabilidad de finito, es decir: $(\Omega, \algbP(\Omega), P)$
...
\[\int (\Phi + \Psi) = \sum_i \sum_j (a_i+b_j)\ind_{E_i \cap F_j} = \sum_i \sum_j (a_i \ind_{E_i \cap F_j} + b_j \ind_{E_i \cap F_j}) = \]
\[= \sum_i ( \sum_j (a_i \ind_{E_i \cap F_j}) +\sum_j (b_j \ind_{E_i \cap F_j}) = \sum_i a_i \ind_{E_i} + \sum_j b_j \ind_{F_j} = \int \Phi + \int \Psi\]
\end{proof}

De esta proposición se deduce fácilmente que
\[\forall c > 0 \ c\int \Phi = \int c \Phi\]
...
\begin{proof}
Sean $\Phi = \sum a_i \ind_{E_i}$ y $\Psi  = \sum b_j \ind_{F_j}$, tenemos
\[\int \Phi = \sum_i a_i \ind_{E_i} \leq \sum_j b_j \ind_{F_j} = \int \Psi\]
\end{proof}
...
\[\int f = \sup\{\int \Phi \tq \Phi \text{ es simple }, 0 \leq \Phi \leq f\}\]
\end{defn}
