...
Entonces
%TODO intercambiar A <->E
\[\algbM = \{E \subset X: \ \forall A \subset X \ µ^*(A)=µ^*(A \cap E) + µ^*(A \cap E^c)\}\]
es una $\salgb$ y $µ^*_{|\algbM}$ es una medida completa.
\end{theorem}
\begin{proof}
En primer lugar vamos a ver que $\algbM$ es cerrada por complementación.

Esto es bastante obvio ya que la expresión que define $\algbM$ es simétrica para $E$ y $E^c$.

Ahora vamos a ver que es cerrada por uniones finitas.
...
Por último tenemos que ver que $\algbM$ es cerrada por uniones numerables, comprobando así que es una $\salgb$.
...
Realmente aún nos faltaría ver que es numerablemente aditiva, pero esto puede demostrarse repitiendo los pasos anteriores considerando $E=B$.
Quedaría:
\[µ^*(B)=\sum_{j=0}^{\infty}µ^*(B \cap A_j) = \sum_{j=0}^{\infty} µ^*(A_j)\]

Ahora vamos a comprobar que la medida es completa. Vemos claramente que si $µ^*(A)=0$ entonces $A \in \algbM$
\[µ^*(E) \leq µ^*(E \cap A) + µ^*(E \cap A^c)\leq µ^*(E)\]
\end{proof}
%TODO estructurar en apartados la demostración. Él ha llegado hasta el sexto, todos del tirón.  Yo lo separaría en bloques.
