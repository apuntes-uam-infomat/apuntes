...
\begin{defn}[Delta\IS de Dirac] También llamada \index{Medida!puntual} medida puntual, déntico a los anteriores salvo que nuestra $f$ vale 1 en un único punto y 0 en todos los demás.
\end{defn}
...
\begin{defn}[Medida\IS de Lebesgue-Stieltjes] Sea $\algbM$ la σ-álgebra de los intervalos semiabiertos por la izquierda (de la forma $(a,b]$). Entonces definimos la medida de Lebesgue-Stieltjes como la aplicación $\appl{μ}{\algbM}{[0,∞]}$ tal que la medida del conjunto vacío es $0$, y la medida de los intervalos es \[ μ((a,b]) = b - a \]
\end{defn}

¿Qué diferencia hay entre la medida de Lebesgue y la de Lebesgue-Stieltjes? Pues no lo tengo muy claro, pero en una usamos intervalos cerrados\footnote{O quizás no.} y en otra semiabiertos, y en la de Lebesgue-Stieltjes hablamos explícitamente de los intervalos vacíos\footnote{Porque es lo que dice \href{http://math.stackexchange.com/questions/468545/why-is-the-lebesgue-stieltjes-measure-a-measure}{un tipo en math.sx} y parece que tiene lógica.}.
