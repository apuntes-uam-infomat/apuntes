...
Sabiendo que $F(b_{i_{k-1}}+\delta_{i_{k-1}})$=$2^{-1_N}\frac{\epsilon}{2}$ llegamos a:
...
Vamos a fijar ahora una función $F$, una medida $µ = µ_F$ y una $\salgb$ $\algbM = \algbM_F$; entonces, por construcción, para todo medible $E \subset \algbM$:
...
Esta parte es trivial puesto que el ínfimo de un conjunto de valores es, por definición, menor que todos los valores del conjunto
...
\[\forall E \subset \bigcup_{i=1}^{\infty}(a_i, b_i + \delta_i), \ \sum_{i=1}^{\infty}µ(a_i,b_i+\delta_i) \leq \sum_{i=1}^{\infty}µ((a_i, b_i+\delta_i]) = \sum_{i=1}^{\infty}(F(b_i+\delta_i)-F(a_i))\]
