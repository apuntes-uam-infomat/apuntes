...
\textcolor{blue}{Los $I_k$  son intervalos. En geneal nos estamos restringiendo siempre a trabajar en $\real$. Entonces si ya tengo $m^*(S)$ tengo ya un recubrimiento. Ahora busco un intervalo de longitud menor que $a-m^*(S)$ que es fácil de encontrar y lo uno a los intervalos que me permitían calcula $m^*(S)$. Ya tengo un recubrimiento de S con suma de longitudes menor que el ínfimo dado. Y puedes seguir con la demostración}

\textcolor{red}{Se puede decir, que ese ínfimo no se aplica sobre un O cualquiera, sino que es el ínfimo de todos los O posibles no?, quedaría más claro así?:\\
...
\item Basándonos en el ejemplo anterior podemos ver el caso particular de la \textbf{Medida de Cantor}.
Definios $\appl{f}{X}{[0, \infty]}$ con $f(x)=1 \forall x \in N$.

Definimos µ($E$) como en el apartado anterior. Entonces si $E$ es finito, $µ(E)$ = \#$E$ y si $E$ es infinito, $µ(E)$=$\infty$

\item \textbf{Medida puntual ó Delta de Dira}
...
\item Podemos extender el espacio de probabilidad simple anterior a uno mayor:
...
Dado ($X, \algb{M}, µ$), decimos que µ es finita si µ($X$)< $\infty$
\end{defn}

\begin{defn}[.$\sigma -finita$]
Dado ($X, \algb{M}, µ$), si $X=\bigcup_{n=1}^{\infty}E_n, \ E_n \algb{M}$ y $µ(E_n)< \infty$, se dice que µ es una $\sigma$-finita.
\end{defn}

\begin{defn}
Si $E\subset \algb{M}, \ E=\bigcup_{n=0}^{\infty} E_n, \ E_n \in \algb{M} \text{con} \ µ(E_n)< \infty$ se dice que $E$ es $\sigma$-finito.
\end{defn}

\begin{defn}
Si todo $E\in \algb{M}$ tiene un subconjunto $F\in \algb{M}$ tal que 0<µ($E$) < $\infty$, se dice que µ es semifinita
\end{defn}
\end{example}
...
%TODO completar esto para infinitos elementos y bla bla bla
\end{proof}
...
\begin{proof}
%TODO Idea de siempre. Pasar a una unión disjunta y ver que podemos tomar el límite ???????
...
Tenemos así:
\[\bigcup_{n=1}^{\infty}E_n=\sum{n=1}^{\infty}µ(D_n)=\lim_{n}
\sum{j=1}^{n}µ(D_j)=\lim_{n}\bigcup{j=1}^{n}D_j = \lim_{n}µ(E_n)\]
\end{proof}
...
µ(A)= su cardinal si es finita (si es finito ok y sino pos también)
Si tomamos $E_i=\{i, i+1, i+2,...\}$ su intersección es el vacío, por lo que la medida de la intersección sería 0 mientras que el límite de las medidas es infinito, puesto que todas son infinitas.
\end{example}
...
Sin pérdida de generalidad, suponemos que $µ(E_1 < \infty$
\textcolor{red}{pa home}
\end{defn}
