...
\ppart Comprobar que un modelo con $S_0 = 10$ y $S_1(ω_1)=S_1(ω_2)=12$ no satisface la ley de único precio.
...
\[S_0 = \frac{qS_1(ω_1) + (1-q)S_1(ω_2)}{1+r} = \frac{12}{1+r}\]

En este caso nos falta conocer el interés del activo de riesgo neutro, aunque lo podemos calcular suponiendo que el model satisfaciese la ley de único precio. En ese caso tendríamos:
\[10 = \frac{12}{1+r} \implies 10 + 10r = 12 \implies r = 0.2\]
es decir, el rendimiento de la cuenta bancaria sería del 20 \%.

Es evidente que este rendimiento no se aproxima, ni de lejos, a la realidad por lo que el valor $S_0$ que calculásemos una vez conocido $r$ no sería igual al valor actual del activo. Por tanto tendríamos dos precios para el mismo subyacente.
...
La relación entre $V_0$ y $V_1$ viene dada por la condición de martingala:
\[V_0 = \frac{qV_1(ω_1)+(1-q)V_1(ω_2)}{1+r}\]


En este caso concreto tenemos:
\[V_0 = \frac{10\cdot (1+r) - 12 \cdot q - (1-q)\cdot 8}{1+r} = 10-\frac{12q+(1-q)8}{1.01} = 10 - \frac{4q +8}{1.01}\]

Cualquier otra estrategia de este modelo consistirá en comprar o vender $a$ acciones del activo $S^0$ y $b$ acciones del activo $S^1$.

Para garantizar que la estrategia $\varphi$ es dominante debemos estudiar aquellas estrategias con precio inicial $V_0$, es decir:
\[a+10b = 10 - \frac{4q +8}{1.01} \implies 1.01a+10.1b-10.1=-4q-8 \implies q = \frac{2.1-1.01a-10.1b}{4}\]

Tal estrategia tendrá un precio final:
\[V_1(\varphi') = 1.01a + 4bq+8b\]
La estrategia $\varphi$ tendrá un precio final:
\[V_1(\varphi)=10.1 -4q-8\]

Para comparar ambos valores sustituimos la $q$ por su valor expresado en función de $a$ y $b$:
\[\begin{array}{l}
V_1(\varphi') = 1.01a+2.1b-1.01ab-10.1b^2 +8b = 1.01a+10.1b-1.01ab-10.1b^2 \\
V_1(\varphi) = 10.1-2.1+1.01a+10.1b - 8 = 1.01a + 10.1b
\end{array}\]
\[V_1(\varphi)>V_1(\varphi') \iff V_1(\varphi)-V_1(\varphi') > 0 \iff 1.01ab + 10.1b^2 > 0 \iff b(1.01a+10.1b) > 0\]
Empleando de nuevo la ecuación que relaciona la $q$ con los valores de $a$ y $b$ tenemos:
\[V_1(\varphi)>V_1(\varphi')  \iff b(-4q+2.1) > 0\]
puesto que $q$ es una probabilidad, sabemos que $q \in (0,1) \implies -4q+2.1>0$. Por tanto necesitamos que $b>0$
\end{problem}
