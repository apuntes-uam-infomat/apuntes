...
Puesto que estamos bajo la suposición de que nos movemos en una economía libre de arbitrajes, el precio de esta cartera debe coincidir con el precio actual del activo $C$ (la call).
...
\[a=e^{σ\sqrt{Δt}} = 1.08, \ b = \frac{1}{a} = 0.92, \ p = \frac{e^{r_cΔt}-b}{a-b}=0.76\]
...
\[V_T(CON) = 0.8461 \implies V_0(CON) = V_T(CON)e^{-r_cΔtn} = 0.649\]
...
Con estas condiciones tenemos
\[V_T(CON) = 11.3127 \implies V_0(CON) = V_T(CON)e^{-r_cΔtn} = 8.68\]
...
\[V(X) = K_1e^{-rT} + C(T, S_0, K_1) - C(T,S_0,K_2)\]
...
\[1 \text{ año } + 3 \text{ meses } + 1 \text{ día } = 1 + \frac{3}{12} + \frac{1}{360} = 1.253 \text{ años}\]
...
\[PV = 15\cdot e^{-0.1} + 35 \cdot e^{-0.2} + 47\cdot 1.1 \cdot e^{-0.3} + 55 \cdot 1.1 \cdot e^{-0.4} + 15 \cdot 1.1\cdot e^{-0.5} + 12 \cdot 1.1 \cdot e^{-0.6} = 138.33\]
Lo que nos da
\[NPV = -150 + 138.33 = -11.67\]

El NPV obtenido es mayor que el calculado en el apartado B). Por tanto, interesa el cambio en las ganancias, aunque la diferencia es muy pequeña.
\end{problem}
...
Por tanto sólo tenemos que evaluar los bonos como hemos visto en la sección anterior.
...
El valor será lo obtenido gracias al interés dado.
\[V_{\text{FRA}}(T_2) = N \cdot \left(e^{R_k\cdot (T_2-T_1)} \right)\]
Ahora descontamos este valor para traerlo al comienzo del intervalo
\[V_{\text{FRA}}(T_1) = N \cdot \left(e^{R_k\cdot (T_2-T_1)} \right) \cdot e^{-R_F (T_2-T_1)} = N \cdot e^{(R_k-R_f)\cdot (T_2-T_1)} \]
...
\begin{itemize}
\item La empresa $A$ paga cada intervalo un $7\%$ al banco, LIBOR a $B$ y recibe un $7.5\%$. El cómputo global es que ha conseguido el préstamo a LIBOR-50pb.
...
\[f_{t_0} = (F_{t_0}-F_0)e^{-r(T-t_0)} = S_{t_0}-F_0e^{-r(T-t_0)} = 22 - 22.10\cdot e^{-0.08\cdot 0.5} = 0.77 \]
