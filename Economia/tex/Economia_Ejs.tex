...
\addplot[domain=6:10,blue] {x-1};
\draw[dotted] (axis cs:6,5) -- node[left] {F} (axis cs:6,12);
\draw[dotted] (axis cs:-1,5) -- node[below] {F-K} (axis cs:11,5);
\end{axis}
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[ymin=2.5,ymax=5.5]
\addplot[domain=0:4,blue] {3};
\addplot[domain=4:6,blue] {x-1};
\addplot[domain=6:10,blue] {5};
\draw[dotted] (axis cs:4,7) -- (axis cs:4,2.5);
\draw[dotted] (axis cs:6,7) -- (axis cs:6,2.5);
\draw[dotted] (axis cs:-1,4) -- (axis cs:11,4);
\draw[dotted] (axis cs:-1,6) -- (axis cs:11,6);
\end{axis}
\end{tikzpicture}
...
%Con las fórmulas del apartado anterior, siendo en este caso $K=12$, $S_0=10$ tenemos:
%\[d_{\pm} = \frac{1}{0.27\sqrt{0.25}}\left(\ln \frac{10}{12e^{-0.05\cdot 0.25}}\pm\frac{1}{2}0.27^2\cdot 0.25 \right) \implies d_+ = -1.19, \;\; d_-=-1.325 \]
%lo que nos lleva a
%\[C_0(10,12,0.27,0.05,1) = 10 \cdot 0.62 - 10\cdot 0.95 \cdot 0.6 = 0.5\]

%Por tanto, con la venta de la opción descrita, obtenemos un capital de 500.
...
\begin{problem}[8]
Calcular el valor teórico de una acción que paga en 0 un dividendo del 3\% y  que aumentará su valor un 1.5\% anualmente considerando que el tipo libre de riesgo es del 4\%.
\solution

Al pagar un dividendo del $3\%$ el valor de la acción cae a un $97\%$ de su valor inicial y, a partir de ahí, comienza a subir según la situación descrita por el enunciado. Para estudiar su precio no tenemos más que descontar los valores de la acción en cada año:

\[PV = 0.97V_0\sum_{i=1}^{\infty}\frac{e^{0.015\cdot i}}{e^{0.04\cdot i}} 0.97V_0\sum_{i=1}^{\infty}e^{-0.025\cdot i} = \frac{0.97\cdot V_0}{1-e^{-0.025}} = 39.29\cdot V_0 \]
...
Al preguntar por el rendimiento esperado quiere decir cuál es el rendimiento que hace que el valor de producto neto ($NPV$) sea nulo, es decir, nos piden la TIR. Para ello tenemos que calcular el valor $r$ tal que:
\[PV = 20\cdot e^{-r} + 40 \cdot e^{-r \cdot 2} + 47 \cdot e^{-r \cdot 3} + 55 \cdot e^{-r \cdot 4} + 15 \cdot e^{-r \cdot 5} + 12 \cdot e^{-r \cdot 6} = 150\]
...
\begin{problem}[11]
Consideramos un bono que paga un cupón anual del 6\% compuesto semestralmente al que le quedan dos años de vida.
