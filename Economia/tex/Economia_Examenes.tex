...
En este momento tenemos una opción que paga $a^2S_0-S_0$ con probabilidad $p$ y $aS_0-abS_0$ con probabilidad $(1-p)$.
...
\varphi_0 e^{0.12\cdot 0.25}+\varphi_1 \cdot 50 = 12 \\
\end{array}\right. \implies \varphi_1 = \frac{10}{22} = 0.45, \;\; \varphi_0 = -10.41\]
...
\[B_{\text{fix}} = \frac{1.5}{1.0225^{\frac{2}{3}}} + \frac{1.5}{1.0225^{\frac{5}{3}}} + \frac{1.5}{1.0225^{\frac{8}{3}}} + \frac{1.5}{1.0225^{\frac{11}{3}}} + \frac{101.5}{1.0225^{\frac{14}{3}}} = 97.31395154\]

El tipo LIBOR publicado a tres meses es del 12\% con composición trimestral lo que significa que cada pago es de un 3\%.

\[B_{\text{fl}} = \frac{103}{1.0225^{\frac{2}{3}}} = 101.4834011\]

Así, para la parte que paga variable, el valor del swap el valor del mismo es:
\[V = B_{\text{fix}} - B_{\text{fl}} = -4169449.54 \text{ euros }\]
...
La empresa buscará cubrise respecto a los cambios en el precio de la acción para lo que recurrimos a las sensibilidades de la fórmula de la call Black-Scholes. Recordemos que
\[δ = \partial_S = N(d_+)\]
...
\begin{tikzpicture}
  ...
  \draw[->, thick] ($(A.north east)!0.25!(A.south east)$) -- ($(C.north west)!0.25!(C.south west)$) node[black, midway, above]{$L$};
  \draw[->, thick] ($(C.north west)!0.75!(C.south west)$) -- ($(A.north east)!0.75!(A.south east)$) node[black, midway, below]{$2.95 \%$};

  \draw[->, thick] ($(B.south west)!0.25!(B.north west)$) -- ($(C.south east)!0.25!(C.north east)$) node[black, midway, below]{$3.05 \%$};
  \draw[->, thick] ($(C.north east)!0.25!(C.south east)$) -- ($(B.north west)!0.25!(B.south west)$) node[black, midway, above]{$L$};
