...
\spart
%\[\sum_{n=0}^{\infty} \frac{\sin(nz)}{n^2} = \sum_{n=0}^{\infty}e^{nx}\frac{e^{iny}-e^{-iny}}{2^{n+i}}=\sum_{n=0}^{\infty}\left(\left( \frac{e^x}{2}\right)^n \left(\left(\frac{e^{ny}}{2}\right)^i-\left(\frac{1}{2e^{ny}}\right)^i\right)\right) \]
...
\[= \int_0^π \left(\cos(t)-i\sin(t)\right)\left(-\sin(t)+i\cos(t)\right)dt - \int_0^2 \left(1-t \right)^2dt=\]
\[=0-\frac{8}{3}-2+4=-\frac{2}{3}\]
...
Si en el ejercicio anterior calculamos la parte real del resultado obtenemos $-2/3$ mientras que si calculamos la integral de la parte real nos queda
\[\int_α |z|Re(z) dz\]
con lo que el primer sumando que hemos desarrollado cambiaría su valor, manteniéndose el segundo igual, puesto que no tiene parte imaginaria, obteniendo un resultado evidentemente distinto
...
\solution
%\[\left| \int_γ \frac{dz}{z^2+1}\right| = ... =\left|\int_0^{\frac{π}{4}}\frac{-2\sin(t)+2i\cos(t)}{2\cos^2(t)+i\sin(2t)} \right| = \left|\int_0^{\frac{π}{4}} \frac{re^{it}}{re^{2it}} \right| = \left|\int_0^{\frac{π}{4}} e^{-it} \right| =\]
%\[=\left|\int_0^{\frac{π}{4}} \cos(t}-i\sin(t)\right| = \left| \right|\]
...
\[=\frac{1}{i2^{2n}}\int_{|z|=1}\sum_{j=0}^{2n}{2n \choose j }z^{2n-2j-1}dz=\footnote{Los sumanos con exponente positivo dan como resultado 0 aplicando el Teorema de Cauchy}\frac{1}{i2^{2n}} \int_{|z|=1} \sum_{j=n+1}^{2n}{2n \choose j }z^{2n-2j-1}dz =\]
\[\frac{1}{i2^{2n}} \int_{|z|=1} \sum_{j=0}^{2n}{2n \choose j+n+1 }\frac{1}{z^{2j+1}}dz=\frac{1}{i2^{2n}} \sum_{j=0}^{2n}{2n \choose j+n+1 } \int_{|z|=1}\frac{1}{z^{2j+1}}dz=\]
\[=\frac{1}{i2^{2n}}2πi =\frac{π}{2^{2n-1}} \]
%\[=\frac{1}{i2^{2n}} \sum_{j=n+1}^{2n}{2n \choose j } \int_0^{2π}\frac{-\sin(t)+i\cos(t)}{(\cos(t)+i\sin(t))^{2j+1}}dt=\frac{1}{i2^{2n}} \sum_{j=n+1}^{2n}{2n \choose j }\frac{1}{-2j} (\cos(t)+i\sin(t))^{-2j}\]
...
\[|f'(z)| = \left| \frac{1}{2πi}\int_{\partial \mathbb{D}(z,r)}\frac{f(w)}{w-z}dw \right|\leq  \left| \frac{1}{2πi}\right| \left|\int_{\partial \mathbb{D}(z,r)}\frac{M}{w-z}dw\right|\leq\left|\int_{\partial \mathbb{D}(z,r)}\frac{M}{w-z}dw\right|=\]
\[=M \left|\int_{\partial \mathbb{D}(z,r)}\frac{dw}{w-z}\right|\]
...
\begin{problem}[6]
Śea α un número irracional y $q=e^{2πiα}$. Demuéstrese que las únicas soluciones holomorfas de la ecuación funcional $f(z)=f(qz)$ en la corona $Ω = \{\frac{1}{2}<|z|<\frac{3}{2}\}$ son las funciones constantes.
...
\begin{problem}[7]
Desmuestre que si $f$ es holomorfa en el disco unidad y $|f(z)|\leq 1-|z|$ allí, entonces $f=0$. ¿Puede una función holomorfa satisface $|f(z)| \geq 1/(1-|z|)$ para |z| < 1?
\solution
\end{problem}
