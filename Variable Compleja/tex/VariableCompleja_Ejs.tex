...
Podemos ver que uno de estos puntos se encuentra en el interior del disco unidad y otro fuera. Por tanto vemos que dependiento del valor de $t$ tendremos un resultado para la integral u otro.

Vamos a calcular los residuos correspondientes a estas singularidades, ambas polos de orden 1.

\[Res\left(f,\frac{1+\sqrt{3}}{2}\right)=\lim_{z \to \frac{1+\sqrt{3}}{2}}\left(z-\frac{1+\sqrt{3}}{2}\right)f(z)=\frac{1}{1-\frac{1-\sqrt{3}}{2}}\]
\[Res\left(f,\frac{1-\sqrt{3}}{2}\right)=\lim_{z \to \frac{1-\sqrt{3}}{2}}\left(z-\frac{1-\sqrt{3}}{2}\right)f(z)=\frac{1}{1-\frac{1+\sqrt{3}}{2}}\]

Ahora podemos ver que el valor de la integral, según el valor de $t$ será:
\[\left\{ \begin{aligned}
0 &\text{ si } \ t<\frac{1-\sqrt{3}}{2}\\
\frac{1}{1-\frac{1+\sqrt{3}}{2}} &\text{ si } \ \frac{1-\sqrt{3}}{2}\leq t < 1 \\
\frac{1}{1-\frac{1+\sqrt{3}}{2}} &\text{ si } \ 1 \leq t < \frac{1+\sqrt{3}}{2} \\
\frac{1}{1-\frac{1+\sqrt{3}}{2}} +\frac{1}{1-\frac{1-\sqrt{3}}{2}} &\text{ si } \ \frac{1+\sqrt{3}}{2} \leq t
\end{aligned}\right.\]
...
\[
Res\left(f,0\right) = \lim_{z\to 0} z\frac{1-z}{1-\cos(z)} = \lim_{z\to 0} \frac{1+2z}{\sin(z)} = \lim_{z\to 0}\frac{2}{\cos(z)} = 2 \implies z=0 \text{ polo de orden 2}
\]

Para calcular el índice, sabemos que al ser una circunferencia ($|z| = 1$) tiene índice=1.

\[\int_{\abs{z}=1} \frac{1 + z}{1 - \cos(z)} \dif z = 2πi · Res(f,0) · Ind(γ,0) = 4 π i\]
...
Puesto que la exponencial crece mucho más rápido que un poliniomio, es encillo ver que en el origen, esta función presenta una singularidad.

Puesto que se trata de una singularidad esencial, para calcular el residuo debemos buscar el desarrollo de la función somo serie de Laurent en torno al punto de singularidad (en este caso el origen).
...
Ahora que tenemos el desarrollo de Laurent en torno a $z_0=0$, el residuo de $z_0$ es el coeficiente correspondiente a la potencia de exponente $-1$, es decir $\frac{1}{(n+1)!}$ y este es el valor de la integral.
...
\[x²+2x+2 =0 \implies x = \frac{-2\pm \sqrt{4-8}}{2}=\frac{-2\pm 2i}{2}=1\pm i\]

Podemos ver que ambas son polos dobles y que sólo $i+1$ se contiene dentro de la región encerrada por $γ_R$ a partir de un cierto $R$.

Tenemos por tanto:
\[\int_{γ_R}\frac{z^2}{z^2+2z+2}\dif z=2πi Res(f,1+i)\]
\[Res(f,1+i)=\lim_{z \to 1+i}\left(\frac{z^2}{(z-1+i)^2}\right)'=\lim_{z\to 1+i}\left(\frac{2z(z-1+i)-z^22(z-1+i)}{(z-1+i)^4}\right)=\]
\[=\frac{2(1+i)2i+2i2(2i)}{(2i)^4}=\frac{4i-12}{16}=\frac{i}{4}-\frac{3}{4}\]

Y la integral queda:
\[\int_{γ_R}\frac{z^2}{(z^2+2z+2)^2}\dif z=2πi Res(f,1+i)=-\frac{π}{2}-\frac{3πi}{2}\]

Por otro lado, la integral sobre la semicircunferencia nos queda:
\[\int_{C_R}\frac{z^2}{(z^2+2z+2)^2}\dif z \leq \left|\int_{C_R}\frac{z^2}{(z^2+2z+2)^2}\dif z\right| \leq \int_{C_R}\frac{|z^2|}{|(z^2+2z+2)^2|}|\dif z|\leq \]
\[\leq \int_{C_R} \frac{R^2}{(R-\sqrt{2})^4}|\dif z|=2πR\frac{R^2}{(R-\sqrt{2})^4}\]
que tiende a 0 cuando $R\to \infty$

Por tanto tenemos
\[\int_{-\infty}^{\infty} \frac{x^2}{(x^2 + 2x + 2)^2} \dif x = -\frac{π}{2}-\frac{3πi}{2}\]

\textcolor{blue}{Me huele a que he metido la gamba en el cálculo de los residuos. Ya veré cómo soluciono esto.}
\end{problem}
...
La forma de resolver este ejercicio es similar a la del anterior. Realizaremos las cuentas sin las explicacines detalladas del por qué de nuestras acciones
...
Ámbas singularidades son simples y sólo una de ellas se contiene dentro de la region encerrada por $γ_R$ a partir de un cierto radio, la singularidad $z_0=1+i$.
...
Por tratarse de un polinomio complejo va a tener 4 raices complejas, teniendo en cuenta las posibles multiplicidades.
...
Vamos a estudiar el efecto de la aplicación sobre cada circunferencia de radio $r$

Podemos ver que
\[\left\{\begin{aligned}
f(r)&=\frac{r-i}{r+i}=\frac{r^2-1-2ir}{r^2+1}\\
f(ir)&=\frac{ri-i}{ri+i} = \frac{r-1}{r+1}\\
f(-r)&=\frac{r-i}{r+i}=\frac{-r^2+1+2ir}{r^2+1}
\end{aligned}\right.\]

Podemos ver que nos lleva primero a un punto de parte imaginaria negativa, luego a uno de parte imaginaria cero y luego a otro de parte imaginaria positiva. Podemos concluir que nos lleva a una banda vertical de altura infinita, pues la parte imaginaria de las imágenes no está acotada y de anchura: $[-1,1]$

\textcolor{blue}{Esto último es un triple sobre la bocina desde mi casa}

\spart

\textcolor{blue}{Este apartado puede deducirse del anterior ya que las cuentas serían totalmente equivalentes. No obstante, tirarme otro triple sobre otro triple es demasiado hasta para mi.}
...
Podemos ver que nos lleva al eje real recorrido en sentido habitual.
\end{problem}
