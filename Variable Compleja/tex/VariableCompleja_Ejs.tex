...
\spart
\doneby{Edu} \approvedby{Guille} (Tutoría)
...
Vamos a calcular los residuos correspondientes a estas singularidades, ambas polos de orden 1. Por comodidad y no escribir mucho, diremos que $\frac{1 + \sqrt{3}i}{2} = r_+$ y $\frac{1- \sqrt{3}i}{2} = r_-$. Calculamos el residuo para $r_+$:
\[ \Res(f, r_+) = \lim_{z\to r_+} (z - r_+) f(z) = \lim_{z\to r_+} \frac{(z - r_+)}{(z - r_+) · (z - r_-)} = \lim_{z\to r_+} \frac{1}{r_+ - r_-} \]

Análogamente, para $r_-$ tendremos que $\Res(f, r_-) = \frac{1}{r_- - r_+}$. Es decir, que los dos residuos son opuestos, y por lo tanto la integral de ahí vale $0$ para todo $t ≠ 1$.
...
Vamos a ello. Puesto que $f$ sólo tiene singularidades en $\pm i$, $i$ nunca se contiene en el dominio encerrado por $γ_R$, al aplicar el teorema de los residuos nos queda:
...
\begin{problem} ¿Cuántos ceros tiene la ecuación $e^z - 4 z^n + 1 = 0$ en el disco unidad?
\solution
...
\end{tikzpicture}
\caption{Transformación del apartado B.}
\label{figEj7B}
\end{figure}

Con la exponencial lo que hacemos es llevar la banda al medio disco (ver figura \ref{figEj7B}), de tal forma que le hemos quitado una de las fronteras y ya podemos hacer mejor la transformación.
