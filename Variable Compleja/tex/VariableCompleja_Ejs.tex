...
\[\log(f(z)) = \log|f(z)|+i\underbrace{\text{arg}}_{\text{una rama del argumento de f}}(f(z))\]

Vamos a definir esa rama del argumento. Para ello recordamos que una rama del argumento de $\appl{f}{Ω}{\cplex}$ es una función $\appl{g}{Ω}{\real}$ continua tal que
\[e^{ig(z)}=\frac{f(z)}{|f(z)|} \ \forall z \in Ω\]

En nuestro caso tendíamos
\[\frac{e^z+1}{|e^z+1|}=e^{ig(z)}\]
pero no sabemos cuál sería el dominio Ω, todavía.

Podemos ver que si en el dominio hay una curva α tal que su imagen por $f$ de una vuelta al origen tendremos problemas. Para evitar este caso, podemos considerar el dominio que queda al restringirnos a la banda horizontal $(-πi,πi)$.

Se trata de un dominio simplemente conexo lo que implica que existe la rama del logaritmo de $f$.

Ya vimos que si $F(z)$ es rama del logaritmo de $f$ se cmple que $e^{F(z)}=f(z)$ y derivando a ambos lados tenemos
