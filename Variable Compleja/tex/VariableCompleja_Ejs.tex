...
\ppart Demuestre que si $a$ es un número real mayor que un, entonces la serie converge uniformemente en $\{z \in \cplex \tq Re(z) \geq a\}$
...
\begin{problem}[20]
Supongamos que la serie de potencias $f(z)=\sum_{k=0}^{\infty}a_0z^k$ tiene un radio de convergencia $R=1$ y que $\sum_{k=0}^{\infty}a_k=0$. Denotemos
\[s_n(z)=\sum_{k=0}^n a_k z^k \text{  y  } s_n=\sum_{k=n}^na_n\]

\ppart Demuestre que $s_n(z)=(1-z)\sum_{k=0}^{n-1}s_kz^l+s_nz^n$ y concluya que $f(z)=(1-z)\sum_{n=0}^{\infty}s_nz^n$
...
\ppart Prueba que si $z \in \algb{D}$, entonces
...
\[\frac{1}{n}\sum_{j=1}^nP(ω_j)=a_+a_n\]
