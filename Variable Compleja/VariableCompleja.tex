...
\begin{defn}[Superficie de Riemann]
...
\begin{defn}[Rama de una función]
...
Sea $f(t)=u(t)+iv(t)$ una función continua $\appl{f}{[a,b]}{\cplex}$ definimos la \concept{integral} como:
...
\[\int_a^bf(t)dt=\left|\int_a^b f(t)dt \right| e^{iα}\]
multiplicando a ambos lados por $e^{-iα}$ tenemos:
\[\left| \int_a^bf(t)dt\right| = Re\left[ e^{-iα} \int_a^bf(t)dt\right] = Re\left[  \int_a^be^{-iα} f(t)dt\right] = \int_a^bRe\left[ e^{-iα} f(t)dt\right]\]
...
\begin{defn}[Curva de Jordan]
Una curva es una función continua $\appl{α}{[a,b]}{\cplex}$.

Si $α(a)=α(b)$ diremos que α es \concept{cerrada}.

si $α(t_1)=α(t_2)$ implica que $t_1=t_2$ para todo $t_1,t_2 \in (a,b)$. En caso de ocurrir esto diremos que α es \concept{simple}.
...
\[long(α \circ β) = \int_a^b |(α \circ β)'|ds = \int_a^b |β'(s)α'(β(s))|ds\]
%TODO ponemos la composición y hacemos cambio de variable y listo
\end{example}

Sea $\appl{α}{[a,b]}{\cplex}$ un camino y sea $\appl{f}{α([a,b])}{\cplex}$ una función continua, definimos la \concept{integral de línea}
...
\begin{defn}[Integral de línea respecto a la longitud de arco]
...
Para α rectificable y $f$ continua en $α([a,b])$ existe la \concept{integral de Riemann-Stieldjes}
