...
\[\{s_n\} \text{ es de Caucchy } \iff \forall δ > \ \exists N \tq \text{ si } n,n \geq N \implies |s_n-s_m| < ε\]
...
\[|s_m - s_n| = |\sum_{k=m+1}^m z_k| \leq \sum_{k=n+1} ^m | z_k| = |s_m'-s_n'| < ε\]
...
\[\iff \forall ε > 0 \forall z \in \Omega \exists N=N(ε,z) \tq \forall n \geq N |f(z_n)-f(z)| < ε \]
\end{defn}
...
\[f_n \rightarrow f \iff \forall ε >0 \ \exists N=N(ε) \tq \forall n \geq N \ |f_n(z)-f(z)| < ε \forall z \in \Omega\]
\end{defn}

\begin{prop}[Criterio de M de Weierstrass]
Sea $\appl{f_n}{\Omega}{\cplex}$ funciones tales que $|f_n(z)| \leq M_n$ para todo $z \in \Omega$, si $\sum M_n$ converge, entonces $\sum f_n$ conerge \textbf{abolsuta} y \textbf{uniformemente}.
\end{prop}
...
\[|g_m(z)-g_n(z) | = \left|\sum_{k=n+1}^m f_k(z)\right| \leq \sum_{k=n+1}^m | f_k(z)| \leq \sum_{k=n+1}^m M_k = |s_m-s_n| < ε\]
...
Dado ε > 0 $\exists N$ t.q. para $n \geq N$, $|g_n(z)-g_{n+k}(z)| < ε \ \forall z \in \Omega$.
