...
\begin{example}
Sean $z_0, w_0 \in \cplex$
\[\lim_{z\to z_0} f(z) = \infty \iff \lim_{z\to z_0}\frac{1}{f(z)}=0\]
\[\lim_{z\to \infty} f(z) = w_0 \iff \lim_{z\to 0}\frac{1}{f(z)}=w_0\]
...
Con la última propiedad hemos obtenido algo realmente nuevo, que no se cumple en las funciones de variable real. Veamos qué le estamos pidiendo a las funciones holomorfas que nos gartantiza esto y que no lo pedimos a las funciones de variable real.
...
\[\frac{df}{dx}=\frac{df}{dy}(x_0,y_0) = -i \frac{df}{dy} \text{ en } (x_0,y_0)\]
es decir
...
Igualando obtenemos las ecuaiones de Cauchy Riemann. Por tanto si existe f' existen las 4 derivadas parciales ocn las que hemos trabajado y verifican las ecuaciones de cauchy riemann
