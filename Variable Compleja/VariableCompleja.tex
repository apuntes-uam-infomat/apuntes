...
La última igualdad, evidentemente, se deriva del toerema de los residuos
...
Vamos a continuar ahora con el ejemplo. Para ello debemos determinar la región Ω que se encontrará en la parte positiva del eje $OY$ (ya que queríamos que la parte imaginaria fuese positiva). Por comoddida la consideraremos un semicírculo de radio $R$ donde $R$ será suficientemente grande como para contener todas las singularidades de la función.
...
y por el lema de Jordna\footnote{La demostración de este lema se deja como ejercicio para el lector por ser sencilla y de escasa importancia} tenemos que la última integral es menor que $π/R$.
...
Veamos algunos ejemplos para entender mejor el conecpto de conformidad:
...
\obs Toda transformación de Möbius es composición de estas transofrmación básicas
...
\begin{example}
Consideremos la transofmración de Möbius tal que
\begin{align*}
...
\[T(z)=[z,i,1,-1]=\frac{(z-1)(i+1)}{(z+1)(1-i)}=\frac{(z-1)}{(z+1)}\frac{(1+i)(-1-i)}{(i-1)(-i-1)}=\frac{z-1}{z+1}\frac{-(1+i)^2}{2}=i\frac{1-z}{1+z}\]
