...
\[\{s_n\} \text{ es de Cauchy} \iff \forall δ > \ \exists N \tq \text{ si } n,n \geq N \implies |s_n-s_m| < ε\]
\end{defn}
...
Sea $\appl{f_n}{\Omega}{\cplex}$ funciones tales que $|f_n(z)| \leq M_n$ para todo $z \in \Omega$, si $\sum M_n$ converge, entonces $\sum f_n$ conerge \textbf{absoluta} y \textbf{uniformemente}.
\end{prop}
...
Entonces, para todo $e \tq 0 < \rho < |ε|$ la serie converge absoluta y uniformemente en $\{z \tq |z-z_0| < \rho\}$
\end{prop}
\begin{proof}
...
Básicamente ese ε nos va a dar el tamaño de la bola centrada en $z_0$ que no permite garantizar la convergencia y, obviamente, estamos interesados en localizar el mayor ε. Tenemos dos posibilidades:
\begin{enumerate}
...
pero teníamos que $R > r$ lo que nos lleva a contradicción.
\item
...
\[a_{n+1} < \frac{|a_n|}{r^-}\]
desplazándonos un término a la izquierda en la sucesión ($n-1$) y multiplicando a ambos lados por $r^-$ obtenemos
...
%TODO completar cómo se justifica esto con el teorema anterior
\end{itemize}
...
Vamos a estudiar ahora cómo e relacionan los radios de convergencia para la suma y el producto mediante un ejemplo.
...
Sea $\appl{f}{\Omega \subset \cplex}{\cplex}$, decimos que $f$ es \textbf{anaĺitica} en $\Omega$ si para todod $z_0 \in \Omega$ existe una bola $B(z_0, r)=\{z: |z-z_0|<r\}$ donde $F$ coincide con una serie de potencias centrada en $z_0$, es decir
