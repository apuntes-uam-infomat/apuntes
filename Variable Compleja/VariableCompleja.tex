...
Si $α(a)=α(b)$ diremos que α es una \concept{curva\IS cerrada}.
\end{defn}
...
Una curva $\appl{α}{[a,b]}{\cplex}$ es \concept{diferenciable a trozos} si existe una partición de [a,b]:
...
La \concept{longitud de α} se define como:
...
Definimos
\[F(z)=\int_{α_z}f(z)dz =\int_{x_0}^xf(t+iy_0)dt+i \int_{y_0}^yf(x+it)dt\]
...
\[F(z)=\int_{\bar{α_z}}f(z)dz =i\int_{y_0}^yf(x_0+it)dt + \int_{x_0}^xf(t+iy)dt\]
...
\begin{defn}[Indice\IS de un punto con respecto a un camino cerrado]
...
\[f(z)=\frac{1}{2πi}\int_α \frac{f(w)}{w-z} dw\footnote{Consideramos α tal que sólo de una vuelta en torno a z de modo que el índice sea 1}\]
...
\begin{theorem}[Teorema de Cauchy\IS]
...
\begin{theorem}[Teorema de continuación única]
...
\[f'(z)=\frac{1}{2πi}\int_{\partial B(z,r)} \frac{f(w)}{(w-z)^2}dw \implies |f'(z)|=\frac{1}{2πi}\int_{\partial B(z,r)} \frac{|f(w)|}{|w-z|^2}|dw| \leq \frac{M}{2πr^2} \int_{\partial B(z,r)} |dz| = \]
\[= \frac{M}{2πr^2}2πr = \frac{M}{r} \to 0 \text{ cuando } r \to \infty\]

Por tanto tenemos que $f'(z)=0 \ \forall z \in \cplex \implies f$ constante.
\end{proof}


\begin{theorem}[Teorema de Loiuville (General)]
...
\[a_n = \frac{f^{(n)}(0)}{n!}=\frac{1}{2π}\int_{\partial B(0,r)} \frac{f(w)}{w^{n+1}}dw \text{ con } r>b\]

Calculando el valor absoluto a ambos lados de la igualdad tenemos:
\[|a_n| \leq \frac{1}{2πi}\int_{\partial B(0,r)} \frac{|f(w)|}{|w|^{n+1}}|dw| \leq \frac{1}{2πi} \int_{\partial B(0,r)} \frac{a|w|^n}{|w|^{n+1}}|dw| = \frac{a}{2πir^{n+1-m}}2πri =\frac{a}{r^{n-m}}\]
...
\begin{proof}
\[f(z)=\frac{1}{2π i} \int_{\partial B(a,r)}\frac{f(w)}{w-a}dw = \frac{1}{2πi} \int_0^{2π} \frac{f(a+r'e^{it})}{r'e^{it}}ir'e^{it}dt = \frac{1}{2πi}\int_0^{2π}f(a+r'e^{it}) dt = \]
...
\[|f(a)| \leq \frac{1}{2πi}\int_0^{2π} |f(a+r'e^{it})|dt \leq\footnote{Por la hipótesis de que a es un máximo de la función} |f(a)|\]

Si la integral está acotada superior e inferiormente por el mismo valor, es que ese es su valor. Por tanto
\[|f(a)| )= \frac{1}{2π}\int_0^{2π} |f(a+r'e^{it})|dt = \frac{1}{2π} \int_0^{2π}\left[ |f(a)|-|f(a+r'e^{it})|\right] dt = 0\]
...
\[\sum_j Ind_α(z_j)=\frac{1}{2πi}\int_α \frac{f'(z)}{f(z)}dw\]
...
\[\frac{1}{2πi}\int_α \frac{f'(z)}{f(z)} = Ind_α(z_1)+...+Ind_α(z_n)+\underbrace{\frac{1}{2πi}\int_α \frac{g'(z)}{g(z)}}_{0\footnote{por el teorema de Cauchy, pues el cociente es una función holomorfa en B}}\]

\end{proof}

\begin{obs}
\begin{itemize}
\item Si $Ind_α(z_j)$=0 ó 1 entonces $\frac{1}{2πi}\int_α  \frac{f'(z)}{f(z)} dz$= número de ceros contados con multiplicidad que encierra α.

\item La función $f$ manda α en un camino cerrado τ y
\[\frac{1}{2πi}\int_α  \frac{f'(z)}{f(z)} dz = \frac{1}{2πi}\int_τ \frac{1}{w}dw = Ind_τ(0)\]
...
\item Al aplicar el resultado a $f(z)=a$ con $a \in \cplex$
 ...
 \[\sum_j Ind_α(w_j)=\frac{f'(z)}{f(z)-a}dz\]
 y
 \[Ind_τ(a)=\sum_j Ind_α(w_j)\]

\item Si a,b están en la misma región determinada por τ $Ind_τ(z)=Ind_τ(b)$.

\item Si $v_j$ son las raíces de $f(z)=b$ se tiene que
\[\sum_j Ind_α(w_j)=\sum_j ind_α(v_j)\]
...
con
\[a_n=\frac{1}{2πi}\int_{|w-z_0|=ρ} \frac{f(w)}{(w-z_0)^{n+1}}dw \text{ con } r < ρ < R\]
\end{theorem}
\begin{proof}
...
\[f(z)=\frac{1}{2πi}\int_{\partial Ω_1} \frac{f(w)}{w-z}dw+\frac{1}{2π i}\int_{\partial Ω_2}\frac{f(w)}{w-z}dw\]
...
\[f(z)= \frac{1}{2πi}\int_{|w-z_0|=R'}\frac{f(w)}{w-z}dw-\frac{1}{2πi}\int_{|w-z_0|=r'}\frac{f(w)}{w-z}dw\]
...
Por otro lado
\[B(z)=-\frac{1}{2πi} \int_{\partial B(z_0, r')} \frac{f(w)}{w-z}dw = -\frac{1}{2πi} \int_{\partial B(z_0, r')} \frac{f(w)}{z-z_0-(w-z_0)}dw=\]
\[=-\frac{1}{2πi(z-z_0)} \int_{\partial B(z_0, r')} \frac{f(w)}{1-\frac{w-z}{z-z_0}}dw=-\frac{1}{2πi(z-z_0)} \int_{\partial B(z_0, r')} f(w) \sum_{n=0}^{\infty} \left(-\frac{w-z}{z-z_0} \right)^ndw =\]
\[=\sum_{n=0}^{\infty}\frac{1}{2πi}\int_{\partial B(z_0, r')} f(w)(w-z_0)^ndw\frac{1}{(z-z_0)^{n+1}}\]
...
\[Res(f(z),z_0)=\frac{1}{2πi}\int_{\partial B(z_0, ε)}f(w)dw \ \forall 0 < ε < r\]
\end{defn}

Veamos algunas propiedades:
\begin{enumerate}
\item Sea $0<ε_1<ε_2<r$ por el teorema de Cauchy
\[\int_{\partial B(z_0,ε_2)}f(w)dw-\int_{\partial B(z_0,ε_1}f(w)dw = 0\]
...
\[Res(f(z),z_0)=\frac{1}{(m-1)!}\frac{d^{(m-1)}}{dz}\left((z-z_0)^mf(z)\right)\]
\begin{proof}
Tomamos la función $g(z)=(z-z_0)^m f(z)$ que tiene singularidad evitable en $z_0$. Entonces tenemos que
\[\frac{1}{2πi}\int_{\partial B(z_0,ε)} f(z)dz = \frac{1}{2πi}\int_{\partial B(z_0,ε)} \frac{g(z)}{(z-z_0)^m} = \frac{1}{(m-1)!}g^{(m-1)}(z_0)\]
\end{proof}
...
\begin{proof}
\[Res(f(z),z_0)= \frac{1}{2πi}\int_{\partial B(z_0,ε)}\left( \sum_{n=-\infty}^{\infty} a_n(z-z_0)^n \right)dz \]
\[\frac{1}{2πi}\sum_{n=-\infty}^{\infty} a_n\int_{\partial B(z_0,ε)}(z-z_0)^n dz = \frac{1}{2πi}a_{-1}\int_{\partial B(z_0,ε)}\frac{1}{z-z_0} dz = a_{-1}\]
\end{proof}
...
\[\int_α f(z)dz = 2πi\sum_j Res(f(z),z_j)Ind_α(z_j)\]
