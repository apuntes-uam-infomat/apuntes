...
\[F_x(z)=f(z)=-iF_y(z)\]
que son las ecuaciones de Cauchy-Riemann. Sabiendo que $f$ es continua podemos decir que $F$ cumple las ecuaciones de Cauchy-Riemann y que su derivada $F'=f$ es continua, y por tanto podemos asegurar que $F(z)$ es holomorfa en $|z-z_0| < r$
...
\begin{defn}[Residuos]
Sea $z_0$ una singularidad aislada de una función $\appl{f}{B(z_0,r)\setminus\{z_0\}}{\cplex}$ entonces definimos
\[Res(f(z),z_0)=\frac{1}{2πi}\int_{\partial B(z_0, ε)}f(w)dw \ \forall 0 < ε < r\]
\end{defn}

\obs Sea $0<ε_1<ε_2<r$ por el teorema de Cauchy
