...
\[f'(z_0)=\lim_{k \to 0} \frac{u(x_0,y_0+k)+iv(x_0,y_0+k)-u(x_0, y_0)-iv(x_0,y_0)}{h}=\]
...
\[\partial f = \partial_z f = \frac{1}{z}(d_x f - i d_y f)\]
\end{defn}

\begin{defn}[Notación: $\bar{\partial}$]
Sea una función $\appl{f}{\cplex}{\cplex}$ definimos
\[\bar{\partial} f = \partial_{\bar{z}} f = \frac{1}{z}(d_x f + i d_y f)\]
\end{defn}
...
Esta función es holomorfa en $\cplex\setminus\{\pm i\}$. Para verlo nos basamos en que es el cociente de dos funciones holomorfas por lo que $f(x)$ será holomorfa en todo $\cplex$ salvo los puntos en que se anula el denominador.

Si calculamos la derivada obtenemos
\[f'(x)=\frac{2z(z^2+1)^2-z^22(z^2+1)2z}{(z^2+1)^4}\]
...
Por tanto, sabemos que existe $f'(0)=0$. Es decir, la función es holomorfa en $(0,0)$
...
Por tanto, el límite no existe y la función \textbf{no es holomorfa}. Sin embargo, las ecuaciones de Caucy-Riemann si se satisfacen para esta función, con lo que queda claro que las ecuaciones de Cauchy-Riemann no son condición suficiente para que la función sea holomorfa.
\end{enumerate}
\end{example}
...
Existe un entorno $U_{z_0}\subset \Omega$ de $z_0$ tal que $f|_{U_{z_0}}$ es isomorfismo holomorfo (biyectiva, holmorfo con inversa holomorfa) si y sólo si $f'(z_0)=0$
\end{theorem}
\begin{proof}
...
%Por el teorema de la función inversa para funciones diferenciables sabemos que $g(u,v)$ tiene inversa diferneciable en un entorno de $(x_0,y_0)$ sii el Jacobiano es d
Puesto que $f$ es un isomorfismo holomorfa, sabemos que es un isomorfismo diferneciable pues veremos un resultado que garantiza que siendo $f$ holomorfa, sabemos que $f'$ también lo es y por tanto $u,v \in \cplex^{\infty}$.

Por tanto ya tenemos que, siendo $f$ una función holomorfa y siendo $f'(z_0)\neq 0$, garantizamos la existencia del isomorfismo holomorfo que describe el enunciado del teorema.
