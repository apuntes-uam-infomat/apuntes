...
\section{Aplicaciones}
	...
	\subsection{Formato JPEG}
		...
		La imagen $N \times M$ se divide en bloques $8 \times 8$\footnote{Si la imagen no tiene un número de píxeles múltiplo de 8×8, entonces nos \textit{intventamos} píxeles para que cuadre}. Cada uno de ellos se puede interpretar como una función:
...
% % % % % % % % % % % % % % % % % % % % % Clase que ha copiado dejuan.

En principio, con 3 de estas funciones $f$ tenemos toda la información de los píxeles del bloque.

\textbf{Cosa técnica:} Al utilizar $YC_RC_B_R$, si cambiamos el tono azul, no se aprecia diferencia. Además, para $C_BB_R$ se suelen despreciar entre la mitad y las $\frac{3}{4}$ partes de los píxeles de la imagen original.
