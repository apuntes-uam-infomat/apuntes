...
\begin{problem}[5]
	\solution
	S = {0,1,2}
	Vemos que $p_1 = \frac{1}{4}$ , $p_2 = \frac{1}{2}$ ,$p_3 = \frac{1}{4}$ 
	...
	Entonces $\frac{3}{2}$ bits es la "cantidad de información" de cada experimento.
	
	Para calcular el número medio de bits para 100 experimentos hacemos:
	
	$$\frac{3}{2} \leq \frac{l^{*}(C)}{100} < \frac{3}{2} + \frac{1}{100}$$
	
	Por lo tanto en número mínimo sería $100 \cdot \frac{3}{2} = 150$ bits.
	
	Ahora vamos a ver si se puede alcanzar ese mínimo.
	
	Vemos que
	$$0 \rightarrow p_1 = \frac{1}{4} \textbf{ con codificación }$$
	COMPLETAR
\end{problem}

\begin{problem}[6]
	\solution
	Aproximadamente se emplean 40 caracteres ASCII  de un byte.
	
	Sólo con estos datos ¿Cuánto podremos esperar comprimir un texto?
	$$S \{ s_1, s_2 , ...., s_{40}\}$$
