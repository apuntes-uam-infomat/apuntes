...
\begin{problem}[2]
	...
	\textbf{solución esquemática}

	$$f * g = g * f$$

	$f: \real \rightarrow \mathbb{C} $

	$$ (f * g) (x) = \int_{-\infty}^{\infty} f(x - t) g(t) dt = \int^{\infty}_{-\infty} f(u) g(x - u) du = (g * f)(x) $$
...
\begin{problem}[3]
	$f(x) = (-1)^{[2x]} \;\;\; x \in [0,1] $ y $f$ es 1-periódica.

	Hallar los coeficioentes de Fourier y escribir la serie de Fourier con senos

	\solution

	\begin{figure}
		\inputtikz{fourier/onda_cuadrada}
		\caption{f(x): función de onda cuadrada}
	\end{figure}

	$$ \hat{f}(n) = \int^{1}_{0} f(x) e(-nx)dx = \int^{1/2}_{0} e(-nx)dx - \int^{1}_{1/2} e(-nx) dx = \frac{-1}{2 \pi i n} e(-nx) \left. \right|^{1/2}_{0} + \frac{1}{2 \pi i n} e(-nx) \left. \right|^{1}_{1/2} $$

	$$ = \frac{-1}{2 \pi i n}\left( e\left( -\frac{n}{2} \right) -1 \right) + \frac{1}{2 \pi i n}\left(1 - e\left( -\frac{n}{2} \right) \right)$$

	Como $e(-1/2) = e^{2 \pi i (-1/2)} = -1$ :
	...
	Teniendo en cuenta que $e(nx) = \cos(2 	pi n x) + i \sin(2 \pi n x)$ y $\frac{e(nx)}{n} + \frac{e(-nx)}{-n} = 2i \frac{\sin(2\pi n x)}{n}$.

	$$ = \frac{2}{\pi i} \sum\limits^{\infty}_{n = -\infty \text{ y } 2 \nmid n}  \frac{\sin (2 \pi n x)}{n} = \frac{4}{\pi} \sum^{\infty}_{k=0} \frac{\sin(2 \pi (2 k + 1) x)}{2k + 1} $$

\end{problem}

\begin{problem}[4]

	$f = \chi_{[-a/2, a/2]} \quad a^{-1} f, a^{-1} \hat{f} \convs[][a][0] ?$

	\solution 

	$\lim_{a \rightarrow 0} a^{-1} f(x) = \lim_{a \rightarrow 0} a^{-1} \chi_{[-a/2,a/2]} (x) = 0 $ para cualquier $x \neq 0$

	Y para $x = 0$ : $\lim_{a \rightarrow 0} a^{-1} f(x) = \lim_{a \rightarrow 0} a^{-1} = \infty $

	$$\int^{\infty}_{-\infty} a^{-1} f(x) = 1 $$

	En cierto modo $a^{-1} \convs[][a][0]$ delta de Dirac.

	$$\lim_{a \rightarrow 0} a^{-1} \hat{f}(\xi) = \lim_{a \rightarrow 0} a^{-1} \int^{\infty}_{-\infty} f(x) e(-x \xi) dx $$

	$$\int^{a/2}_{-a/2} e(-x\pi) dx = 2 \frac{\sin (2 \pi \xi a/2))}{2 \pi \xi} $$

	$$ = \lim_{a \rightarrow 0} \frac{\sin( \pi \xi a)}{\pi \xi a} = 1 $$

	Esto sugiere que $\hat{\delta} = 1$

	Fórmula de inversión $\Rightarrow \delta (x) = \int\limits^{\infty}_{-\infty} 1 e(x \xi) d \xi$

	No converge, es $0 \; \forall $ punto salvo en 1, que es $\infty$.
