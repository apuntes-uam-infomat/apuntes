...
(ya adelantamos que r y $\theta$ no se podrán calcular eplícitamente.)
...
\begin{proof}
	...
	$$ \int_{\partial B} \overrightarrow E \dif \overrightarrow S = \int _{\partial{B_\delta}} \overrightarrow E \dif \overrightarrow S \overbrace{=}^{esfericas} \int_0^{\pi} \int_0^{2\pi} \frac{\delta ^2 \sin\theta}{\delta ^3} \delta \dif \phi \dif \theta\footnote{Escribimos $\overrigharrow{E}$ en polares multiplicado por el radio δ} = 4\pi$$
	\end{itemize}
\end{proof}
...
Si $q_i(t) \rightarrow q_i(t,h)$ dejan invariante $L$ (no depende de la última variable) y $L$ satisface las ec. de Euler-Lagrange. Entonces $\sum_{i = 1}^n \frac{\partial L}{\partial \dot{q_i}} \frac{\partial q_i}{\partial h}$ se conserva (no varía en t).
...
\begin{obs}
	...
	Siempre que $L$ no dependa de $q_{i_0}$ se puede tomar $q_{i_0} \rightarrow q_{i_0} + h$ y el Tma de Noether implica, como en el ejemplo que $\frac{\partial L}{\partial \dot{q_{i_0}}} 1$ se conserva y esto se deduce solo con las ecuaciones de Euler Lagrange directamente.
