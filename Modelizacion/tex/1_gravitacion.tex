...
\begin{op}{Ecuaciones de Euler-Lagrange}
 \frac{d}{dt}\left(\frac{\sigma L}{\sigma \dot{q_i}}\right) = \frac{\sigma L}{\sigma q_i} \text{ para } 1 \leq i \leq n
\end{op}
...
$$\frac{\dif}{\dif t}(\frac{\partial L}{\partial \dot r})= \frac{\partial L}{\partial r}$$
$$\frac{\dif}{\dif t}(\frac{\partial L}{\partial \dot \theta})= \frac{\partial L}{\partial \theta}$$
$$E= \dot r \frac{\partial L}{\partial \dot r} + \dot{\theta }\frac{\partial L}{\partial \dot{\theta}} - L$$
Sabemos que E es conservativo....
Queremos hallar la r en función de $\theta$ porque queremos hallar la función de la elipse:
...
$$\dif \theta = \frac{-\dif(\frac{K_1}{r} - K_2)}{K_1 \sqrt{....}}\\\text{ Y como $K_1$ y $K_2$ son ctes podemos escribir }\\ -\dif(\frac{K_1}{r} - K_2) = \frac{K_1}{r^2}\dif r$$
Y nos queda:
$$ \frac{\dif r}{r^2\sqrt{...}}=\dif \theta \implies \theta = \int \frac{\dif ( \frac{K_1}{r} - K_2 )}{\sqrt{1 - (\frac{K_1}{r} - K_2)^2}}$$

$$ \text{Sabiendo que }- \int \frac{1}{\sqrt{1-x^2}}\dif x = -\arcsin + cte = \arccos + cte \implies \theta = \arccos ( \frac{K_1}{r} - K_2) + cte$$
...
Vamos a hacer ahora algunos ejercicios de la hoja 1:
\begin{example}[1]
	...
	De esto último deducimos que $\overrightarrow x \times \overrightarrow x'' = \overrightarrow 0$
	...
	Y con esto ya tenemos que $\overrightarrow x $ está en un plano porque:\\
	...
	$$ \text{ Definimos la curva } t \mapsto \overrightarrow{x(t)} \text{ y la torsión :} T = \frac{(\overrightarrow x' \times \overrightarrow x'') \cdot \overrightarrow x'''}{||\overrightarrow x' \times \overrightarrow x ''||}$$
	...
	Con esto podemos ver que $\overrightarrow x'''$ es combinación lineal de $\overrightarrow x''$ y $\overrightarrow x'$ y por eso:
...
\begin{example}[2]
	...
	$$\frac{\frac{GMm}{R^2}}{\frac{GMm}{(R + 400000 m)^2}} \implies \frac{(R + 4\cdot 10^5)^2}{R^2} = (\frac{67.8}{63.8})^2$$
	Por lo tanto, si m = 80, el peso a 400 km sería:
	$$\frac{80}{(\frac{67.8}{63.8})^2} = 70 .84 Kg$$
...
\begin{example}[3]
	Tenemos la elipse:$$r(\theta)= \frac{a^{-1} \cdot b^2}{1 + e\cos\theta} \text{  siendo  } e = \frac{c}{a} = \sqrt{1 - \frac{b^2}{a^2}}$$
		...
		r\cos\theta = x\\ 
		\end{cases}$$
		Despejando y haciendo: $(\sqrt{x^2 + y^2})^2 = (....)^2$\\
		$$x^2 + y^2 = a^{-2} b^4 + e^2x^2 - 2a^{-1}b^2ex$$
		Como $1-e^2 = \frac{b^2}{a^2} \implies$
